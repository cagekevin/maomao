# 回归测试补齐 · 存储层 kvStore（TASK-061）

> 委派给：执行 AI
> 类型：**C/D（代码改写：新增回归测试）** —— 目标明确，需精确源文件定位
> 唯一产出：`tests/unit/kvStore.test.js`。**你只能写这个文件，碰任何其他文件视为失败。**
...
### 1. `src/components/base/kvStore.js`

**模块职责**：localTool KV 存储层：kvGet/kvSet/kvDelete（fetch 到 /api/kv/*）+ storageGet/storageSet/storageDelete（按 CANVAS_STATE_PREFIX 分流 KV/localStorage）+ KV 失败降级 localStorage。

**测试覆盖点（至少覆盖以下，可补充更多）**：
- isKvKey(key)：canvas-state-v1- 前缀返回 true，否则 false
- kvGet/kvSet/kvDelete：请求 URL/方法/body 构造；非 ok 抛错
- storageGet：KV 前缀走 kvGet；否则 sGet + JSON.parse
- storageSet：KV 前缀走 kvSet，KV 失败时降级 localStorage（返回 {ok:true, degraded:true}）；非 KV 前缀 sSet
- storageDelete：KV 前缀走 kvDelete；否则 sRemove

**测试策略**：mock global.fetch 返回可控响应（含非 ok 分支），mock storageAdapter 的 sGet/sSet/sRemove。
...
5. **测试必须能跑通**：`npx vitest run tests/unit/kvStore.test.js` 0 失败。
...
只创建一个文件：`tests/unit/kvStore.test.js`，内容为完整可运行的 vitest 测试。
文件首行注释写：`// 回归测试：kvStore.js`

---

## 六、验收标准（必须全部满足）

- [ ] 只创建了 `tests/unit/kvStore.test.js`，**未修改/删除任何其他文件**
- [ ] 只运行过 `npx vitest run tests/unit/kvStore.test.js` 这一个验证命令
- [ ] `tests/unit/kvStore.test.js` 所有测试通过（0 fail）
