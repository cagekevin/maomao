# 回归测试补齐 · 日志与配置 logger + apiBase（TASK-062）

> 委派给：执行 AI
> 类型：**C/D（代码改写：新增回归测试）** —— 目标明确，需精确源文件定位
> 唯一产出：`tests/unit/logger.test.js`。**你只能写这个文件，碰任何其他文件视为失败。**
...
### 1. `src/components/base/logger.js`

**模块职责**：统一日志：log(info/warn/error) 格式 `[log] HH:MM:SS | category | action | detail` + fire-and-forget 上报 localTool /api/logs（含 taskId/nodeId 提取 + 200ms 上报去重）。

**测试覆盖点（至少覆盖以下，可补充更多）**：
- log 按 level 调 console.info/warn/error，输出格式含 category/action/detail
- 从 detail 提取 taskId/task_id/nodeId/node_id 传给上报
- 上报去重：同一 (category:action:detail) 在 200ms 内只上报一次
- 上报失败静默不抛错（fetch 拒绝时 catch 吞掉）

**测试策略**：mock console 各方法断言输出；mock global.fetch 捕获上报 body；断言上报去重与失败静默。

---

### 2. `src/components/base/apiBase.js`

**模块职责**：localTool 后端地址统一入口，导出 API_BASE 常量（http://127.0.0.1:18080）。

**测试覆盖点（至少覆盖以下，可补充更多）**：
- API_BASE 是字符串且以 http 开头
- 所有 API 层 import 统一从这里取（可断言该常量被引用）
...
5. **测试必须能跑通**：`npx vitest run tests/unit/logger.test.js` 0 失败。
...
只创建一个文件：`tests/unit/logger.test.js`，内容为完整可运行的 vitest 测试。
文件首行注释写：`// 回归测试：logger.js、apiBase.js`

---

## 六、验收标准（必须全部满足）

- [ ] 只创建了 `tests/unit/logger.test.js`，**未修改/删除任何其他文件**
- [ ] 只运行过 `npx vitest run tests/unit/logger.test.js` 这一个验证命令
- [ ] `tests/unit/logger.test.js` 所有测试通过（0 fail）
