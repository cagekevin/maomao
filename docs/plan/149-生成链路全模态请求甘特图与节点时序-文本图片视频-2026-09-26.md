...
> 🔴 **不变量：文本全同步**（用户裁定 2026-09-26）—— 所有与文本相关的链路都是同步：POST 一次、无异步句柄、不轮询、结果在 `content`、无 `resultUrl`、不进恢复轮询。
>
> **现状违反项（唯一一处）**：文本节点借用了**异步原语** `runGenerationOrchestration`，而该原语 R7 自述「非异步任务不要走本原语（它假定有结果 URL）」（`generationOrchestration.ts:32-33`）。它给文本带来的四件事对文本全是无意义或副作用：
> 1. `reportGenerate` → **建「文本」任务行**（`taskStore.ts:229 TASK_TYPE_META.text`）且**自动弹开任务中心**（`taskStore.ts:344 openTaskCenter`）；
> 2. `taskCtl.progress(5/10/30)` → 给一个同步操作画进度条；
> 3. `saveResultToTasks('')` → 恒 `skipped`（空 url 早返回，`filesApi.ts:726`）；
> 4. `taskCtl.done('')` → 不广播（空 URL 拒发）。
>
> **对比**：其余文本消费者（剧本盒 5 处、AI 助手、会话摘要压缩）**都不走这条**，直接 `chatCompletions` / `chatStream` ⇒ **只有文本节点这一家不合群**。
> 修法（不改代码，仅记结论）：见 §六·13。
...
- `temperature`：**不传**（`responseFormat` 已于 2026-09-26 整条删除，见 §八）。
...
|---|---|---|---|---|---|---|---|---|
| ① | 剧本+分镜+资产（一次出全） | `onGenerateScript` `:322`；`chatCompletions` `:377` | 步骤1「生成分镜脚本」`StepShots.tsx:197` | `resolveTextModel()` `:340` | `resolveSystem(playbookId,'script') \|\| SCRIPT_WRITER_SYSTEM` + `SCRIPT_WRITER_FORMAT` + 镜头数要求 + 风格要求（`:348-363`） | `story`（手填 `data.story` + 上游 `data.upstreamStory` 合并，`:327-329`） | `temperature:0.7`；`images: 上游参考图`（`:380-383`） | `parseJsonText`（`:399`）→ `shots[]`（含 `dialogue` 归一化，`:426`）+ `assets[]`（`prompt` 由 `ZgPrompt` 拼，`:466`）→ `updateData`（`:479-487`） |
| ② | 分镜生图/生视频提示词 | `onGenerateShotPrompts` `:658`；`chatCompletions` `:760` | 步骤3「生成/批量生成提示词」`StepPrompt.tsx:271/326` | 同上 | `resolveSystem(playbookId,'shot') \|\| SHOT_DIRECTOR_SYSTEM` + `resolveSystem(playbookId,'qg')`（`:682-684`） | `assembleShotUser(shot, refAssets, globalStyle, {负面/上下文/前后镜…})` + 生图约束 + 生视频约束 + `videoPrompt` 格式硬要求 + 用户修改意见（`:741-758`） | `temperature:0.7`；**无 images** | `parsedData.prompt` / `parsedData.videoPrompt`（`:799-800`）→ `shots[].prompt` / `shots[].videoPrompt`（`:829-835`，200ms 批量 patch） |
| ③ | AI 生图提示词（关键帧/宫格/俯视） | `onGenerateShotImage` `:897`；`chatCompletions` `:920` | 步骤3 关键帧类 | 同上 | `resolveImageGenSys(playbookId,type).sys`（`:907`） | `buildShotImageUser(shot, type, {globalStyle, assets})`（`:908`） | `temperature:0.7` | 解析可 string/`{prompt}`，非 JSON 降级原文（`:930-938`）→ `shots[].imgGen = {type,label,prompt,ts}`（`:947-962`） |
| ④ | 审计式「按意见改」 | `onReviewShotPrompt` `:986`；`chatCompletions` `:1030` | 步骤3 双击弹窗「按意见改」`StepPrompt.tsx:106` | 同上 | `resolveSystem(playbookId,'audit') \|\| SHOT_AUDIT_SYSTEM`（`:1000`） | `buildAuditUser(shot, field, feedback, assetNames)`（`:1001-1006`） | `temperature:0.7` | 返回 `Promise<{ok,text}>`（`:1053`）→ **组件确认后才写回** `shots[idx][field]`（引擎只置 `promptLoading`，`:1009`） |
| ⑤ | 多镜合并视频提示词 | `onGenerateMergedVideo` `:1410`；`chatCompletions` `:1456` | 步骤3「合并生成视频」`StepPrompt.tsx:358` | 同上 | `MERGE_VIDEO_SYSTEM`（内置常量，`:1462`） | `buildMergedVideoUser(picked, d.assets)`（`:1463`） | `temperature:0.7` | `r.content` 原文（`:1470`）→ **新建 `videoGenerateNode`**，`data.prompt = mergedPrompt`（`:1476-1483`） |
...
13. **文本全同步 ⇒ 文本节点不该走 `runGenerationOrchestration`**（该原语 R7 自述不适用非异步任务）。
    - **必须保留**：`claimNodeRun`/`releaseNodeRun`（单节点互斥锁）、`validate→error`、`loading`（`nodeRuntimeStore`）、`AbortController`、**`registerTaskRetry`**（`useNodeGeneration.ts:302` —— Agent `generate_node` / `canvasPlanExecutor` / `upstreamLink` 全靠 `runNodeGeneration` 驱动节点，**丢了它 Agent 就驱动不了文本节点**）、`onSuccess` 写回 `data.text`。
    - **应当去掉**：`reportGenerate`（任务行 + 弹任务中心）、`saveResultToTasks`、`done`。
    - **推荐切法（无新开关）**：`useNodeGeneration` 的 `resultKey` 语义本就是「声明即表示结果可随任务中心恢复」（`:86-91`）——文本节点**有意不声明**。⇒ 可按「有无 `resultKey`」分派：**有 → 异步原语；无 → 同步轻路径**。这是既有声明的自然分支，不是新加的布尔开关。
    - **回归点**：任务中心不再出现「文本」卡片（`TASK_TYPE_META.text` 只服务历史行）；`openTaskCenter` 不再被文本触发；`GeneratedView` 的「文本」tab **不受影响**（它读 resources，来自 `saveTextToTasks`，与任务行无关）。
...
### 7.2 对账中发现并**已就地修正**的参照物过时点（2026-09-26）

发现即改（按两份文件的维护规矩：就地改该行，不追加叙事）。改动清单：

| # | 文件 | 原记载 | 改为（当前工作区事实） |
|---|---|---|---|
| 1 | `spec/DATAFLOW.md` §一 | `agentRuntime.ts → chatStream（AI 助手 SSE 对话）` | `chatStream（AI 助手对话；stream 由设置「响应方式」决定）` |
| 2 | `spec/DATAFLOW.md` §五 | filesApi 消费方挂 `scriptBoxEngine（uploadFileToLocal/saveResultToTasks）· useNodeGeneration（saveResultToTasks）` | `scriptBoxEngine（uploadFileToLocal）· TextGenerate（saveTextToTasks）· generationOrchestration（saveResultToTasks，节点与剧本盒共用）` |
| 3 | 登记表 §二 #1 | `relayProxy.ts:41` | `relayProxy.ts:36` |
| 4 | 登记表 §二 #4 | `config.ts:181` · `relayProxy.ts:213` | `config.ts:174` · `relayProxy.ts:210` |
| 5 | 登记表 §二 #5 | `relay-poll.ts:133` | `relay-poll.ts:176` |
| 6 | 登记表 §二 #8 | `relay-poll.ts:143` · `relayProxy.ts:32` | `relay-poll.ts:186` · `relayProxy.ts:27` |
| 7 | 登记表 §二 #13 | `generateEngine.ts:69` | `generateEngine.ts:68` |
| 8 | 登记表 §四 #6 | `agentConfig.ts:54` | `agentConfig.ts:74` |
| 9 | 登记表 §四 #7 | `agentConfig.ts:60` | `agentConfig.ts:80` |
| 10 | 登记表 §四 #9 | `agentConfig.ts:164` | `agentConfig.ts:184` |
| 11 | 登记表 §八 #2 | `taskStore.ts:890` · `relayProxy.ts:66` · `generationOrchestration.ts:193-196` | `taskStore.ts:958` · `relayProxy.ts:63` · `generationOrchestration.ts:204-216` |
| 12 | 登记表 §八 #10 | `taskStore.ts:827/842` | `taskStore.ts:934/949` |
| 13 | 登记表 §八 #11 | `taskStore.ts:154` | `taskStore.ts:193` |
| 14 | 登记表 §八 #13 | `taskStore.ts:433/479` | `taskStore.ts:461/498` |
| 15 | 登记表 §九 #13 | `relay-poll.ts:638` | `relay-poll.ts:676` |

> 说明：本次**只改了我逐条验证过的行**；登记表共 181 行，其余引用文件（`providerConfigStore` / `ai-relay/types` / `providers.default.json` / `nodePrefs` / `promptChips` / `utils` 等）的行号**未逐条复核**，可能仍有漂移。
> 另：`relayProxy.ts` / `generationOrchestration.ts` 在工作区有未提交改动，本表行号以**当前工作区**为准。

---

## 八、能力退役：`response_format`（`json_object`）已整条删除（2026-09-26 · TD-01-35）

本文 §2.2 原记录的「剧本盒 `responseFormat: useJsonObject(modelId) ? 'json_object' : undefined`」**已不存在**。该字段于 2026-09-26 按**用户裁定**整条跨栈链删除（`ADR-0053` 四关——目的 / 能力 / 消费 / 红线——全过）。

### 8.1 删了什么（本文相关处已同步）

`scriptBoxEngine.ts`（`useJsonObject()` + 两处写入）→ `generate.ts`（`GenerateRequest` / `ChatCompletionsOptions` / `json→json_object` 归一 / `chatCompletions` 透传）→ `relayProxy.ts`（入参 + `body.response_format`）→ `routes/generate.ts`（路由转发）→ `generateEngine.ts`（`RelayGenerateInput.responseFormat` + 失实注释「TextNode JSON 依赖」）→ `ai-relay/protocol/variables.ts` 白名单项 `'response_format'`。

### 8.2 为什么可删（消费端实证，非推定）

- **默认路径结构上无法消费**：默认文本模型 = `lovart::lovart-chat`（`providers.default.json` primary + 列表首位）→ `generateEngine.ts:192` 分支 → `chatLovartText` 走 **Lovart 私有 HMAC 协议**（请求体只有 `prompt / attachments / tool_config`，**无 messages 数组、无该字段位**）；且 `extractChatTextAndImages`（`providers/lovart/index.ts:74-116`）把 system / user **拍平成一个字符串**。
- **非 lovart 分支才是真"漏接"**：`ai-relay/generate.ts:43-55` 的 `postChatRaw` body 里无该字段，`ChatOptions`（`types.ts:335-344`）亦无。
- ⇒ 该参数**从未到达任何上游**。

### 8.3 历史真相（勿再判为"它本来就是废物"）

| 日期 | 提交 | 状态 |
|---|---|---|
| 2026-08-15 | `e96f6096` 初始提交 | 随原型进仓；前端直发、形态**正确** `{type:'json_object'}` ⇒ **有效**（满血 **19 天**） |
| 2026-09-03 | `1b0d72eb` | **特意接上**（`relay.ts:162` 注入 + preset 模板 `{{response_format}}`）；**但同一提交把值拍平成裸字符串** `'json_object'` ⇒ 形态已非法（2 天） |
| **2026-09-05** | **`fbe0a3d2`「lovart 直连收口」** | **能力失效点**：改原生 HMAC 协议（无该字段位），模板行移除 ⇒ 字段自此不在任何请求里（21 天） |
| 2026-09-20 | `54a8c77f` | 清残留：删注入行，**漏清声明** ⇒ 留下"看起来还在"的**假护栏** |

### 8.4 仍然存在、但属**另判据**（本批未动）

| 位置 | 判据归属 | 状态 |
|---|---|---|
| `ai-relay/protocol/presets.ts:47` `extra_body: { response_format: 'url' }` | **图片**（Images API 的 `response_format`，值域 `url` / `b64_json`）；且 `extra_body` 在协议引擎内**零消费**（grep 零命中）⇒ 只会作为**字面键**进入请求体 | 疑似另一笔，待查 |
| `localTool/src/routes/requestModes.ts:214/271`（`body.text = { format: { type } }`） | **另一层语义**（已退役的 responses 形态）；整文件 `refs` 模块引用 **0 处**、唯一引用者是自带单测 | 疑似整文件真幽灵，另立一案 |

> ⚠️ **不再有"JSON 硬保证"**：删后写侧**只剩提示词契约**这一招软约束（`SCRIPT_WRITER_FORMAT` / `MANGA_SHOT` 等 system 里的自然语言）。读侧的失败契约问题是 `TD-01-34`，**未随之解决**。

---

*文档位置：`docs/plan/149-生成链路全模态请求甘特图与节点时序-文本图片视频-2026-09-26.md`（第三版 · 2026-09-26 追加 §八 能力退役）*
