...
| **ADR-0052** | **机器可判定性** —— 每条规则须可写成检查 + 负例探针自证；**闸绿不算证据** | 本计划 §5 验收总表须照它写（判定命令 + 期望输出，禁止写「跑 X ✅」当结论） |
...
| 41-43 | ~~`captureFrame`~~ · `videoEngine` · `timeScale` · `sourceTime` | `video/lib/`（后三件**已落位**；**`captureFrame` 除外** —— 2026-09-19 裁定**留 `base/utils/`**：跨 scriptbox+video 真横切，见 `video/index.ts:11`） |
