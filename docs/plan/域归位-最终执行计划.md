...
│   ├── depthVideo/
│   └── lib/    videoEngine · timeScale · sourceTime        ← captureFrame 不在本域（2026-09-19 裁定留 base/utils/）
│
├── text/       TextGenerate
...
`editors/index.ts` → **保留为子域门面**（【2026-09-19 推翻原「删」· **ADR-0044 §3**】域门面唯一 + 每个子域可有自己的子域门面；实测其 6 个消费方**全在域内**、域外 0 处。原裁定：并入 `image/index.ts`，不留双门面）。
...
| 41-43 | ~~`captureFrame`~~ · `videoEngine` · `timeScale` · `sourceTime` | `video/lib/`（后三件**已落位**；**`captureFrame` 除外** —— 2026-09-19 裁定**留 `base/utils/`**：跨 scriptbox+video 真横切，见 `video/index.ts:11` 与 ADR-0043 §2） |
