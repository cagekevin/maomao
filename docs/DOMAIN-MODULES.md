...
### 2.0 🔴🔴 **归类的根本判据：按事实归属**（用户 2026-09-19 校准）

> **用户原话**：「**你如果不把它归类正确的话，人都看不懂，AI 一样也看不懂**，
> 最后还是乱七八糟地找一堆。」
> （前一句是：「只要 AI 能够迅速找到东西就行，反正我又不看代码」——**我把它误读成
> 「好找 = 独立判据、可优先于正确性」，造了一个不存在的对立，已纠正。**）

**判据**：归类要**正确** —— 即「这个东西**事实上属于哪个东西**」。按事实定，不按"方便"定。

**「好找」不是独立判据，它是「归类正确」的*结果***：

| | 归类正确 | 归类错误 |
| --- | --- | --- |
| 人 | 看得懂 | 看不懂 |
| AI | **一眼找到** | 也得满仓乱找，**且会反复找错** |

⇒ **禁止**用「这样更好找」当理由改归属 —— 那是循环论证（**先错了，才需要"找"**）。
⇒ **禁止**用「理论上更优雅」当理由改归属 —— 理论自洽 ≠ 事实归属。
⇒ **唯一合法的判据 = 它事实上是什么、属于哪个「东西」。**

**推论（对我自己的约束）**：两个归属都"说得通"时，**说明事实还没查清** ——
回去取证（逐文件消费者实测 · 头注自述 · 用户视角），**不是二选一、更不是挑个好听的理由**。

### 2.0-bis 🔴🔴 **域 = 用户能「指着」说的那个东西**（用户 2026-09-19 定案）

> **判据**：**域 = 用户能用手**指着**说的那个东西**（一级视图 / 节点类型 / 应用）。
> **节点上的工具（hover 工具 · 编辑器 · 输入控件 · 按钮）不是域** ——
> 它们归 **「所在节点所属的那个能力」**。
> **⚠️ 宿主数不是判据 —— 它在界面上「长在哪儿」才是。**

**三条用户判例（原话）—— 它们是同一条判据的三个实例**：

| 件 | **用户原话** | 归属 |
| --- | --- | --- |
| `PromptInput`（图片/视频/文本三类节点**共用**的输入控件） | 「`PromptInput.tsx` 这个**根本就是画布上面的东西**」 | **画布**（节点通用部件） |
| `cameraStudio` + `cameraParams/*`（相机） | 「相机也不是真独立能力啊，相机是他妈的**图像生成节点下面的一个按钮**」 | **图片能力**（图片节点上的按钮） |
| `depthVideo`（深度视频） | 「你他妈的那个深度视频，是**视频节点上面的 hover tool 工具**」 | **视频能力**（视频节点上的 hover 工具） |

**⇒ 🔴 推翻我 2026-09-19 提出的两条错判**（都在本会话内产生、当日被用户否掉）：
1. ❌ 「**宿主数 ≥2 ⇒ 独立能力**」—— 我据此把相机判为独立能力。**错**：
   `depthVideo` 同样被 **2 个节点**（`VideoGenerate` + `AssetNode`）消费，
   但它仍然只是**视频节点上的一个 hover 工具**。**宿主数不能推独立能力。**
2. ❌ 「**`base/editors` = 独立域「图像编辑域」**」（沿自 §2.3 P2）—— **错**：
   裁剪/图层/全景/打码/相机**全是图片类节点上的工具** ⇒ 归**图片能力**，不是独立域。

**⇒ 正确读法（本仓适用的唯一读法）**：**按"它在界面上长在哪儿"归属** ——
· 长在**某个内容类型节点**上（图片/视频/文本）⇒ 归**该内容能力**
· 长在**所有节点都有的通用位**上（外壳/输入控件/hover 工具栏）⇒ 归**画布**（节点机制）
· 长在**非节点的界面位**上（左栏页签 / 设置页 / 独立应用）⇒ 归**它所在的那个域**

**这与 §3.1.3 的能力轴（产品 `NodePalette.ts:106-109` 文本/图片/视频/其他工具）是同一件事** ——
产品的「工具分类」= 用户的「指着说」= 我们的「域」。

**附：归类错误的四个症状（拿来自查，不是判据本身）**

| # | 症状 | 本仓实例 |
| --- | --- | --- |
| **S1 一名多址** | 一个「东西」的零件散在多处 | 「图片能力」散在 `canvas/nodes` + `editors` + `base/utils` 三处 |
| **S2 名字对不上内容** | 看目录名猜不中里面是什么 | `base/panels`（app-shell + 横切 kit + 域 UI）· `base/prompt`（画布控件 + 社区库） |
| **S3 宿主与零件分居** | 东西在 A，它的宿主在 B | 「提示词输入」在 `base/prompt`，宿主（节点）在 `canvas/nodes` |
| **S4 需要开例外** | 归类对了就不该有例外；要开例外 = 归类错了的信号 | `base/prompt` 原本要开 3 处白名单；拆对后**自然消解** |
...
> ⚠️ **本条 2026-09-19 被推翻（见 §2.0-bis / §3.0）**：用户原话「相机是**图像生成节点下面的一个按钮**」+
> §3.0 界面位置地图实测（`editors/**` 挂在「图片生成 / 图片拼图 / 人脸打码 / 素材节点」上）
> ⇒ 它们**全是图片类节点上的工具** ⇒ 归 **图片能力**（`cat: image`），**不是独立域「图像编辑域」**。
> 「不归画布域」这句仍成立，但理由变了：不是"它自成一域"，而是"它属**图片能力**"。
...
### 3.0 🔴🔴 **界面位置地图 —— 归属判据的真源**（用户 2026-09-19：「你又分不清什么东西在什么位置」）

> **为什么要这张表**：§2.0-bis 的判据是「**它在界面上长在哪儿**」。但我此前**没有这份映射**，
> 每次都在**猜**（本会话因此连错 4 次）。⇒ 正解不是「让我分得清」，而是**把它机械生成出来**。
> **生成方式（可复现）**：① 节点清单从 `canvas/NodePalette.ts` 的 `paletteNodes` 抽（产品写死的 `cat`）；
> ② 每个界面位挂的件从其组件文件的 import 抽（`@/components/**`，排除 `base/{core,utils,ui,store,api,storage}` 这类横切原语）。
> **⚠️ 表会漂移** ⇒ 归属有疑问时**重新生成一次**，不照抄本表。

**A · 一级视图与界面位**

| 界面位（用户能指着的） | 代码位置 |
| --- | --- |
| **一级视图 · 画布** | `src/App.tsx` + `components/canvas/**` |
| **一级视图 · 多开 / 账号** | `base/store/accountsStore`（+ 账号环境组） |
| **左栏页签 · 任务** | `base/panels/TaskCenter.tsx` |
| **左栏页签 · 生成** | `base/panels/GeneratedView.tsx` |
| **左栏页签 · 素材** | `base/panels/ResourceLibrary.tsx` |
| **左栏页签 · 提示词** | `base/prompt/PromptHub.tsx`（+ `promptHubStore`） |
| **设置页** | `base/panels/SettingsFrame.tsx` + `sections/*` |
| **AI 助手面板** | `agent/panels/AgentPanel.tsx` |
| **顶部导航 / 项目选择** | `base/panels/TopNav.tsx` + `ProjectSelector.tsx` |
| **剪辑器（独立应用）** | `components/videoEditor/**` |

**B · 画布上的节点（产品 `cat` = 能力归属的**权威真源**）**

| 中文名 | type | **cat** | 组件 |
| --- | --- | --- | --- |
| 图片 | `imageGenerateNode` | **image** | `ImageGenerate` |
| 图片盒子 | `imageBoxNode` | **image** | `ImageBoxNode` |
| 图片切分 | `gridSplitNode` | **image** | `GridSplitNode` |
| 图片拼图 | `gridMergeNode` | **image** | `GridMergeNode` |
| 全景图 | `panoramaNode` | **image** | `PanoramaNode`（懒加载） |
| 3D导演台 | `director3dNode` | **image** | `Director3DNode`（懒加载） |
| 人脸打码 | `faceMosaicNode` | **image** | `FaceMosaicNode` |
| 循环生成 | `loopNode` | **image** | `LoopNode` |
| 图片视频素材节点 | `assetNode` | **image** | `AssetNode` |
| 视频生成 | `videoGenerateNode` | **video** | `VideoGenerate` |
| 视频抽帧 | `videoExtractNode` | **video** | `VideoExtractNode` |
| 视频处理 | `videoProcessNode` | **video** | `VideoProcessNode`（懒加载） |
| 文本 | `textGenerateNode` | **text** | `TextGenerate` |
| 编组 | `group` | **other** | `GroupNode` |
| 剧本盒子 | `scriptBoxNode` | **other** | `ScriptBoxNode` |

**C · 各节点挂的「工具件」（= 节点界面上的东西 ⇒ 归该节点所属能力）**

| 节点 | 挂的域外工具件 |
| --- | --- |
| 图片（生成） | `editors`（相机/编辑器）· `creative` · `base/nodeImage` |
| 图片拼图 | `editors`（`OverlayEditor`） |
| 人脸打码 | `editors`（`FaceMosaicEditor`） |
| 图片视频素材节点 | `editors`（相机）· `video`（深度视频）· `base/nodeImage` |
| 视频生成 | `video`（深度视频）· `creative` |
| 文本 | `creative` |
| 剧本盒子 | `scriptbox/*`（`StepShots`/`StepAssets`/`StepPrompt`/`StepNav`/`GearSettings`/`ScriptBoxFullscreen`/`scriptBoxSchema`） |
| 其余（盒子/切分/全景/3D/循环/抽帧/处理/编组） | （无域外工具件） |

**⇒ 用这张表读出的结论（三条，都是机械事实、不是猜）**：
1. **`editors/**` 挂在「图片生成 / 图片拼图 / 人脸打码 / 素材节点」上** ⇒ 全是**图片类节点上的工具**
   ⇒ 归 **图片能力**（`cat: image`），**不是**独立域「图像编辑域」。
2. **`depthVideo` 挂在「视频生成 / 素材节点」上** ⇒ 是**视频类节点上的工具** ⇒ 归 **视频能力**。
3. **`PromptInput` 是三类节点共用的输入控件**（图片/视频/文本节点都挂）⇒ 它不属于任何**内容能力**，
   是**节点通用部件** ⇒ 归 **画布**（节点机制）。
...
**⑥ 🔴🔴 决定性修正（2026-09-19 · 用户指出「`PromptInput.tsx` 根本就是画布上面的东西」）——
`base/prompt/` **不是一个域**，它混着两块**归属不同**的东西**（逐文件实测消费者）：

| 文件 | 消费点 | 消费者（实测） | **归属** |
| --- | --- | --- | --- |
| **`PromptInput.tsx`** | 5 | **`canvas/nodes/{Image,Text,Video}Generate` + `_template/TemplateNode`** + `FullscreenEditor`（`§3.3 C4` 已判**画布域**） | **画布域**（节点输入控件） |
| **`promptChips.ts`** | 5 | **`canvas/nodes` ×4** + `creative/creativePresets` | **画布域**（输入控件配套：`@{id:label}` 素材引用芯片） |
| **`promptLayout.ts`** | 5 | **`canvas/nodes` ×4** + `FullscreenEditor`（画布域） | **画布域**（输入控件 + 素材条 + 底栏的对齐基准） |
| **`promptMention.ts`** | 0（仅被 `PromptInput` 用） | — | **画布域**（输入控件的 `@提及` 逻辑） |
| `PromptHub.tsx` | 1 | `LeftPanel`（左栏「提示词」页签 `LeftPanel.tsx:21-24`） | **提示词域** |
| `promptHubStore.ts` | 0（仅域内自用） | — | **提示词域** |

**判据链（三条独立证据同向）**：
1. **§2.3.2 P5（用户可感知性）**：「提示词」**不在**用户的「一个东西」清单里
   （视频 / 图片 / 文本 / AI 助手 / 表格 / 多开 / 设置 / 其他）⇒ 它是**能力 / 子域**，
   归到「**它能被用户感知的那个母域**」。而 `PromptInput` 长在**画布节点**上 ⇒ **母域 = 画布**。
   真正对应用户可感知「提示词」的是**左栏那个页签** = `PromptHub`。
2. **§2.3.1 P4（宿主判据）**：`PromptInput` 的消费者**全在画布域**（4 节点 + `FullscreenEditor`）
   ⇒ 不存在「≥2 宿主」情形 ⇒ 它是**画布域的私有 UI 件**，不是独立能力。
3. **实测**：`PromptHub`/`promptHubStore` 的消费者只有 `LeftPanel` 一处
   ⇒ 它们才是「左栏页签」这一**用户可感知单元**。

**⇒ 连带修正**：`§3.1.3.1 ②`「`base/prompt/*` 是跨内容类型共享原语」**只对了一半** ——
被三内容能力共用的是**画布节点上的同一个输入控件**（因为它们同属画布域），
**不等于**「它是独立于画布的域」。**`base/prompt` 必须拆**：
· 画布域控件组（4 件）⇒ 随画布域归位
· 提示词域（2 件：`PromptHub` + `promptHubStore`）⇒ `components/prompt/`

**附带收益**：本批的 3 处「横切消费点」**全部自然消解** —— `FullscreenEditor`（画布域）→
`PromptInput`/`promptLayout`（同归画布域）= **域内引用**；`LeftPanel`（宿主层，§7 已改判）→
`PromptHub` = **宿主→域** ✓ 合法。**不需要任何白名单。**
...
⇒ **撞真实号比编假号更危险**：**没有任何机器能替你判「这个号指的事与引用上下文是否相符」** ——
原 `check:doc-refs` 也只验「债号存在」，**且该闸已于 2026-09-20 按用户裁定删除**
（「文档不需要任何测试」，判据真源 `docs/adr/ADR-0049`）⇒ 撞号引用**必然静默通过**，
后人按号去查会读到完全不相干的内容（比幽灵债号更难发现）。**故这条只能靠纪律，不能靠闸。**
...
| **2026-09-19** | ⚠️ **我提出的**「**宿主数 ≥2 ⇒ 独立能力**」 | **🔴 推翻（用户当日否掉）**：`depthVideo` 被 **2 个节点**（`VideoGenerate` + `AssetNode`）消费，但用户原话「那个深度视频，是**视频节点上面的 hover tool 工具**」⇒ 它**不是**独立能力。**宿主数不能推独立能力** —— **它在界面上「长在哪儿」才是判据**（见 §2.0-bis）。我据此把 `cameraStudio`（相机）判为独立能力也是错的 | §2.0-bis · 用户校准 |
| **2026-09-19** | **§2.3 P2「`base/editors` = 独立域『图像编辑域』」**（本会话 S2-8 还据此把它迁成 `components/editors/`） | **🔴 推翻**：用户原话「相机是**图像生成节点下面的一个按钮**」+ 逐件实测（`ImageEditor`/`InlineImageCropper`/`OverlayEditor`/`FaceMosaicEditor`/`PanoViewer` 各只被**1 个节点**消费）⇒ 它们**全是图片类节点上的工具** ⇒ 归 **图片能力**，**不是独立域**。⇒ **S2-8 的 `editors` 落点（`components/editors/`）是错的**，需改落 `components/image/`（并入图片能力域） | §2.0-bis · 用户校准 |
| **2026-09-19** | **§3.1.1「`depthVideo` 降级为视频域的子域」** | **修正**：归属（视频）**对**，但**不是"子域"** —— 它是**视频节点上的 hover 工具**（用户原话）。⇒ `components/video/depthVideo/` 落点可用，**描述要改**（不是子域，是视频能力的工具件） | §2.0-bis |
...
| **S2-8** | **其余域迁出 `base/`** ⇒ `components/<域>/`（`creative` ✅ · `depthVideo` ✅ · `editors` ✅ · `store`；`media` 经 §3.1.4 C-1 裁定为**横切协议层**，**不迁**）。⚠️ **`prompt` 已改判 ⇒ 移出本批**：它**不是一个域**（§3.1.3.5 ⑥ 实测），须先拆成「画布域控件组（4 件）」+「提示词域（2 件）」⇒ 见 **S2-10**。**它是「base/ 只留横切层」这个目标形态的收尾**（§2.6 / §7 裁定）；S2-4 落地时**顺带做**，不另开批。**手法已跑通三轮**：搬移 → 手动后置工序 → 门面 → 收口 → 计划回改 |
...
| **S2-10** | **`prompt` 命名审计与收口**（用户 2026-09-19 指出「很多名字叫 prompt，但实际命的不对」→ 取证后**升级为「拆域」**，见 §3.1.3.5 ⑤⑥）。**三层问题**：<br>① **一词覆盖 5 个域**（§3.1.3.5 ④）：`base/prompt/*` · `base/creative/*` · `agent/assistantTable/assistantTablePrompt` · `scriptbox/scriptBoxPrompt*` ×3 · `base/editors/cameraParams/cameraPrompt` ⇒ 必须分别命名<br>② **`base/prompt` 域内 3 文件名不副实**（逐文件读头注）：`promptChips.ts`（实为 **`@{id:label}` 素材引用芯片 ↔ DOM**）· `promptLayout.ts`（实为**素材条 + 正文 + 底栏的公共对齐基准**）· `promptMention.ts`（`@提及` 是**输入控件**的通用逻辑）<br>③ **🔴 `base/prompt` 不是一个域，必须拆**（用户指出「`PromptInput` 根本就是画布上面的东西」，实测坐实）：<br>　· **画布域控件组**（4 件）⇒ `PromptInput` · `promptChips` · `promptLayout` · `promptMention`（消费者全在画布域：4 节点 + `FullscreenEditor`）<br>　· **提示词域**（2 件）⇒ `PromptHub` + `promptHubStore`（消费者只有 `LeftPanel` 的左栏「提示词」页签）<br>⇒ **附带收益**：本批 3 处「横切消费点」**全部自然消解**（`FullscreenEditor`→`PromptInput`/`promptLayout` 变域内引用；`LeftPanel`→`PromptHub` 是宿主→域）⇒ **不需要任何白名单** |
...
| **S2-10 `prompt` 命名审计** | **待做** | 三层：① 一词覆盖 5 个域 ② `base/prompt` 内 3 文件名不副实 ③ **`base/prompt` 不是一个域，必须拆**（画布域控件组 4 件 + 提示词域社区库 2 件）—— 用户指出「`PromptInput` 根本就是画布上面的东西」，逐文件实测坐实（§3.1.3.5 ⑤⑥） |
...
| **S1-1c-2（类型件）** | **已完成** | ① `volumePolicy` 的 `ChatMessage`/`Conversation`/`ConversationMemory` **实测全仓零 import**（死导出）⇒ 按 **ADR-0053** 去 `export` 收私有 ⇒ **消歧零 churn**（原判"须与文件迁移同批"**被推翻**：不必迁文件，消歧只需收私有）② `agentCore.ChatMessage`→`agentChatMessage`(41/7) ③ `generate.ChatMessage`→`relayChatMessage`(7/2)；**类型改名无 `vi.mock` 风险**（type 运行时被擦除）；相关测试 6 文件 / 77 例全绿；残留旧名全为注释 |
