...
| **S1-1c-2（类型三组 + volumePolicy 迁移同批）** | **待做** | `ChatMessage`(68 处) · `Conversation`(49) · `ConversationMemory`(17) —— 因 `volumePolicy` 属 **agent 域物住错层**，按 **D20**「一批 = 一次 move 完成位置 + 名字」**须与迁移同批**，目标形态待定 |
| S1-1 … S2-7 | 待做 | — |
