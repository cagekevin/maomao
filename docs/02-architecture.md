...
└───────┬──────────────────────────────┬───────────────────────────────┬────────────┘
        ...
        ▼                               ▼                                ▼
┌──────────── localTool（本地数据层） ──────────┐   ┌── 网关 apimart-gateway ──┐   ┌── background.ts ──┐
│ Node/TS，SQLite WASM(kv/tasks/resources)       │   │ FastAPI → Lovart 转发    │   │ 右键采集/资源注入  │
│ + 磁盘 uploads/；路由 files/kv/tasks/resources │   │ 异步任务轮询 /v1/tasks   │   │ → resourceAdded   │
└──────────────────────────────────────────────┘   └────────────────────────┘   └──────────────────┘
                                                                                          │
                                                                                          ▼
                                                                              GAS 云同步（可选，跨设备）
...
1. **AI 生成**：`Jn`@L32490 → 网关异步任务 → 轮询 → 落盘 `uploads/tasks/` → `mutiwindow-task-completed` → rescan。
2. **资源采集**：`background.ts` 右键 → `resourceAdded`（跨进程）→ 前端 `Zr`@L1827 下载 → `uploads/migrated/` → 入库。
3. **画布拖入**：`onDrop`(Lr@L36243) → `ii({subfolder:'canvas/drop'})` → `POST /api/files/upload`。
4. **统一同步**：遍历 `globalTasks` → 未本地化则 `uploadFile` → `mutiwindow-task-completed` → rescan。
5. **GAS 云同步**：`ei`(push)/`ti`(pull)（同名遮蔽，见 `glossary.md`）→ POST GAS URL。
6. **重跑任务**：`mutiwindow-rerun-task` → 按 type 分派 `tr`/`er`/`Qn`/`$n`/`_r`。
