...
REM 使用系统全局 Python 3.12（不用 venv）。优先用 PATH 里的 python，找不到再用常见安装路径。
cd /d "%~dp0"
...
set "PYTHON=python"
if not exist "%PYTHON%" set "PYTHON=C:\Users\xinye\AppData\Local\Programs\Python\Python312\python.exe"
