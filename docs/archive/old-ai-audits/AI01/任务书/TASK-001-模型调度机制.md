# TASK-001（AI01）· 撰写《模型调度机制》模块专题文档

> 本文件是【任务书】，复制下方"任务 prompt"整段发给执行 AI（AI01）即可。
> 产出文件名固定为 `docs/AI01/模型调度机制.md`，执行 AI 禁止改动其他文件或其他 AI 的目录。
...
- ❌ 禁止修改、覆盖项目里任何已有文件，尤其不要动其他 AI（AI02）目录下的文件。你只【新增】本 AI 自己的产出文件。
...
- 必须把文档写入：**`docs/AI01/模型调度机制.md`**
- 禁止写入、修改、删除 `docs/AI02/` 下任何文件，也禁止改动 `docs/模块专题/` 下任何已有文件。
...
- 产出文件名为指定的 `docs/AI01/模型调度机制.md`，未触碰其他文件/目录。
