...
| **域内落点判据** | **`docs/adr/ADR-0044`** | 域根口径 · D2 优先于 D3 · 子域门面合法 · 类型件落点 |
...
| **本轮过程与证据** | `daily/架构日志/跨区-域籍判定与搬迁-2026-09-19.md` | **只作证据，不作判据**（判据去 ADR） |
...
```
1) 提交工作区（104 项）—— 一把提交；提交后 git log 核对 HEAD
2) ★ 放开所有闸（§4.1 ①→⑧，逐条：撤 → 探针先红后绿 → 真违规/误报二分 → 元层两道）
3) videoEditor(259 件) / director3d(30 件) 的**域内错位**（用户裁定：归其域内自理，本轮未审）
4) 债账本回改（`debt.mjs`）：TD-18-35/36/37/38（实测已偿/已过期/已搬）· TD-25-20（锁死留原地）· TD-18-34（描述过期）· TD-11-15（母债摘要过期）
5) DATAFLOW 回改（计划 §A9；§7 的 `agentKeys` 错前提必须回改）
