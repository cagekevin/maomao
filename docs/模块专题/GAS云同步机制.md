...
## 二、引擎 `CloudSyncEngine`（`src/App.js` ≈41729）

- 配置 `config.gasUrl = GAS_CLOUD_SYNC_URL`（`src/config.js` ≈75，一个 Google Apps Script `exec` 地址）。
...
- **push（上传/同步）** `ei()`（≈41782）：
  1. 遍历固定键集合：`['app_settings','api_configs','users','membership','projects','presetPrompts','customNodeTemplates','modelSchedules','cloud_storage_config','local_templates']`（≈41786）；
  2. 每个键取本地值；`local_templates` 特殊处理：**strip 掉 `coverUrl` 和节点里的 `imageUrl/thumbnailUrl/images/videoUrl/...Ref/...Uploaded` 等二进制字段**，只留纯 JSON 结构（≈41787-41812）；
...
- **pull（拉取/恢复）** `ti()`（≈41831）：
  ...
  2. 逐键写回本地：
     - `modelSchedules` → `Sa(n)`；
     - **`local_templates` → 合并而非覆盖**：云端模板按 `id` 去重，`localIds` 没有的才 `unshift` 进本地数组（≈41855-41866），避免覆盖本地新增；
     - 其余键 → `Q.setObject(e, n)` 直接覆盖；
  3. 写完后 `setTimeout(reload, 1e3)` 重载页面。
- **兜底 `ni()`**（≈41881，带默认参数）：用于无用户态时的初始化拉取。
