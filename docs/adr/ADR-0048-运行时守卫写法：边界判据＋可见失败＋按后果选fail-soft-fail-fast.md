...
**与静态 `check-arch` 的分工**：`check-arch` 管结构/分层（机器可判、CI 跑、`ADR-0052` 机器可判定性 + 反向判据哲学）；本 ADR 管**请求/写回/归一处的实时边界判据**。两者哲学相通（判据可机判、优先反向判据），落点不同——一个是"提交前拦结构漂移"，一个是"运行时拦数据越权"。
...
13. **开放式集合优先"结构/反向判据"而非枚举白名单**（跨 `ADR-0052`/check-arch 反向判据）。能用结构特征判定（"是否含 `_relayPoll`""是否走唯一写入口"）就别枚举。
...
- ✅ 与 `ADR-0052` 机器可判定性、`ADR-0045` 断环不造第二入口、`ADR-0016` 闸不许凌驾正确性形成"静态闸 + 运行时守卫 + 唯一入口"三层防线。
