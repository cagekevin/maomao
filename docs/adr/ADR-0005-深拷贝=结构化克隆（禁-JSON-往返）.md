...
- ⚠️ **诚实边界（2026-09-21 实测）**：`tests/unit/utils.test.ts:25` 测的是 **deepClone 的行为**（保留 Date/Map/Set、循环引用、含函数抛错），
  上列 **1、3 两条（别处改用 JSON 往返 / 出现第三份实现）它一条都不会红**。⇒ 本条**未毕业、不许判毕业**
  （毕业口径见已毕业的 `ADR-0027`/`0035`/`0036`：载体须是**闸或类型层**）。按 `ADR-0052` 第 6 级：**不能机器判定**，不许硬凑 grep。
