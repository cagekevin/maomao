# ADR-0009 · 数据生命周期·写：节点结果走声明式 resultKey 唯一路径
...
- **裁定人**：架构师（取证后自定）
- **触发**：TD-01-21
- **与 ADR-0034 的边界（2026-09-20 补）**：本条管「**生成结果回填**」（唯一路径 = 声明式 `resultKey`，hook 内 `writeBackResult`）；节点 **UI 编辑**写回归 **ADR-0034**（`setNodes` 不可变更新）。两条不冲突，别互相当对方。
