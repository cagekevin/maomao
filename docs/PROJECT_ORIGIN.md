...
`App.js` 是混淆反编译代码，函数/变量名（`ii()` `Xr()` `Zr()` `ri()` `Ev()` `Jn()` `Oa()` `er()` …）**无稳定语义**，不同次反编译可能重命名，**引用必须带行号**（如 `Ev()` 在某行、`Jn` 在某行），不能只说"改 ii()"。行号映射查 `docs/reference/var-mapping.txt` / `func-mapping.txt`（行号会漂移，以实际打开为准）。
