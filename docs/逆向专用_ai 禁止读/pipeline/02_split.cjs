...
if (!inputFile || !outputDir) {
  console.error('用法: node 02_split.cjs <输入.js> <输出目录>');
  process.exit(1);
}
...
async function run() {
  ...
  function collectRefsInPath(sp, selfName) {
    ...
    sp.traverse({
      MemberExpression(ip) {
        ...
        const o = ip.node.object,
          pr = ip.node.property;
        if (
          t.isIdentifier(o) &&
          t.isIdentifier(pr) &&
          REACT_APIS.has(pr.name) &&
          !GLOBALS.has(o.name) &&
          !reactNS.includes(o.name)
        )
          reactNS.push(o.name);
      },
      ...
      JSXOpeningElement(ip) {
        ...
        if (
          ...
          !/^[a-z]/.test(n.name) &&
          !GLOBALS.has(n.name) &&
          !jsxTags.includes(n.name)
        )
          jsxTags.push(n.name);
      },
      Identifier(ip) {
        if (
          ip.parent.type === 'MemberExpression' &&
          ip.parent.property === ip.node &&
          !ip.parent.computed
        )
          return;
        if (
          ip.parent.type === 'OptionalMemberExpression' &&
          ip.parent.property === ip.node &&
          !ip.parent.computed
        )
          return;
      ...
      progPath.get('body').forEach((sp) => {
        ...
        sp.traverse({
          ...
          CallExpression(cp) {
            ...
            if (t.isIdentifier(callee) && ['_jsx', '_jsxs', 'jsx', 'jsxs'].includes(callee.name))
              isComponent = true;
            if (
              t.isMemberExpression(callee) &&
              ['createElement', 'jsx', 'jsxs'].includes(callee.property.name)
            )
              isComponent = true;
          },
        });
        if (isComponent) {
          ...
          else if (sp.isExpressionStatement()) {
            const e = sp.node.expression;
            if (t.isAssignmentExpression(e) && e.left.name) compName = e.left.name;
          }
        }
        if (isComponent && compName) {
          components.push({
            name: compName,
            node: nodeToStore,
            ...collectRefsInPath(sp, compName),
          });
        } else {
          sharedNodes.push(nodeToStore);
          if (sp.isVariableDeclaration())
            sp.node.declarations.forEach((d) => {
              if (d.id?.name && !seenNames.has(d.id.name)) {
                sharedDeclared.add(d.id.name);
                seenNames.add(d.id.name);
              }
            });
          else if (sp.isFunctionDeclaration()) {
            if (sp.node.id?.name && !seenNames.has(sp.node.id.name)) {
              sharedDeclared.add(sp.node.id.name);
              seenNames.add(sp.node.id.name);
            }
          }
        }
      });
  ...
  function safeFilename(name) {
    const lower = name.toLowerCase(),
      ex = usedCase.get(lower);
    if (ex && ex !== name) {
      const c = usedCase.get(lower + '_count') || 1;
      const withSuffix = name + '_' + c;
      usedCase.set(lower + '_count', c + 1);
      usedCase.set(lower, name);
      return withSuffix;
    }
    usedCase.set(lower, name);
    return name;
      ...
      ...new Set(
        comp.freeIds.filter(
          (d) =>
            ...
            !rn.has(d) &&
            !compNames.has(d) &&
            !(sharedDeclared.has(d) || importedNames.has(d)),
        ),
      ),
    ];
    comp.compDeps = [
      ...new Set(
        [...comp.jsxTags, ...comp.freeIds].filter((d) => compNames.has(d) && d !== comp.name),
      ),
    ];
    comp.sharedDeps = [
      ...new Set(
        comp.freeIds.filter(
          (d) => (sharedDeclared.has(d) || importedNames.has(d)) && !compNames.has(d) && !rn.has(d),
        ),
      ),
    ];
    comp.leftDeps = [
  ...
  function emitProgram(bodyNodes, label) {
    ...
    for (const n of prog.body) {
      if (t.isImportDeclaration(n))
        for (const s of n.specifiers)
          if (s.local?.name) bound.add(s.local.name);
          else if (t.isVariableDeclaration(n))
            n.declarations.forEach((d) => d.id?.name && bound.add(d.id.name));
    ...
    try {
      ...
      fs.writeFileSync(path.join(outputDir, `fail_${label}.js`), code);
  ...
  function renameInAst(pa, renameMap) {
    traverse(pa, {
      Identifier(p) {
        ...
        if (
          t.isImportDefaultSpecifier(par) ||
          t.isImportNamespaceSpecifier(par) ||
          t.isImportDeclaration(par) ||
          t.isExportNamedDeclaration(par) ||
          t.isExportDefaultDeclaration(par)
        )
          return;
  ...
  function rewriteSharedRefs(pa, depSet, ns) {
    traverse(pa, {
      Identifier(p) {
        ...
        if (par.type === 'OptionalMemberExpression' && par.property === p.node && !par.computed)
          return;
        if (par.type === 'JSXMemberExpression' && par.property === p.node) return;
        if (
          t.isImportSpecifier(par) ||
          t.isExportSpecifier(par) ||
          t.isImportDeclaration(par) ||
          t.isExportNamedDeclaration(par)
        )
          return;
        ...
        if (
          (par.type === 'ObjectMethod' ||
            par.type === 'ClassProperty' ||
            par.type === 'ClassMethod' ||
            par.type === 'ClassPrivateProperty') &&
          par.key === p.node &&
          !par.computed
        )
          return;
        if (t.isLabeledStatement(par) || t.isBreakStatement(par) || t.isContinueStatement(par))
          return;
  ...
  function collectWritten(node, sdSet, out) {
    ...
    if (Array.isArray(node)) {
      for (const c of node) collectWritten(c, sdSet, out);
      return;
    }
  ...
  for (const comp of components) {
    ...
    for (const c of comp.compDeps) {
      const safe = nameMap[c],
        alias = `_cmp_${safe}`;
      renameMap[c] = alias;
      importStmts.push(
        t.importDeclaration(
          [t.importDefaultSpecifier(t.identifier(alias))],
          t.stringLiteral(`./${safe}.jsx`),
        ),
      );
    }
    // 修复 A：shared 引用
    ...
    if (namedShared.length)
      importStmts.push(
        t.importDeclaration(
          namedShared.map((d) => t.importSpecifier(t.identifier(d), t.identifier(d))),
          t.stringLiteral('./shared.js'),
        ),
      );
    if (writtenSet.size)
      importStmts.push(
        t.importDeclaration(
          [t.importNamespaceSpecifier(t.identifier('_shared'))],
          t.stringLiteral('./shared.js'),
        ),
      );
    ...
    for (const X of comp.reactDeps) {
      importStmts.push(
        t.importDeclaration(
          [t.importNamespaceSpecifier(t.identifier(X))],
    ...
    if (comp.reactBare.length)
      importStmts.push(
        t.importDeclaration(
          comp.reactBare.map((n) => t.importSpecifier(t.identifier(n), t.identifier(n))),
          t.stringLiteral('react'),
        ),
      );

    const body = [...importStmts.map((n) => t.cloneNode(n, true))];
    if (t.isFunctionDeclaration(comp.node))
      body.push(t.exportDefaultDeclaration(t.cloneNode(comp.node, true)));
    else {
      body.push(t.cloneNode(comp.node, true));
      body.push(t.exportDefaultDeclaration(t.identifier(comp.name)));
    }
    ...
    if (comp.leftDeps.length)
      code = `// TODO(全局, 无需 import): ${comp.leftDeps.join(', ')}\n` + code;
    let formatted;
    try {
  ...
  const sharedCompDeps = new Set();
  {
    ...
    traverse(scanAst, {
      Identifier(p) {
        const n = p.node.name;
        if (compNames.has(n) && !p.scope.hasBinding(n)) sharedCompDeps.add(n);
      },
    });
  }
  const sharedImportStmts = [],
    sharedRename = {};
  for (const c of sharedCompDeps) {
    const safe = nameMap[c],
      alias = `_cmp_${safe}`;
    sharedRename[c] = alias;
    sharedImportStmts.push(
      t.importDeclaration(
        [t.importDefaultSpecifier(t.identifier(alias))],
        t.stringLiteral(`./${safe}.jsx`),
      ),
  ...
  for (const node of sharedNodes) {
    if (t.isExportNamedDeclaration(node) && node.specifiers)
      for (const s of node.specifiers) originalExported.add(s.exported?.name || s.exported?.value);
    if (t.isExportNamedDeclaration(node) && node.declaration) {
      const d = node.declaration;
      if (t.isVariableDeclaration(d))
        d.declarations.forEach((dd) => originalExported.add(dd.id?.name));
      else if ((t.isFunctionDeclaration(d) || t.isClassDeclaration(d)) && d.id)
        originalExported.add(d.id.name);
    }
  ...
  for (const node of sharedNodes) {
    if (t.isImportDeclaration(node))
      for (const s of node.specifiers)
        if (s.local?.name) sharedBound.add(s.local.name);
        else if (t.isVariableDeclaration(node))
          node.declarations.forEach((d) => d.id?.name && sharedBound.add(d.id.name));
  ...
  const sharedBody = [
    ...sharedImportStmts.map((n) => t.cloneNode(n, true)),
    ...sharedNodes.map((n) => t.cloneNode(n, true)),
  ];
  ...
  for (const n of sharedBody) {
    if (t.isImportDeclaration(n))
      for (const s of n.specifiers)
        if (s.local?.name) realBound.add(s.local.name);
        else if (t.isVariableDeclaration(n))
          n.declarations.forEach((d) => {
            if (d.id?.name) {
              realBound.add(d.id.name);
              declaredBound.add(d.id.name);
            }
          });
        else if (t.isFunctionDeclaration(n) && n.id?.name) {
          realBound.add(n.id.name);
          declaredBound.add(n.id.name);
        } else if (t.isClassDeclaration(n) && n.id?.name) {
          realBound.add(n.id.name);
          declaredBound.add(n.id.name);
        }
  }
  ...
  const exportStmt = exportSet.size
    ? t.exportNamedDeclaration(
        null,
        [...exportSet].map((n) => t.exportSpecifier(t.identifier(n), t.identifier(n))),
      )
    : null;
  ...
  let sharedFormatted;
  try {
    sharedFormatted = await prettier.format(sharedCode, {
      parser: 'babel',
      printWidth: 100,
      tabWidth: 2,
      singleQuote: true,
