...
if (!ROOT) {
  console.error('用法: node fix_esm.cjs <工程根目录>');
  process.exit(1);
}
...
function isSharedFile(p) {
  return /(^|[\\/])shared\.jsx?$/.test(p);
}

// ---------- C. 清理 webcrack [native code] 伪迹（兼容空格/内嵌形式）----------
function cleanArtifacts(code) {
  return code.replace(
    /function\s+([A-Za-z_$][\w$]*)\s*\(\s*\)\s*\{\s*\[\s*native code\s*\]\s*\}/g,
    '$1',
  );
}
...
function exportSync(code) {
  ...
  for (const stmt of ast.program.body) {
    if (t.isVariableDeclaration(stmt)) {
      ...
      for (const s of stmt.specifiers)
        if (s.local && t.isIdentifier(s.local)) topNames.add(s.local.name);
    }
  }
    ...
    if (t.isExportNamedDeclaration(stmt)) {
      ...
      if (stmt.declaration) {
        if (t.isFunctionDeclaration(stmt.declaration) && stmt.declaration.id)
          existing.add(stmt.declaration.id.name);
          ...
          for (const d of stmt.declaration.declarations)
            if (d.id && t.isIdentifier(d.id)) existing.add(d.id.name);
      }
    }
  ...
  let keep = null;
  for (const stmt of ast.program.body) {
    if (t.isExportNamedDeclaration(stmt) && !stmt.declaration && stmt.specifiers) {
      keep = stmt;
      break;
    }
  }
  if (!keep) {
    const gen = generate(
      t.exportNamedDeclaration(
        null,
        toAdd.map((n) => t.exportSpecifier(t.identifier(n), t.identifier(n))),
      ),
    ).code;
    return code + '\n' + gen + '\n';
  }
  ...
  const seen = new Set();
  keep.specifiers = keep.specifiers.filter((s) => {
    const n = s.exported.name;
  ...
  for (const stmt of ast.program.body) {
    if (t.isVariableDeclaration(stmt))
      for (const d of stmt.declarations)
        if (d.id && t.isIdentifier(d.id)) set.add(d.id.name);
          ...
          for (const s of stmt.specifiers)
            if (s.local && t.isIdentifier(s.local)) set.add(s.local.name);
  }
  return set;
}
...
function fixImportAssignments(code) {
  const ast = parseCode(code);
  const sharedImports = ast.program.body.filter(
    (s) => t.isImportDeclaration(s) && /shared\.jsx?$/.test(s.source.value),
  );
  ...
  traverse(ast, {
    AssignmentExpression(p) {
      const l = p.node.left;
      if (t.isIdentifier(l)) assigned.add(l.name);
    },
    UpdateExpression(p) {
      const a = p.node.argument;
      if (t.isIdentifier(a)) assigned.add(a.name);
    },
  });
  ...
  for (const imp of sharedImports) {
    ...
    const existingNs = ast.program.body.find(
      (s) =>
        t.isImportDeclaration(s) &&
        s.source.value === imp.source.value &&
    ...
    if (existingNs) {
      ...
      ast.program.body.unshift(
        t.importDeclaration(
          [t.importNamespaceSpecifier(t.identifier(nsLocal))],
          t.stringLiteral(imp.source.value),
        ),
      );
    }

    imp.specifiers = imp.specifiers.filter(
      (s) => !(t.isImportSpecifier(s) && nsNames.has(s.local.name)),
    );
    ...
    traverse(ast, {
      Identifier(p) {
        ...
        if (p.isReferencedIdentifier()) {
          p.replaceWith(t.memberExpression(t.identifier(nsLocal), t.identifier(name)));
          return;
        }
        const par = p.parentPath;
        if (par.isAssignmentExpression() && par.node.left === p.node) {
          p.replaceWith(t.memberExpression(t.identifier(nsLocal), t.identifier(name)));
          return;
        }
        if (par.isUpdateExpression() && par.node.argument === p.node) {
          p.replaceWith(t.memberExpression(t.identifier(nsLocal), t.identifier(name)));
          return;
        }
      },
      ...
      if (t.isExportNamedDeclaration(stmt)) {
        ...
        if (stmt.declaration) {
          if (t.isFunctionDeclaration(stmt.declaration) && stmt.declaration.id)
            ...
            for (const d of stmt.declaration.declarations)
              if (d.id && t.isIdentifier(d.id)) names.add(d.id.name);
        }
      }
      ...
      if (t.isVariableDeclaration(stmt))
        for (const d of stmt.declarations)
          if (d.id && t.isIdentifier(d.id)) names.add(d.id.name);
            ...
            for (const s of stmt.specifiers)
              if (s.local && t.isIdentifier(s.local)) names.add(s.local.name);
    }
    return names;
  } catch (e) {
    return null;
...
function fixDanglingImports(code, sharedExports, dir) {
  ...
  try {
    ...
    if (fs.existsSync(mp)) {
      ...
      compKeys = new Set(Object.keys(m));
    }
  } catch (e) {}
  ...
  for (const n of [...missing]) {
    ...
    if (fs.existsSync(path.join(dir, n + '.jsx')) || fs.existsSync(path.join(dir, n + '.js')))
      missing.delete(n);
  }
  if (missing.size === 0) return code;
  let imp = ast.program.body.find(
    (s) => t.isImportDeclaration(s) && /shared\.jsx?$/.test(s.source.value),
  );
  if (imp) {
    const have = new Set(
      imp.specifiers.filter((s) => t.isImportSpecifier(s)).map((s) => s.imported.name),
    );
    for (const n of missing)
      if (!have.has(n)) imp.specifiers.push(t.importSpecifier(t.identifier(n), t.identifier(n)));
  } else {
...
function fixExtractedComponentRefs(code, fp) {
  ...
  let cmap;
  try {
    cmap = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
  } catch (e) {
    return code;
  }
  const targets = [];
  ...
  for (const stmt of ast.program.body) {
    if (t.isImportDeclaration(stmt) && /shared\.jsx?$/.test(stmt.source.value)) {
      const filtered = stmt.specifiers.filter(
        (s) => !(t.isImportSpecifier(s) && refs.has(s.imported.name)),
      );
      ...
      if (stmt.specifiers.length === 0) {
        const i = ast.program.body.indexOf(stmt);
        if (i >= 0) ast.program.body.splice(i, 1);
      }
    }
  }
  ...
  for (const name of refs.keys()) {
    ...
    const exist = ast.program.body.some(
      (s) =>
        t.isImportDeclaration(s) &&
        s.specifiers.some((sp) => t.isImportDefaultSpecifier(sp) && sp.local.name === tg.local),
    );
    if (!exist) {
      ...
      ast.program.body.splice(
        ...
        0,
        t.importDeclaration(
          [t.importDefaultSpecifier(t.identifier(tg.local))],
          t.stringLiteral('./' + tg.file + '.jsx'),
        ),
      );
    }
    need.add(name);
  ...
  traverse(ast, {
    JSXIdentifier(p) {
      ...
      if (p.parentPath.isJSXMemberExpression() && p.parentPath.node.property === p.node) return;
      if (p.parentPath.isJSXMemberExpression() && p.parentPath.node.object === p.node) {
        p.node.name = origToTarget.get(name).local;
        changed = true;
        return;
      }
      if (p.parentPath.isJSXOpeningElement() || p.parentPath.isJSXClosingElement()) {
...
walk(ROOT, (fp) => {
  ...
  try {
    code = fixConstructorArtifact(code);
  } catch (e) {
    console.log(`  ⚠️ constructor 还原跳过: ${fp} (${e.message.split('\n')[0]})`);
  }
  if (isShared) {
    try {
      code = exportSync(code);
    } catch (e) {
      console.log(`  ⚠️ 导出同步跳过: ${fp} (${e.message.split('\n')[0]})`);
    }
  } else {
    try {
      code = fixExtractedComponentRefs(code, fp);
    } catch (e) {
      console.log(`  ⚠️ 组件引用改写跳过: ${fp} (${e.message.split('\n')[0]})`);
    }
    ...
    if (se) {
      try {
        ...
        console.log(`  ⚠️ 悬空补全跳过: ${fp} (${e.message.split('\n')[0]})`);
      }
    }
  }
  try {
    code = fixImportAssignments(code);
  } catch (e) {
    console.log(`  ⚠️ 改写跳过: ${fp} (${e.message.split('\n')[0]})`);
  }
  if (code !== before) {
    fs.writeFileSync(fp, code);
    if (isShared) syncCount++;
    else fixCount++;
  }
});
