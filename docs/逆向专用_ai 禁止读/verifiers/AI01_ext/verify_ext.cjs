...
const DIST = process.env.EXT_PATH
  ? path.resolve(process.env.EXT_PATH)
  : path.resolve(ROOT, 'output', 'project', 'dist');
...
(async () => {
  ...
  if (!extId) {
    try {
      ...
      console.log('target dump failed', e.message);
    }
  }
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ extId, events, errorCount: errors.length }, null, 2),
  );
