// 审计 AI02.md：提取 App.js 中每个缩写函数的真实定义行，与表格声明对比
const fs = require('fs');
const path = require('path');

const appJs = path.join(__dirname, '..', 'src', 'App.js');
const ai02 = path.join(__dirname, '..', 'docs', 'ai-tasks', 'AI02.md');
const src = fs.readFileSync(appJs, 'utf8');
const lines = src.split('\n');
const md = fs.readFileSync(ai02, 'utf8');
...
// 用首字母大写启发式抽取 AI02 给出的语义名（结果列第一个 token，去掉 (?) 等）
...
console.log('idx | name | claimed | actualDefLines | semanticGuess | delta | firstDefLineSnippet');
console.log('---');
for (const r of rows) {
  ...
  console.log(`${r.idx} | ${r.name} | L${r.claimedLine} | [${defLines}] | ${guess} | ${delta} | ${snippet}`);
}
