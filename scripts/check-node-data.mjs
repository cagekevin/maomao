 ...
 *   Q1 守什么：**结构偏好闸**（`node.data` 形状对账 = 数据契约治理，非物理红线）。
 *   Q2 何时该改：`RESULT_EXEMPT` 出现**第 3 条同型豁免** → 说明判据（`isResultish` 启发式 / 读侧已知字段集）
 *               该改，而不是继续加豁免；`--strict` 长期红 → 是**真源表**（`NODE_OUTPUTS` 等三张表）缺项。
 *   Q3 怎么改：优先改真源 `canvas/nodeDataSchema.ts`（新建默认值）+ `useConnectedInputs` 的三张写侧表；
 *               `RESULT_EXEMPT`（**本闸内清单**）**只收窄**（它已有"必须带原因 + 过期自检"的缓解，保持）。
 *
 * 边界（诚实标注，避免误读）：
