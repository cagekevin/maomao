 ...
 *   Q1 守什么：**红线闸** —— `apiRegistry` 是前后端唯一契约真源（CLAUDE §五.4 字符串契约零损伤）。
 *   Q2 何时该改：新增/退役端点 → 改**真源两侧**（本闸只做对账，不承载业务判断）；
 *               若某类端点长期只能判 `info`（"信封形态无法静态判定"）→ 该改**判定口径**，不是加豁免。
 *   Q3 怎么改：改 `src/components/base/core/contracts.ts::apiRegistry` + `localTool/src/router.ts`（真源），
 *               再跑 `npm run check:api` 双向自证；形态豁免清单（`stream/sse/raw/probe/stub`）**只收窄不放宽**。
 *
