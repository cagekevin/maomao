...
// ── 3. 跑 tsc（只读，扫副本）──
const res = spawnSync('npx', ['tsc', '--noEmit', '-p', 'tmp/tsconfig.json'], { cwd: root, encoding: 'utf8', shell: true })
...
const byCode = {}
errorLines.forEach(l => { const c = l.match(/error TS(\d+)/); if (c) byCode[c[1]] = (byCode[c[1]] || 0) + 1 })
...
console.log('错误码分布: ' + Object.entries(byCode).sort((a, b) => b[1] - a[1]).map(([c, n]) => `TS${c}×${n}`).join('  '))

// 按错误数降序排 tests 文件
const sorted = testFiles.map(([k, v]) => ({ file: rawToNorm[k], ...v })).sort((a, b) => b.count - a.count)
console.log('\n── tests/unit 各文件错误数（降序，前 40）──')
sorted.slice(0, 40).forEach((f, i) => {
  const top = Object.entries(f.codes).sort((a, b) => b[1] - a[1]).map(([c, n]) => `TS${c}×${n}`).join(', ')
...
if (srcFiles.length) {
  ...
  srcFiles.map(([k, v]) => ({ file: rawToNorm[k], ...v })).sort((a, b) => b.count - a.count)
...
if (values.export) {
  const lines = [
    ...
    '| 错误码 | 数量 |',
    '|---|---|',
    ...Object.entries(byCode).sort((a, b) => b[1] - a[1]).map(([c, n]) => `| TS${c} | ${n} |`),
    '',
    '## tests/unit 各文件（错误数降序）',
    '',
    '| 文件 | 错误数 | 错误码分布 |',
    '|---|---|---|',
    ...sorted.map(f => `| \`${f.file}\` | ${f.count} | ${Object.entries(f.codes).sort((a, b) => b[1] - a[1]).map(([c, n]) => `TS${c}×${n}`).join(', ')} |`),
    '',
    srcFiles.length
