 ...
 *
 * 【★闸的申诉口 · 每个闸都应回答的三问（2026-09-14 入规 → 架构师心法 §零.4）】
 *   Q1 **这道闸守什么**？—— 红线闸（机器化物理契约 / 唯一入口 / 安全 → 不可绕、不可自行放宽）
 *                           还是**结构偏好闸**（分层 / 目录白名单 / 命名 = 默认值 → 可被"正确性/复杂度"证据推翻）。
 *   Q2 **什么时候应该改它**？—— 判据：它拦住了"让同一语义的**份数下降**"的动作（收口 / 下沉 / 去重）→ 该改。
 ...
 *   ▶ 本文件答案：**结构偏好闸**（分层边界，非 CLAUDE 级物理契约）。改它的先例见规则 2 与规则 4 的两次
 *     「清单 → 反向判据」；规则 4 的【★改】段就是本条款的实证 —— 原「文件级白名单」把 `core/` 逼成内联重写 7 遍
 *     （文件头写着"严禁在此重写"，正文却只能重写 = **闸把作者逼成了它禁止的样子**）。
 *
