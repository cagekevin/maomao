...
function extractImportAbs(code, filepath) {
  const out = []
  try {
    const ast = parse(code, { sourceType: 'unambiguous', plugins: ['jsx', 'typescript', 'decorators-legacy'], errorRecovery: true })
    const walk = (node) => {
      if (!node) return
      ...
      for (const k in node) {
        if (k !== 'loc' && k !== 'range' && typeof node[k] === 'object' && node[k] !== null) walk(node[k])
      }
    }
    walk(ast.program)
  } catch (e) {
...
function resolveSourceFile(abs) {
  ...
  for (const e of EXTS) { const c = stem + e; try { if (statSync(c).isFile()) return c } catch {} }
  return null
}
...
function dfs(node) {
  ...
  for (const dep of graph.get(node) || []) {
    ...
    if (c === 1) {
      ...
      const cycle = stack.slice(idx).concat(dep).map((x) => x.slice(root.length + 1))
