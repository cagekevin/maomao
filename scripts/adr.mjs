 ...
 * 用法（写 / 维护）：
 *   node scripts/adr.mjs add --title "…" --conclusion "…" --q6 "<AI 对 6 问的逐条回答>" --user-approved
 *                           [--status 生效] [--decider 用户|架构师]
 *                           [--trigger "…"] [--date YYYY-MM-DD] [--supersedes NNNN] [--dry]
 *                           # ⚠️ `--q6` + `--user-approved` **真落盘时两者都必填**（`--dry` 不要求）：
 *                           #    缺任一 ⇒ 拒绝落盘 exit 2。六问的**答卷人是 AI**（逐条答），
 *                           #    用户的角色只有**同意 / 不同意** ⇒ 必须先由 AI 逐条回答这 6 个问题，
 *                           #    并把答卷呈给用户 —— 用户只会说"同意"或"不同意"。（2026-09-20 立）
...
function cmdAdd() {
  ...
  console.log(`
   ...
   ⚠️ 本打印**只是提醒，不做机器校验** —— 别把"命令跑过了"当成"六问答过了"。
   🔴 **必须先由 AI 逐条回答这 6 个问题，并把答卷呈给用户 —— 用户只会说"同意"或"不同意"。**
`);
  ...
  // 【落盘门槛（2026-09-20 立）】六问的**答卷人是 AI**，用户的角色只有**同意 / 不同意** ⇒ 两样都要：
  //   `--q6 "<AI 对 6 问的逐条回答>"` + `--user-approved`（用户已明确说"同意"）。缺任一 ⇒ **拒绝落盘**。
  //   【为什么不是"再打印一遍"】打印挡不住"打了不看"。2026-09-20 实证：命令跑过、六问从眼前过去、
  //     我没答就落了 ADR-0050（前提错：把一次性能妥协升格成架构维度），当场 `rm` 撤回。
  //     ⇒ 改成**写入口校验**（同 `debt.mjs` 的"摘要 >120 字即拒"），把"忘了问"变成"必须主动声明答过、并拿到同意"。
  //   【为什么不是闸】不进 `gates.manifest.json`、不挂 git hook ⇒ **不过「建闸前置评审」**。
  //   【为什么合法成本更低】合法 = 逐条答 + 呈给用户拿同意；绕行 = 编一段假答卷 + 假同意写进命令 ⇒ 可归责。
  //   【诚实边界】它拦不住"蓄意撒谎"，也拦不住"答卷敷衍"（只校验非空）—— 故末行仍打那句强制话。
  const q6 = val('--q6');
  const userApproved = flag('--user-approved');
  if (!q6 || !q6.trim() || !userApproved) {
    console.log(`
   ❌ 拒绝落盘：需要 \`--q6 "<AI 对 6 问的逐条回答>"\` **且** \`--user-approved\`。
      这 6 问的**答卷人是 AI**（你）—— **逐条答**，别写"都过了"（那不算答）。
      答完把答卷**呈给用户**；用户只会说「同意」或「不同意」——
      拿到「同意」才加 \`--user-approved\`，缺任一项都落不了盘。
   🔴 **必须先由 AI 逐条回答这 6 个问题，并把答卷呈给用户 —— 用户只会说"同意"或"不同意"。**
`);
    process.exit(2);
  }
