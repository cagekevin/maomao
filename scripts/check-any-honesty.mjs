 ...
 *   Q1 守什么：**红线闸（零白名单）** —— 类型诚实性（CONTEXT §三 + TD-09-1）。文件头已定调
 *               "不留只守部分目录的假护栏"（本仓批过的形态）→ **本闸是"零豁免面"的健康样本**。
 *   Q2 何时该改：**不为它加豁免**。若某处 `as any` 被判红而其语义确属合法收窄 → 该改的是**代码**
 *               （补运行时守卫 / 写真实类型），不是改闸。
 *   Q3 怎么改：确需调判定规则时，必须同时说明"为什么这不是把问题藏起来"；
 *               **禁止新增白名单/目录豁免**（那会把它从健康样本变成成本倒挂源）。
 *
