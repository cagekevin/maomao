 ...
 *   Q1 守什么：**正确性闸（零豁免口）** —— 守"可编译性"。它既非红线闸、也非结构偏好闸，
 *               且**不存在"改它换绿灯"的通道**（过不了就是语法错，绕不过去）→ 结构上免疫成本倒挂。
 *   Q2 何时该改：几乎不改。仅在扩展支持范围（新扩展名 / 新 loader）时同步。
 *   Q3 怎么改：改收集范围（`SCAN_EXTS` / 收集分支）与 esbuild `loader`；**不引入豁免清单**。
 *
 * 用法：
