...
var c_ = Z.memo(({ id: e, data: t, selected: n }) => {
  let { updateNodeData: r, setEdges: i, getNodes: a } = We();
  let o = it();
  let c = nn();
  let l = Vt((e) => {
...
`,
  ...
  let $e = (e, t) => {
    ...
    return (
      <Component2416
        className={`absolute z-50 top-full ${E && (t === `motion` || t === `sound`) ? `right-0` : `left-0`} mt-1 w-56 rounded-lg border border-[#3a3a3a] bg-[#202020] p-2 shadow-xl nowheel nopan nodrag space-y-2`}
        onWheel={(e) => {
      ...
      >
        ...
        {t === `duration` && (
          <Component2408 className={`flex flex-wrap gap-1`}>
            {t_.map((t) => {
              return (
                <Component2407
                  className={`px-2 py-1 rounded text-[11px] nodrag ${n === t ? `bg-gray-100 text-gray-950` : `bg-[#2a2a2a] text-gray-300 hover:bg-[#333]`}`}
                  onClick={() => {
                    return He(e.id, {
                      duration: t,
                    });
        ...
        {t === `duration` ? (
          <Component2409
            ...
            placeholder={Qe(t)}
            className={`w-full rounded bg-[#262626] border border-[#3a3a3a] px-2 py-1.5 text-[11px] text-gray-200 outline-none`}
          />
        ) : (
          <Component2410
            autoFocus={true}
            ...
            placeholder={Qe(t)}
            className={`w-full rounded bg-[#262626] border border-[#3a3a3a] px-2 py-1.5 text-[11px] text-gray-200 outline-none resize-none`}
          />
        )}
        {t === `motion` && (
          <Component2413 className={`flex flex-wrap gap-1`}>
            {n_.map((t) => {
              return (
                <Component2411
                  className={`px-2 py-1 rounded text-[11px] bg-[#2a2a2a] text-gray-300 hover:bg-[#333] nodrag border border-[#3a3a3a]`}
                  onClick={() => {
                    let r = n ? `${n}，${t}` : t;
                    He(e.id, {
                      motion: r,
            ...
            <Component2412
              className={`w-full mt-1 text-[10px] text-gray-500`}
            >{`点击预设直接填入`}</Component2412>
          </Component2413>
        )}
        <Component2415 className={`flex justify-end mt-1`}>
          <Component2414
            className={`px-3 py-1.5 rounded-lg bg-gray-100 text-gray-950 font-medium text-[11px] hover:bg-white transition-colors`}
            onClick={() => {
              return R(null);
            }}
          >{`保存`}</Component2414>
    ...
    return (
      ...
      >
        <Component2417
          ...
          title={`双击编辑`}
          data-placeholder={r}
          className={`${n} whitespace-pre-wrap break-words cursor-text empty:before:content-[attr(data-placeholder)] empty:before:text-gray-600`}
        >
          {i ? _cmp_a_(i, p) : null}
        </Component2417>
      </Component2424>
...
`);
  ...
  let _Component116 = ({ idx: e }) => {
    return (
      <Component2425
        className={`absolute top-0 right-0 h-full w-1 cursor-col-resize hover:bg-white/30 nodrag`}
        onMouseDown={(t) => {
          return ut(e, t);
        }}
      />
  ...
  let ft = (e, t) => {
    return (
      <Component2470
        className={`relative border-t border-[#2a2a2a] hover:bg-[#202020] min-h-[88px]`}
        key={e.id}
      >
        <Component2469
          className={`grid ${t ? `grid-cols-[44px_92px_minmax(220px,1.6fr)_96px_120px_150px_110px_96px_34px]` : ``} items-stretch min-h-[88px]`}
          style={
            t
              ? undefined
              : {
        ...
        >
          ...
          <Component2429 className={`relative flex items-center gap-1 px-1.5 py-4`}>
            <Component2427
              ...
              max={60}
              step={1}
              value={Math.max(1, Number.parseInt(e.duration || `5`, 10) || 5)}
              onChange={(t) => {
                return He(e.id, {
          ...
          <Component2430 className={`relative`}>
            {rt(
              e,
              `description`,
              `w-full h-full bg-transparent text-xs text-gray-300 outline-none resize-none px-2 py-5 leading-relaxed focus:bg-[#262626] rounded min-h-[72px] overflow-hidden`,
              `画面描述，输入 @ 引用资产`,
            )}
          </Component2430>
          <Component2435 className={`relative`}>
            <Component2432
              className={`w-full h-full bg-transparent text-xs text-gray-300 outline-none resize-none px-2 py-5 leading-relaxed focus:bg-[#262626] rounded nodrag text-left flex items-center justify-between`}
              onClick={(t) => {
                t.stopPropagation();
                F(P === e.id ? null : e.id);
                j(null);
            ...
            {P === e.id && (
              <Component2434
                className={`absolute z-50 top-full left-0 mt-1 w-28 max-h-52 overflow-y-auto rounded-lg border border-[#3a3a3a] bg-[#202020] p-1 shadow-xl nowheel nopan nodrag`}
                onWheel={(e) => {
                  return e.stopPropagation();
                }}
                onClick={(e) => {
              ...
              >
                {e_.map((t) => {
                  return (
                    <Component2433
                      role={`button`}
                      className={`px-2 py-1.5 rounded text-[11px] cursor-pointer ${e.shotType === t ? `bg-[#333] text-white` : `text-gray-300 hover:bg-[#2a2a2a]`}`}
                      onClick={() => {
                        He(e.id, {
                          shotType: t,
                        });
          ...
          {t && (
            <Component2438 className={`relative`}>
              <Component2437
                className={`w-full h-full bg-transparent text-xs text-gray-300 outline-none resize-none px-2 py-5 leading-relaxed focus:bg-[#262626] rounded nodrag text-left truncate`}
                onClick={(t) => {
                  t.stopPropagation();
                  Xe(e.id, `lighting`);
                }}
          ...
          {t && (
            <Component2460 className={`relative`}>
              <Component2445
                className={`w-full h-full bg-transparent text-xs text-gray-300 outline-none resize-none px-2 py-5 leading-relaxed focus:bg-[#262626] rounded nodrag text-left truncate`}
                onClick={(t) => {
                  t.stopPropagation();
              ...
              >
                {e.dialogue ? (
                  <Component2443 className={`flex flex-col gap-1`}>
                    {at(e.dialogue).map((e, t) => {
                      return (
                        <Component2442 className={`text-gray-300 truncate text-[10px]`} key={t}>
                          {e.kind === `旁白` ? (
                            <Component2439 className={`text-gray-200`}>
                              {`[旁白] `}
                              {e.text}
                            </Component2439>
                          ) : (
                  ...
                  <Component2444 className={`text-gray-600`}>{`对白/旁白`}</Component2444>
                )}
              </Component2445>
              {I === e.id && (
                <Component2459
                  className={`absolute z-50 top-full left-0 mt-1 w-[360px] rounded-lg border border-[#3a3a3a] bg-[#202020] p-3 shadow-xl nowheel nopan nodrag space-y-2`}
                  onWheel={(e) => {
                    return e.stopPropagation();
                  }}
                  onClick={(e) => {
                ...
                >
                  {(() => {
                    ...
                    return (
                      <Component2456 className={`space-y-2`}>
                        {t.map((n, r) => {
                          return (
                            <Component2452 className={`flex items-center gap-2`} key={r}>
                              <Component2446
                                type={`button`}
                                className={`w-[52px] shrink-0 rounded border border-transparent px-1 py-1 text-[10px] outline-none nodrag ${n.kind === `旁白` ? `bg-purple-500/20 text-purple-300` : `bg-cyan-500/20 text-cyan-400`}`}
                                onClick={() => {
                                  let i = [...t];
                              ...
                              {n.kind === `台词` && (
                                ...
                                >
                                  ...
                                  {h.map((e) => {
                                    return (
                                      <Component2448 value={e.name} key={e.id}>
                                        {e.name}
                                      </Component2448>
                                    );
                                  })}
                              ...
                              <Component2450
                                ...
                                placeholder={n.kind === `旁白` ? `旁白内容` : `台词内容`}
                                className={`flex-1 rounded bg-[#1a1a1a] border border-[#3a3a3a] px-2 py-1.5 text-[11px] text-gray-200 outline-none resize-none min-h-[28px] nodrag`}
                              />
                              <Component2451
                                className={`p-1 text-gray-500 hover:text-red-400 shrink-0 nodrag mt-0.5`}
                                onClick={() => {
                                  let n = [...t];
                                  n.splice(r, 1);
                                  He(e.id, {
                        ...
                        <Component2455 className={`flex items-center gap-2 pt-1`}>
                          <Component2453
                            className={`px-2 py-1 rounded bg-[#2a2a2a] text-gray-300 hover:bg-[#333] text-[10px] flex items-center gap-1 nodrag`}
                            onClick={() => {
                              return He(e.id, {
                                dialogue: ot([
                                  {
          ...
          {t && (
            <Component2463 className={`relative`}>
              <Component2462
                className={`w-full h-full bg-transparent text-xs text-gray-300 outline-none resize-none px-2 py-5 leading-relaxed focus:bg-[#262626] rounded nodrag text-left truncate`}
                onClick={(t) => {
                  t.stopPropagation();
          ...
          <Component2468 className={`px-1 py-5 flex items-center justify-center`}>
            <Component2467
              className={`p-1 text-gray-500 hover:text-red-400 nodrag`}
              onClick={(t) => {
                t.stopPropagation();
                Ge(e.id);
              }}
  ...
  let pt = Z.useMemo(() => {
    return (d.drawingModelForScript || ``)
      .split(
        `
`,
      )
  ...
  let xt = () => {
    ...
    let i = (e, t) => {
      ...
      return (
        <Component2474
          role={`button`}
          className={`w-full flex items-center gap-1.5 mb-1 last:mb-0 text-left px-2 py-1.5 text-[11px] rounded-md transition-colors cursor-pointer ${gt === e ? `bg-[#333] text-white` : `text-gray-400 hover:bg-[#2a2a2a] hover:text-gray-200`}`}
          onClick={() => {
            return r(e);
          }}
          title={e}
        ...
        >
          <Component2471
            className={`shrink-0 px-1 rounded text-[9px] leading-[14px] border ${t ? `bg-white/10 text-white/90 border-white/30` : `bg-gray-500/15 text-gray-400 border-gray-500/40`}`}
          >
            {t ? `内置` : `三方`}
          </Component2471>
    ...
    return (
      ...
      >
        <Component2479
          className={`flex items-center gap-1 h-7 px-2 bg-[#262626] hover:bg-[#2a2a2a] border border-[#3a3a3a] hover:border-[#555] rounded-lg text-[11px] text-gray-300 transition-colors cursor-pointer max-w-[200px]`}
          onClick={(e) => {
            e.stopPropagation();
            q((e) => {
              return !e;
        ...
        {je && (
          <Component2495
            className={`absolute bottom-full right-0 mb-1 min-w-[17rem] w-max max-w-[29rem] bg-[#222] border border-[#333] rounded-lg shadow-xl p-2 z-50 max-h-72 overflow-y-auto custom-scrollbar nowheel nopan nodrag`}
            onClick={(e) => {
              return e.stopPropagation();
            }}
            onWheel={(e) => {
          ...
          >
              ...
              <Q.Fragment>
                ...
                {Ne.map((e) => {
                  ...
                  return (
                    <Component2486
                      role={`button`}
                      className={`w-full flex items-center gap-1.5 mb-1 last:mb-0 text-left px-2 py-1.5 text-[11px] rounded-md transition-colors cursor-pointer ${gt === t ? `bg-[#333] text-white` : `text-gray-400 hover:bg-[#2a2a2a] hover:text-gray-200`}`}
                      onClick={() => {
                        return r(t);
                      }}
                      title={`${e.name}（${e.steps.length} 个模型按序重试）`}
                      key={e.id}
                    >
                      <Component2483
                        className={`shrink-0 px-1 rounded text-[9px] leading-[14px] border bg-blue-500/20 text-blue-200 border-blue-400/40`}
                ...
                {(t.length > 0 || n.length > 0) && (
                  <Component2487 className={`h-px bg-[#333] my-1.5`} />
                )}
              </Q.Fragment>
            )}
            {t.length > 0 && (
            ...
            {e.length === 0 && Ne.length === 0 && (
              <Component2494
                className={`px-2 py-2 text-[11px] text-gray-600`}
              >{`请先在设置中配置生图模型`}</Component2494>
            )}
          </Component2495>
        )}
      </Component2496>
      ...
      <Component2533>
        <Component2500 className={`flex items-center justify-end mb-2 gap-2`}>
          <Component2499 className={`flex items-center gap-2`}>
            {xt()}
            <Component2497
              className={`inline-flex items-center gap-1 rounded-lg bg-[#2a2a2a] px-3 py-1.5 text-xs font-medium text-gray-200 hover:bg-[#333] nodrag nopan shrink-0`}
              title={`上传全部已生成图片资产到视频网关`}
              onClick={(t) => {
                t.stopPropagation();
            ...
            <Component2498
              className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-gray-100 text-gray-950 text-xs font-medium hover:bg-white nodrag nopan shrink-0`}
              title={we.size > 0 ? `生成勾选的 ${we.size} 项` : `生成全部空缺项`}
              onClick={(t) => {
                t.stopPropagation();
                let n = p
                  .filter((e) => {
        ...
        <Component2503 className={`mb-3`}>
          ...
          <Component2502
            ...
            placeholder={`例如：中世纪童话·皮克斯3D`}
            className={`w-full rounded-lg bg-[#242424] border border-transparent focus:border-gray-500 px-2.5 py-2 text-xs text-gray-200 outline-none nodrag transition-colors`}
          />
        </Component2503>
        {[`character`, `scene`, `prop`].map((t) => {
          return (
            <Component2532 className={`mb-5`} key={t}>
              ...
              <Component2531 className={`grid ${n} gap-1.5`}>
                {m[t].map((t) => {
                  return (
                    <Component2528
                      className={`rounded-lg border bg-[#212121] overflow-hidden transition-colors ${we.has(t.id) ? `border-gray-300` : `border-transparent hover:border-[#3a3a3a]`}`}
                      key={t.id}
                    >
                      <Component2524
                        className={`aspect-square bg-[#1a1a1a] relative flex items-center justify-center cursor-pointer nodrag overflow-hidden`}
                        onClick={() => {
                          return k(t.id);
                        }}
                        onDoubleClick={(e) => {
                      ...
                      >
                        {t.imageUrl ? (
                          <Q.Fragment>
                            <Component2505
                              ...
                              loading={`lazy`}
                              decoding={`async`}
                              className={`w-full h-full object-cover ${t.loading ? `opacity-55` : ``}`}
                              onError={(e) => {
                            ...
                            {t.loading && (
                              ...
                              >
                                ...
                                <Component2507
                                  className={`absolute inset-y-0 w-1/2 bg-gradient-to-r from-transparent via-white/40 to-transparent animate-shimmer`}
                                />
                                <Component2508
                          ...
                          >
                            <Component2510
                              className={`absolute inset-y-0 w-1/2 bg-gradient-to-r from-transparent via-white/25 to-transparent animate-shimmer`}
                            />
                            <_Component25
                              size={14}
                              className={`animate-spin text-gray-400 relative z-10`}
                            />
                          </Component2511>
                        ) : (
                          <Component2512
                            className={`text-[9px] text-gray-600 px-1 text-center`}
                          >{`点击编辑`}</Component2512>
                        )}
                        <Component2513
                          className={`absolute top-1 left-1 w-3.5 h-3.5 rounded flex items-center justify-center text-[9px] border ${we.has(t.id) ? `bg-white border-white text-black` : `bg-black/50 border-white/40 text-transparent`}`}
                          title={`勾选以批量生成`}
                          onClick={(e) => {
                            e.stopPropagation();
                            Ee(t.id);
                        ...
                        <Component2523 className={`absolute top-1 right-1`}>
                          {d.videoAssetUploadStatus?.[t.imageUrl || ``] === `uploading` && (
                            <Component2514
                              className={`flex items-center gap-1 rounded bg-black/70 px-1.5 py-1 text-[9px] text-gray-200`}
                            >
                              <_Component25 size={10} className={`animate-spin`} />
                              {`上传中`}
                            </Component2514>
                          )}
                          {d.videoAssetUploadStatus?.[t.imageUrl || ``] === `failed` && (
                            <Component2515
                              className={`rounded bg-red-500/80 px-1.5 py-1 text-[9px] text-white hover:bg-red-500`}
                              title={d.videoAssetUploadErrors?.[t.imageUrl || ``] || `上传失败`}
                              onClick={(n) => {
                                n.stopPropagation();
                          ...
                          {d.videoAssetUploadStatus?.[t.imageUrl || ``] === `uploaded` && (
                            <Component2516
                              className={`rounded bg-emerald-600/80 px-1.5 py-1 text-[9px] text-white`}
                            >{`已上传`}</Component2516>
                          )}
                          {t.loading ? (
                            <Component2517
                            ...
                            <Q.Fragment>
                              <Component2518
                                className={`p-1 rounded bg-black/60 text-gray-200 hover:text-white`}
                                title={`更多操作`}
                                onClick={(e) => {
                                  e.stopPropagation();
                              ...
                              {se === t.id && (
                                <Component2522
                                  className={`absolute right-0 top-full z-50 mt-1 w-28 rounded-lg border border-[#3a3a3a] bg-[#202020] p-1 shadow-xl`}
                                  onClick={(e) => {
                                    return e.stopPropagation();
                                  }}
                                >
                                  <Component2519
                                    className={`w-full rounded px-2 py-1.5 text-left text-[10px] text-gray-300 hover:bg-[#2a2a2a]`}
                                    onClick={() => {
                                  ...
                                  >{`上传`}</Component2520>
                                  <Component2521
                                    className={`w-full rounded px-2 py-1.5 text-left text-[10px] text-red-400 hover:bg-red-500/10`}
                                    onClick={() => {
                                      r(e, {
                                        assets: p.filter((e) => {
                                          return e.id !== t.id;
                      ...
                      <Component2527 className={`px-1.5 py-1`}>
                        ...
                        <Component2526
                          className={`mt-0.5 text-[9px] leading-relaxed text-gray-500 truncate`}
                        >
                          {t.description}
                        </Component2526>
                      </Component2527>
                ...
                <Component2530
                  className={`rounded-lg border border-dashed border-[#3a3a3a] bg-transparent hover:bg-[#202020] hover:border-gray-500 cursor-pointer flex flex-col items-center justify-center transition-colors nodrag min-h-[120px]`}
                  onClick={() => {
                    let n = {
                      id: `asset-${Date.now()}`,
                      category: t,
  ...
  let Ct = (t = false) => {
    ...
    return (
      <Component2559
        className={`shrink-0 self-stretch bg-[#191919] border-l border-white/10 flex flex-col nodrag`}
        style={{
          width: n,
        }}
        onClick={(e) => {
      ...
      >
        ...
        >
          ...
          <Component2535
            className={`p-1.5 text-gray-400 hover:text-white hover:bg-[#333] rounded-md shrink-0`}
            onClick={() => {
              return k(null);
            }}
            title={`收起`}
        ...
        <Component2558
          className={`flex-1 overflow-y-auto custom-scrollbar p-3 space-y-3 nowheel`}
          onWheel={(e) => {
            return e.stopPropagation();
          }}
        >
          <Component2545
            className={`bg-[#222] rounded-xl overflow-hidden flex items-center justify-center relative`}
            style={{
              minHeight: r,
            }}
          >
            {Y.imageUrl ? (
              <Q.Fragment>
                <Component2537
                  src={Y.thumbnailUrl || Y.imageUrl}
                  alt={Y.name}
                  loading={`lazy`}
                  decoding={`async`}
                  className={`w-full object-contain cursor-zoom-in ${Y.loading ? `opacity-55` : ``}`}
                  onError={(e) => {
                ...
                />
                {Y.loading && (
                  <Component2541 className={`absolute inset-0 pointer-events-none overflow-hidden`}>
                    <Component2538 className={`absolute inset-0 bg-black/20`} />
                    <Component2539
                      className={`absolute inset-y-0 w-1/2 bg-gradient-to-r from-transparent via-white/35 to-transparent animate-shimmer`}
                    />
              ...
              <Component2544 className={`text-xs text-gray-600 py-8`}>{`暂无参考图`}</Component2544>
            )}
          </Component2545>
          <Component2548>
          ...
          <Component2551>
            ...
            <Component2550
              ...
              placeholder={`该资产的主体外观描述`}
              className={`w-full min-h-[80px] rounded-lg bg-[#242424] border border-transparent focus:border-gray-500 px-2.5 py-2 text-xs text-gray-200 outline-none resize-y custom-scrollbar nowheel transition-colors`}
            />
          </Component2551>
          <Component2554>
            <Component2552
              className={`text-[11px] text-gray-500 mb-1`}
            >{`完整生图提示词（可编辑）`}</Component2552>
            <Component2553
              value={Y.prompt || ``}
              onChange={(e) => {
                return J(Y.id, {
              ...
              placeholder={`该资产的完整生图提示词`}
              className={`w-full min-h-[220px] rounded-lg bg-[#242424] border border-transparent focus:border-gray-500 px-2.5 py-2 text-xs text-gray-200 outline-none resize-y custom-scrollbar nowheel transition-colors`}
            />
          </Component2554>
          <Component2557 className={`flex justify-end gap-2`}>
            <Component2555
              className={`px-3 py-1.5 rounded-lg bg-[#262626] text-xs text-gray-300 hover:bg-[#333]`}
              onClick={() => {
                return W(true);
              }}
            >{`从资源选择`}</Component2555>
            <Component2556
              className={`px-3 py-1.5 rounded-lg bg-gray-100 text-gray-950 text-xs font-medium hover:bg-white`}
              onClick={() => {
  ...
  let Et = () => {
    if (w) {
      return (
        <Component2562
          className={`flex items-center bg-red-500/10 rounded-full p-1 pl-3 border border-red-500/30 hover:border-red-500/50 transition-colors cursor-pointer flex-shrink-0 nodrag nopan`}
          onMouseDown={(e) => {
            return e.stopPropagation();
          }}
          onClick={(t) => {
        ...
        >
          ...
          <Component2561
            className={`bg-red-500/20 text-red-400 w-6 h-6 rounded-full flex items-center justify-center hover:bg-red-500/30`}
          >
      ...
      return (
        <Component2567
          className={`flex items-center rounded-full p-1 pl-3 border transition-colors flex-shrink-0 nodrag nopan ${v.trim() ? `bg-[#2a2a2a] border-[#333] hover:border-gray-500 cursor-pointer` : `bg-[#222] border-[#2a2a2a] opacity-50 cursor-not-allowed`}`}
          onMouseDown={(e) => {
            return e.stopPropagation();
          }}
          onClick={qe}
  ...
  let Dt = () => {
    return (
      ...
      >
        <Component2571
          className={`flex items-center gap-1 px-3 py-2 rounded-xl bg-[#262626] border border-[#3a3a3a] text-xs text-gray-200 hover:border-[#555] nodrag`}
          onClick={(e) => {
            e.stopPropagation();
            x((e) => {
              return !e;
        ...
        >
          {Ve && (
            <Component2568
              className={`shrink-0 px-1 rounded text-[9px] leading-[14px] border ${ca(Ve) ? `bg-white/10 text-white/90 border-white/30` : `bg-gray-500/15 text-gray-400 border-gray-500/40`}`}
            >
              {ca(Ve) ? `内置` : `三方`}
            </Component2568>
          )}
          ...
          {wt !== null && (
            <Component2570
              className={`inline-flex items-center gap-0.5 text-[10px] text-orange-400 tabular-nums`}
            >
              <_Component20 className={`w-2.5 h-2.5`} strokeWidth={2.5} />
              {la(wt)}
            </Component2570>
        ...
        {b && (
          <Component2577
            className={`absolute z-50 bottom-full left-0 mb-1 w-60 max-h-60 overflow-y-auto rounded-xl border border-[#3a3a3a] bg-[#202020] p-1 shadow-xl custom-scrollbar nowheel nopan nodrag`}
            onWheel={(e) => {
              return e.stopPropagation();
            }}
            onMouseDown={(e) => {
            ...
            {Be.length > 0 ? (
              Be.map((t) => {
                ...
                return (
                  <Component2575
                    role={`button`}
                    className={`w-full flex items-center gap-1.5 text-left px-2 py-1.5 rounded-lg text-[11px] cursor-pointer ${Ve === t ? `bg-[#333] text-white` : `text-gray-400 hover:bg-[#2a2a2a]`}`}
                    onClick={(n) => {
                      n.stopPropagation();
                      r(e, {
                        selectedModel: t,
                  ...
                  >
                    <Component2572
                      className={`shrink-0 px-1 rounded text-[9px] leading-[14px] border ${ca(t) ? `bg-white/10 text-white/90 border-white/30` : `bg-gray-500/15 text-gray-400 border-gray-500/40`}`}
                    >
                      {ca(t) ? `内置` : `三方`}
                    </Component2572>
  ...
  let kt = () => {
    return (
      <Component2598
        className={`w-[240px] shrink-0 flex flex-col gap-3 pr-3 border-r border-[#2a2a2a]`}
      >
        <Component2584>
          ...
          <Component2581 className={`relative nodrag`}>
            <Component2580
              ...
              placeholder={`选择或输入风格`}
              className={`w-full rounded-lg bg-[#262626] border border-[#3a3a3a] px-2 py-1.5 text-xs text-gray-200 outline-none`}
            />
          </Component2581>
          <Component2583 className={`flex flex-wrap gap-1 mt-1.5`}>
            {$g.map((t) => {
              return (
                <Component2582
                  className={`px-1.5 py-0.5 rounded text-[10px] nodrag ${d.globalStyle === t ? `bg-gray-100 text-gray-950` : `bg-[#262626] text-gray-400 hover:text-gray-200`}`}
                  onClick={() => {
                    return r(e, {
                      globalStyle: t,
                    });
        ...
        <Component2588 className={`flex-1 flex flex-col`}>
          ...
          <Component2587 className={`relative flex-1 min-h-[180px]`}>
            <Component2586
              ...
              className={`w-full h-full min-h-[180px] rounded-xl bg-[#262626] border border-[#3a3a3a] px-3 py-2 pb-8 text-sm text-gray-200 outline-none resize-none nodrag custom-scrollbar nowheel`}
            />
            <_cmp__Component23
              targetRef={ie}
              onRequestFullscreen={() => {
        ...
        <Component2594>
          ...
          <Component2593 className={`flex items-center gap-1.5 flex-wrap`}>
            <Component2590
              className={`px-2 py-1 rounded-lg text-[11px] nodrag ${d.shotCount === undefined || d.shotCount === `auto` ? `bg-gray-100 text-gray-950` : `bg-[#262626] text-gray-400 hover:text-gray-200`}`}
              onClick={() => {
                return r(e, {
                  shotCount: `auto`,
                });
            ...
            {[10, 20, 30, 50].map((t) => {
              return (
                <Component2591
                  className={`px-2 py-1 rounded-lg text-[11px] nodrag ${d.shotCount === t ? `bg-gray-100 text-gray-950` : `bg-[#262626] text-gray-400 hover:text-gray-200`}`}
                  onClick={() => {
                    return r(e, {
                      shotCount: t,
                    });
            ...
            <Component2592
              ...
              placeholder={`自定义`}
              value={
                typeof d.shotCount == `number` && ![10, 20, 30, 50].includes(d.shotCount)
                  ? d.shotCount
                  : ``
              }
              onChange={(t) => {
  ...
  let At = () => {
    if (_) {
      return (
        <Component2602
          className={`mt-3 flex items-center justify-between gap-3 text-[11px] text-gray-500`}
        >
          <Component2599
            className={`px-2 py-1 text-gray-400 hover:text-gray-200 disabled:opacity-30 nodrag transition-colors`}
            disabled={pe === 0}
            onClick={() => {
              return he((e) => {
  ...
  let jt = (e) => {
    return (
      <Component2625 className={`flex-1 min-w-0`}>
        {d.errorMsg && (
          <Component2603 className={`text-xs text-red-400 mb-2`}>{d.errorMsg}</Component2603>
        )}
        <Component2617 className={`relative`}>
          <Component2613
            className={`grid ${e ? `grid-cols-[44px_56px_minmax(220px,1.6fr)_96px_120px_150px_110px_96px_34px]` : ``} bg-[#222] text-[11px] text-gray-400 rounded-t-lg`}
            style={
              e
                ? undefined
                : {
          ...
          <Component2616>
            {_ ? (
              ye.map((t) => {
                return (
                  <Component2614 className={`relative`} key={t.id}>
                    {ft(t, e)}
                    {!e && !be.has(`shot-${t.id}`) && (
                      <Kt
                        type={`source`}
                        position={X.Right}
                        id={`shot-${t.id}`}
                        className={`!w-3 !h-3 !bg-white !border-2 !border-[#1c1c1c]`}
                        style={{
                          top: `50%`,
                          right: -10,
              ...
              >{`输入剧情后点“生成”，自动生成多镜头分镜表`}</Component2615>
            )}
          </Component2616>
        </Component2617>
        {At()}
        {!e && xe.length > 0 && (
          <Component2623 className={`mt-2 border-t border-white/10 pt-2`}>
            <Component2620
              className={`mb-1.5 flex items-center justify-between text-[10px] text-gray-500`}
            >
            ...
            <Component2622 className={`flex flex-wrap gap-1.5`}>
              {xe.map((e) => {
                return (
                  <Component2621
                    className={`relative rounded border border-white/10 bg-[#242424] px-2 py-1 text-[10px] text-gray-300 hover:bg-[#2b2b2b] nodrag transition-colors`}
                    onClick={() => {
                      return he(
                        Math.floor(
                          f.findIndex((t) => {
                  ...
                  >
                    ...
                    <Kt
                      ...
                      position={X.Right}
                      id={`shot-${e.id}`}
                      className={`!h-2.5 !w-2.5 !border-2 !border-[#1c1c1c] !bg-white`}
                      style={{
                        right: -7,
                        top: `50%`,
                        transform: `translateY(-50%)`,
        ...
        <Component2624
          className={`mt-3 inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#262626] hover:bg-[#333] text-xs text-gray-300 nodrag transition-colors`}
          onClick={Ue}
        >
          <Xt size={12} />
          {` 添加镜头`}
  ...
  let Mt = (t, n) => {
    let r =
      n === `image`
        ? new Set([`promptNode`])
        : new Set([`videoNode`, `discountVideoNode`, `sd2VideoNode`]);
    return o.some((n) => {
  ...
  let Nt = (t, n, r = false) => {
    let i = p.filter((e) => {
      return (
        ...
        e.imageUrl &&
        Fa(
          `${t.description || ``} ${t.prompt || ``} ${t.videoPrompt || ``} ${t.dialogue || ``}`,
          e.name,
        )
      );
    });
    ...
    return (
      <Component2692
        className={`relative border-b border-[#2a2a2a] transition-colors ${De.has(t.id) ? `bg-[#242424]` : `bg-transparent`} ${r ? `p-7 space-y-1` : `p-2.5`}`}
        key={t.id}
      >
        {t.promptLoading && (
          <Component2629
            className={`absolute inset-0 z-40 overflow-hidden bg-[#181818]/55 backdrop-blur-[1px] cursor-not-allowed`}
          >
            <Component2626
        ...
        <Component2637 className={`flex items-center justify-between mb-2`}>
          {!r && (
            <Component2633
              className={`flex items-center gap-2 text-[11px] text-gray-400 cursor-pointer nodrag`}
            >
              <Component2630
                type={`checkbox`}
                checked={De.has(t.id)}
              ...
              {t.duration && (
                <Component2632 className={`text-[10px] text-gray-500`}>{t.duration}</Component2632>
              )}
            </Component2633>
          )}
          {!r && (
            <Component2636 className={`ml-auto`}>
              {t.promptLoading ? (
                <Component2634
                  type={`button`}
                  className={`p-1 rounded text-white hover:bg-white/10 nodrag`}
                  title={`生成中，点击停止`}
                  onClick={() => {
                ...
                <Component2635
                  type={`button`}
                  className={`p-1 rounded text-gray-500 hover:text-white hover:bg-white/10 nodrag`}
                  title={t.prompt || t.videoPrompt ? `重生成提示词` : `生成提示词`}
                  onClick={() => {
        ...
        {r && (
          <Component2683 className={`mb-6 space-y-5`}>
            {i.length > 0 && (
              <Component2645
                className={`flex items-center gap-5 rounded-lg bg-[#1b1b1b] px-4 py-3`}
              >
                <Component2640
                  className={`flex shrink-0 items-center pl-2 py-1 ${ne.has(t.id) ? `max-w-[420px] flex-wrap gap-2` : ``}`}
                >
                  {(ne.has(t.id) ? i : i.slice(0, 4)).map((e, n) => {
                    return (
                      <Component2638
                        ...
                        loading={`lazy`}
                        decoding={`async`}
                        className={`w-16 h-16 object-cover border-2 border-white/90 shadow-[0_8px_18px_rgba(0,0,0,0.38)] cursor-zoom-in nodrag ${ne.has(t.id) ? `rounded-md` : `${n > 0 ? `-ml-7` : ``} ${[`-rotate-3`, `rotate-2`, `-rotate-1`, `rotate-3`][n] || ``}`}`}
                        style={{
                          zIndex: n + 1,
                        }}
                        onError={(t) => {
                  ...
                  {i.length > 4 && (
                    <Component2639
                      type={`button`}
                      className={`relative z-10 flex h-8 min-w-8 items-center justify-center rounded-full border border-white/40 bg-[#303030] px-1.5 text-[10px] font-medium text-white shadow-lg hover:bg-[#444] nodrag ${ne.has(t.id) ? `` : `-ml-5`}`}
                      onClick={() => {
                        return B((e) => {
                          let n = new Set(e);
                          if (n.has(t.id)) {
                ...
                <Component2644 className={`min-w-0 flex-1`}>
                  ...
                  <Component2643
                    className={`mt-1.5 truncate text-[11px] text-gray-300`}
                    title={i
                      .map((e) => {
                        return e.name;
                      })
            ...
            <Component2667 className={`grid grid-cols-2 gap-4`}>
              <Component2649 className={`relative`}>
                ...
                >{`时长`}</Component2646>
                <Component2648
                  className={`w-full rounded-md bg-[#262626] border border-transparent focus:border-gray-500 px-2 py-1.5 text-[11px] text-left text-gray-200 outline-none nodrag transition-colors`}
                  onClick={(e) => {
                    e.stopPropagation();
                    Xe(t.id, `duration`);
                  }}
            ...
            <Component2680>
              ...
              {(() => {
                ...
                return (
                  <Component2679 className={`space-y-2`}>
                    {e.map((n, r) => {
                      return (
                        <Component2675 className={`flex items-center gap-2`} key={r}>
                          <Component2669
                            type={`button`}
                            className={`w-[52px] shrink-0 rounded border border-transparent px-1 py-1 text-[10px] outline-none nodrag ${n.kind === `旁白` ? `bg-purple-500/20 text-purple-300` : `bg-cyan-500/20 text-cyan-400`}`}
                            onClick={() => {
                          ...
                          {n.kind === `台词` && (
                            ...
                            >
                              ...
                              {h.map((e) => {
                                return (
                                  <Component2671 value={e.name} key={e.id}>
                                    {e.name}
                                  </Component2671>
                                );
                              })}
            ...
            <Component2682 className={`relative`}>
              <Component2681
                className={`text-[10px] text-gray-500 mb-0.5`}
              >{`画面描述（输入 @ 引用资产）`}</Component2681>
              {rt(
                t,
                `description`,
                `w-full min-h-[64px] rounded-md bg-[#262626] border border-transparent focus:border-gray-500 px-2.5 py-2 text-[11px] text-gray-200 outline-none resize-y nodrag custom-scrollbar nowheel transition-colors`,
                `画面描述，输入 @ 引用资源`,
              )}
            </Component2682>
          </Component2683>
        ...
        <Component2691 className={`grid ${r ? `grid-cols-1 gap-3` : `grid-cols-2 gap-2`}`}>
          <Component2688>
            <Component2687 className={`flex items-center justify-between gap-2 mb-1.5`}>
              ...
              <Component2686 className={`flex items-center gap-0.5`} title={`图像提示词宫格构图`}>
                {[
                  ...
                  return (
                    <Component2685
                      className={`px-1.5 py-0.5 rounded text-[10px] nodrag ${(t.gridMode || 0) === e ? `bg-gray-100 text-gray-950` : `bg-[#2a2a2a] text-gray-500 hover:text-gray-300`}`}
                      onClick={() => {
                        return He(t.id, {
                          gridMode: e,
                        });
            ...
            {rt(
              t,
              `prompt`,
              `w-full ${r ? `min-h-[100px] bg-[#262626]` : `min-h-[60px] bg-transparent group-hover:bg-[#262626] focus:bg-[#262626]`} rounded-lg border border-transparent hover:border-[#3a3a3a] focus:border-gray-500 text-xs text-gray-200 outline-none px-2.5 py-2 nodrag nowheel transition-colors`,
              `点上方按钮生成，或输入 @ 引用资产`,
            )}
          </Component2688>
          <Component2690>
            <Component2689
              className={`text-xs font-medium text-gray-300 mb-1.5`}
            >{`生视频`}</Component2689>
            {rt(
              t,
              `videoPrompt`,
        ...
        <Kt
          ...
          position={X.Right}
          id={`shot-${t.id}`}
          className={`!w-3 !h-3 !border-2 !z-20 ${n ? `!opacity-0 !pointer-events-none` : `!bg-white !border-[#1c1c1c]`}`}
          style={{
            top: r ? 32 : `50%`,
            right: n ? 0 : r ? -10 : -22,
  ...
  let Pt = (t) => {
    return (
      <Component2731>
        {g === 1 && (
          <Component2693 className={`flex gap-3 ${t ? `p-2` : `px-4 py-3`}`}>
            {kt()}
            {jt(t)}
          </Component2693>
        )}
        {g === 2 && (
          <Component2695 className={`flex items-stretch`}>
        ...
        {g === 3 && (
          <Component2730 className={t ? `p-2` : `px-4 py-3`}>
            {d.videoAssetUploadProgress?.status === `uploading` && (
              <Component2701
                className={`mb-3 rounded-lg border border-white/10 bg-[#202020] px-3 py-2`}
              >
            ...
            <Component2710 className={`flex items-center justify-between mb-4 gap-2`}>
              <Component2704 className={`flex items-center gap-0.5 p-0.5 rounded-lg bg-[#262626]`}>
                <Component2702
                  className={`px-2.5 py-1 rounded-md text-[11px] transition-colors nodrag ${le === `list` ? `bg-gray-100 text-gray-950` : `text-gray-400 hover:text-gray-200`}`}
                  onClick={() => {
                    return G(`list`);
                  }}
                >{`列表`}</Component2702>
                <Component2703
                  className={`px-2.5 py-1 rounded-md text-[11px] transition-colors nodrag ${le === `single` ? `bg-gray-100 text-gray-950` : `text-gray-400 hover:text-gray-200`}`}
                  onClick={() => {
                    return G(`single`);
                  }}
                >{`单镜头`}</Component2703>
              ...
              {le === `list` && (
                <Component2709 className={`flex items-center gap-1.5 nodrag`}>
                  <Component2705
                    className={`mr-0.5 text-[11px] text-gray-500`}
                  >{`批量操作：`}</Component2705>
                  <Component2706
                    className={`inline-flex items-center gap-1 rounded-lg bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-950 hover:bg-white`}
                    onClick={() => {
                      let t = f
                        .filter((e) => {
                          return De.has(e.id);
                  ...
                  <Component2707
                    className={`rounded-lg bg-[#2a2a2a] px-2.5 py-1.5 text-xs text-gray-200 hover:bg-[#333]`}
                    onClick={() => {
                      return window.dispatchEvent(
                        new CustomEvent(`script-box-connect-shots`, {
                          detail: {
                            nodeId: e,
                            shotIds: f
                              .filter((e) => {
                      ...
                      <Q.Fragment>
                        <Component2727 className={`flex items-center justify-between mb-3`}>
                          <Component2721 className={`flex items-center justify-start gap-2`}>
                            <Component2713
                              className={`text-5xl font-light leading-none text-gray-500 hover:text-white nodrag`}
                              disabled={i <= 0}
                              onClick={() => {
                                de(i - 1);
                            ...
                            <Component2719 className={`relative text-center`}>
                              <Component2716
                                className={`flex items-baseline gap-2 nodrag`}
                                onMouseEnter={() => {
                                  return _e(true);
                                }}
                              >
                              ...
                              {ge && (
                                <Component2718
                                  className={`absolute left-1/2 top-full z-50 mt-1 grid max-h-64 w-[360px] -translate-x-1/2 grid-cols-10 gap-1 rounded-lg border border-[#3a3a3a] bg-[#202020] p-2 shadow-xl`}
                                  onClick={(e) => {
                                ...
                                >
                                  {f.map((e, t) => {
                                    return (
                                      <Component2717
                                        className={`h-7 w-7 rounded border border-white/10 text-center text-xs text-gray-300 hover:border-white/50 hover:bg-[#333]`}
                                        onClick={() => {
                                          de(t);
                                          he(Math.floor(t / s_));
                                          _e(false);
                          ...
                          <Component2726 className={`flex items-center justify-end gap-2`}>
                            {a.promptLoading ? (
                              <Component2722
                                type={`button`}
                                className={`h-8 rounded-md bg-[#2a2a2a] px-3 text-xs text-gray-200 nodrag`}
                                title={`生成中，点击停止`}
                                onClick={() => {
                              ...
                              <Component2723
                                type={`button`}
                                className={`h-8 rounded-md bg-[#2a2a2a] px-3 text-xs text-gray-200 hover:bg-[#333] nodrag`}
                                onClick={() => {
                            ...
                            <Component2724
                              className={`inline-flex min-w-[76px] items-center justify-center gap-1.5 px-3.5 py-2 rounded-md bg-white text-gray-950 text-xs hover:bg-gray-200 nodrag`}
                              onClick={() => {
                                return window.dispatchEvent(
                                  new CustomEvent(`script-box-connect-shot`, {
                                    detail: {
                                      nodeId: e,
                                      shotId: a.id,
                                      target: `image`,
  ...
  let Ft = (() => {
    ...
    return (
      <Component2743 className={`flex items-center flex-1 max-w-3xl mx-auto text-xs w-full`}>
        {o.map((t, n) => {
          let i = t.n === g;
          let a = n === o.length - 1;
          ...
          return (
            <Component2742 className={`flex items-center flex-1`} key={t.n}>
              <Component2740
                className={`flex items-center gap-3 nodrag text-left rounded-xl px-3 py-1.5 transition-colors w-[160px] ${i ? `bg-[#2a2a2a]` : `hover:bg-[#222]`}`}
                onClick={() => {
                  return r(e, {
                    step: t.n,
                  });
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <Component2733
                      ...
                      strokeDashoffset={Math.PI * 2 * 11 * (1 - t.progress)}
                      className={`transition-all duration-300 ease-out`}
                    />
                  </Component2734>
                  <Component2735
                    className={`relative z-10 text-[11px] font-medium ${i ? `text-white` : `text-gray-400`}`}
                  >
                    {t.n}
                  </Component2735>
                </Component2736>
                <Component2739 className={`flex flex-col flex-1 min-w-0`}>
                  <Component2737
                    className={`text-[12px] font-medium truncate ${i ? `text-white` : `text-gray-400`}`}
                  >
                    {t.title}
                  </Component2737>
                  <Component2738
                    className={`text-[10px] truncate ${i ? `text-gray-400` : `text-gray-500`}`}
                  >
                    {t.desc}
                  </Component2738>
                </Component2739>
  ...
  if (l < 0.4 && !n && !w) {
    ...
    return (
      <Component2749
        className={`relative flex flex-col items-stretch ${n ? `z-50` : `z-10`}`}
        style={{
          width: 860,
        }}
      >
        <_cmp__Component8
          ...
          data={t}
          defaultTitle={`脚本盒子`}
          icon={<_Component49 size={11} className={`text-gray-500`} />}
          className={`w-full justify-start text-left`}
        />
        <Component2748
          className={`w-full rounded-[20px] border border-white/10 bg-[#1c1c1c] px-6 py-8 shadow-[0_12px_40px_rgba(0,0,0,0.35)]`}
        >
    ...
    return (
      <Component2815
        className={`relative flex flex-col items-stretch group/node transition-colors ${n ? `z-50` : `z-10`}`}
        style={{
          width: 860,
        }}
      >
        ...
        <Component2763 className={`relative w-full ${It ? `isolate pt-2 pb-3` : ``}`}>
          {It && (
            <Q.Fragment>
              <Component2751
                className={`pointer-events-none absolute inset-x-2 top-4 bottom-0 z-0 translate-x-3 rotate-2 rounded-[22px] border border-white/10 bg-[#2a2a2a] shadow-xl`}
              />
              <Component2752
                className={`pointer-events-none absolute inset-x-1 top-2 bottom-1 z-0 -translate-x-2 -rotate-1 rounded-[22px] border border-white/15 bg-[#242424] shadow-xl`}
              />
            </Q.Fragment>
          )}
          <Component2762
            className={`relative z-10 w-full bg-[#1c1c1c] rounded-[20px] border shadow-[0_12px_40px_rgba(0,0,0,0.35)] ${n ? `border-white/35` : `border-white/10 hover:border-white/20`}`}
          >
            <Component2757
              className={`flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#191919] rounded-t-[20px]`}
            >
              {Ft}
              <Component2756 className={`flex items-center gap-2`}>
                ...
                <Component2754
                  className={`p-1.5 text-gray-400 hover:text-white hover:bg-[#333] rounded-md nodrag`}
                  onClick={() => {
                    return z(true);
                  }}
                  title={`总体提示词设置`}
            ...
            <Component2761 className={`relative`}>
              {w && (
                <Component2760
                  className={`absolute inset-0 z-30 flex flex-col items-center justify-center gap-2 bg-[#1a1a1a]/75 rounded-b-[20px]`}
                >
                  ...
                  <Component2758 className={`text-xs text-gray-200`}>
                    {typeof Tt == `number` && Tt > 0
                      ? `正在生成分镜脚本… 已接收 ${Tt} 字 · 已用 ${Re}s`
                      : `正在生成分镜脚本… 已用 ${Re}s`}
                  </Component2758>
                  <Component2759
                    className={`mt-1 px-3 py-1 rounded-full bg-red-500/20 text-red-400 text-xs hover:bg-red-500/30 nodrag nopan`}
                    onMouseDown={(e) => {
                      return e.stopPropagation();
                    }}
                    onClick={(t) => {
        ...
        {E &&
          Fn.createPortal(
            <Component2770
              className={`fixed inset-0 z-[2147483646] bg-black/85 backdrop-blur-sm flex items-center justify-center p-6`}
              onMouseDown={(e) => {
                return e.stopPropagation();
              }}
              onWheel={(e) => {
            ...
            >
              <Component2769
                className={`relative bg-[#1c1c1c] rounded-2xl border border-[#3a3a3a] shadow-2xl flex flex-col`}
                style={{
                  width: `92vw`,
                  height: `88vh`,
                }}
              >
                <Component2767
                  className={`flex items-center justify-between px-5 py-3 border-b border-[#2c2c2c]`}
                >
                  {Ft}
        ...
        {re &&
          Fn.createPortal(
            <Component2776
              className={`fixed inset-0 z-[2147483647] bg-black/85 backdrop-blur-sm flex items-center justify-center p-8`}
              onMouseDown={() => {
                return V(false);
              }}
            >
              <Component2775
                ref={ae}
                className={`relative w-[min(1000px,90vw)] h-[80vh] rounded-2xl border border-[#3a3a3a] bg-[#1c1c1c] p-5 shadow-2xl`}
                onMouseDown={(e) => {
                  return e.stopPropagation();
                }}
              >
                ...
                <Component2774
                  ...
                  placeholder={`描述剧情片段、故事，为你生成分镜脚本`}
                  className={`h-[calc(100%-40px)] w-full resize-none rounded-xl border border-[#3a3a3a] bg-[#262626] p-4 text-sm text-gray-200 outline-none custom-scrollbar`}
                />
        ...
        {te &&
          Fn.createPortal(
            <Component2811
              className={`fixed inset-0 z-[2147483647] bg-black/85 backdrop-blur-sm flex items-center justify-center p-6`}
              onMouseDown={(e) => {
                return e.stopPropagation();
              }}
              onWheel={(e) => {
            ...
            >
              ...
              >
                ...
                <Component2807
                  className={`flex-1 overflow-y-auto custom-scrollbar p-5 space-y-5 nowheel`}
                  onWheel={(e) => {
                    return e.stopPropagation();
                  }}
                >
                  <Component2784>
                    <Component2780
                      className={`mb-1.5 text-xs text-gray-200`}
                    >{`画面比例`}</Component2780>
                    <Component2782 className={`flex flex-wrap gap-2`}>
                      {[`16:9`, `9:16`, `4:3`, `3:4`, `1:1`, `custom`].map((t) => {
                        return (
                          <Component2781
                            className={`rounded-md px-3 py-1.5 text-xs ${(d.aspectRatio || `16:9`) === t ? `bg-white text-gray-950` : `bg-[#262626] text-gray-400 hover:text-white`}`}
                            onClick={() => {
                              return r(e, {
                                aspectRatio: t,
                              });
                    ...
                    {(d.aspectRatio || `16:9`) === `custom` && (
                      <Component2783
                        value={d.customAspectRatio || ``}
                        onChange={(t) => {
                          return r(e, {
                        ...
                        placeholder={`例如 21:9`}
                        className={`mt-2 w-44 rounded-md border border-[#3a3a3a] bg-[#262626] px-3 py-1.5 text-xs text-gray-200 outline-none`}
                      />
                    )}
                  </Component2784>
                  <Component2791 className={`grid grid-cols-2 gap-4`}>
                    <Component2787>
                      <Component2785
                        className={`mb-1.5 text-xs text-gray-200`}
                      >{`生图强制约束`}</Component2785>
                      <Component2786
                        value={d.imageGlobalConstraint || ``}
                        onChange={(t) => {
                          return r(e, {
                            imageGlobalConstraint: t.target.value,
                          });
                        }}
                        placeholder={`例如：禁止画面文字、水印、边框；人物肢体结构正确……`}
                        className={`min-h-[110px] w-full resize-y rounded-lg border border-[#3a3a3a] bg-[#262626] p-3 text-xs text-gray-200 outline-none`}
                      />
                    </Component2787>
                    <Component2790>
                      <Component2788
                        className={`mb-1.5 text-xs text-gray-200`}
                      >{`生视频强制约束`}</Component2788>
                      <Component2789
                        value={
                          d.videoGlobalConstraint ??
                          [...(d.globalConstraints || []), d.customGlobalConstraint || ``]
                            .filter(Boolean)
                            .join(`；`)
                        }
                        onChange={(t) => {
                  ...
                  <Component2796>
                    <Component2794 className={`flex items-center justify-between mb-1.5`}>
                      <Component2792
                        className={`text-xs text-gray-200`}
                      >{`剧本生成提示词`}</Component2792>
                      <Component2793
                        className={`text-[11px] text-gray-500 hover:text-gray-300 nodrag`}
                        onClick={() => {
                          return r(e, {
...
只返回可解析的纯 JSON，不要解释，不要 Markdown，不要在 JSON 前后添加任何文字。`
                  ...
                  {[
                    [
                      `character`,
                      `角色参考图模板`,
                      `高质量专业角色设定图，横向构图，纯白色纯净背景，中性摄影棚灯光，平光布光；布局结构：正面半身特写 + 全身正面居中 + 左侧面视图 + 背面视图，无任何道具或背景物体。光影：中性摄影棚灯光，柔和的前侧光，清晰的轮廓定义，自然的肤色，面部清晰服装可辨识，平视镜头，完整全身，无裁剪。不得出现任何道具 / 武器 / 食物 / 饮料 / 手持物（角色空手）；不得出现复杂动作、夸张表情、面部遮挡；不得出现环境背景（仅白色）；不得出现其他角色；确保所有视图中的面部特征、发型、体型和服装保持一致；不得出现文字、水印、标签、UI元素；无背景场景，无过度风格化。`,
                    ],
                    [
                      `scene`,
                      `场景参考图模板`,
                    ...
                    return (
                      <Component2806 key={t}>
                        ...
                        <Component2805
                          value={d.customAssetTemplates?.[t] ?? i}
                          onChange={(n) => {
                            return r(e, {
                              customAssetTemplates: {
                                ...(d.customAssetTemplates || {}),
                ...
                <Component2809
                  className={`flex justify-end gap-2 px-5 py-3 border-t border-[#2c2c2c]`}
                >
                  <Component2808
                    className={`px-4 py-1.5 rounded-lg bg-gray-100 text-gray-950 text-xs font-medium hover:bg-white`}
                    onClick={() => {
                      return z(false);
                    }}
                  >{`完成`}</Component2808>
        ...
        {oe &&
          Fn.createPortal(
            <Component2814
              className={`fixed inset-0 z-[2147483647] bg-black/90 flex items-center justify-center p-6 cursor-zoom-out`}
              onMouseDown={(e) => {
                return e.stopPropagation();
              }}
              onWheel={(e) => {
            ...
            >
              <Component2812
                className={`absolute top-4 right-4 p-2 text-white hover:text-gray-300 bg-black/50 rounded-full`}
                onClick={(e) => {
                  e.stopPropagation();
                  H(null);
                }}
        ...
        {U && Fe.current && (
          <_cmp__Component118
            ...
            transitResources={d.transitResources || []}
            canvasNodes={a()}
          ...
          (() => {
            ...
            return Fn.createPortal(
              <>
                ...
                >
                  <Component2426
                    className={`shotedit-panel`}
                    onClick={(e) => {
                      return e.stopPropagation();
                    }}
                  >
                    <Component2427 className={`shotedit-head`}>
                      <Component2428 className={`shotedit-title`}>
                        {{
                          description: `画面描述`,
                          prompt: `生图提示词`,
                          videoPrompt: `生视频提示词`,
                      ...
                      <div className={`shotedit-head-actions`}>
                        <Component2437
                          className={`shotedit-format nodrag`}
                          title={`把长段落按句/逗号拆成多行，方便逐句阅读修改`}
                          onClick={() => {
                            return ems((o) => {
                              if (!o) return o;
                              let s = o.value || ``;
                        ...
                        >{`一键排版`}</Component2437>
                        <Component2429
                          className={`shotedit-close nodrag`}
                          onClick={() => {
                            return ems(null);
                          }}
                        >
                    ...
                    <Component2430
                      ...
                      placeholder={`输入或修改该${{ description: `画面描述`, prompt: `生图提示词`, videoPrompt: `生视频提示词` }[em.field] || ``}，可直接键入 @资产名 引用资产`}
                      className={`shotedit-area`}
                    />
                    {p.length > 0 && (
                      <Component2431 className={`shotedit-assets`}>
                        <Component2432
                          className={`shotedit-assets-title`}
                        >{`点资产名插入 @引用`}</Component2432>
                        <Component2433 className={`shotedit-assets-list`}>
                          {p.map((n) => {
                            return (
                              <Component2434
                                className={`shotedit-asset nodrag`}
                                onClick={() => {
                                  let r = emRef.current;
                                  let o = em.value || ``;
                    ...
                    <Component2435 className={`shotedit-foot`}>
                      <Component2436
                        className={`shotedit-save nodrag`}
                        onClick={() => {
                          ems((o) => {
                            if (o) {
                              He(o.shotId, {
