...
try {
  ...
  for (const c of Object.values(dict.contracts)) {
    for (const p of c.patterns) {
      ...
      if (
        p.type === 'fixed' &&
        /^[a-zA-Z][a-zA-Z0-9_-]{3,}$/.test(p.value) &&
        !p.value.includes('/')
      )
        set.add(p.value);
    }
...
function extractFeatures(file) {
  ...
  // API 路径
  const apiRe =
    /\/api\/[a-zA-Z0-9_/-]+|\/v1\/[a-zA-Z0-9_/-]+|\/public\/[a-zA-Z0-9_/-]+|\/files\/[a-zA-Z0-9_/-]*|\/proxy[a-zA-Z0-9_/-]*/g;
  let m;
...
function extractSymbols(file) {
  ...
  const syms = [];
  const re =
    /^(?:export\s+(?:default\s+)?)?(?:async\s+)?(?:function|const|let|var)\s+([A-Za-z0-9_$]+)\s*(?:=|\(|\{)/gm;
  let m;
  while ((m = re.exec(c))) {
    const name = m[1];
    // 行号
...
function inferSymbolRole(body, file) {
  ...
  // 中文文案（UI 组件/面板）
  const cn = [
    ...body.matchAll(/>([\u4e00-\u9fff][\u4e00-\u9fff\s，。！？、；：""''（）《》\-+]{1,})/g),
  ].map((x) => x[1].trim());
  if (cn.length >= 2) clues.push('UI:' + [...new Set(cn)].slice(0, 4).join('|'));
  // hooks → 组件
  const hooks = [...new Set(body.match(/\b(use[A-Z]\w+)\s*[\(<]/g) || [])].map((h) =>
    h.replace(/[\s(<]/g, ''),
  );
  ...
  const apis = [
    ...new Set(
      body.match(
        /\/api\/[a-zA-Z0-9_/-]+|\/v1\/[a-zA-Z0-9_/-]+|\/public\/[a-zA-Z0-9_/-]+|\/files\/[a-zA-Z0-9_/-]*/g,
      ) || [],
    ),
  ];
  ...
  // data 字段（区分"读/写节点数据"）
  const fields = [
    ...new Set([...body.matchAll(/(?:data|n|t)\.([A-Za-z][A-Za-z0-9]{2,})/g)].map((x) => x[1])),
  ].filter(
    (f) =>
      ![
        'type',
...
for (const tc of topChunks) {
  let type = 'facade',
    note = '';
  if (/vendor|runtime|shim|jsx_runtime|browser-external/.test(tc.name)) {
    type = '运行时';
    note = '勿改（React 单实例/外链垫片）';
  } else if (tc.name === 'endpointConfig-Bt85xi8d.js') {
    type = '逻辑';
    note = '接入点/端口/18080 配置（契约 critical）';
  } else if (tc.name === 'main-CYvt_zul.js') {
    type = '入口';
    note = '应用入口';
  } else if (tc.name === 'share-CyPsaet6.js') {
    type = '入口';
    note = '分享页入口';
  } else if (tc.name === 'mediabunny-mp3-encoder-CZeRAvEV.js') {
    type = '库';
    note = 'MP3 编码器';
  } else if (tc.name.startsWith('src-')) {
    type = 'facade';
    note = '映射至同名 _components';
  } else {
    note = '映射至同名 _components';
  }
...
for (const f of big) {
  ...
  L.push(
    `| \`${f.rel}\` | ${f.lines} | ${[...ft.apis].slice(0, 5).join(' ') || '—'} | ${[...ft.kvKeys].join(' ') || '—'} | ${[...ft.hooks].slice(0, 4).join(' ') || '—'} | ${expShow} |`,
  );
}
L.push('');
...
for (const k of REVERSE_KEYS) {
  ...
  if (!fs2.length) {
    ...
    const note =
      {
        9004: 'bundle 内无字面量（前端经变量拼接，见 contracts.json scope=localTool/apimart）',
        'x-proxy-url': 'bundle 内无字面量（上游头，见 contracts.json scope=localTool）',
      }[k] || 'bundle 内未命中，确认是否只在 localTool/apimart 端';
    L.push(`| \`${k}\` | 0 | ⚠ ${note} |`);
    continue;
  }
  const detail = fs2
...
L.push('| 文件 | 被引用次数 |');
L.push('|---|---|');
const topDeps = Object.entries(importCount)
  .filter(([k]) => k.includes('_components'))
  .sort((a, b) => b[1] - a[1])
  .slice(0, 15);
...
const shadow = Object.entries(byBase)
...
const DOMAINS = [
  {
    name: '应用入口 / 启动',
    match: (f) => /main-CYvt_zul|App-BX6o9fW5\.js|share-CyPsaet6|src-kC58-PF2/.test(f.rel),
  },
  {
    name: '接入点 / 端口 / 代理配置',
    match: (f) =>
      /endpointConfig|proxyMode|local-tool|18080|x-proxy-url|\/api\/proxy/.test(f.content),
  },
  {
    name: 'HTTP 客户端 / 代理转发层',
    match: (f) =>
      /httpClient-BknZwXjG/.test(f.dir) &&
      /proxyMode|local-tool|\/api\/proxy|x-proxy-url|9004/.test(f.content),
  },
  {
    name: '画布编辑器核心 UI / 状态',
    match: (f) => /App-BX6o9fW5_components/.test(f.dir) && f.feats.hooks.size > 0,
  },
  {
    name: '资源 / 文件上传',
    match: (f) => /\/api\/assets\/upload|\/api\/files|\/files\/resources|upload/.test(f.content),
  },
  {
    name: '任务 / 工作流管理',
    match: (f) =>
      /\/api\/tasks|\/v1\/gateway\/task|\/api\/workflow|task\/save|batch-save/.test(f.content),
  },
  {
    name: '分享页（ShareAppPage）',
    match: (f) => /ShareAppPage|share-CyPsaet6|share\//.test(f.rel),
  },
  {
    name: 'AI 对话 / 绘图接口',
    match: (f) =>
      /\/v1\/chat\/completions|\/v1\/draw\/completions|\/v1\/images\/generations|\/v1\/images\/edits/.test(
        f.content,
      ),
  },
...
for (const dom of DOMAINS) {
  const hits = allCompFiles
    .filter((f) => dom.match(f))
    .sort((a, b) => b.lines - a.lines)
    .slice(0, 8);
  ...
  for (const f of hits) {
    ...
    L.push(
      `| \`${f.rel}\` | ${f.lines} | ${sig || (f.content.includes('proxyMode') ? 'proxyMode 配置' : '—')} |`,
    );
  }
  L.push('');
}
...
try {
  ...
  console.warn('[symbol_map.json 写入失败]', e.message);
}
...
console.log(
  `  顶层 chunk: ${topChunks.length}, _components 目录: ${compDirs.length}, 组件文件: ${allCompFiles.length}, 大文件(>500行): ${big.length}, 高危文件标记: ${topDeps.length}, 符号: ${Object.keys(SYMBOL_MAP).length}`,
);
