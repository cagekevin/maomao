...
function restoreAll() {
  ...
  if (restoreMap.size > 0) {
    ...
    for (const [file, originalSrc] of restoreMap.entries()) {
      try {
        writeFileSync(file, originalSrc, 'utf8');
      } catch (e) {
        /* 忽略个别写入异常 */
      }
    }
...
if (cmd === 'add-nocheck') {
  if (!fileArg) {
    console.error('用法：node scripts/ts-tests.mjs add-nocheck <dir>');
    process.exit(1);
  }
  ...
  console.log(
    `✔ add-nocheck 完成：处理 ${files.length} 个文件 (新增 ${added} 个，跳过 ${skipped} 个)`,
  );
  process.exit(0);
}
...
if (cmd === 'rm-nocheck') {
  if (!fileArg) {
    console.error('用法：node scripts/ts-tests.mjs rm-nocheck <file>');
    process.exit(1);
  }
...
if (cmd === 'verify') {
  if (!fileArg) {
    console.error('用法：node scripts/ts-tests.mjs verify <file>');
    process.exit(1);
  }
  const abs = resolve(projectRoot, fileArg);
  ...
  try {
    ...
    const tscRes = spawnSync('npx', ['tsc', '--noEmit', '-p', tsconfigPath], {
      cwd: projectRoot,
      encoding: 'utf8',
      shell: true,
    });
    ...
    if (!hasProject) {
      ...
      const vitestRes = spawnSync('npx', ['vitest', 'run', fileArg], {
        cwd: projectRoot,
        encoding: 'utf8',
        shell: true,
        stdio: 'inherit',
...
if (cmd === 'check') {
  if (!fileArg) {
    console.error('用法：node scripts/ts-tests.mjs check <file>');
    process.exit(1);
  }
  const abs = resolve(projectRoot, fileArg);
  if (!existsSync(abs)) {
  ...
  try {
    const res = spawnSync('npx', ['tsc', '--noEmit', '-p', tsconfigPath], {
      cwd: projectRoot,
      encoding: 'utf8',
      shell: true,
    });
    ...
    if (errs.length === 0) {
      console.log(
        `✔ 无类型错误！现在运行 \`node scripts/ts-tests.mjs verify ${fileArg}\` 确立战果！`,
      );
    } else {
      ...
      const byCodeRows = Object.entries(byCode).map(([c, n]) => ({
        c,
        n: /** @type {number} */ (n),
      }));
...
if (cmd === 'status') {
  ...
  files.forEach((f) =>
    /^\s*\/\/\s*@ts-nocheck/m.test(readFileSync(f, 'utf8'))
      ? nocheckFiles.push(f)
      : cleanFiles.push(f),
  );
...
`;
    ...
    console.log(
      `ℹ 进度总览：已解 ${cleanFiles.length} / 总计 ${files.length}。待处理：${nocheckFiles.length} 个。`,
    );
