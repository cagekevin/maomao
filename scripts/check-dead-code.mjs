 ...
 * 与原 `check-strict-src.mjs`（白名单渐进收口 · 2026-09-24 并入 `type-check`）同构：防回潮，
 * 而不是逼一次清空存量（全量门禁弊大于利）。
 * 基线内条目消失会提示"可移除"（stale，仅告警，保持基线诚实而不制造摩擦）。
 *
 * 【禁止】新增死代码一律**先删**；但有两类**不是死代码**（不该删、也不该塞基线）→ 走下方唯一豁免通道。
 *
 * 【★时机（2026-09-21 · 用户裁定 · `ADR-0053` §决议 ④）：清偿是「收尾动作」，不是「施工准入」】
 *   新增 / 替换功能时**必然**会报出两类"零消费"：① **本次新写、尚未接线**的新实现；② 被替代、
 *   新实现刚接上的**旧实现**。二者都是**替换动作的过渡态**，不是本闸要治的死代码。
 *   ⇒ **先把功能写完、跑通，再同批清偿**（新实现接上 + 旧实现删除 ⇒ 本闸自然不报「基线外新增」）。
 *   ⛔ **禁止**为让本闸变绿而**删掉本次新写的代码 / 退回被替代的老实现** —— 那等于闸凌驾正确性
 *      （`ADR-0016`）：闸的合法通过成本被抬到"放弃本次功能"，它就在生产它本要禁止的行为。
 *   ⚠️ 过渡态**不是永久豁免**：只许存在于本次施工内，落账前清干净，且**不许塞基线**（见下方红线）。
 *
...
const ADJUDICATION =
  '\n裁决路径（按序，别跳步）：\n' +
  '   0. 先分清时机：报的是「本次新写、尚未接线」的新实现、或「刚被替代」的旧实现 ⇒ 那是**过渡态**\n' +
  '        （ADR-0053 §决议 ④），先把功能写完、跑通再**同批清偿**；\n' +
  '        ⛔ 不许为过闸删掉本次新写的代码 / 退回被替代的老实现（闸不许凌驾正确性，ADR-0016）。\n' +
