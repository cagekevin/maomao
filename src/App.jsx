...
function Canvas() {
  ...
  const canvasMenuItems = (state) => {
    ...
    //
    // 设计意图（对应需求：「只要一级显示了，二级就不必显示，除了固定项目的图钉」）：
    //   - 一级菜单 = 顶部固定快捷(文本/图片/视频/剧本盒子) + 用户固定到一级的节点(pinnedItems)。
    //   - 二级(小工具子菜单)理论上不必重复展示一级已有的节点；
    //     但固定项的「图钉」必须留在二级（已固定节点仍在二级显示、图钉高亮=已固定），
    //     否则用户没有取消固定的入口。所以二级仍列全部节点，靠图钉区分固定态。
    //   - 例外：脚本盒已在一级硬编码展示（「剧本盒子」快捷项），二级不再重复列出（见下）。
    //   - ⚠️ 禁止项：不要因为「一级已显示」就把二级的已固定节点项删掉——二级必须保留它们，
    //     否则图钉(取消固定入口)会一起消失。真要删，必须先给一级固定区补取消按钮。
    ...
    // 与二级关系：固定项一级、二级都显示，二级的图钉承担「取消固定」；若删掉二级固定项，
    // 必须给一级固定区补取消按钮，否则无法取消固定。
