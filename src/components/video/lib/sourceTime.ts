 ...
 *
 * 【依赖方向】本文件零 import —— 2026-09-19 域归位迁入本域（`refs` 实测非测试消费者
 * 只有 `video/nodes/VideoProcessNode.tsx` ⇒ 单域 ⇒ ADR-0040 L4）。
 *
 * 【刻意不并入】同域 `video/lib/videoEngine.ts` 的媒体处理链不做时间轴映射，与本模块无关；
 * 时间↔**像素**换算是另一维（见 `video/lib/timeScale.ts` 内的时间轴刻度原语），不要混进这里。
 */
