...
import { KEY_YIMAO_PROMPT_HUB_CACHE } from '../base/core/contracts.ts';
...
async function runPromptSource(source: PromptSource): Promise<Prompt[]> {
  ...
  try {
    ...
    throw new Error(
      `「${source.name}」拉取失败：${err instanceof Error ? err.message : String(err)}`,
    );
  }
...
export async function loadPromptHub(): Promise<{
  items: Prompt[];
  tags: string[];
  categories: string[];
}> {
