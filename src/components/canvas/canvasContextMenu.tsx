...
// 【TD-04-26 已删，2026-09-13】原此处 `import './lazyNode.tsx';` 是**副作用 import**：
...
import type { ContextMenuItem } from './shell/ContextMenu.tsx';
...
export function buildCanvasMenuItems(ctx: MenuActionCtx): ContextMenuItem[] {
  ...
  const toolsSubmenu = paletteCategories
    .map((cat) => {
      const catNodes = getNodesByCategory(cat.key).filter(
        (n) => !EXCLUDED_FROM_SUBMENU.includes(n.type),
      );
  ...
  return [
    ...
    {
      key: 'scriptBox',
      icon: <Clapperboard size={16} className="text-fuchsia-300" />,
      label: '剧本盒子',
      badge: { text: 'Beta', tone: 'new' },
      onClick: () => ctx.addNodeFromMenu('scriptBoxNode'),
    },
    ...
    ...(toolsSubmenu.length
      ? [
          {
            key: 'tools',
            icon: <Zap size={13} className="text-secondary" />,
            label: '小工具',
            items: toolsSubmenu,
          },
          { type: 'divider' as const },
...
export function buildNodeMenuItems(
  ...
  if (!node) return [];
  const isImageLike = node.type === 'assetNode' || node.type === 'imageGenerateNode';
  ...
  // 「复制图片」仅图片类节点（assetNode/imageGenerateNode）有：把图片本身复制到剪贴板（对齐官方 Ei）
  if (isImageLike) {
    items.push({
      key: 'copyImage',
  ...
  items.push(
    { type: 'divider' },
    {
      ...
      label: '删除',
      danger: true,
      onClick: () => ctx.deleteNode(node.id),
    },
  );
  return items;
}
