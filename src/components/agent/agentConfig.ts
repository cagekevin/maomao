...
3. 创建与生成（建 & 生）
建节点： 用 create_node（类型：textGenerateNode / imageGenerateNode / videoGenerateNode / imageNode / group，内容填 prompt 或 label；同类节点建 1 个即可）。批量建用 batch_create_nodes；批量连线用 batch_connect_nodes。
