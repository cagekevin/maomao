...
/**
 * 【docs/107 决策记录 · 2026-09-06】画布工具 parameters 批量补严（纯提示层，零行为改动）：
 * 凡带 properties 的 object 顶层统一加 `additionalProperties: false`（含空 properties 的无参工具
 * read_canvas/list_nodes/list_edges/run_existing_plan/undo_ai）；batch_create_nodes.nodes、
 * batch_connect_nodes.connections、execute_plan.generations、execute_plan.artifacts、
 * show_plan_for_confirm.generations、show_plan_for_confirm.artifacts 的空壳 items 收敛为带
 * additionalProperties:false 的子 schema（键与 execute 实际解析一致）；若干字符串字段补 minLength:1。
 * description 为 create_node / update_node / generate_node / execute_plan 增补「何时不用」负向边界句。
 * 未动 execute / name / 注册顺序 / MUTATING_TOOLS / buildCanvasAgentToolSchemas。
 */
...
const createNodeTool = {
  ...
  execute(args, ctx) {
    ...
    return {
      ok: true,
      data: { id: built.id, position: built.newNode.position, connected: built.edges.length > 0 },
    };
  },
};
...
const batchCreateNodesTool = {
  ...
  description:
    '批量创建多个节点（元素结构与 create_node 相同）。适合一次搭建整条流程。返回全部新节点 id。',
  parameters: {
    type: 'object',
    additionalProperties: false,
    properties: {
      nodes: {
        type: 'array',
        description:
          '节点数组，每个元素结构与 create_node 相同（type 必填，可选 prompt/label/text/aspectRatio/resolution/position/connectFrom）',
        items: {
          type: 'object',
          additionalProperties: false,
          required: ['type'],
  ...
  execute(args, ctx) {
    ...
    return {
      ok: ids.length > 0,
      data: { ids },
      ...(lastError && ids.length === 0 ? { error: lastError } : {}),
    };
  },
};
...
const updateNodeTool = {
  name: 'update_node',
  description:
    '更新节点可编辑字段（白名单）。可改：prompt(提示词)、label(标题)、selectedModel(模型)、aspectRatio(宽高比 16:9/9:16/1:1)、resolution(720p/1080p)、seconds(秒数)、text(文本)、locked(锁定)。只改传入字段。只改用户要求改的字段；不改白名单之外的 data。',
  parameters: {
    type: 'object',
    additionalProperties: false,
    properties: {
...
const updateNodeRawTool = {
  ...
  description:
    '直接改节点任意原始 data 字段（高级）。patch 整体合并进 node.data，仅改必要字段避免覆盖。',
  parameters: {
    type: 'object',
    additionalProperties: false,
    properties: {
...
function buildConnect(conn, ctx, currentEdges) {
  ...
  if (!ctx.getNodes().some((n) => n.id === source))
    return { status: 'error', error: `源节点不存在：${source}` };
  if (!ctx.getNodes().some((n) => n.id === target))
    return { status: 'error', error: `目标节点不存在：${target}` };
  if (currentEdges.some((e) => e.source === source && e.target === target))
...
const connectNodesTool = {
  ...
  parameters: {
    ...
    additionalProperties: false,
    properties: {
      source: { type: 'string', minLength: 1, description: '数据源节点 id' },
      target: { type: 'string', minLength: 1, description: '数据目标节点 id' },
    },
...
const batchConnectNodesTool = {
  ...
  parameters: {
    ...
    properties: {
      connections: {
        type: 'array',
        description: '连线数组，每个元素 { source, target }',
        items: {
          type: 'object',
          additionalProperties: false,
          required: ['source', 'target'],
...
const getNodeDetailsTool = {
  ...
  description:
    '读取指定节点完整 data（提示词/模型/尺寸/结果 URL 等）。⚠️ 只返回结构化数据，不含图像/视频的画面内容：要分析图片（反推提示词、描述图片、识别图中文字等）请直接看用户消息里的图，不要调用本工具。',
  parameters: {
    type: 'object',
    additionalProperties: false,
  ...
  execute(args) {
    ...
    if (rowId) {
      ...
      if (!row)
        return {
          ok: false,
          error: `行不存在：${rowId}。当前行 id：${sb.rows.map((r) => r.id).join('、') || '（空表）'}`,
        };
      return {
        ok: true,
        data: { rowId, index: sb.rows.indexOf(row) + 1, fields: rowToObj(sb, row), globalStyle },
      };
    }
    return {
      ok: true,
...
const triggerGenerationTool = {
  name: 'generate_node',
  description:
    '触发生成并等待完成，返回结果 URL（供后续做参考图/前序依赖）。调用前该节点须已有提示词。节点须已有提示词才触发生成；不要为空白节点盲目生成。',
  parameters: {
    type: 'object',
    additionalProperties: false,
    properties: {
  ...
  execute: async (args, ctx) => {
    ...
    if (!node) {
      ...
      return {
        ok: false,
        error: `节点不存在：${id}${hint}。请使用工具返回的真实 id（如 create_node 的 data.id），不要自行推测节点 id。`,
      };
    }
    ...
    if (!res) {
      ...
      return {
        ok: false,
        error: `节点 ${id} 未注册生成契约（类型 ${node.type} 暂不支持由 Agent 驱动）`,
        nodeId: id,
      };
    }
    ...
    return {
      ...
      data: {
        ...
        submitted: true,
        resultUrl: '',
        note: '已提交生成（异步后台进行中），请勿重复触发或重复建同类节点',
      },
    };
  },
};
...
const executePlanTool = {
  name: 'execute_plan',
  description:
    '按计划批量建节点并生成（多图/多步骤）。输入 generations（每步含 prompt/比例/分辨率/是否依赖前序），按依赖分批执行，返回每步结果 URL。引用参考图有两种方式：①本轮用户参考图用 attachment_indices（0-based，参考图1→0）；②引用历史图/上一轮生成图用 direct_refs（把 system 里「当前可引用的图」中对应图的 url 填进该步 direct_refs 数组，prompt 里写「图N」，执行层自动反查）。适合大批量任务。一次只针对用户当前要的那批图；若阶段1已暂存策划则可不传 generations，系统自动取；提交后不要重复触发。',
  parameters: {
    type: 'object',
    additionalProperties: false,
    properties: {
      generations: {
        type: 'array',
        description:
          '【可选】步骤数组。每项 { id, title, prompt, ratio, resolution, depends_on_previous, dependency_mode, use_attachments, attachment_indices, direct_refs }。attachment_indices 是 0-based 数组，指向本轮用户参考图的编号（参考图1→0，参考图2→1），仅当该步要基于某参考图图生图时填。direct_refs 是数组，项 { url, name? }，引用 system 里「当前可引用的图」（图N，含上一轮生成图），prompt 里用「图N」指代。此参数仅作兜底：若阶段1 已把 generations 暂存（回复正文），这里可省略，系统自动从暂存读取。',
        items: {
          type: 'object',
          additionalProperties: false,
          required: ['prompt'],
          properties: {
            id: { type: 'string', description: '步骤 id（可选，用于引用/排序）' },
            title: { type: 'string', description: '步骤标题（可选）' },
            prompt: { type: 'string', description: '该步生图提示词（必填）' },
            ratio: { type: 'string', description: '比例，如 9:16 / 16:9 / 1:1（可选）' },
            resolution: { type: 'string', description: '画质档位 720p/1080p/2K/4K（可选）' },
            depends_on_previous: {
              type: 'boolean',
              description: '是否依赖前序步骤结果当参考图（可选）',
            },
            dependency_mode: {
              type: 'string',
              enum: ['none', 'product_reference', 'fusion'],
              description: '依赖模式：none/product_reference/fusion（可选）',
            },
            use_attachments: {
              type: 'boolean',
              description: '是否使用本轮用户参考图（attachment_indices 取图）',
            },
            attachment_indices: {
              type: 'array',
              items: { type: 'integer' },
              description: '0-based 参考图编号数组（参考图1→0）',
            },
            direct_refs: {
              type: 'array',
              description: '引用历史图/上一轮生成图（每项 { url, name? }）',
              items: {
                type: 'object',
                additionalProperties: false,
                required: ['url'],
      ...
      auto_run: {
        type: 'boolean',
        description: '是否自动触发生成（默认 true）。false 时只建节点不跑，供用户确认',
      },
      ...
      referenceImages: {
        ...
        items: { type: 'string' },
        description:
          '参考图 url 数组（可选；整批共享，写进所有生图节点作参考。若用 attachment_indices 则按步精确指定，优先于它）',
      },
      global_contract: {
        type: 'object',
        additionalProperties: false,
        description:
          '统一风格契约 {visual_positioning, unified_style_prompt, unified_negative_prompt}，逐字锁定每步 prompt 头部',
        properties: {
      ...
      artifacts: {
        type: 'array',
        description:
          '跨步成果资产 [{id,type,title,description,nodeId?,url?}]，供依赖步 input_artifact_ids 注入参考图',
        items: {
          type: 'object',
          additionalProperties: false,
          required: ['id'],
          properties: {
            id: { type: 'string', description: '资产 id（被后续步 input_artifact_ids 引用）' },
            type: { type: 'string', description: '资产类型（可选）' },
            title: { type: 'string', description: '资产标题（可选）' },
            description: { type: 'string', description: '资产说明（可选）' },
            nodeId: { type: 'string', description: '关联节点 id（可选）' },
            url: { type: 'string', description: '资产 url（可选）' },
          },
        },
      },
    },
    ...
    try {
      ...
      if (!hasPlanSource) {
        ...
        return {
          ok: false,
          error:
            '未找到生成计划。请确保本次生图已有计划来源：\n1. AI 在回复正文用 ```json 输出 generations 数组，或在 execute_plan 的 generations 参数中提供每步计划。\n（若走 Skill 流程，需先完成阶段1 策划确认。）',
        };
      }
      const gens = pendingUsed ? pending : argsGens;
      clearPendingGenerations();
      // 【D3/D4 积分闸判定 · 2026-08-27 简化（架构决策）】credit = creditSwitch（全局总闸，与模式正交）。
      // 删掉旧版「分步确认残留 semi」特殊分支：它把分步确认与直接生图混为一谈，
      ...
      //     【更新 2026-09-05】执行模型已收敛恒 auto（direct/step-confirm 已删）：
      ...
      const globalRefs = Array.isArray(args.referenceImages)
      ...
      const resolvedGens = (gens || []).map((g) => {
        const isPrevDep = !!(
          g?.depends_on_previous ||
          g?.use_previous_results ||
      ...
      const lockedGens = gcText
        ...
        : resolvedGens;
      const artifactTable =
        Array.isArray(args.artifacts) && args.artifacts.length
          ? args.artifacts
          : getCurrentArtifacts() || [];
      ...
      logger.debug(
        'AI助手',
        '[plan] execute_plan 入参',
        {
          gensSource: pendingUsed ? 'pending(阶段1暂存)' : 'args(本次参数)',
          gensCount: gens.length,
          stepIds: gens
            .map((g) => String(g?.id ?? g?.title ?? '').slice(0, 24))
            .filter(Boolean)
            .slice(0, 40),
          autoRun,
          model,
          awaited: getAwaitingConfirm(),
          refPoolLen: refPool.length,
          globalRefsLen: globalRefs.length,
          gcApplied: !!gcText,
          artifactsCount: artifactTable.length,
          refTypePerStep: resolvedGens.map((g) =>
            g?.direct_refs?.length && !g?.depends_on_previous
              ? 'direct_refs'
              : Array.isArray(g.referenceImages) && g.referenceImages.length
                ? 'attachment_indices'
                : 'none',
          ),
          refMetrics: resolvedGens
            .map((g) => ({
              id: String(g?.id ?? g?.title ?? '').slice(0, 16),
              n: Array.isArray(g.referenceImages) ? g.referenceImages.length : 0,
              head:
                Array.isArray(g.referenceImages) && g.referenceImages[0]
                  ? String(g.referenceImages[0]).slice(0, 48)
                  : '',
            }))
            .slice(0, 40),
        },
      ...
      const result = await executePlan({
        ctx,
        generations: lockedGens,
        autoRun,
        model,
        defaults: panel,
        referenceImages: globalRefs,
        globalContract: gc,
      ...
      if (!result || !result.entries) {
        logger.debug(
          'AI助手',
          '[plan] execute_plan 失败：无 entries',
          { result },
          { module: 'agent' },
        );
      ...
      if (creditHit) {
        ...
        logger.info('AI助手', '[plan] execute_plan 命中积分闸（节点已建好待确认）', {
      ...
      logger.debug(
        'AI助手',
        '[plan] execute_plan 完成',
        {
          entriesCount: result.entries.length,
          anyFailed,
          status: result.workflow?.status,
          stepStatuses: result.entries
            .map((e) => ({
              id: String(e?.id ?? e?.stepId ?? '').slice(0, 16),
              status: e?.status,
              hasUrl: !!e?.resultUrl,
            }))
            .slice(0, 40),
      ...
      setCurrentMemory({
        ...mem,
        lastPlan: {
          plan_text: args.plan_text || mem?.lastPlan?.plan_text || '',
          generations: gens,
          ts: Date.now(),
        },
      });
...
const focusNodeTool = {
  ...
  parameters: {
    ...
    additionalProperties: false,
    properties: {
      nodeId: { type: 'string', minLength: 1, description: '要聚焦的节点 id' },
      zoom: { type: 'number', minimum: 0.1, description: '聚焦后的缩放级别（可选，默认 1.0）' },
    },
    required: ['nodeId'],
  },
  execute(args, ctx) {
...
const memorySuggestTool = {
  name: 'memory_suggest',
  description:
    '提议保存一条项目长期记忆（仅在用户表达稳定偏好、确定事实、明确约束或做出决定时调用；内容精简成一句话）。禁止把文件全文/网页正文/密钥/绝对路径或临时结果作为记忆内容。调用后本工具进入"待确认"门禁，用户确认后才真正保存。',
  parameters: {
    type: 'object',
    additionalProperties: false,
    properties: {
      kind: {
        ...
        description: '记忆类别：preference偏好/fact事实/constraint约束/decision决定',
      },
      content: {
        type: 'string',
        minLength: 1,
        maxLength: 500,
        description: '一句话长期记忆内容（写前会脱敏密钥/凭据/本地路径并截断）',
      },
    },
  ...
  execute(args) {
    ...
    if (!PROJECT_MEMORY_KINDS.includes(kind))
      return { ok: false, error: `kind 非法：${kind}。应为 ${PROJECT_MEMORY_KINDS.join('/')}` };
  ...
  const callTool = useCallback(
    (name, args = {}) => {
      ...
      if (!fn) {
        ...
        return {
          ok: false,
          error: `未知工具：${name}。可用：${AGENT_TOOLS.map((t) => t.name).join('、')}`,
        };
      }
      return fn(args);
    },
