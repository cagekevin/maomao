...
function looksLikeFusionPrompt(text = '') {
  return /组合|结合|合成|融合|拼在一起|合并|合在一起|放在一起|拼合|合成为|合成一张|合成一图|把这[两三三四五六七八九十\d]+张|将这[两三三四五六七八九十\d]+张/.test(
    String(text || ''),
  );
}
...
function looksLikeSeriesPrompt(text = '') {
  ...
  if (
    /(套图|系列|批量|一组|若干|多张).{0,20}(详情|主图|产品图|详情页)|(详情页|主图|产品图|海报|banner|宣传图|首页图).{0,20}(套图|系列|多张|几张|若干|一组|批量)/.test(
      t,
    )
  )
    return true;
  // ② 数量表达的主图/详情页套图（如「5主图+8详情页」「生成4张详情页」「3张主图」）——最常见电商表达
  if (/(\d+|[一二三四五六七八九十两])张?(主图|详情页|详情图|产品图|海报|banner|宣传图)/.test(t))
    return true;
...
function extractSubjectLabel(text = '', index = 0) {
  ...
  const animal = t.match(
    /(?:一只|一个|一位)(?!与)([\u4e00-\u9fffA-Za-z0-9]{1,12}?(?:猫猫|猫咪|黑猫|橘猫|白猫|猫|狗狗|小狗|犬|狗|老虎|狮子|小熊|兔子|小鸟|金鱼|女孩|男孩|男人|女人|人物|包装|产品))/,
  );
...
function normalizeRatio(ratio) {
  ...
  if (
    ...
    return '16:9';
  if (r === 'portrait' || r === '3:4' || r === '3比4' || r === '三比四' || r === '竖版三比四')
    return '3:4';
...
function normalizeQuality(q) {
  ...
  if (v === 'medium' || v === '中' || v === '中画质' || v === '标准' || v === '一般')
    return 'medium';
...
export async function executePlan({
  ...
  logger.info('AI助手', '执行计划', {
    steps: steps.length,
    autoRun,
    model: model || defaults.model || '',
    hasGlobalContract: !!globalContract,
  });
  ...
  if (independent.length === 0 && dependent.length > 0) {
    ...
    independent = [
      { ...seed, depends_on_previous: false, use_previous_results: false },
      ...independent,
    ];
    dependent = rest;
    log(
      'warn',
      `计划缺少独立种子步，已将「${seed.title || seed.id}」提升为独立首步作为一致性锚点（依赖步 ${dependent.length} 将以此为前序）`,
    );
    logger.debug(
      'AI助手',
      '[执行计划] 计划补种',
      {
        promotedStepId: seed.id || seed.title,
        independentNow: independent.map((s) => s.id),
        dependentNow: dependent.map((s) => s.id),
      },
      { module: 'agent' },
    );
  }
  log(
    'info',
    `开始执行计划：共 ${steps.length} 步（独立批 ${independent.length} + 依赖批 ${dependent.length}）`,
  );
  logger.debug(
    'AI助手',
    '[执行计划] 分批',
    { independent: independent.map((s) => s.id), dependent: dependent.map((s) => s.id) },
    { module: 'agent' },
  );
  try {
    ...
    const maxX = nodes.length
      ? Math.max(...nodes.map((n) => (n.position?.x || 0) + (n.width || 400)))
      : 0;
    ...
    const lockNodeSettings = (
      nodeId: string,
      { m = model, ratio, resolution, quality }: NodeSettings = {},
    ) => {
      host.updateNodeData(nodeId, {
        ...(m ? { selectedModel: m } : {}),
        aspectRatio: ratio,
    ...
    const createGenNode = async (step, index, anchor) => {
      ...
      const gcText = [
        globalContract?.visual_positioning,
        globalContract?.unified_style_prompt,
        globalContract?.unified_negative_prompt,
      ]
        .filter(Boolean)
        .map((t, i) => ['视觉整体定位：', '统一风格提示词：', '统一负面提示词：'][i] + t)
        .join('\n');
      const lockedPrompt = gcText
        ? `[统一风格锁定]\n${gcText}\n\n${step.prompt || ''}`
        : step.prompt || '';
      const data = {
      ...
      logger.debug(
        ...
        '[执行计划] 建节点',
        {
          ...
          refCount: stepRefImages(step).length || (referenceImages ? referenceImages.length : 0),
          useAttachments: step.use_attachments,
        },
        { module: 'agent' },
      );
    ...
    const runNode = async (nodeId, step) => {
      ...
      if (live) {
        ...
        lockNodeSettings(nodeId, {
          ...
          ratio: normalizeRatio(step.ratio || defaults.ratio),
          resolution: normalizeResolution(step.resolution || defaults.resolution),
          quality: normalizeQuality(step.quality || defaults.quality || 'auto'),
        });
      }
    ...
    if (mode === 'runExisting') {
      ...
      log(
        'info',
        `补跑：触发已建节点 ${entries.length} 个（独立批 ${wave1.length} + 依赖批 ${wave2.length}）`,
      );
      // Wave 1：独立批并行触发
      if (autoRun && wave1.length) {
        ...
        wave1.forEach(({ entry }, i) => {
          ...
          entry.error = r.error || '';
          log(
            r.status === 'completed' ? 'ok' : 'error',
            `补跑独立步「${entry.id}」${r.status === 'completed' ? '完成' : r.error ? `失败：${r.error}` : '未触发'}`,
          );
        });
      }
      ...
      if (autoRun && wave2.length) {
        ...
        if (wave1Failed > 0) {
          wave2.forEach(({ entry }) => {
            entry.status = 'failed';
            entry.error = `前置独立批失败 ${wave1Failed} 项，依赖批已跳过`;
          });
          ...
          wave2.forEach(({ entry }, i) => {
            ...
            log(
              r.status === 'completed' ? 'ok' : 'error',
              `补跑依赖步「${entry.id}」${r.status === 'completed' ? '完成' : r.error ? `失败：${r.error}` : '未触发'}`,
            );
          });
        }
      }
    ...
    for (let i = 0; i < independent.length; i++) {
      ...
      log(
        'info',
        `第 ${i + 1} 步「${step.title || entry.id}」开始生成，挂载参考图 ${stepRefImages(step).length || referenceImages.length} 张`,
      );
    }
    ...
    if (autoRun && wave1.length > 0) {
      ...
      wave1.forEach(({ entry }, i) => {
        ...
        log(
          r.status === 'completed' ? 'ok' : 'error',
          `第 ${i + 1} 步「${entry.id}」${r.status === 'completed' ? '完成' : `失败：${r.error || '未知错误'}`}`,
        );
      });
    }
    ...
    if (dependent.length || (userText && looksLikeFusionPrompt(userText))) {
      ...
      const hasFusion = dependent.some(
        (s) => String(s.dependency_mode || '').toLowerCase() === 'fusion',
      );
      ...
      const independentFailedCount = entries
        .slice(0, independent.length)
      ...
      const stepsIdxOf = (s) => {
        ...
        if (s && s.id != null)
          return steps.findIndex((t) => t && t.id != null && String(t.id) === String(s.id));
        return -1;
      };
      ...
      for (let k = 0; k < dependent.length; k++) {
        ...
        const inIds = Array.isArray(step.input_artifact_ids)
          ? step.input_artifact_ids.map(String)
          : [];
        if (inIds.length && Array.isArray(artifacts)) {
          const matched = artifacts
            .filter((a) => inIds.includes(String(a.id)) && a.url)
            .map((a) => a.url);
          if (matched.length && !stepRefImages(step).length) {
            ...
            const entry = {
              ...
              status: 'failed',
              resultUrl: '',
              error: `步骤 ${step.id} 声明 input_artifact_ids=${inIds.join(',')} 但资产表无对应 url`,
            };
      ...
      const runDep = async (job) => {
        ...
        const edges = prevOk.map((e) => ({
          id: `e-plan-${nodeId}-${e.nodeId}`,
          source: e.nodeId,
          target: nodeId,
        }));
        ...
        if (autoRun) {
          ...
          log(
            r.status === 'completed' ? 'ok' : 'error',
            `依赖步 ${k + 1}「${entry.id}」${r.status === 'completed' ? '完成' : `失败：${r.error || '未知错误'}`}`,
          );
        } else {
          entry.status = 'ready';
          entry.resultUrl = '';
    ...
    if (autoRun) {
      log(
        anyFailed ? (anyDone ? 'warn' : 'error') : 'ok',
        anyFailed
          ? `执行结束：完成 ${doneCount} 步，失败 ${failCount} 步${anyDone ? '（可重试失败项）' : ''}`
          : `执行完成：共 ${doneCount} 步全部成功`,
      );
    } else {
      log('info', `已建 ${entries.length} 个节点，等待确认后执行`);
    }
    // 【A层】执行计划终态：总完成/失败步数 + 状态——定位 AI 批量出图的最终成败（高价值）
    logger.info('AI助手', '执行计划完成', {
      status,
      done: entries.filter((e) => e.status === 'completed').length,
      failed: entries.filter((e) => e.status === 'failed').length,
      total: entries.length,
