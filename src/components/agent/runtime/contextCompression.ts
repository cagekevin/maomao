...
import { withTimeout, isTimeoutError } from '@/components/base/utils/net/asyncGuard';
...
export async function compressToSummary({
  ...
  const input = serializeMessagesForSummary(
    previousSummary,
  ...
  logger.debug(
    'AI助手',
    '[记] 压缩输入',
    { inputLen: input.length, previousLen: previousSummary.length },
    { module: 'agent' },
  );
  ...
  logger.info('AI助手', '[记] 压缩成功', {
    summaryLen: summary.length,
    anchors: extractSummaryAnchors(summary).length,
  });
  return summary;
}
