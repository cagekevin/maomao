 ...
 *   buildIntentHint / INTENT_HINT / LocalIntent 类型。
 *   【更新 2026-09-06】删除 LOCAL_INTENT_THRESHOLD 伪门槛：原三个 confidence 为写死常量且
 *   恒 ≥ 阈值，threshold 从不过滤（命中即注入）。现去 confidence/threshold，命中即注入。
...
export function buildRequestMessages(
  ...
  if (
    memory &&
    memory.lastPlan &&
    (memory.lastPlan.plan_text || memory.lastPlan.generations?.length)
  ) {
    const planLines = (memory.lastPlan.generations || [])
      .map((g) => `- ${g.title || g.id || ''}: ${String(g.prompt || '').slice(0, 80)}`)
      .join('\n');
    out.push({
      role: 'system',
  ...
  if (
    memory &&
    memory.global_contract &&
    (memory.global_contract.visual_positioning || memory.global_contract.unified_style_prompt)
  ) {
  ...
  if (Array.isArray(imageCatalog) && imageCatalog.length > 0) {
    ...
    imageCatalog.forEach((img) => {
      lines.push(
        `图${img.num}：${img.name || `图${img.num}`}${img.source === 'gen' ? '（上一轮生成）' : '（本轮参考图）'}`,
      );
    });
    lines.push(
      '在 generations 某步里，若需引用这些图，把其 url 填进该步的 direct_refs 数组，并在 prompt 里写「图N」；执行层会自动把它当作该步参考图。',
    );
  ...
  if (
    ...
    typeof projectMemoryContext === 'string' &&
  ...
  for (let i = startIdx; i < messages.length; i++) {
    ...
    if (m.role === 'user' && m.attachments && m.attachments.length > 0) {
      if (currentHasImages && i === lastUserIdx) {
        ...
        else if (coordLines.length > 0)
          content.push({
            type: 'text',
            text: `${coordLines.join('\n')}\n${String(m.content || '')}`,
          });
...
export async function parseAgentError(
  res: Response,
 ...
 *
 * 【判定顺序（2026-09-08 修订：与 agentConfig.CANVAS_RULES「通道 A 默认 + 通道 C 断路器」对齐）】
 *   求方法 > 生成 > 内容理解 > 纯聊天 > 不注入。
 *   修订前「生成动词绝对优先」，与系统提示词冲突：系统提示词默认只给文字、
 *   出图须二次确认，而本地却对任何含「生成/批量/出图」字样的句子注入
 *   「按生成流程执行」——「批量出图怎么写提示词」这类【求方法句】会被误判成真要出图，
 *   等于本地规则反过来盖掉了系统提示词立的规矩。现把【求方法】提到最前。
 *   仍保住原底线：HOWTO_RE 极窄（只收语言文字类动词 + 提示词/描述名词），
 *   「反推提示词并生成一张」无求方法词 → 照旧判 generate，不会被引导成不用工具。
 *
...
export const INTENT_HINT: Record<LocalIntent, string> = {
  ...
  generate:
    '生成/改图 —— 若用户确实在要求出图或改图，按【三、工具操作规范】执行；但若用户其实是在问方法（如「怎么做 / 怎么写 / 怎么描述」只是求一段文字答案），以【通道 A】为准，只给文字不动手。【硬底线】凡是涉及调用 generate_node / execute_plan 的动作（新建生成、重画、改图、改成黑白、图生图、批量），一律走【通道 C】先文字二次确认，本预判绝不引导「直接动手」',
};

// ── 正则（按判定顺序；越靠前优先级越高）──
/**
 * 【第 0 优先】求方法句：用户在问「该怎么写/怎么描述/用什么提示词」，要的是【一段文字答案】，
 *   不是真要出图。必须排在 GENERATE_RE 前面，否则「批量出图怎么写提示词」会被
 *   GENERATE_RE 的「批量/出图」抢先判成 generate（本次修复的直接原因）。
 * 【刻意收窄，宁漏勿误】只匹配「疑问词 + 语言文字类动词（写/描述/表达/形容/措辞/说）」
 *   和「用什么/需要什么 + 提示词/描述」——【不含】做/弄/生成/出图。
 *   因此「怎么生成一张图」「这个怎么做成节点」不命中，照常走 generate / 交 LLM。
 */
const HOWTO_RE =
  /(怎么|怎样|如何|咋)(写|描述|表达|形容|措辞|说)|(该|应)(怎么|怎样|如何)(写|描述|表达|说)|(用|需要)(什么|啥)(提示词|prompt|描述)|(提示词|prompt).{0,6}(怎么|怎样|如何)/i;
/**
 * 生成/改图动词：命中即判 generate（用户真要出图，绝不引导成「不用工具」）。
 * 【2026-09-06 修正边界】不再把「改成/变成/换成」当独立命中词——它们同时承载
 * 「内容改造」（把这段改成英文）与「图像改造」（把图改成黑白），裸命中会误判出图。
 * 现在改动词只在「图类对象紧随其后」时才算 generate（保住「把这张图改成黑白」→generate），
 * 内容改造类语言交由 CONTENT_RE 或交给 LLM 自判（宁漏勿误）。
 */
const GENERATE_RE =
  /(生成|出图|生图|画一张|画个|画出来|重画|重生成|重新生成|做一张|跑一下|批量)|(图|图片|照片|画面|画像).{0,8}(改成|变成|换成)/i;
/**
 * 内容理解/文字产出：反推提示词 / 描述图片 / 提取图上文字 / 翻译润色 / 起标题。
 * 【2026-09-09 回滚本处 9-08 补漏】求方法句已统一由上方 HOWTO_RE 以更高优先级承载，
 *   此处不再重复。原因：本处补的 `(该|应)(怎么|如何|怎样)` 缺动词约束，会让
 *   「这个该怎么弄 / 该怎么做成节点」被误判成 content → 注入"不用工具" → AI 装死。
 *   HOWTO_RE 的同类分支带动词约束（写/描述/表达/说），安全。保留本文件「单一职责」：
 *   HOWTO_RE 管「怎么写一段文字」，CONTENT_RE 管「内容理解与文字产出」。
 */
const CONTENT_RE =
  /(反推|逆向|倒推|提取|描述|解读|讲讲|识别)|(这张图|这张图片|这张|图中|图上|这里).{0,10}(有什么|是什么|什么风格|什么内容|写的什么|啥)|什么(风格|内容|品牌|材质|构图)|(提示词|prompt|文案|卖点|标题|广告语).{0,8}(是什么|怎么写|帮我写|来一个|给我|怎么写)|帮我(写|想|起|取)(个)?(提示词|文案|标题|名字|卖点|名字)|(翻译|润色|改写|扩写|缩写)/i;
/** 纯聊天：整句为短问候/致谢才命中（锚定首尾，避免误伤长句）。 */
const CHAT_RE =
  /^(你好|您好|您|hi|hello|嗨|哈喽|嗨喽|在吗|在么|测试|谢谢|感谢|好的|好嘞|嗯|哦|明白了|辛苦了|不错|厉害|牛|666)[\s。.!！?？~～,，]*/i;

/**
 * 本地意图判定（纯函数）。
 * 【2026-09-06 精简】移除 confidence/阈值伪机制：三个 confidence 全是写死常量且恒 ≥ 阈值，
 * threshold 从不过滤（命中即注入）。现去掉 confidence/threshold，命中即返回意图，判不出返回 null。
 * @param {string} text 用户本轮输入
 * @param {object} ctx 附加信号（hasAttachments：本轮是否带图片附件；当前不参与判定，预留）
 * @returns {{ intent: LocalIntent | null, rule?: string }}
 *   intent 为 null = 本地判不出，调用方不注入提示（行为同现状）。
 */
export function classifyLocalIntent(
  text: string,
  ctx: { hasAttachments?: boolean } = {},
): { intent: LocalIntent | null; rule?: string } {
  const t = String(text || '').trim();
  if (!t) return { intent: null };

  // 0) 【2026-09-08 新增·最高优先】求方法句：用户问的是「怎么写一段文字」，
  //    即使句中同时出现「生成/出图/批量」等字眼，要的仍是文字答案 → 判 content。
  //    必须压过 GENERATE_RE，否则与 CANVAS_RULES 的「通道 A 默认 + 通道 C 断路器」打架。
  if (HOWTO_RE.test(t)) return { intent: 'content', rule: 'howto' };
  // 1) 生成动词——绝不把真实出图需求引导成「不用工具」
  if (GENERATE_RE.test(t)) return { intent: 'generate', rule: 'generate-verb' };
  // 2) 内容理解/产出文字（本次故障中无处安放的那一档，docs/76 §0）
  if (CONTENT_RE.test(t)) return { intent: 'content', rule: 'content' };
  // 3) 纯聊天（短问候）
  if (CHAT_RE.test(t)) return { intent: 'chat', rule: 'chat' };
