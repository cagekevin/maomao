...
export const ASSISTANT_TABLE_SYSTEM =
  '你正在一个 AI 助手表格工作区里协作：内容以表格形式组织，可整表生成、也可只改某行。列与行完全由用户需求/粘贴决定，你不预设模板、不限定用途（可以是分镜、产品页、提示词集、任何结构化清单）。';
...
export function buildRefineRowUser(
  ...
  const parts = [
    `请针对下面这一行表格内容做修改（只改文字；列结构/其它行/顺序保持不变）：`,
    '',
