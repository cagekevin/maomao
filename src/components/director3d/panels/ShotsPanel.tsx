...
function shotKeyframeCount(shot: ShotCard): number {
  const cameraKeys = Array.isArray(shot.keyframes)
    ...
    : Object.values(shot.keyframes || {}).reduce<number>(
        (sum, list) => sum + (Array.isArray(list) ? list.length : 0),
        0,
      );
  const objectKeys = Object.values(shot.objectKeyframes || {}).reduce<number>(
    (sum, track) => sum + (Array.isArray(track) ? track.length : 0),
    0,
  );
  ...
  return (
    <div className="shots-panel">
      <div className="shots-panel-head">
        ...
        <button type="button" onClick={onAdd} disabled={shots.length >= 30}>
          <Plus size={13} /> 新建镜头
        </button>
      </div>
        ...
        {shots.map((shot, index) => {
          ...
          return (
            <article className={`shot-card ${active ? 'is-active' : ''}`} key={shot.id}>
              <button
                type="button"
                className="shot-card-select"
                onClick={() => onSelect(shot.id)}
                aria-label={`切换到${shot.name}`}
              >
              ...
              <div className="shot-card-copy">
                <div className="shot-card-copy-head">
                  ...
                  <div className="shot-card-actions">
                    <button
                      type="button"
                      title="更新当前镜头缩略图"
                      aria-label="更新当前镜头缩略图"
                      onClick={() => onCapture(shot.id)}
                      disabled={!active}
                    >
                      <FileImage size={12} />
                    </button>
                ...
                <span>
                  {shot.durationSeconds} 秒 · {shot.fps} FPS · {shotKeyframeCount(shot)} 个关键帧
                </span>
              </div>
            </article>
          );
      ...
      <p className="shots-panel-note">
        每个镜头独立保存场景、人物骨骼、摄像机、参考图和关键帧。切换镜头前会自动更新当前缩略图。
      </p>
    </div>
  );
}
