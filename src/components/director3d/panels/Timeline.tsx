...
export function Timeline({
  ...
  const minPxPerFrame = Math.max(
    PX_PER_FRAME_HARD_MIN,
    viewportWidth / Math.max(1, totalFrames * 2),
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onWheel = (event) => {
      ...
      setPxPerFrame((value) =>
        clamp(value * (event.deltaY < 0 ? 1.1 : 1 / 1.1), minPxPerFrame, PX_PER_FRAME_MAX),
      );
    };
  ...
  const beginKeyDrag = (event, key, kind, trackId, trackFrames = []) => {
    ...
    const up = (upEvent) => {
      ...
      const moved =
        Math.hypot(upEvent.clientX - downX, upEvent.clientY - downY) > MARQUEE_THRESHOLD;
      if (!moved) return;
  ...
  const renderTrack = (frames, kind, onDelete, trackId = null) => (
    <div
      ...
      onPointerDown={beginTrackScrub(kind, trackId)}
    >
      ...
      {frames.map((key) => {
        ...
        return (
          <button
            ...
            title={title}
            aria-label={title}
            onPointerDown={(event) => beginKeyDrag(event, key, kind, trackId, frames)}
            onDoubleClick={(event) => {
              event.stopPropagation();
              onDelete(key.frame);
              onSelectKeyframe(null);
  ...
  return (
    <section className="timeline panel">
      ...
      <div className="timeline-key-toolbar">
        ...
        <button className="timeline-toolbar-skip" onClick={() => onSeek(0)} title="回到开头">
          <SkipBack size={13} />
        </button>
        <button
          className={`play-button timeline-toolbar-play ${playing ? 'is-playing' : ''}`}
        ...
        <button
          className="timeline-toolbar-skip"
        ...
        {selectedKeyframe && !isMulti && (
          <>
            <span className="timeline-key-toolbar-divider" />
            <button onClick={onCopyKeyframe} title="复制关键帧">
              <Copy size={12} />
            </button>
            <button onClick={onDeleteSelectedKeyframe} title="删除关键帧">
              <Trash2 size={12} />
            </button>
          </>
        )}
      ...
      <div className="timeline-body" ref={bodyRef}>
        ...
        <div className="timeline-labels">
          ...
          <div className="track-label camera-track-label">
            <Camera size={13} />
            <span>主摄像机</span>
          </div>
          {objectTrack && (
            <div className="track-label object-track-label">
              {objectTrack.type === 'person' ? <UserRound size={13} /> : <Box size={13} />}
              <span>{objectTrack.name}</span>
            </div>
          )}
        </div>
        ...
        <div className="timeline-scroll" ref={scrollRef}>
          <div className="timeline-ruler-row">
            <div className="ruler timeline-ruler" style={{ width: timelineWidth }}>
              {rulerFrames.map((frame) => (
                <span key={frame} style={{ left: `${frame * pxPerFrame}px` }}>
                  {frame}
                </span>
              ))}
            </div>
          ...
          {objectTrack &&
            renderTrack(objectTrack.keyframes, 'object', onDeleteObjectKeyframe, objectTrack.id)}
        </div>
        {/* 右列（第三列）：吸附 + 缩放 + keyframe 按钮 */}
        <div className="timeline-actions">
          <div className="timeline-view-tools">
            <button
              className={`timeline-toolbar-toggle ${snapEnabled ? 'is-on' : ''}`}
              onClick={() => setSnapEnabled((value) => !value)}
              title="吸附：拖动关键帧时贴齐整数帧/播放头（开/关）"
            >
              <Magnet size={13} />
            </button>
            <input
              ...
              min={minPxPerFrame}
              max={PX_PER_FRAME_MAX}
              step={0.5}
              value={pxPerFrame}
              onChange={(event) =>
                setPxPerFrame(clamp(Number(event.target.value), minPxPerFrame, PX_PER_FRAME_MAX))
              }
              title="时间轴缩放"
              aria-label="时间轴缩放"
            />
          </div>
          <button className="keyframe-button camera-keyframe-button" onClick={onAddKeyframe}>
            <Plus size={13} /> 镜头关键帧
          </button>
          {objectTrack && (
            <button
