...
export function AxisSlider({
  ...
  return (
    <div
      className={`axis-slider ${onToggleLock ? 'has-axis-lock' : ''} ${locked ? 'is-axis-locked' : ''}`}
      style={{ '--axis-color': accent } as React.CSSProperties}
    >
      <span>{label}</span>
      <input
      ...
      {onToggleLock && (
        <button
          type="button"
          className="axis-lock-button"
          aria-label={`${locked ? '解除' : '锁定'}缩放 ${label} 轴`}
          aria-pressed={locked}
          title={`${locked ? '解除' : '锁定'} ${label} 轴缩放`}
...
export function VectorFields({
  ...
  scaleAxisLocks = [false, false, false],
  ...
  const update = (index, next) => {
    ...
    if (kind === 'scale' && proportionalScale) {
      ...
      display.forEach((current, axis) => {
        copy[axis] = scaleAxisLocks[axis] ? current : Math.max(0.05, current * ratio);
      });
    ...
    <div className="property-group">
      <div className="property-label-row">
        ...
        {kind === 'scale' && (
          <button
            type="button"
            className={`proportional-lock ${proportionalScale ? 'is-active' : ''}`}
            aria-pressed={proportionalScale}
            onClick={onToggleProportionalScale}
            disabled={disabled}
            title="开启后拖动任意未锁定轴，其他未锁定轴按原比例同步缩放"
          >
            {proportionalScale ? <Link2 size={11} /> : <Unlink2 size={11} />} 等比
          </button>
        )}
      </div>
