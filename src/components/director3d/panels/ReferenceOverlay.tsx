...
export function ReferenceOverlay({
  ...
  const referenceLayer =
    hasImage && reference.visible ? (
      <div
        className={`reference-layer ${editing ? 'is-editing' : ''}`}
        aria-label={`参考图 ${reference.name}`}
      >
        <img
          src={reference.image}
    ...
    <>
        ...
        <div className={`reference-panel floating-panel ${!expanded ? 'is-collapsed' : ''}`}>
          {!expanded ? (
            ...
            <>
              <button
                type="button"
                className={reference.visible ? 'is-active' : ''}
                onClick={() => update({ visible: !reference.visible })}
              >
                {reference.visible ? '隐藏' : '显示'}
              </button>
              <button
                type="button"
              ...
              <button
                ...
                aria-label="移除参考图"
                onClick={() => {
                  onChange(cloneProjectValue(DEFAULT_REFERENCE));
                  setEditing(false);
                  setExpanded(true);
                }}
              >
              ...
              <label className="reference-export-toggle">
                <input
                  aria-label="参考图随 PNG 和 MP4 导出"
                  type="checkbox"
                  checked={reference.includeInExport}
                  onChange={(event) => update({ includeInExport: event.target.checked })}
                />
                <span>进入导出</span>
              </label>
              <button type="button" onClick={() => update({ x: 0, y: 0, scale: 1 })}>
                居中
              </button>
              <button
                type="button"
      ...
      {cameraMode ? (
        <div className="camera-edit-frame">
          <div
            className="camera-edit-stage"
            style={
              { aspectRatio: cameraAspect, '--camera-aspect': cameraAspect } as React.CSSProperties
            }
          >
            {referenceLayer}
            {children}
