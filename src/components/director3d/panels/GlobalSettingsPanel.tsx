  ...
  const lightRange = (label, key, minimum, maximum, step, suffix = '') => (
    <label className="lighting-range" key={key}>
      ...
      <input
        ...
        step={step}
        value={lighting[key]}
        onChange={(event) => updateLighting({ [key]: Number(event.target.value) })}
      />
      <output>
        {Number(lighting[key]).toFixed(step < 1 ? 2 : 0)}
        {suffix}
      </output>
    </label>
  );
      ...
      <div className="inspector-scroll global-settings-scroll">
        <div className="inspector-section">
          <div className="property-group">
            <label className="select-field">
              ...
              <input
                ...
                maxLength={40}
                onChange={(event) =>
                  onApplySettings({ ...settings, name: event.currentTarget.value })
                }
              />
            </label>
            <div className="global-settings-row">
              <label className="select-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    onApplySettings({ ...settings, fps: Number(event.target.value) })
                  }
                >
              ...
              <label className="select-field">
                ...
                <input
                  ...
                  onKeyDown={(event) => {
                    if (event.key === 'Enter')
                      commitDuration((event.currentTarget as HTMLInputElement).value);
                  }}
                />
              </label>
            </div>
            <label className="select-field">
              <span>画幅比例</span>
              <select
                value={aspectSelectValue(cameraAspect)}
                onChange={(event) =>
                  onAspectChange(
                    event.target.value === 'custom'
                      ? customAspectFrom(cameraAspect)
                      : event.target.value,
                  )
                }
              >
            ...
            {selected === 'custom' && (
              <div className="custom-aspect-inputs">
                ...
                <input
                  ...
                  value={customWidth}
                  onChange={(event) =>
                    onAspectChange(customAspectValue(event.target.value, customHeight))
                  }
                />
                <i>:</i>
                <input
                  ...
                  value={customHeight}
                  onChange={(event) =>
                    onAspectChange(customAspectValue(customWidth, event.target.value))
                  }
                />
              </div>
            )}
            <label
              className={`global-check loop-playback-check ${settings.loopPlayback ? 'is-checked' : ''}`}
            >
              <input
                type="checkbox"
                checked={Boolean(settings.loopPlayback)}
                onChange={(event) =>
                  onApplySettings({ ...settings, loopPlayback: event.target.checked })
                }
              />
              <span>循环播放</span>
            </label>
            {hasDurationConflict && (
              <p className="settings-warning">
                当前时长放不下关键帧，请至少设置 {requiredSeconds} 秒。
              </p>
            )}
          </div>

          {/* 视口（日常开关） */}
          <div className="section-title timeline-section-title">
            <span>视口</span>
          </div>
          <div className="property-group">
            <div className="global-check-row">
              <label className={`global-check ${showGrid ? 'is-checked' : ''}`}>
                <input type="checkbox" checked={Boolean(showGrid)} onChange={onToggleGrid} />
                <span>显示网格</span>
              </label>
              <label className={`global-check ${performanceMode ? 'is-checked' : ''}`}>
                <input
                  type="checkbox"
                  checked={Boolean(performanceMode)}
                  onChange={onTogglePerformance}
                />
                <span>性能模式</span>
              </label>
              <label className={`global-check ${seamlessBackground ? 'is-checked' : ''}`}>
                <input
                  type="checkbox"
                  checked={Boolean(seamlessBackground)}
                  onChange={onToggleSeamless}
                />
                <span>无缝背景</span>
              </label>
            </div>
          ...
          <div className="section-title timeline-section-title section-title-with-action">
            ...
            <button
              type="button"
              className="global-reset-light"
              onClick={() => onLightingChange(cloneDefaultLighting)}
            >
