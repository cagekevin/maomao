...
export function Director3DOverlay({ nodeId, onExit }) {
  ...
  useEffect(() => {
    ...
    let rootCaptureCleanup = []
    if (hostRoot) {
      const rootBlock = (e) => { e.preventDefault(); e.stopImmediatePropagation() }
      const rootEvents = ['dragover', 'drop', 'dragenter', 'dragleave']
      rootEvents.forEach((name) => { hostRoot.addEventListener(name, rootBlock, true); rootCaptureCleanup.push(() => hostRoot.removeEventListener(name, rootBlock, true)) })
    }
    ...
    return () => {
      pointerEvents.forEach((name) => document.removeEventListener(name, blockPointerOutside, true))
