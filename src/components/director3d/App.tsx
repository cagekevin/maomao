...
import {
  ...
  readCachedProject,
  readCustomPoses,
  referenceCanvasForExport,
  referenceImageFromFile,
...
export function Director3DApp({ storageKey, onExport, onExit, onThumbnail }) {
  ...
  const activeObject = useMemo(
    () => (selected?.id && selected.id !== CAMERA_ID ? selected : null),
    [selected],
  );
  const activeShot = useMemo(
    () => shots.find((shot) => shot.id === activeShotId) || shots[0],
    [activeShotId, shots],
  );
  ...
  const maxKeyframeFrame = useMemo(
    () => keyframeMaxFrame(keyframes, characterKeyframes),
    [characterKeyframes, keyframes],
  );
  ...
  const animatedObjects = useMemo(() => {
    ...
    return objectsAtFrame(objects, characterKeyframes, currentFrame, fps, paths).map(
      (object) => objectDrafts[object.id] || object,
    );
  ...
  const inspectorSelected = useMemo(
    () =>
      selectedId === CAMERA_ID
        ? selected
        : animatedObjects.find((object) => object.id === selectedId),
    [animatedObjects, selected, selectedId],
  );
  ...
  const exportDimensions = useMemo(
    () => exportDimensionsForAspect(camera.aspectRatio),
    [camera.aspectRatio],
  );
  ...
  useEffect(() => {
    if (selectedKeyframe?.kind === 'object' && selectedKeyframe.trackId !== selectedId)
  ...
  const applyProjectSnapshot = useCallback((snapshot) => {
    ...
    setCurrentFrame((frame) => {
      const nextFrame = clamp(
        frame,
        0,
        normalized.settings.fps * normalized.settings.durationSeconds,
      );
  ...
  useEffect(() => {
    ...
    hydrateProject(projectStorageKey)
      ...
      .catch((error) => {
        log.error(
          'director3d 工程 hydrate 失败（已用本地种子，不阻塞）',
  ...
  const applyPathBake = useCallback(
    (path) => {
      ...
      if (target === CAMERA_ID) {
        ...
        setKeyframes((channels) =>
          bakeCameraPath(channels, path, camera, baked.frames, [...removeFrames]),
        );
      } else {
      ...
      setPaths((all) => ({
        ...all,
        [target]: { ...path, sourceKeyframeFrames: baked.sourceKeyframeFrames },
      }));
  ...
  const focusSelected = useCallback(() => {
    if (selectedId === CAMERA_ID) {
      setViewFocusRequest({
        position: [...displayCamera.position],
        height: 0,
        distance: 4,
        nonce: Date.now(),
      });
  ...
  useEffect(() => {
    const onKeyDown = (event) => {
      ...
      if ((event.key === 'Delete' || event.key === 'Backspace') && selectedId !== CAMERA_ID) {
        event.preventDefault();
        event.stopImmediatePropagation();
        deleteSelected();
      }
      ...
      if (event.key === 'F1') {
        event.preventDefault();
        event.stopImmediatePropagation();
        gizmoApiRef.current?.('perspective');
      }
      if (event.key === 'F2') {
        event.preventDefault();
        event.stopImmediatePropagation();
        gizmoApiRef.current?.('top');
      }
      if (event.key === 'F3') {
        event.preventDefault();
        event.stopImmediatePropagation();
        gizmoApiRef.current?.('left');
      }
      if (event.key === 'F4') {
        event.preventDefault();
        event.stopImmediatePropagation();
        gizmoApiRef.current?.('back');
      }
    };
      ...
      list.map((shot) =>
        shot.id === activeShotId
          ? {
              ...
              loopPlayback: next.loopPlayback,
            }
          : shot,
      ),
    );
  ...
  const applyShotState = (shot) => {
    ...
    setSettings((current) => ({
      ...
      loopPlayback: shot.loopPlayback,
    }));
  ...
  const duplicateShot = (shotId) => {
    ...
    const source =
      shotId === activeShotId
        ? liveShotRecord(storedSource, thumbnail || storedSource.thumbnail)
        : storedSource;
    const duplicate = cloneProjectValue({
      ...source,
      id: `shot-${uid()}`,
      name: uniqueShotName(shots, `${source.name} 副本`),
    });
    setShots((list) => {
      const persisted = list.map((shot) =>
        shot.id === activeShotId ? liveShotRecord(shot, thumbnail || shot.thumbnail) : shot,
      );
      const sourceIndex = persisted.findIndex((shot) => shot.id === shotId);
      return [
        ...persisted.slice(0, sourceIndex + 1),
        duplicate,
        ...persisted.slice(sourceIndex + 1),
      ];
    });
  ...
  const addPerson = (bodyType) => {
    ...
    const person = {
      id,
      name: `人物 · ${objects.filter((item) => item.type === 'person').length + 1}`,
      type: 'person',
      bodyType,
      pose: 'idle',
      poseTime: presetPhase('idle'),
      joints: presetJoints(),
      rigRoot: [0, 0, 0],
      footLock: false,
  ...
  const addPrimitive = (type) => {
    ...
    setObjects((list) => [
      ...
      {
        id,
        name: `${labels[type] || type} · ${list.filter((item) => item.type === type).length + 1}`,
        type,
        position: [0, positionY, 0],
        rotation: [0, 0, 0],
        scale: type === 'plane' ? [2, 1, 2] : defaultScales[type] || [1, 1, 1],
        color: type === 'tree' ? '#9ca68d' : '#c7c2b7',
      },
    ]);
    setSelectedId(id);
  };
  ...
  const addKeyframe = () => {
    const existing = snapshotKeysForTrack(keyframes, 'camera').find(
      (key) => key.frame === currentFrame,
    );
  ...
  const addObjectKeyframe = () => {
    ...
    const animatedSource =
      animatedObjects.find((item) => item.id === activeObject.id) || activeObject;
    const source = objectDrafts[activeObject.id] || animatedSource;
    const existing = snapshotKeysForTrack(
  ...
  const deleteObjectKeyframe = (frame) => {
    ...
    if (
      ...
      selectedKeyframe.trackId === activeObject.id &&
  ...
  const moveKeyframes = useCallback((moves) => {
    ...
    for (const move of moves) {
      if (move.kind === 'camera') cameraMap[move.fromFrame] = move.toFrame;
        ...
        objectMaps[move.trackId] = { ...objectMaps[move.trackId], [move.fromFrame]: move.toFrame };
    }
    if (Object.keys(cameraMap).length)
      setKeyframes((channels) => moveCameraFrames(channels, cameraMap));
  ...
  const deleteSelectedKeyframe = () => {
    ...
    else if (activeObject?.id === selectedKeyframeInfo.trackId)
      deleteObjectKeyframe(selectedKeyframeInfo.frame);
  };
  ...
  const handleExportMp4 = async () => {
    ...
    const originalCamera = countChannelKeyframes(keyframes)
      ? {
          ...cameraAtFrame(keyframes, originalFrame, camera.aspectRatio),
    ...
    try {
      ...
      const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-');
      const safeName =
        settings.name.replace(/[\\/:*?"<>|]+/g, '-').trim() || 'director3d-animation';
    ...
    <main className="app-shell" aria-busy={exporting || capturingImage}>
      <header className="topbar">
        ...
        <div className="project-title">
          <i className={`status-dot ${saveStatus === '保存中…' ? '' : 'live'}`} />
          <span className="project-title-name">{settings.name}</span>
          <small>
            {activeShot?.name} · {saveStatus}
          </small>
        </div>
        <div className="export-actions">
          <button
            className="project-export-button"
            onClick={() => saveProject({ download: true })}
            disabled={exporting || capturingImage}
          >
            <Download size={14} /> 导出工程
          </button>
          <button
            className="project-export-button capture-image-button"
            onClick={handleCaptureImage}
            disabled={exporting || capturingImage}
          >
            <FileImage size={14} /> {capturingImage ? '截图中…' : '截图'}
          </button>
          <button
            ...
            disabled={exporting || capturingImage}
          >
            <FileVideo2 size={14} /> {exporting ? `${exportProgress}%` : '导出 MP4'}
          </button>
        </div>
      </header>
      ...
      <div className="workspace">
        ...
        <MemoLeftSidebar
          ...
          onSelectShot={switchShot}
          onAddShot={addShot}
          onDuplicateShot={duplicateShot}
          onDeleteShot={deleteShot}
          onRenameShot={renameShot}
          onCaptureShot={captureShotThumbnail}
        />
        <section className="viewport-shell">
          <div className="viewport-toolbar floating-panel">
            <ToolButton
              icon={MousePointer2}
              label="选择"
              active={!['translate', 'rotate', 'scale'].includes(transformMode)}
            ...
            <ToolButton
              icon={Axis3D}
              label={transformSpace === 'world' ? '世界坐标' : '局部坐标'}
              active={transformSpace === 'local'}
              disabled={transformMode === 'select'}
              onClick={() => setTransformSpace((space) => (space === 'world' ? 'local' : 'world'))}
            />
            <ToolButton
              icon={Magnet}
              label={snapEnabled ? '关闭吸附' : '开启吸附'}
              active={snapEnabled}
              disabled={transformMode === 'select'}
            ...
            <ToolButton
              icon={ImagePlus}
              label={reference.image ? '更换背景图' : '上传背景图'}
              active={Boolean(reference.image)}
            ...
            <div className="floating-panel reference-panel path-panel">
              <label>
                ...
                <input
                  type="range"
                  min="0"
                  max={settings.durationSeconds}
                  step="0.5"
                  value={pathTiming.startSec}
                  onChange={(e) =>
                    updatePathTiming({
                      startSec: clamp(Number(e.target.value), 0, settings.durationSeconds),
                    })
                  }
                />
                <em>{pathTiming.startSec}s</em>
              </label>
              <label>
                <span>终点</span>
                <input
                  ...
                  max={settings.durationSeconds}
                  step="0.5"
                  value={pathTiming.endSec}
                  onChange={(e) =>
                    updatePathTiming({
                      endSec: clamp(Number(e.target.value), 0, settings.durationSeconds),
                    })
                  }
                />
                <em>{pathTiming.endSec}s</em>
              </label>
              <label>
                <span>帧数</span>
                <input
                  ...
                  min="2"
                  max="5"
                  value={pathTiming.keyframeCount}
                  onChange={(e) =>
                    updatePathTiming({ keyframeCount: clamp(Number(e.target.value), 2, 5) })
                  }
                />
                <em>{pathTiming.keyframeCount}</em>
              </label>
              <button
                type="button"
                className={pathDrawing ? 'is-active' : ''}
                onClick={togglePathDrawing}
              >
                {pathDrawing ? '完成绘制' : '开始绘制'}
              </button>
              {paths[selectedId] && (
                <button
                  type="button"
          ...
          {pathContextMenu && (
            <>
              <div
                className="path-context-backdrop"
                onMouseDown={closePathContextMenu}
                onContextMenu={(event) => {
                  event.preventDefault();
                  closePathContextMenu();
                }}
              />
              <div
                className="floating-panel path-context-menu"
                style={{ left: pathContextMenu.x, top: pathContextMenu.y }}
              >
                <button
                  type="button"
                  disabled={pathContextMenu.index < 0}
                  onClick={() => deletePathPoint(pathContextMenu.index)}
                >
                  删除此点
                </button>
                <button type="button" onClick={deleteWholePath} disabled={!paths[selectedId]}>
                  删除整条曲线
                </button>
              </div>
            </>
          ...
          />
          <div
            className={`viewport-view-options floating-panel ${viewOptionsCollapsed ? 'is-collapsed' : ''}`}
          >
            {viewOptionsCollapsed ? (
              <button
                className="view-options-expand"
                onClick={() => setViewOptionsCollapsed(false)}
                title="展开视角工具"
                aria-label="展开视角工具"
              >
                <ChevronLeft size={14} />
              ...
              <>
                <button
                  className={!cameraView ? 'is-active' : ''}
                  onClick={openEditorView}
                  title="使用固定的编辑观察相机自由布置场景"
                >
                  <RotateCw size={14} /> 编辑视角
                </button>
                <button
                  className={cameraView ? 'is-active' : ''}
                  onClick={openCameraView}
                  title="切换到场景中主摄像机的实际画面"
                >
                  <Camera size={14} /> 摄像机视角
                </button>
                {cameraView && (
                  <button
                    className={cameraAnglePanelOpen ? 'is-active' : ''}
                    onClick={() => setCameraAnglePanelOpen((value) => !value)}
                    title="调整参考图视角中的地面和水平线"
                  >
                    <SlidersHorizontal size={14} /> 镜头角度
                  </button>
                )}
                <button
                  className="view-options-collapse"
                  onClick={collapseViewOptions}
                  title="收起视角工具"
                  aria-label="收起视角工具"
                >
          ...
          >
            <div className="viewport-canvas-layer">
              <MainViewport
                key={cameraView ? 'shot-view' : 'scene-view'}
                ...
                referenceVisible={Boolean(reference.image && reference.visible)}
                pathEditing={pathEditing}
                pathDrawing={pathDrawing}
                pathDraft={pathDraft}
                pathAnchorY={pathAnchorY}
                pathDensity={pathTiming.density}
                onPathChange={commitPathPoints}
                onClearPath={clearPath}
                onContextMenuAt={openPathContextMenu}
              />
            </div>
          </ReferenceOverlay>
          ...
          <div className={`camera-monitor is-${monitorMode}`}>
            <div className="monitor-head">
              <div>
                <Video size={13} />
                <strong>主摄像机 01</strong>
              </div>
              <div className="monitor-head-actions">
                {monitorMode === 'minimized' ? (
                  <button title="恢复摄像机窗口" onClick={() => setMonitorMode('normal')}>
                    <Maximize2 size={13} />
                  </button>
                ) : (
                  <>
                    <button title="选择场景中的主摄像机" onClick={() => setSelectedId(CAMERA_ID)}>
                      <Camera size={12} />
                    </button>
                    <button title="最小化摄像机窗口" onClick={() => setMonitorMode('minimized')}>
                      <Minus size={13} />
                    </button>
                    <button
                      title={monitorMode === 'expanded' ? '恢复摄像机窗口大小' : '放大摄像机窗口'}
                      onClick={() =>
                        setMonitorMode((mode) => (mode === 'expanded' ? 'normal' : 'expanded'))
                      }
                    >
                      {monitorMode === 'expanded' ? (
                        <Minimize2 size={13} />
                      ) : (
                        <Maximize2 size={13} />
                      )}
                    </button>
                    <button title="切换到摄像机视角" onClick={openCameraView}>
                      <ZoomIn size={13} />
                    </button>
                  </>
                )}
            ...
            {monitorMode !== 'minimized' && (
              <div className="monitor-frame">
                <div
                  className={`monitor-canvas ${previewAspectClass}`}
                  style={{ '--preview-aspect': previewAspect } as React.CSSProperties}
                >
                  <CameraPreview
                    objects={animatedObjects}
                    animationTime={currentFrame / fps}
                  ...
                  <span className="monitor-focal">
                    {Math.round(displayCamera.focalLength)} mm ·{' '}
                    {aspectLabel(displayCamera.aspectRatio)}
                  </span>
                </div>
              </div>
            )}
        ...
        <MemoInspector
          ...
          seamlessBackground={seamlessBackground}
          onToggleSeamless={() => setSeamlessBackground((value) => !value)}
          lighting={lighting}
          ...
          onToggleLock={() => activeObject && updateSelected({ locked: !activeObject.locked })}
          onGround={groundSelected}
          onResetRotation={resetSelectedRotation}
          onResetScale={resetSelectedScale}
          onSaveCustomPose={saveCustomPose}
          onApplyCustomPose={applyCustomPose}
          onDeleteCustomPose={deleteCustomPose}
        />
        <Timeline
      ...
      {capturingImage && (
        <div
          ...
          style={{ width: exportDimensions.width, height: exportDimensions.height }}
          aria-hidden="true"
        >
          <CameraPreview
            objects={animatedObjects}
            animationTime={currentFrame / fps}
            cameraData={displayCamera}
            cameraAspect={exportDimensions.width / exportDimensions.height}
            lighting={lighting}
            exportMode
          ...
          <div className="export-progress-overlay" role="status" aria-live="polite">
            <div className="export-progress-card">
              ...
              <div className="export-progress-copy">
                <strong>正在编码 MP4</strong>
                <span>
                  {exportDimensions.width} × {exportDimensions.height} · {fps} FPS · 第{' '}
                  {Math.round((exportProgress / 100) * totalFrames)} / {totalFrames} 帧
                </span>
              </div>
