...
const deriveChannels = (registry: PropertyRegistry): EntityChannelMap => {
  ...
  for (const [type, fields] of Object.entries(registry)) {
    ...
    for (const [field, meta] of Object.entries(fields)) {
      ...
      if (!meta.channel)
        throw new Error(`[PROPERTY_REGISTRY] ${type}.${field}: animatable 字段必须声明 channel`);
...
export function snapshotToChannelKeys(
  entityType: string,
  ...
  for (const [channel, fields] of Object.entries(definition)) {
    ...
    if (Object.keys(fieldValues).length) {
      keys[channel] = [
        {
          frame: normalizeFrameNumber(frame),
          interpolation: normalizeInterpolation(interpolation),
          fields: fieldValues,
        },
      ];
    }
...
export function channelsToSnapshotKeys(entityType: string, channels: ChannelTracks = {}) {
  ...
  const merged = new Map<
    number,
    { frame: number; interpolation: string; fields?: Record<string, unknown> }
  >();
    ...
    .map((entry) => {
      ...
      for (const field of fieldList) {
        if (entry.fields?.[field] !== undefined)
          key[field] = cloneProjectValue(entry.fields[field]);
      }
      return key;
    });
}
...
export function countChannelKeyframes(channels: ChannelTracks | ChannelKey[]) {
  ...
  return new Set(
    Object.values(channels).flatMap((list) =>
      (Array.isArray(list) ? list : []).map((key) => key?.frame),
    ),
  ).size;
}
...
export function removeChannelFrames(
  channels: ChannelTracks = {},
  frames: number[] = [],
): ChannelTracks {
...
const collectTrackFrames = (track: ChannelTracks | ChannelKey[]): number[] => {
  ...
  return lists.flatMap((list) =>
    (Array.isArray(list) ? list : []).map((key) => normalizeFrameNumber(key?.frame)),
  );
};

export function clampKeyframeFrames(keys: ChannelTracks | ChannelKey[], maxFrame: number) {
  const clampList = (list: ChannelKey[]) =>
    uniqueSortedKeyframes(
      (Array.isArray(list) ? list : []).map((key) => ({
        ...key,
  ...
  if (keys && typeof keys === 'object') {
    return Object.fromEntries(
      Object.entries(keys)
        .map(([channel, list]) => [channel, clampList(list as ChannelKey[])])
...
export function keyframeMaxFrame(
  cameraKeys: ChannelTracks | ChannelKey[] = [],
  objectTracks: Record<string, ChannelTracks | ChannelKey[]> = {},
) {
  const cameraFrames = collectTrackFrames(cameraKeys);
  const objectFrames = Object.values(objectTracks || {}).flatMap((track) =>
    collectTrackFrames(track),
  );
...
const normalizeCameraField = (field: string, value: unknown, fallbackCamera: ProjectCamera) => {
  ...
  if (field === 'rotation')
    return Array.isArray(value)
      ? finiteVector3(value, fallbackCamera.rotation)
      : cameraRotationToward(fallbackCamera.position, LEGACY_DEFAULT_CAMERA_TARGET);
  if (field === 'focalLength') return clamp(Number(value) || fallbackCamera.focalLength, 18, 120);
  return value;
};

export function normalizeCameraKeyframes(
  keys: ChannelTracks | ChannelKey[] = [],
  fallbackCamera: ProjectCamera = initialCamera,
): ChannelTracks {
  ...
  const channels: ChannelTracks = Array.isArray(keys)
    ? (Array.isArray(keys) ? keys.filter(Boolean) : []).reduce(
        (merged: ChannelTracks, key) =>
          upsertChannelKeys(
            merged,
            snapshotToChannelKeys('camera', key, key.frame, key.interpolation),
          ),
        {},
      )
    : keys || {};
...
export function normalizeProjectSettings(settings: Partial<ProjectSettings> = {}): ProjectSettings {
  const fps = FPS_OPTIONS.includes(Number(settings.fps))
    ? Number(settings.fps)
    : DEFAULT_PROJECT_SETTINGS.fps;
  const durationSeconds = clamp(
    Math.round(Number(settings.durationSeconds) || DEFAULT_PROJECT_SETTINGS.durationSeconds),
    1,
    60,
  );
  return {
    name:
      String(settings.name || DEFAULT_PROJECT_SETTINGS.name)
        .trim()
        .slice(0, 40) || DEFAULT_PROJECT_SETTINGS.name,
    fps,
    durationSeconds,
...
function normalizeObjectField(entityType: string, field: string, value: unknown) {
  ...
  if (field === 'poseTime')
    return Number.isFinite(Number(value))
      ? Number(value)
      : presetPhase(normalizePoseId(value as string));
  if (field === 'rigRoot')
    return Array.isArray(value) ? value.slice(0, 3).map((item) => Number(item) || 0) : [0, 0, 0];
...
const normalizeObjectTrack = (
  ...
  const channels: ChannelTracks = Array.isArray(track)
    ? track
        .filter(Boolean)
        .map((key) =>
          pathFrameSet.has(normalizeFrameNumber(key.frame))
        ...
        .reduce(
          (merged: ChannelTracks, key) =>
            upsertChannelKeys(
              merged,
              snapshotToChannelKeys(entityType, key, key.frame, key.interpolation),
            ),
          {},
        )
    : track || {};
...
export function normalizeObjectTracks(
  ...
  return Object.fromEntries(
    Object.entries(tracks)
      .map(([id, track]): [string, ChannelTracks] => [
        id,
        normalizeObjectTrack(
          track,
          entityTypes[id] || inferChannelEntityType(track),
          paths?.[id]?.sourceKeyframeFrames || [],
        ),
      ])
...
export function normalizeShot(shot, index, fallback) {
  ...
  let objectKeyframes = normalizeObjectTracks(
    shot?.objectKeyframes || shot?.characterKeyframes || fallback.objectKeyframes || {},
    entityTypes,
    paths,
  );
  const timing = normalizeProjectSettings({
    ...
    loopPlayback:
      shot?.loopPlayback ?? shot?.settings?.loopPlayback ?? fallback.settings.loopPlayback,
  });
  const maxFrame = keyframeMaxFrame(keyframes, objectKeyframes);
  if (maxFrame > timing.fps * timing.durationSeconds)
    timing.durationSeconds = clamp(Math.ceil(maxFrame / timing.fps), 1, 60);
  ...
  if (maxFrame > supportedMaxFrame) {
    ...
    objectKeyframes = Object.fromEntries(
      Object.entries(objectKeyframes).map(([id, track]) => [
        id,
        clampKeyframeFrames(track, supportedMaxFrame),
      ]),
    );
  }
  return {
    ...
    name:
      String(shot?.name || defaultShotName(index))
        .trim()
...
export function normalizeProjectData(data) {
  ...
  const settings = normalizeProjectSettings({
    ...data.settings,
    durationSeconds:
      migrateLegacyDefaultDuration && legacySettingsDuration === 5
  ...
  const rawShots =
    Array.isArray(data.shots) && data.shots.length
      ? data.shots
      : [
          {
            id: 'shot-01',
  ...
  const shots = rawShots.slice(0, 30).map((shot, index) => {
    ...
    const migratedShot =
      migrateLegacyDefaultDuration && legacyShotDuration === 5
        ? {
            ...shot,
  ...
  const activeShotId = shots.some((shot) => shot.id === data.activeShotId)
    ? data.activeShotId
    : shots[0].id;
  ...
  return {
    ...
    settings: {
      ...
      loopPlayback: activeShot.loopPlayback,
    },
    activeShotId,
    shots,
...
export function projectData({
  ...
  const serializedShots = sourceShots.map((shot) =>
    shot.id === resolvedActiveShotId ? liveShot : shot,
  );
  return {
    version: PROJECT_VERSION,
...
function locateChannelKey(keys: ChannelKey[], frame: number) {
  if (!Array.isArray(keys) || !keys.length)
    return { exact: null, left: null, right: null, t: 0, bounds: 'empty' };
...
function evaluateNumericChannel(
  keys: ChannelKey[],
  frame: number,
  { angleFields = new Set<string>() } = {},
  ...
  for (const field of allFields) {
    ...
    if (a === undefined || b === undefined) {
      ...
      fields[field] = (a as number[]).map((value, index) =>
        lerpAngle(value, (b as number[])[index], t),
      );
...
function evaluateActionChannel(
  keys: ChannelKey[],
  ...
  const motionEnabled = (key: ChannelKey) =>
    poseCanLoop(poseOf(key)) &&
    (field(key, 'continuousMotion') === undefined
      ? Boolean(object.continuousMotion)
      : Boolean(field(key, 'continuousMotion')));
  const sameState = (leftKey: ChannelKey, rightKey: ChannelKey) =>
    normalizePoseId(poseOf(leftKey)) === normalizePoseId(poseOf(rightKey)) &&
...
function evaluateSkeletonChannel(
  keys: ChannelKey[],
  actionContext: { interpolateState: boolean } | null,
  object: ProjectObject,
  frame: number,
) {
  if (!Array.isArray(keys) || !keys.length) return null;
...
function synthesizeObjectState(
  object: ProjectObject,
  channels: ChannelTracks,
  frame: number,
  fps: number,
  path: object | null = null,
) {
...
export function cameraAtFrame(keyframes, frame, aspectRatio = '16:9') {
  ...
  const channels = Array.isArray(keyframes)
    ? normalizeCameraKeyframes(keyframes, initialCamera)
    : keyframes || {};
  const result = { ...initialCamera, aspectRatio };
  const transform = evaluateNumericChannel(channels.transform || [], frame, {
    angleFields: new Set(['rotation']),
  });
...
export function objectsAtFrame(
  ...
  return objects.map((object) =>
    objectAtFrame(object, objectKeyframes[object.id], frame, fps, paths?.[object.id] ?? null),
  );
}
...
export function createEmptyPath(timing = DEFAULT_PROJECT_SETTINGS) {
  const totalFrames = Math.max(
    ...
    (timing.durationSeconds || DEFAULT_PROJECT_SETTINGS.durationSeconds) *
      (timing.fps || DEFAULT_PROJECT_SETTINGS.fps),
  );
  return {
    startFrame: 0,
...
export function pathSamplePoints(points, closed = false, samplesPerSegment = 8) {
  ...
  for (let segment = 0; segment < segmentCount; segment += 1) {
    const i0 = clampSegmentIndex(
      pts,
      closed ? (segment - 1 + pts.length) % pts.length : segment - 1,
    );
...
export function bakePathKeyframes(path, _fps = DEFAULT_PROJECT_SETTINGS.fps) {
  ...
  for (let index = 1; index < samples.length; index += 1) {
    cumulative.push(
      cumulative[index - 1] + Math.sqrt(pathPointDistanceSq(samples[index - 1], samples[index])),
    );
  }
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    frames.push({
      frame: start + Math.round(u * totalFrames),
...
export function referenceImageFromFile(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      reject(new Error('参考图不能超过 50 MB'));
      return;
    }
