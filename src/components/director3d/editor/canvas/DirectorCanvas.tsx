...
function DirectorViewCameraSync({
  ...
  useLayoutEffect(() => {
    ...
    // 仅当「外部更新」（切视角 / 点坐标轴）时才把相机写回 snapshot；
    // 用户用 OrbitControls 拖动时 snapshot 本身就来自控件，回写会造成
    // onChange → snapshot → 回写 的自反馈无限渲染（drei 10 + three 0.185 会触发）。
    if (viewMode !== "director") return;

    if (!!isExternalUpdateRef?.current || viewModeChanged) {
      if (isExternalUpdateRef) isExternalUpdateRef.current = false;
  ...
  }, [camera, controlsRef, snapshot, viewMode, isExternalUpdateRef]);
