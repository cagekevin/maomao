  ...
  if (transformMode !== 'select') {
    return (
      event.intersections?.some(
        (intersection) => sceneObjectIdFromIntersection(intersection) === selectedId,
      ) ?? false
    );
  }
...
function SceneObject({
  ...
  const scaleAxisLocks = Array.isArray(data.scaleAxisLocks)
    ? data.scaleAxisLocks
    : [false, false, false];
  const stateAnimationTime = Math.max(
    0,
    animationTime -
      (Number.isFinite(Number(data.motionStartTime)) ? Number(data.motionStartTime) : 0),
  );
  useEffect(() => {
    if (
      !groundRequest ||
      groundRequest.id !== data.id ||
      !groupRef.current ||
      data.locked ||
      preview
    )
  ...
  const syncTransform = useCallback(() => {
    ...
    if (transformMode === 'scale' && scaleTransformStart.current) {
      ...
      const locks = Array.isArray(data.scaleAxisLocks)
        ? data.scaleAxisLocks
        : [false, false, false];
      if (data.proportionalScale) {
        const changedAxis = nextScale
          .map((value, axis) => ({
            axis,
            change: locks[axis] ? -1 : Math.abs(value / Math.max(0.0001, baseline[axis]) - 1),
          }))
          .sort((left, right) => right.change - left.change)[0];
        const factor =
          changedAxis?.change > 0.00001
            ? nextScale[changedAxis.axis] / Math.max(0.0001, baseline[changedAxis.axis])
            : 1;
        nextScale = baseline.map((value, axis) =>
          locks[axis] ? value : Math.max(0.05, value * factor),
        );
      } else
        nextScale = nextScale.map((value, axis) =>
          locks[axis] ? baseline[axis] : Math.max(0.05, value),
        );
  ...
  const content = (
    ...
    >
      {data.type === 'person' ? (
        ...
        <DepthMeshModel
          url={String(data.depthMapUrl)}
          settings={
            data.depthSettings as {
              invert?: boolean;
              near?: number;
              far?: number;
              fov?: number;
        ...
        >
          <ImportedModel url={String(data.url)} selected={selected} />
        </Suspense>
      ) : (
        <PrimitiveModel
          type={String(data.type)}
          color={String(data.color || '#c7c2b7')}
          selected={selected}
        ...
        <TransformControls
          ...
          onMouseDown={() => {
            if (transformMode === 'scale' && groupRef.current)
              scaleTransformStart.current = groupRef.current.scale.toArray();
          }}
          onObjectChange={syncTransform}
          onMouseUp={() => {
            syncTransform();
            scaleTransformStart.current = null;
          }}
        />
      )}
    </>
...
function sceneObjectPropsEqual(prev, next) {
  ...
  if (sceneObjectNeedsAnimation(next.data) && prev.animationTime !== next.animationTime)
    return false;
  return true;
}
...
function CameraModel({
  ...
  const groupRotation = useMemo(
    () =>
      new THREE.Euler(data.rotation[0] || 0, data.rotation[1] || 0, data.rotation[2] || 0, 'YXZ'),
    [data.rotation],
  );
  const syncTransform = useCallback(() => {
  ...
  const rig = (
    <group
      ...
      userData={{ sceneObjectId: CAMERA_ID }}
      onPointerDown={selectCamera}
    >
...
function ViewFocusController({
  ...
  useEffect(() => {
    ...
    if (!Number.isFinite(direction.x) || direction.lengthSq() < 0.001)
      direction.set(1, 0.65, 1).normalize();
...
function PathEditor({
  ...
  useEffect(() => {
    if (drawing || selectedDot >= 0)
      document.body.style.cursor = selectedDot >= 0 ? 'grabbing' : 'crosshair';
    else if (hoveredDot >= 0) document.body.style.cursor = 'grab';
    else if (curveHover)
      document.body.style.cursor = CURVE_ADD_CURSOR; // 金色加号：点击可在曲线上加点
  ...
  const syncFromDraft = useCallback(() => {
    const list = (pathDraft?.points || []).map((p) => ({
      x: normalizeNum(p.x),
      y: anchorY,
      z: normalizeNum(p.z),
    }));
  ...
  const handleMove = useCallback(
    (event) => {
      ...
      if (activeMode.current === 'draw') {
        ...
        setPoints((list) =>
          list.map((item, index) => (index === activeMode.current ? point : item)),
        );
      }
      invalidate();
    },
    ...
    <>
      {!referenceVisible &&
        (seamlessBackground ? (
          ...
          <color attach="background" args={['#4b4b48']} />
        ))}
...
function PreviewCameraController({
  ...
  useFrame(() => {
    ...
    const nextAspect =
      Number.isFinite(cameraAspect) && cameraAspect > 0
        ? cameraAspect
  ...
  return (
    <Canvas
      ...
      gl={{
        ...
        preserveDrawingBuffer: exportMode || Boolean(onCanvasReady),
        toneMapping: THREE.ACESFilmicToneMapping,
