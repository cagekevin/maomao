      ...
      <div className="flex gap-3 min-h-0">
          ...
          {CATS.map((c) => {
            ...
            return (
              <div key={c.k} className="flex flex-col gap-1.5 min-w-0">
                <div className="flex items-center gap-1.5 text-caption-sm text-body">
                  {c.icon}
                  <span>{c.n}</span>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  ...
                  <button
                    className="flex items-center justify-center gap-1 h-9 w-full border border-dashed border-edge hover:border-edge-strong rounded-lg text-caption text-muted hover:text-body"
                    onClick={() => {
                      const id = addAsset(updateData, c.k, assets);
                      setEditIdx(id);
                    }}
                  >
                    <Plus size={10} /> 新增{c.n}
        ...
        <div className="w-[340px] shrink-0 border-l border-edge-faint pl-3 min-h-0 overflow-visible">
          {editIdx !== null && assets[editIdx] ? (
            <AssetPanel
              key={assets[editIdx].id}
              asset={assets[editIdx]}
              idx={editIdx}
              data={d}
              updateData={updateData}
              onGen={() => callbacks.onGenerateAssetImage?.(assets[editIdx].id)}
...
function AssetCard({
  ...
  return (
    <div
      className={`relative w-full min-w-0 flex flex-col p-2 bg-surface-1 border rounded-lg transition-colors ${selected ? 'border-blue-500/70' : 'border-edge-faint hover:border-edge-muted'}`}
    >
      {/* 大图区（在上，放大；点击打开抽屉，双击查看大图） */}
      <div
        className="w-full h-20 rounded-md overflow-hidden bg-surface-1 flex items-center justify-center cursor-pointer nodrag"
        onClick={onOpen}
      >
        {asset.loading ? (
          ...
          <span className="text-emerald-400 text-body-xs">✓</span>
        ) : (
      ...
      <div className="absolute top-1 right-1 z-50 shrink-0" ref={moreRef}>
        <button
          className="text-white/80 hover:text-white bg-black/30 hover:bg-black/50 rounded p-0.5"
          title="更多"
          onClick={(e) => {
            e.stopPropagation();
            setMore(!more);
        ...
        {more && (
          <div
            className="absolute right-0 top-full mt-1 bg-surface-menu border border-edge rounded-lg shadow-2xl py-1 min-w-[120px]"
          ...
          >
            <MenuItem
              ...
              text="生成"
              onClick={() => {
                callbacks.onGenerateAssetImage?.(asset.id);
                setMore(false);
              }}
            />
            <MenuItem
      ...
      <div className="flex items-center justify-between gap-1 mt-1.5">
        <span
          className="text-caption-sm text-primary truncate min-w-0 cursor-pointer"
          onClick={onOpen}
        >
          {asset.name}
        </span>
        <input
          type="checkbox"
          checked={!!asset.picked}
          onChange={onTogglePick}
          className="nodrag cursor-pointer shrink-0"
        />
      </div>
...
function MenuItem({
  ...
  return (
    <button
      className={`flex items-center gap-2 w-full px-2.5 py-1.5 text-caption-sm text-left ${danger ? 'text-red-400 hover:bg-red-500/10' : 'text-body hover:bg-surface-hover'}`}
      onClick={onClick}
    >
      {icon}
      {text}
    ...
    const assets = (data.assets || []).map((a, i) =>
      i === idx
        ? {
            ...
            prompt:
              prompt ||
              ZgPrompt(a.category, desc, data.globalStyle, resolveAssetTemplates(data.playbookId)),
          }
        : a,
    );
  ...
  return (
    <div className="flex flex-col gap-3 text-caption-sm">
      <div className="flex items-center justify-between">
        ...
        <button className="text-muted hover:text-white text-sm" title="收起" onClick={onClose}>
          ×
        </button>
      </div>
      <div className="flex flex-col gap-3">
        <label className="text-secondary">
          ...
          <input
            ...
            className="w-full mt-1 bg-surface-strong border border-edge rounded-md px-2 py-1.5 text-primary outline-none nodrag"
          />
        </label>
        <label className="text-secondary">
          描述
          <textarea
            ...
            className="w-full mt-1 h-20 bg-surface-strong border border-edge rounded-md p-2 text-primary outline-none custom-scrollbar nodrag nowheel"
          />
        </label>
        <label className="text-secondary">
          生图提示词
          <textarea
            ...
            className="w-full mt-1 h-40 bg-surface-strong border border-edge rounded-md p-2 text-primary outline-none custom-scrollbar nodrag nowheel"
          />
        </label>
      </div>
      <div className="flex gap-2">
        <button
          className="flex-1 py-2 bg-surface-hover hover:bg-surface-hover-2b text-primary text-body-xs rounded-lg"
          onClick={() => save(false)}
        >
          保存
        </button>
