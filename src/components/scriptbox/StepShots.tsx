...
export default function StepShots({ data, updateData, callbacks }: StepShotsProps) {
  ...
  const commitDlg = () => {
    if (dlgEditing !== null) patchShot(dlgEditing, 'dialogue', textToDlg(dlgText));
    setDlgEditing(null);
  };
  ...
  return (
    <div data-testid="step-shots" className="grid grid-cols-[190px_1fr] gap-4">
      ...
      <div className="flex flex-col gap-3">
        <div>
          ...
          <input
            ...
            className="w-full bg-surface-strong border border-edge rounded-md px-2 py-1 text-caption-sm text-primary outline-none nodrag"
            placeholder="如：中世纪童话 皮克斯3D"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="输入你的故事……"
            className="w-full h-32 bg-surface-strong border border-edge rounded-lg p-2.5 text-body-xs text-primary outline-none resize-none custom-scrollbar nodrag nowheel"
          />
        </div>
        ...
        <div>
          ...
          <div className="flex gap-1">
            {[
              ...
              <button
                key={String(v)}
                onClick={() => setShotCount(v)}
                className={`px-2 py-0.5 text-caption rounded-md border ${String(d.shotCount ?? 'auto') === String(v) ? 'border-white/40 text-white bg-surface-hover' : 'border-edge text-secondary'}`}
              >
                {l}
              </button>
            ))}
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setCustomCount(v);
                setShotCount(v === '' ? 'auto' : Number(v));
              }}
              className="w-20 bg-surface-strong border border-edge rounded-md px-2 py-0.5 text-caption-sm text-primary outline-none nodrag"
            />
          </div>
        </div>
      ...
      <div className="flex flex-col min-h-0 min-w-0">
        ...
        <div className="border border-edge-faint rounded-lg">
          <table className="w-full table-fixed text-caption-sm border-collapse">
              ...
              <tr className="bg-surface-deep">
                {[
                  ...
                  <th
                    key={k}
                    style={{ width: w }}
                    className="text-left px-2 py-1.5 text-muted font-normal whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {shots.length === 0 && (
                <tr>
                  <td colSpan={9} className="text-center py-10 text-muted-2 text-body-xs">
                    输入剧情后点「生成分镜脚本」，自动生成分镜表格
                  </td>
                </tr>
              )}
              {shots.map((s, i) => (
                <tr key={s.id} className="hover:bg-surface-raised">
                  ...
                  <td className="px-1 py-1.5 whitespace-nowrap">
                    <input
                      value={parseShotSeconds(s.duration)}
                      onChange={(e) =>
                        patchShot(i, 'duration', `${parseShotSeconds(e.target.value)}s`)
                      }
                      className="w-8 bg-transparent text-body text-caption-sm outline-none nodrag"
                    />
                  </td>
                  <td className="px-2 py-1.5 align-top" title="双击编辑">
                    <div
                      className="text-primary break-words cursor-text hover:bg-surface-1 rounded px-1 -mx-1 whitespace-normal"
                      onDoubleClick={() => openField(i, 'description', '画面描述')}
                      dangerouslySetInnerHTML={{
                        __html:
                          hlAt(
                            s.description,
                            (d.assets || []).map((a) => a.name),
                          ) || '<span class="text-muted-2">双击编辑画面描述</span>',
                      }}
                    />
                  </td>
                  ...
                  <td className="px-2 py-1.5">
                    <div
                      className="text-body line-clamp-2 break-words cursor-text hover:bg-surface-1 rounded px-1 -mx-1"
                      title="双击编辑"
                      onDoubleClick={() => openDlg(i)}
                    >
                      {dialogueText(s.dialogue) || <span className="text-muted-2">双击编辑</span>}
                    </div>
                  </td>
                  <td className="px-2 py-1.5">
                    <div
                      className="text-body line-clamp-2 break-words cursor-text hover:bg-surface-1 rounded px-1 -mx-1"
                      title="双击编辑"
                      onDoubleClick={() => openField(i, 'sound', '音效')}
                    >
                      {s.sound || <span className="text-muted-2">双击编辑</span>}
                    </div>
                  </td>
                  <td className="px-1.5 py-1.5 whitespace-nowrap">
                    <DropTable
        ...
        <button
          className="self-start flex items-center gap-1 px-2 py-1 mt-2 text-caption-sm text-secondary hover:text-white hover:bg-surface-1 rounded"
          onClick={addShot}
        >
          <Plus size={11} /> 添加镜头
        </button>
        ...
        >
          <div className="flex-1 flex flex-col min-h-0 px-5 pt-4 pb-3">
            <textarea
              ...
              className="flex-1 w-full bg-transparent outline-none resize-none rounded-lg px-0 py-3.5 text-primary custom-scrollbar nodrag nowheel transition-colors"
              style={{ fontSize: '14px', lineHeight: 1.8 }}
            />
            <div className="text-caption-sm text-muted-2 shrink-0 pt-2">
              提示：用 @资产名 引用，如 @小马
            </div>
          </div>
        </ScriptBoxModal>
        ...
        >
          <div className="flex-1 flex flex-col min-h-0 px-5 pt-4 pb-3">
            <textarea
              ...
              placeholder="每行一条：角色名：台词（旁白写：旁白：内容）"
              className="flex-1 w-full bg-transparent outline-none resize-none rounded-lg px-0 py-3.5 text-primary placeholder:text-muted-2 custom-scrollbar nodrag nowheel transition-colors"
              style={{ fontSize: '14px', lineHeight: 1.8 }}
            />
          </div>
        </ScriptBoxModal>
      ...
      {tfShotId !== null &&
        (() => {
          ...
          return (
            <ScriptBoxModal
              title={`镜头${tfShot.index} · 尾帧变体（视觉起点）`}
              onClose={() => setTfShotId(null)}
            >
              {tfShot.tailFrameVariantsLoading && (
                <div className="flex items-center gap-2 py-3 text-secondary">
                  <Loader2 size={14} className="animate-spin" /> 正在抽帧并生成变体…
                </div>
              )}
              {!tfShot.tailFrameVariantsLoading && tfShot.tailFrameVariantsError && (
                <div className="text-red-400 text-caption-sm mb-2">
                  {tfShot.tailFrameVariantsError}
                </div>
              )}
              <div className="flex flex-wrap gap-2">
                {variants.map((v) => (
                  ...
                  >
                    ...
                    />
                    <span className="text-caption-xs text-secondary">
                      {v.id === 'original' ? '原始尾帧' : '重构变体'}
                    </span>
                  </button>
                ))}
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                {!tfShot.tailFrameVariantsLoading && (
                  <button
                    className="text-caption-sm px-2 py-1 rounded border border-edge text-body hover:border-edge-strong"
                    onClick={() => callbacks.onGenerateTailFrameVariants?.(tfShotId)}
                  >
                    重新生成变体
                  </button>
                )}
...
function DropTable({
  ...
  return (
    <div ref={ref} className="relative" onClick={(e) => e.stopPropagation()}>
      <button
        className="w-full text-left text-body text-caption-sm px-1.5 py-0.5 rounded hover:bg-surface-hover line-clamp-2 break-words block"
        title={val || '选择'}
      ...
      {open && (
        <div className="absolute z-modal mt-1 bg-surface-menu border border-edge rounded-lg shadow-2xl py-1 min-w-[90px]">
          {opts.map((o) => (
            <button
              key={o}
              className="block w-full text-left px-2.5 py-1 text-caption-sm text-body hover:bg-surface-hover"
              onClick={() => {
                onPick(o);
                setOpen(false);
              }}
