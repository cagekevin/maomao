...
import {
  ...
  ZgPrompt,
  IMAGE_GEN_DEFAULT,
  SCRIPT_WRITER_FORMAT,
  buildAuditUser,
  normalizeDialogue,
  mergeShotsForVideo,
  MERGE_VIDEO_SYSTEM,
  buildMergedVideoUser,
  SHOT_DIRECTOR_SYSTEM,
  SHOT_AUDIT_SYSTEM,
...
import {
  ...
  SCRIPTBOX_DOWNSTREAM_CELL_W,
  SCRIPTBOX_DOWNSTREAM_CELL_H,
  SCRIPTBOX_DOWNSTREAM_GAP_X,
  SCRIPT_TEXT_TIMEOUT,
 ...
 */
export function createScriptBoxEngine({
  getData,
  updateData,
  addNodes,
  nodeId,
  ...
  const runAbortable = async (
    ...
    try {
      ...
      onReset?.();
      // 【L3c】中止判定统一走 classifyError（看 e.name，不再依赖 message 文案）。
      // 唯一行为变更：业务错误 message 恰含 "abort" 但 name 非 AbortError 时，
      // 由误判「已中止」改为正确 toast（原路径几乎不可达，见 L3c 方案 §1-C2）。
  ...
  const onGenerateScript = async () => {
    ...
    const upstreamImageUrls = (Array.isArray(d.upstreamImages) ? d.upstreamImages : [])
      ...
      .filter(Boolean);
    if (!story) {
      toast('请先输入剧情或连接上游文本节点');
      return;
    }
    const { provider, modelId } = resolveTextModel();
    if (!provider || !modelId) {
      toast('请先在「设置」中配置文本大模型');
      return;
    }
    ...
    const shotCount =
      Number.isFinite(shotCountNum) && shotCountNum > 0 ? Math.floor(shotCountNum) : null;
    const countReq =
      shotCount != null
    ...
    return runAbortable(
      ...
      async (signal) => {
        ...
        if (!r.ok) {
          updateData({ genMask: false });
          if (!r.aborted) toast(r.error || '脚本生成失败');
          logger.error('scriptBox', '生成剧本·上游失败', { nodeId, error: r.error });
          return;
        }
        logger.info('scriptBox', '生成剧本·上游返回', {
          nodeId,
        ...
        if (!parsed.ok) {
          updateData({ genMask: false });
          toast('模型输出的 JSON 不完整或格式有误，请重试');
          logger.error('scriptBox', '生成剧本·JSON解析失败', { nodeId });
          return;
        }

        const m = parsed.data as Record<string, unknown>;
        const rawShots: Record<string, unknown>[] = Array.isArray(m)
          ? (m as Record<string, unknown>[])
          : Array.isArray(m.shots)
            ? (m.shots as Record<string, unknown>[])
            : [];
  ...
  const onGenerateAllAssetImages = async (assetIds?: string[]) => {
    ...
    logger.info('scriptBox', '批量生成资产图·开始', {
      nodeId,
      count: target.length,
      ids: target.map((a) => a.id),
    });
  ...
  const onGenerateShotPrompts = async (shotIds?: string[], feedback?: string) => {
    ...
    logger.info('scriptBox', '生成分镜提示词·开始', {
      nodeId,
      count: target.length,
      provider: provider?.id,
      model: modelId,
    ...
    const genShot = async (shot: ScriptBoxShot) => {
      ...
      logger.info('scriptBox', '生成分镜提示词·单镜开始', {
        nodeId,
        shotId: shot.id,
        index: shot.index,
      });
      ...
      enqueuePatch((latest) => ({
        shots: (latest.shots || []).map((s) =>
          s.id === shot.id ? { ...s, promptLoading: true } : s,
        ),
      }));
      return runAbortable(
        `shot-${shot.id}`,
        () =>
          enqueuePatch((latest) => ({
            shots: (latest.shots || []).map((s) =>
              s.id === shot.id ? { ...s, promptLoading: false } : s,
            ),
          })),
        async (signal) => {
          ...
          const user =
            assembleShotUser(shot, refAssets, globalStyle, {
          ...
          if (!r.ok) {
            ...
            logger.error('scriptBox', '生成分镜提示词·单镜失败', {
              nodeId,
              shotId: shot.id,
              error: r.error,
            });
          ...
          const hasTailRef =
            shot.usePrevShotVideoTail &&
            Array.isArray(shot.prevShotImageRefUrls) &&
            shot.prevShotImageRefUrls.length;
          if (hasTailRef) {
          ...
          enqueuePatch((latest) => ({
            shots: (latest.shots || []).map((s) =>
              s.id === shot.id
                ? { ...s, prompt: pText, videoPrompt: vText, promptLoading: false }
                : s,
            ),
          }));
        },
    ...
    if (Array.isArray(shotIds) && shotIds.length) {
      ...
      toast(
        `已生成 ${target.length - failedCount} 个分镜提示词，${failedCount} 个失败，请重试`,
        'warning',
      );
      logger.error('scriptBox', '生成分镜提示词·部分失败', {
        nodeId,
        total: target.length,
        failed: failedCount,
      });
    ...
    logger.info('scriptBox', '生成分镜提示词·完成', {
      nodeId,
      count: target.length,
      failed: failedCount,
    });
  ...
  const onGenerateShotImage = async (shotId: string, type: string = IMAGE_GEN_DEFAULT) => {
    ...
    updateData({
      shots: d.shots.map((s) => (s.id === shotId ? { ...s, imgGenLoading: true } : s)),
    });
    ...
    return runAbortable(
      ...
      async (signal) => {
        ...
        const prompt =
          parsed.ok && typeof parsed.data === 'string'
            ? parsed.data
            : parsed.ok && (parsed.data as Record<string, unknown>)?.prompt
              ? (parsed.data as Record<string, unknown>).prompt
              : r.ok
                ? r.content
                : '';
        // r.ok 但返回非 JSON：降级写回原文（不丢结果、不吓用户），但必须留痕，便于发现模型格式漂移
        if (r.ok && !parsed.ok)
          logger.warn('scriptBox', 'AI生图提示词·非JSON降级写回原文', {
            nodeId,
            shotId,
            type,
          ...
          shots: getData().shots.map((s) =>
            s.id === shotId
              ? {
                  ...
                  imgGen: {
                    type,
                    label: resolveImageGenSys(d.playbookId, type).label,
                    prompt: prompt || '',
                    ts: Date.now(),
                  },
                }
        ...
        if (r.ok && !r.aborted)
          logger.info('scriptBox', 'AI生图提示词·成功', {
            nodeId,
            shotId,
            type,
          ...
          logger.error('scriptBox', 'AI生图提示词·失败', { nodeId, shotId, type, error: r.error });
  ...
  const onReviewShotPrompt = (
    ...
    const user = buildAuditUser(
      shot,
      field,
      String(feedback).trim(),
      (d.assets || []).map((a) => a.name),
    );
    ...
    const done = new Promise<ReviewShotResult>((res) => {
      resolveResult = res;
    });

    updateData({
      shots: getData().shots.map((s) => (s.id === shotId ? { ...s, promptLoading: true } : s)),
    });
    logger.info('scriptBox', '审计改写提示词·开始', {
      nodeId,
      shotId,
      field,
      feedback: String(feedback).trim(),
    ...
    runAbortable(
      ...
      async (signal) => {
        ...
        if (r.ok) {
          updateData({
            shots: getData().shots.map((s) =>
              s.id === shotId ? { ...s, promptLoading: false } : s,
            ),
          });
          logger.info('scriptBox', '审计改写提示词·成功', {
            nodeId,
            shotId,
            field,
            len: text.length,
          ...
          if (!r.aborted) {
            toast(r.error || '审计改写失败');
            logger.error('scriptBox', '审计改写提示词·失败', {
              nodeId,
              shotId,
              field,
              error: r.error,
  ...
  const onRetryAssetImageUpload = async (assetId: string) => {
    ...
    updateData({
      assets: d.assets.map((a) =>
        a.id === assetId ? { ...a, imageStatus: 'uploading', imageError: undefined } : a,
      ),
    });
  ...
  const onUploadAssetImage = async (assetId: string, file?: File | null) => {
    ...
    updateData({
      assets: getData().assets.map((a) =>
        a.id === assetId
          ? { ...a, loading: true, imageStatus: 'uploading', imageError: undefined }
          : a,
      ),
    });
  ...
  const occupiedLiveSlots = (): Set<number> => {
    ...
    for (const n of getNodes()) {
      if (n?.data?.scriptboxParent === nodeId && Number.isInteger(n.data.scriptboxSlot))
        set.add(n.data.scriptboxSlot as number);
    }
    return set;
  };
    ...
    if (setEdges && nodeId) {
      setEdges((es) => [
        ...es,
        {
          id: `e-${nodeId}-${nodeId2}`,
          source: nodeId,
          sourceHandle: shotHandleId(shotId),
          target: nodeId2,
          type: 'default',
          animated: false,
        },
  ...
  const onGenerateMergedVideo = (shotIds: string[] | undefined, target: string = 'video') => {
    ...
    const refImages = images.map((im) =>
      im && im.url ? { ...im, url: toAbsoluteFileUrl(im.url) } : im,
    );
    const { size } = shotPrefill(picked[0], 'video');
    const firstIdx = picked[0]?.index,
      lastIdx = picked[picked.length - 1]?.index;
    const label =
      firstIdx != null && lastIdx != null
        ? firstIdx === lastIdx
          ? `镜头${firstIdx}视频`
          : `镜头${firstIdx}~${lastIdx}视频`
        : '合并视频';
    ...
    logger.info('scriptBox', '合并生成视频·开始', {
      nodeId,
      shotIds: ids,
      count: picked.length,
      seconds,
    ...
    return runAbortable(
      ...
      async (signal) => {
        ...
        if (!r.ok) {
          if (!r.aborted) toast(r.error || '合并视频提示词生成失败');
          return;
        }
        ...
        logger.info('scriptBox', '合并生成视频·成功', {
          nodeId,
          shotIds: ids,
          count: picked.length,
          seconds,
          promptLen: mergedPrompt.length,
    ...
    return runAbortable(
      ...
      async (signal) => {
        ...
        if (provider && modelId && origUrl) {
          ...
          patchShot((s) => ({
            ...s,
            tailFrameVariantsError: '请先在「设置」中配置资产生图大模型',
          }));
        }

        // 4) 收尾：loading 复位（成功 toast 已在综合图写回分支内发出，此处不再无条件报成功）
        patchShot((s) => ({ ...s, tailFrameVariantsLoading: false }));
        logger.info('scriptBox', '尾帧变体·完成', {
          nodeId,
          shotId,
          angleIds,
          composedOk: !!origUrl,
...
export const TAIL_ANGLE_BY_ID = {
  forward: {
    label: '镜头向前移动',
    action:
      '将摄像机向前（朝向画面主体方向）推进移动，让主体在画面中更突出、更近，画面边缘轻微向外裁剪，但保持主体相对位置居中',
  },
  left: {
    label: '镜头向左移动',
    action: '将摄像机沿水平方向向左平移移动，画面显示更多主体右侧的空间，主体相对视觉重心略微右移',
  },
  closeup: {
    label: '特写镜头',
    action:
      '将景别改为特写：聚焦到主体（人物脸部与肩颈），人物在画面中的占比显著放大，背景相应被进一步虚化/裁掉',
  },
  right: {
    label: '镜头向右移动',
    action: '将摄像机沿水平方向向右平移移动，画面显示更多主体左侧的空间，主体相对视觉重心略微左移',
  },
  rotateLeft45: {
    label: '镜头左转45°',
    action:
      '将摄像机绕垂直轴向左（逆时针）旋转约 45°，画面呈现主体的右后侧斜 45° 视角，更多看到主体右侧面与右后方背景',
  },
  down: {
    label: '镜头向下移动',
    action:
      '将摄像机沿竖直方向向下平移移动，画面呈现略微的俯感，上方空间更多露出，主体相对位置上移',
  },
  rotateRight45: {
    label: '镜头右转45°',
    action:
      '将摄像机绕垂直轴向右（顺时针）旋转约 45°，画面呈现主体的左前侧斜 45° 视角，更多看到主体左侧面与左前方背景',
  },
  topDown: {
    label: '俯视视角',
    action: '将摄像机提升到主体正上方并向下俯视（俯视/顶视视角），摄像机光轴指向地面方向',
  },
  faceCloseup: {
    label: '脸部特写镜头',
    action:
      '将景别改为脸部特写：人物的脸（含额头、下巴、耳朵）占画面核心区，眼神、表情、面部细节锐利清晰，人物发型/妆容保持 100% 不变',
  },
  lowAngle: {
    label: '仰视视角',
    action:
      '将摄像机位置降低到主体下方并向上仰望（仰拍/低角度视角），让人物看起来更挺拔、更有气势，天花板或上方背景更多出现',
  },
  wideAngle: {
    label: '广角镜头',
    action:
      '将镜头改为广角（较短焦距）：画面容纳更广阔的空间，四周有轻微但自然的广角透视感，人物与环境比例不变但环境展现更多',
  },
  backFull: {
    label: '背后全身镜头',
    action:
      '将视角改为从人物正后方看的全身镜头：从头到脚完整入镜，人物背对镜头，能看到发型背面、服装背面、鞋履和前方场景',
  },
  sideFull: {
    label: '正侧面全身镜头',
    action:
      '将视角改为从人物正侧面看的全身镜头：人物侧身完整入镜，能看到侧身轮廓、服装侧面与背景空间关系',
  },
};
...
export function assembleShotUser(
  ...
  const d =
    Number.isFinite(Number(opts.totalShots)) && Number(opts.totalShots) >= u + 1
  ...
  const prevRefs = Array.isArray(shot.prevShotImageRefUrls)
    ? shot.prevShotImageRefUrls.filter(Boolean)
    : [];
  const hasTail = !!shot.usePrevShotVideoTail;
  const ctx = [];
  if (hasTail && prevRefs.length) {
    ctx.push(
      '【视觉起点·必带约束】本镜的起始画面必须以 @图片1（即上一镜视频尾帧图，已作为参考图传入）为视觉起点：人物外貌/服装/发型/妆容/表情/姿态/位置、场景陈设/地面材质/背景物品、光线方向/色彩/材质、道具大小/颜色/摆放，必须与 @图片1 保持 100% 视觉一致。除了用户要求的具体镜头运动/景别变化外，不得添加任何新角色、新道具、新背景元素；不得改变画幅比例、不得跳切、不得更换美术风格。**重要：最终输出的 prompt 与 videoPrompt 文本中必须显式包含 "@图片1" 这一引用标签，让视频生成模型把 @图片1 当作首帧。**',
    );
  } else if (hasTail) {
    ctx.push(
      '【视觉起点·必带约束】本镜的起始画面必须与上一镜视频尾帧保持零帧硬切连续：人物位置、姿态、表情、服装、场景光线和色彩完全一致，不得发生瞬移或跳切。',
    );
  }
  const prevDesc = opts.prevShot?.description && String(opts.prevShot.description).trim();
  if (prevDesc) {
    ctx.push(
      `【剧情承接：上一镜状态描述】${prevDesc}（本镜首秒必须从这一画面状态自然延续，人物位置、姿态、情绪、场景环境不得跳切、不得瞬移、不得更换服装/造型）。`,
    );
  }
  if (shot.description) {
    const seg =
      u <= Math.ceil(d * 0.2)
        ...
        : u <= Math.ceil(d * 0.5)
          ...
          : u <= Math.ceil(d * 0.8)
            ? '情绪爆破/反转打脸/爽点释放'
            : '收束留钩/余韵钩子/为下一部或续作埋伏笔';
    ctx.push(
      `【本镜剧情职责】请围绕本镜画面描述执行，并思考本段在整个戏剧曲线中的叙事功能：${seg}。画面必须服务于这一叙事目的，不要拍无效镜头。`,
    );
  }
  const nextDesc = opts.nextShot?.description && String(opts.nextShot.description).trim();
  if (nextDesc) {
    ctx.push(
      `【剧情钩子：下一镜预告】${nextDesc}（本镜结尾 0.8-1 秒要把人物视线/动作/画面构图自然指向这个结果，做好镜头衔接准备，但不要提前剧透下一镜的核心事件）。`,
    );
  }
  if (ctx.length) {
    rows.push(
      `【本镜剧情上下文（服务于叙事节奏，写 prompt 与 videoPrompt 时请先吃透这一节再落笔）】\n${ctx.join('\n')}`,
    );
  }
  ...
  const others = assets.filter(
    (a) => a.category !== 'character' && a.category !== 'scene' && a.category !== 'prop',
  );
  const allNames = assets.map((a) => a.name).filter(Boolean);
  const roleLines = [];
  if (chars.length) {
    roleLines.push(
      '【本分镜出场人物与叙事分工】（决定站位、景别、视线、动作权重，禁止把背景人物拍成与主角抢焦点）：',
    );
    chars.forEach((c, n) => {
      roleLines.push(
        n === 0
  ...
  if (scenes.length) {
    roleLines.push(
      `【本分镜场景环境角色】${scenes.map((s) => `@${s.name}`).join('、')}：本镜全部动作必须发生在该场景内，场景细节必须与「资产清单」一致，场景不得变换、不得凭空新增外景/路人/NPC。`,
    );
  }
  if (props.length) {
    roleLines.push(
      `【本分镜关键道具】${props.map((s) => `@${s.name}`).join('、')}：这些道具必须在画面中准确呈现并承载叙事功能，不得遗漏或变成模糊背景。`,
    );
  }
  if (others.length) {
    roleLines.push(
  ...
  if (roleLines.length) {
    ...
    rows.push(
      `本分镜涉及以下资源，请在画面里自然呈现，并在画面描述里用 @名称 引用：${allNames.map((a) => `@${a}`).join('、')}`,
    );
  } else {
  ...
  // ── 尾部格式约束（对齐官方 footer）──
  rows.push(
    'prompt 只描述可见画面（主体/动作/场景/景别/光影/色彩/视角/材质/焦段），不要包含对白、旁白、音效、字幕等文字信息；videoPrompt 在 prompt 画面基础上补充镜头连续运动与动态，并把对白/旁白/环境三层音效用"具体角色名说：……""环境层/拟音层/人声层：……"的结构自然融入，使其可直接用于视频生成。只返回包含 prompt、videoPrompt 两个字段的纯 JSON。',
  );
