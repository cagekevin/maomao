...
export default function ScriptBoxPlaybookManager({
  currentId,
  onSelect,
  onClose,
}: ScriptBoxPlaybookManagerProps) {
  ...
  const nameTaken = (t: string, excludeId: string) =>
    playbooks.some((p) => p.label === t && p.id !== excludeId);
        ...
        {renameId === pb.id ? (
          <span className="flex items-center gap-1" onClick={stop}>
            ...
            <button
              ...
              onClick={() => commitRename(pb)}
            >
              <Check size={13} />
            </button>
            <button className="text-muted hover:text-white" onClick={() => setRenameId(null)}>
              <X size={13} />
            </button>
          </span>
          ...
          <span className="flex items-center gap-1 shrink-0 text-2xs text-red-300" onClick={stop}>
            ...
            <button className="hover:text-white" onClick={doDelete}>
              删除
            </button>
            <button className="hover:text-white" onClick={() => setConfirmDel(null)}>
              取消
            </button>
          </span>
        ) : (
    ...
    <ScriptBoxModal
      ...
      footer={
        <div className="flex justify-end px-5 py-3 shrink-0 border-t border-edge-faint">
          <button
            className="px-3 py-1.5 text-body-xs text-secondary hover:text-white rounded-lg"
            onClick={onClose}
          >
            完成
          </button>
      ...
      <div className="flex-1 overflow-auto custom-scrollbar px-3 py-3 flex flex-col gap-4">
        ...
        <div>
          <button
            ...
            className="flex w-full items-center gap-1.5 rounded-md px-2 py-1.5 text-caption-sm text-muted hover:text-primary transition-colors"
          >
        ...
        <div>
          <div className="flex items-center justify-between px-2 py-1.5">
            ...
            {showNew ? (
              <span className="flex items-center gap-1" onClick={stop}>
                ...
                />
                <button className="text-emerald-400 hover:text-emerald-300" onClick={commitNew}>
                  <Check size={13} />
                </button>
                <button
                  className="text-muted hover:text-white"
              ...
              <span className="flex items-center gap-3">
                <button
                  className="flex items-center gap-1 text-2xs text-secondary hover:text-primary"
                  onClick={() => fileRef.current?.click()}
                  title="导入单个 playbook JSON"
                >
                  <Upload size={12} /> 导入
                </button>
                <button
          ...
          <div className="mt-1">
            {mine.length === 0 && (
              <div className="px-2 py-2 text-2xs text-muted-2">
                还没有自己的工作流，点右上「新建」创建
              </div>
            )}
            {mine.map(row)}
