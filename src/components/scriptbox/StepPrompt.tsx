...
export default function StepPrompt({ data, updateData, callbacks }: StepPromptProps) {
  ...
  const genTypes: Record<string, ImageGenTemplate> =
    getPlaybook(d.playbookId).imageGenTemplates || {};
  ...
  const openField = (idx: number, field: string, title: string) => {
    setEditing({ idx, field, title, awaiting: false });
    setDraft(formatLineBreaks(String(shots[idx]?.[field] ?? '')));
    setChatInput('');
  };
  // AI 生图内容（关键帧/四宫格/九宫格/俯视调度图 imgGen）编辑：field 用 'imgGen'，提交写回 shots[i].imgGen.prompt
  const openImgGen = (idx: number, title: string) => {
    setEditing({ idx, field: 'imgGen', title, awaiting: false });
    setDraft(
      formatLineBreaks(String((shots[idx]?.imgGen as ImgGenInfo | undefined)?.prompt ?? '')),
    );
    setChatInput('');
  };
  ...
  const cardFor = (s: ScriptBoxShot, i: number) => {
    const imgGen =
      (s.imgGen as ImgGenInfo | undefined)?.label !== undefined
        ? (s.imgGen as ImgGenInfo)
        : undefined;
    ...
    return (
      <div
        key={s.id ?? i}
        className="relative flex flex-col gap-2 p-3 bg-surface-strong border border-edge-faint rounded-lg"
      >
        ...
        {(s.dialogue || []).length > 0 && (
          <div className="text-caption text-muted italic">「{dialogueText(s.dialogue)}」</div>
        )}
              ...
              <div className="flex flex-wrap items-center gap-1">
                ...
                >
                  {Object.entries(genTypes).map(([key, t]) => (
                    <option key={key} value={key} className="bg-surface-menu text-body">
                      {t.label}
                    </option>
                  ))}
                </select>
              ...
              {imgGen && !s.imgGenLoading && (
                <div className="flex flex-col gap-1 bg-code-bg rounded p-2 border border-edge-faint">
                  <div className="flex items-center gap-1.5">
                    <span className="text-meta text-secondary px-1 py-px rounded bg-surface-1">
                      {imgGen.label}
                    </span>
                    <span className="text-meta text-muted-2">
                      {new Date(imgGen.ts).toLocaleTimeString()}
                    </span>
                    <div className="flex-1" />
                    <button
                      className="flex items-center gap-0.5 text-meta text-secondary hover:text-white"
                      onClick={() => {
                        navigator.clipboard
                          ?.writeText(imgGen.prompt)
                          .catch(() => toastWarning('复制提示词失败'));
                        setCopied((c) => ({ ...c, [i]: true }));
                        setTimeout(() => setCopied((c) => ({ ...c, [i]: false })), 1200);
                      }}
                    >
                      {copied[i] ? (
                    ...
                    <button
                      className="px-1.5 py-0.5 text-meta rounded bg-surface-hover hover:bg-surface-hover-strong text-body"
                      onClick={() => patchShot(i, 'prompt', imgGen.prompt)}
                    >
                      应用到生图
                    </button>
                  </div>
                  <div
                    className="text-caption text-body leading-relaxed line-clamp-4 break-words cursor-text hover:bg-surface-1 rounded px-1 -mx-1"
                    title="双击编辑"
                    onDoubleClick={() => openImgGen(i, imgGen.label)}
                  >
                    <span
        ...
        {cardBusy && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 bg-surface-strong/85 rounded-lg">
            ...
            <span className="text-caption-sm text-body">
              {s.imgGenLoading ? '正在生成关键帧…' : '正在生成提示词…'}
            </span>
          </div>
        )}
      </div>
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center bg-surface-strong rounded-lg p-0.5">
          <button
            className={`flex items-center gap-1.5 px-3 py-1.5 text-caption-sm rounded-md whitespace-nowrap ${view === 'list' ? 'bg-surface-hover text-white' : 'text-secondary'}`}
            onClick={() => setView('list')}
          >
            <Columns2 size={11} /> 列表
          </button>
          <button
            className={`flex items-center gap-1.5 px-3 py-1.5 text-caption-sm rounded-md whitespace-nowrap ${view === 'grid' ? 'bg-surface-hover text-white' : 'text-secondary'}`}
            onClick={() => setView('grid')}
          >
            <LayoutGrid size={11} /> 单镜头
          </button>
        </div>
        <div className="flex-1" />
        ...
        >
          ...
          {mergeLoading
            ? '生成中…'
            : `合并生成视频${selShots.size >= 2 ? `(${selShots.size})` : ''}`}
        </button>
      ...
      {/* 内容 */}
      {shots.length === 0 ? (
        <div className="text-center py-12 text-muted-2 text-body-xs">
          暂无分镜，请先在「确认镜头」步骤生成
        </div>
      ) : view === 'list' ? (
        /* 列表视图：所有镜头小卡片纵向排列 */
      ...
      {(() => {
        ...
        const connected: ConnectedItem[] = (Array.isArray(d.connected) ? d.connected : []).filter(
          (c): c is ConnectedItem =>
            !!c && typeof c === 'object' && typeof (c as { shotId?: unknown }).shotId === 'string',
        );
        return (
          connected.length > 0 && (
        ...
        <ScriptBoxModal
          ...
          footer={
            <div className="flex justify-between items-center px-4 py-3 border-t border-edge-faint shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button
                  className="px-3 py-1 text-caption-sm text-muted hover:text-white transition-colors"
                  onClick={() => {
                    setDraft(null);
                    setEditing(null);
                  }}
  ...
  return (
    <div className="flex flex-col gap-1 bg-code-bg rounded p-2">
      <div className="flex items-center justify-between">
        ...
        {onGen && (
          <button
            className="flex items-center gap-0.5 text-caption text-secondary hover:text-white"
            onClick={onGen}
          >
            {loading ? <Loader2 size={10} className="animate-spin" /> : <ImageIcon size={10} />}{' '}
            生成
          </button>
        )}
      </div>
      <div
        className="text-caption text-secondary leading-relaxed line-clamp-3 break-words cursor-text hover:bg-surface rounded px-1 -mx-1"
        onDoubleClick={onEdit}
        title="双击编辑"
      >
        {text ? (
          <span dangerouslySetInnerHTML={{ __html: hlAt(text, assetNames) }} />
        ) : (
          <span className="text-muted-2">双击编辑</span>
        )}
      </div>
    </div>
  );
...
function ChatEdit({
  ...
  const fieldCurrent = isImgGen
    ? (cur?.imgGen as ImgGenInfo | undefined)?.prompt
    : editing.field === 'prompt'
      ? cur?.prompt
      : cur?.videoPrompt;
    ...
    <div className="flex-1 flex flex-col min-h-0 relative">
      ...
      <div className="flex-1 flex flex-col min-h-0 px-5 pt-4 pb-3">
        <div className="flex items-center gap-1.5 pb-2 shrink-0">
          ...
          <span className="text-caption-sm text-secondary">
            {isImgGen
              ? '提示词（直接编辑）'
              : changed
                ? '预览 · 有改动待应用'
        ...
        <textarea
          ...
          style={{
            fontSize: '14px',
            lineHeight: 1.8,
            color: 'rgb(var(--mao-text-primary))',
