...
{"projectName":"根据故事生成的简洁项目名称，2至8个中文字符，例如：小红帽","globalStyle":"整部片子的统一视觉风格，例如：中世纪童话·皮克斯3D","logline":"一句话故事核心（用于自检，可选）","shots":[{"index":1,"duration":"5s","description":"画面描述：聚焦这一镜要呈现的画面与动作，只对已注册资产用 @名称 引用，例如 @小红帽 走进 @幽暗森林；场景内的普通陈设（家具/门窗/阳台等）与光线/天气/氛围用普通文字描述，不 @","shotType":"景别","lighting":"光影氛围","dialogue":"该镜对白或旁白（如有）","sound":"音效（如有）","motion":"运镜"}],"assets":[{"category":"character|scene|prop","name":"名称","description":"主体外观描述，详细具体。注册 scene/prop 的标准：它在故事里是独立存在的对象——角色会走进去的完整地点，或剧情反复出现、需被镜头单独强调的关键物件。角色身上穿戴的饰品（项圈/铃铛/项链）属角色属性，不单独注册；场景内部家具/门窗/阳台等陈设与光线/天气/氛围不是独立对象，不注册、不 @，归入所属场景的 description；scene=角色站进去的完整空间（如 @客厅、@森林），描述空间结构/主要陈设/氛围光线；prop=全片反复出现、需被镜头单独强调的关键道具（如 @烤鱼）；只描述主体本身，不写构图/视角/布光/负面词，这些由系统自动补全"}]}
【硬性要求】故事中出现的所有场景必须全部注册为 scene 资产，每个分镜的 description 必须 @ 引用其发生的场景；assets 的 name 必须与 shots 的 description 中 @ 引用的名称完全一致；分镜数量与时长要与剧情体量匹配，叙事连贯、有头有尾。本次只需输出 JSON 文本：assets 只写文字描述，不要生成任何图片/参考图，不要调用任何生成类工具。`;
...
export function buildAuditUser(
  ...
  parts.push(
    `【当前${field === 'prompt' ? '生图' : '生视频'}提示词（待改写）】\n${current || '（空）'}`,
  );
  return parts.join('\n\n');
}
...
export function matchAssetNames(text?: string | null, assetNames?: string[] | null): Set<string> {
  const names = Array.isArray(assetNames)
    ? assetNames.map((n) => String(n ?? '')).filter(Boolean)
    : [];
...
export function hlAt(text?: string | null, assetNames?: string[] | null): string {
  ...
  const s = esc(text);
  const names = Array.isArray(assetNames)
    ? assetNames.map((x) => String(x ?? '')).filter(Boolean)
    : [];
...
export function applyTailFrameSelection(
  ...
  return list.map((x, i) =>
    i === idx
      ? {
          ...x,
          usePrevShotVideoTail: useTail,
          selectedTailFrameVariantId: useTail ? variant?.id || 'original' : 'original',
...
export function scriptBoxRemoveResource(
  ...
  const patch: RemoveAssetPatch = {
    assets: next,
    pickedCount: next.filter((a) => a.picked).length,
  };
  if (target?.name) {
...
export function renameAssetRefs(
  ...
  return (shots || []).map((s) => {
    ...
    const next = desc
      .split('@')
      .map((seg, k) =>
        k ? (seg.startsWith(oldName) ? '@' + newName + seg.slice(oldName.length) : '@' + seg) : seg,
      )
      .join('');
...
export function mergeShotsForVideo(
  shots?: Shot[] | null,
  assets?: ScriptAsset[] | null,
): MergedVideoInput {
  const list = Array.isArray(shots) ? shots : [];
  const prompt = list
...
export function buildShotPrompts(shot: Shot): Shot {
  ...
  shot.prompt = base;
  shot.videoPrompt =
    `镜头时长 ${shot.duration || '5s'}，${base}${dlg ? `，对白/旁白：${dlg}` : ''}，音效：${shot.sound || ''}`.trim();
  return shot;
}
...
const SHOT_TPL = [
  {
    desc: '@小马 站在 @河岸边 眺望对岸，晨光洒在草地上。',
    shotType: '中景',
    lighting: '自然光',
    dialogue: [{ kind: '台词', role: '小马', text: '对岸会有更好的草地吗？' }],
    sound: '环境音',
    motion: '缓慢推进',
  },
  {
    desc: '@老牛 缓步走近 @小马，目光温和。',
    shotType: '全景',
    lighting: '暖光',
    dialogue: [{ kind: '旁白', role: '', text: '这头老牛，见过太多四季。' }],
    sound: '环境音',
    motion: '横摇跟随',
  },
  {
    desc: '@松鼠 从树梢跃下，落在 @小马 的背上。',
    shotType: '近景',
    lighting: '自然光',
    dialogue: [{ kind: '台词', role: '松鼠', text: '我带你去看最甜的野莓！' }],
    sound: '动作音',
    motion: '固定',
  },
  {
    desc: '他们一起走向 @森林 深处，@马妈妈 在身后目送。',
    shotType: '大远景',
    lighting: '逆光',
    dialogue: [],
    sound: '鸟鸣',
  ...
  },
  {
    ...
    shotType: '特写',
    lighting: '烛光',
    dialogue: [{ kind: '旁白', role: '', text: '有些路，走过了才知道方向。' }],
    sound: '翻页声',
    motion: '推',
  },
];
 ...
 */
export const MERGE_VIDEO_SYSTEM = `你是资深 AI 视频提示词工程师。下面会给出多个连续镜头，每个镜头单独成一块，标题形如"【镜头N】"（N = 剧本镜号），块内依次是画面描述、景别、光影、运镜、对白、音效。请把这些镜头按顺序合并成一条连贯的中文视频提示词。
