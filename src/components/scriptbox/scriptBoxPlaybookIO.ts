...
export function exportText(pb: Partial<Playbook> | null | undefined): {
  text: string;
  filename: string;
} {
  ...
  const text = JSON.stringify(
    { __meta: { ...FILE_META, exportedAt: Date.now() }, ...rest },
    null,
    2,
  );
...
export function parseImport(text: unknown): ImportResult {
  ...
  const hasContent = [script, shot, audit, qg, assetTemplates, imageGenTemplates].some((v) =>
    Boolean(v),
  );
  if (!hasContent && !label) {
  ...
  const playbook: Playbook = {
    ...
    audit: String(audit || ''),
    qg: String(qg || ''),
    assetTemplates: (typeof assetTemplates === 'object' && !Array.isArray(assetTemplates)
      ? assetTemplates
      : {}) as Record<string, unknown>,
    imageGenTemplates: (typeof imageGenTemplates === 'object' && !Array.isArray(imageGenTemplates)
      ? imageGenTemplates
      : {}) as Record<string, ImageGenTemplate>,
    constraints: {
      image: '',
      video: '',
      ...(typeof constraints === 'object' && constraints ? constraints : {}),
    } as ScriptBoxConstraints,
    negative: {
      common: '',
      image: '',
      video: '',
      ...(typeof negative === 'object' && negative ? negative : {}),
    } as ScriptBoxNegatives,
