...
function eyeLine(e: Detection, imgW: number, imgH: number): EyeLine | null {
  ...
  const lx = kp[1].x * imgW,
    ly = kp[1].y * imgH;
  const rx = kp[0].x * imgW,
    ry = kp[0].y * imgH;
...
function faceBox(e: Detection, imgW: number, imgH: number): FaceBox | null {
  ...
  if (r) {
    ...
    const o = r.dist * 0.7;
    const s = r.dist * 1.55;
    const x = Math.max(0, Math.floor(cx - a));
    const y = Math.max(0, Math.floor(cy - o));
    const w = Math.min(imgW - x, Math.ceil(a * 2));
    const h = Math.min(imgH - y, Math.ceil(o + s));
...
function eyeBox(e: Detection, imgW: number, imgH: number): FaceBox | null {
  ...
  const o = r.dist * 1.15;
  const s = r.dist * 0.42;
  const x = Math.max(0, Math.floor(cx - o));
  const y = Math.max(0, Math.floor(cy - s));
  const w = Math.min(imgW - x, Math.ceil(o * 2));
  const h = Math.min(imgH - y, Math.ceil(s * 2));
  if (w <= 0 || h <= 0) return null;
  return { x, y, w, h };
}
...
export async function applyMosaic(
  dataUrl: string,
  opts: MosaicOptions = {},
): Promise<{ dataUrl: string; width: number; height: number; faceCount: number }> {
  const {
    mode = 'mosaic',
    strength = 0.5,
    color = '#000000',
    format = 'image/png',
    timeoutMs = IMAGE_LOAD_TIMEOUT,
  } = opts;
  const detector = await loadFaceDetector();
  ...
  for (const d of detections) {
    ...
    applyMode(
      ...
      mode,
      strength,
      mode === 'mosaic' || mode === 'blur' ? 'ellipse' : 'rect',
      color,
    );
  }
...
export function drawMosaicOnBox(
  ctx: CanvasRenderingContext2D,
  srcImg: CanvasImageSource,
  box: FaceBox,
  mode: MosaicMode,
...
export const MOSAIC_PALETTE: string[] = [
  '#000000',
  '#ffffff',
  '#ef4444',
  '#22c55e',
