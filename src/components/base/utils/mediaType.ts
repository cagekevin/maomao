 ...
 *  · 消费方：AssetNode.detectMediaType（节点内容态）、useAssetDropPaste.detectFileType（拖入/粘贴建节点）、
 *    assetStore.detectAssetType（素材库分类）、classifyUrl / resolveMediaType（产出类型，含
 ...
 *  - image / video / audio / text：本模块判定的四类（MediaKind）
 *  - other：非以上类型的文件（如压缩包）；empty：无 URL/文件
 */
import type { MediaKind, MediaType, ResultKind } from '@/types';
 ...
 *    而 mediaType 当 audio —— 现统一为 audio；显式 `data:video/ogg` 前缀仍判 video。
 *  · 带查询串/锚点的 URL 先剥 `?` `#` 再比扩展名：此前 detectMediaType 不剥，导致
 *    `a.mp4?token=1` 落到「默认 image」。
 */
const EXT_KIND: ReadonlyArray<readonly [MediaKind, readonly string[]]> = [
...
const EXT_MAP: ReadonlyMap<string, MediaKind> = new Map(
  EXT_KIND.flatMap(([kind, exts]) => exts.map((e) => [e, kind] as const)),
);

/** data: 前缀 → 类别（mime 大类比扩展名权威：data:video/ogg 仍是 video） */
const DATA_PREFIX: ReadonlyArray<readonly [string, MediaKind]> = [
  ['data:video/', 'video'],
  ['data:audio/', 'audio'],
  ['data:image/', 'image'],
  ['data:text/', 'text'],
 ...
 */
export function classifyUrlKind(url: string | null | undefined): MediaKind | null {
  if (!url) return null;
...
export function detectMediaType(url: string | null | undefined): MediaType {
  if (!url) return 'empty';
  return classifyUrlKind(url) ?? 'image'; // data:image / http 图片 / 其它 URL 默认按图片
}
 ...
 */
export function detectFileType(file: TypeProbeLike | null | undefined): MediaType {
  ...
  return classifyUrlKind(file.name) ?? 'other';
}

/**
 * 产出类型判定：产出方声明的 mediaType 优先（blob/无扩展名的产出会被扩展名表漏判，
 * 见 VideoProcessNode extractAudio spawn 的 assetNode），否则按 URL 判型；
 * text / 未识别归 image（产出结果只有图/视频/音频三种）。
 */
export function classifyUrl(url: string | null | undefined): ResultKind {
  const kind = classifyUrlKind(url);
  return kind === 'video' || kind === 'audio' ? kind : 'image';
}

/** resolveMediaType：classifyUrl + 产出方声明优先（useConnectedInputs 连线判型同源） */
export function resolveMediaType(
  url: string | null | undefined,
  mediaType: MediaKind | null | undefined,
): ResultKind {
...
export function isAudio(type: string | null | undefined, url: string | null | undefined): boolean {
  return (
    type === 'audio' || (!!type && type.startsWith('audio')) || classifyUrlKind(url) === 'audio'
  );
}
