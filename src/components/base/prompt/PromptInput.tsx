...
const PromptInput = forwardRef(function PromptInput(
  ...
  const nameSignature = React.useMemo(
    () =>
      [
        ...(refImages || []).map((i) => i.label || ''),
        ...(refTexts || []).map((t) => t.label || ''),
      ].join('\u0001'),
    [refImages, refTexts],
  );
  ...
  React.useEffect(() => {
    ...
    el.innerHTML = '';
    for (const node of renderPromptToNodes(autoLinkAssetsByName(value || '', all), chipMetaMap))
      el.appendChild(node);
    normalizeChipSlots(el);
  ...
  const deleteChipNearCursor = useCallback(() => {
    ...
    if (
      ...
      offset > 0 &&
      isChipEl(node.childNodes[offset - 1])
    ) {
