...
import { subscribeToasts, getToasts, dismissToast } from '@/components/base/core/event/toastStore';
import type { Toast } from '@/components/base/core/event/toastStore';
    ...
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-ceiling flex flex-col items-center gap-1.5 pointer-events-none">
      {items.map((t) => (
        ...
        >
          ...
          <button
            type="button"
            className="ml-0.5 text-secondary hover:text-white transition-colors"
            onClick={() => close(t.id)}
            title="关闭"
          >
            <X size={11} />
