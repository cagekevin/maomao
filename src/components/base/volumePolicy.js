 ...
 */
export const MSG_CONTENT_MAX = 4000      // 单条 user/assistant 正文存储上限（L3 截断用）
export const TOOL_CONTENT_MAX = 6000     // 单条 tool 结果存储上限（L3 截断用）
export const LAST_RESULTS_MAX = 20       // 单条 assistant 消息 lastResults 条数上限（L1/L3）
export const SUMMARY_MAX = 3000          // memory.summary 上限（L3 截断用）
export const FACTS_MAX = 60              // memory.facts 条数上限（L1/L3，保最近）
export const ARTIFACTS_MAX = 20          // memory.artifacts 条数上限（L1/L3，保最近）
export const STEER_QUEUE_MAX = 20        // workflow.steerQueue 条数上限（L1，防排队膨胀）
/** 安全水位：保守远低于典型配额（localStorage ≈5MB、chrome.storage.local ≈10MB），从源头规避超配额写失败 */
export const SAFE_BUDGET_BYTES = 2 * 1024 * 1024
/** L3 单次降级允许的最大截断/剥离轮数（防病态输入死循环） */
const BUDGET_MAX_ITER = 8
/** 超预算后逐层剥离的可丢弃字段名（按代价从低到高，先丢无用户语义的瞬时数据） */
const TRANSIENT_FIELDS = ['pending', 'steerQueue', 'streaming']

/** 截断字符串到 max 字符，超长补省略标记（保留语义可读性；纯函数） */
export function truncateTo(str, max) {
...
export function sanitizeLastResults(list) {
...
export function capConversationMemory(memory) {
...
export function estimateConversationsBytes(conversations) {
 ...
 */
function downgradeConversation(conv, opt = {}) {
 ...
 */
export function applyConversationBudget(conversations, budget, _activeId) {
  ...
  while (bytes > effectiveBudget && iter < BUDGET_MAX_ITER) {
    ...
    let best = null // { path indicators } → 直接重建
    let bestLen = 0
    ...
    projected = projected.map((conv) => {
      if (conv === best.conv) {
        const c = { ...conv, messages: [...(conv.messages || [])] }
