 ...
 * 它原先登记在 `settingRegistry` 的 `app_settings` 里 —— 而那个键**当时整键持久化，且会随云端同步**
 * （⚠️ 2026-09-16 用户裁定后，`app_settings` 已改为**不进云同步**：`contracts.ts` 的 `app_settings`
 * 条目 `backend:'local'` 且缺 `sync:true`。故下面 ② 描述的是**当时**成立、现已不成立的行为，
 * 保留史实以解释"为何当时要迁出"，勿据此认为现状会同步 —— 见判据登记表 §七 #13）。后果：
