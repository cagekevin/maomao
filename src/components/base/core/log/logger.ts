/**
 * 前端日志统一出口（全项目**唯一日志入口**）：所有排查/上报日志走本模块，禁止裸 console.*（否则不进 localTool /api/logs 上报、破坏全链路 grep）。
 *
 * 前端日志（排查用，不建 UI）。
 *
 * 统一输出格式，方便控制台过滤/后端排查：
...
function reportToBackend(p: {
  ...
  try {
    fetch(`${API_BASE}/api/logs`, {
      ...
      keepalive: true,
    }).catch(() => {}); // fire-and-forget：日志上报失败静默；本文件内禁止调 logger 自身（会递归）  // catch-ok: RECURSION_GUARD
  } catch {
...
export const logger: {
   ...
   *      logger.debug('resourceStore', '[SEND] 进入', { ... }, { module: 'asset' })  // 素材库模块位
   */
