...
export function isChromeExtension(): boolean {
  try {
    ...
    // 【TD-18-14 判非债（2026-09-18）· 判据见 ADR-0032】**不是**"未收口的重复"：探测语义（"能力在不在"）的失败
    // 与"不在"对调用方是同一答案，再包一层原语 = 假抽象（Step 3「只有 1 种实现的接缝」）。
    // 同类站点见 `hasLocalStorage`（下方）；别再登记成第二份。
    return false;
  }
}
...
export function hasLocalStorage(): boolean {
  try {
    ...
    // 【TD-18-14 判非债（2026-09-18）· 判据见 ADR-0032】同上：探测语义，不留痕、不收口（失败与"不在"同答案）。
    return false;
  }
}
