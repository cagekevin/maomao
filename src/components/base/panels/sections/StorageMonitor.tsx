    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-surface border border-edge-subtle rounded-xl px-6 py-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted mt-1">查看浏览器与{isExt ? '扩展' : '本地'}存储占用，配额用尽会导致自动保存失败</p>
        </div>
        <button
          type="button"
          onClick={runScan}
      ...
      <div className="bg-surface border border-edge-subtle rounded-xl p-5">
        ...
        <p className="text-xs text-muted mt-1">maomao 项目/设置/素材等数据存放在 {storeLabel}，这是实际占用与压力来源</p>
      ...
      <div className="bg-surface border border-edge-subtle rounded-xl p-5">
        <div className="flex items-center gap-2">
          ...
          {scanning && <span className="text-[10px] text-muted bg-surface-1 px-1.5 py-0.5 rounded-md">扫描中</span>}
        </div>
        ...
        {scanning ? (
          ...
          <div className="mt-3 text-xs text-muted bg-surface-1 rounded-lg px-3 py-2.5">存储读取不可用（隐私模式/权限受限）</div>
        )}
      </div>
    </div>
  )
...
function BrowserQuotaCard({ data, pressure, storeLabel }) {
  ...
  return (
    <div className={`rounded-xl p-5 border ${under ? 'border-amber-500/40 bg-amber-500/10' : 'border-edge-subtle bg-surface'}`}>
      <div className="flex items-center gap-2">
        <Database size={16} className="text-secondary" />
      ...
      <p className="text-xs text-muted mt-1">
        {under
          ...
          : <>IndexedDB / Cache 的浏览器分配配额。maomao 数据存于下方 {storeLabel}，不使用 IndexedDB，故此处占用通常为 0</>}
      </p>
      {data ? (
        <>
          <div className="mt-4 flex items-end gap-2">
            <span className="text-2xl text-strong">{formatBytes(data.usage)}</span>
            <span className="text-xs text-muted mb-1">/ {formatBytes(data.quota)}（{(data.ratio * 100).toFixed(1)}%）</span>
          </div>
          ...
          {idleIndexedDb && (
            <p className="text-xs text-muted mt-2">当前未使用 IndexedDB，占用为 0 属正常；真正占用看下方 {storeLabel}</p>
          )}
        </>
      ) : (
  ...
  return (
    <div>
      ...
      <div className="mt-1.5 h-1.5 rounded-full bg-surface-1 overflow-hidden">
        <div className="h-full bg-cyan-400/70 transition-all" style={{ width: `${Math.min(100, ratio * 100)}%` }} />
      </div>
    </div>
  )
}
