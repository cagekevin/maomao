...
export default function ProjectSelector({ onSwitch, onCreate }: ProjectSelectorProps) {
  ...
  const handleDelete = async () => {
    if (projects.length <= 1) {
      showToast('至少保留一个项目', { type: 'warning' });
      return;
    }
    ...
    <div ref={wrapRef} className="flex items-center gap-1 group/project-selector relative">
      ...
      <div className="relative group/project-dropdown cursor-pointer">
        <div className="flex items-center gap-1 bg-transparent text-body text-sm hover:text-white pl-2 pr-2 py-1 outline-none min-w-[100px] pb-1.5 z-10 relative">
          ...
          <ChevronDown
            size={14}
            className="text-muted group-hover/project-dropdown:text-white transition-colors"
          />
        </div>
        ...
        <div className="absolute left-0 top-full w-48 bg-surface border border-edge rounded-xl shadow-2xl opacity-0 invisible group-hover/project-dropdown:opacity-100 group-hover/project-dropdown:visible transition-all duration-200 z-float overflow-hidden py-1">
          {projects.map((e) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-green-500' : 'bg-transparent'}`}
                />
      ...
      <div className="relative group/project-menu -ml-1 z-10">
        <button
          type="button"
          className="text-muted hover:text-white transition-colors p-1 flex items-center justify-center cursor-pointer border-none bg-transparent"
          aria-label="项目菜单"
        >
          ...
          {projects.length > 1 && (
            <>
              ...
              <button
                ...
                className="w-full text-left px-3 py-2.5 text-sm text-red-400 hover:bg-surface-hover-strong hover:text-red-300 flex items-center gap-2 cursor-pointer border-none bg-transparent"
              >
                <Trash2 size={14} /> 删除项目
              </button>
            </>
      ...
      {modal &&
        createPortal(
          ...
          >
            <div
              className="bg-surface-hover p-4 rounded-lg border border-edge w-64"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="text-primary text-sm font-bold mb-3">
                {modal.mode === 'create' ? '新建项目' : '重命名项目'}
              </div>
              <input
                autoFocus
                value={name}
              ...
              <div className="flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setModal(null)}
                  className="text-secondary hover:text-white text-xs px-2 py-1 cursor-pointer border-none bg-transparent"
                >
                  取消
                </button>
                <button
                  ...
                  className="bg-blue-600 text-white text-xs px-3 py-1 rounded hover:bg-blue-500 cursor-pointer border-none"
                >
