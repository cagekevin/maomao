...
2. 【克制原则，最高优先】简洁精确、够用即可，绝不堆砌字数。普通镜头每个字段 120-280 字、复杂镜头最多 400 字；说清「谁、在哪、做什么、怎么动」即可，删掉逐帧级动作罗列和无关环境细节。
...
【videoPrompt（生视频）要求】
以 prompt 的画面状态为起点，按时间轴写清楚镜头如何发展：起始画面、**摄像机运动**、主体 2-4 个关键动作节点、结束画面。**摄像机运动必须写清**：类型（推/拉/摇/移/跟/环绕/固定）、方向、速度或幅度（如"顺时针环绕180度""缓慢推近"），以及视角（平视/俯视/仰视）。只写主要动作节点变化，不要逐帧级罗列（如"第0.5秒抬右爪、第0.7秒颤一下"这种过细时间点不要写），避免突然跳切、瞬移、身份漂移和无因果动作。
