 ...
 *   · 「画布节点 → 媒体」的真源是 `base/utils/media/nodeMedia.ts`，但它只认 `Node`，不知道素材库；
 *   · 「素材库 → 资源」的真源是 `components/resource/resourceStore.ts` / `base/api/localToolApi.ts`，
 *     但它只认 `Resource`，不知道画布。
...
export interface MediaRefProvider {
   ...
   *
   * 【`entries` 参数（TD-03-15 · 2026-09-18）】分类清单可能**依赖已拉到的数据**
   * （素材库：用户自建的目录不在静态清单里，只能从已拉到的 `type:'folder'` 条目发现）。
   * 消费方把**最近一次 `list()` 的结果**传进来即可；provider **不得**为此另发一次请求
   * ——那会让"分类清单"与"条目列表"来自两次查询，二者可能不一致（＝同一真相两份）。
   * 缺省 `[]`（无数据上下文）时必须能返回**不依赖数据的静态部分**，保证首屏结构稳定。
   */
  categories?: (entries?: readonly MediaRefEntry[]) => MediaRefCategory[];
  /**
