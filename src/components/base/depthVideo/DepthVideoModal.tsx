...
export function DepthVideoModal({ videoUrl, name, onClose, onSave }: DepthVideoModalProps) {
  ...
  function finishRecording(rec: {
    ...
    return withTimeout(
      new Promise<Blob>((resolve) => {
        rec.recorder.onstop = () =>
          resolve(new Blob(rec.chunks, { type: rec.recorder.mimeType || rec.format.mimeType }));
        rec.recorder.resume();
        (rec.track as MediaStreamTrack & { requestFrame?: () => void }).requestFrame?.();
        rec.recorder.stop();
  ...
  async function startConvert() {
    ...
    options.end = Math.min(
      Math.max(options.end, options.start + 1 / options.fps),
    ...
    let rec: {
      ...
      track: MediaStreamTrack;
      chunks: BlobPart[];
      format: { mimeType: string; fellBackToWebm: boolean };
      stream: MediaStream;
    } | null = null;
    ...
    try {
      ...
      logger.debug(
        '深度转视频',
        'fail',
        { message: msg, stack: e instanceof Error ? e.stack : undefined },
        { module: 'depth' },
      );
      if (!abortRef.current) {
        ...
        <div className="flex gap-4 p-4 min-h-0 flex-1">
          ...
          <div className="w-56 shrink-0 flex flex-col gap-3 overflow-y-auto pr-1">
            <div>
              ...
              <select
                ...
                value={engine}
                onChange={(e) => {
                  setEngine(e.target.value as 'ai' | 'fast');
                  if (e.target.value === 'fast') disposeModel();
                }}
              >
                  ...
                  >
                    {DEVICE_OPTIONS.map((d) => (
                      <option
                        key={d.value}
                        value={d.value}
                        disabled={d.value === 'webgpu' && !gpuAvailable}
                      >
                        {d.label}
                        {d.value === 'webgpu' && !gpuAvailable ? '（不可用）' : ''}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div>
              ...
              <select
                className={INPUT_CLS}
                value={outputFormat}
                onChange={(e) => setOutputFormat(e.target.value as 'auto' | 'mp4' | 'webm')}
              >
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-2">
            <div className="flex-1 min-h-0 overflow-hidden rounded-lg border border-edge bg-surface-muted">
              <canvas
                ref={depthCanvasRef}
                className="w-full h-full object-contain nodrag nowheel"
              ></canvas>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 pb-4 flex flex-col gap-2 flex-shrink-0">
          <div className="flex items-center gap-2">
            <div className="flex-1 h-1.5 rounded bg-surface-2 overflow-hidden">
              <div
                ...
                style={{ width: `${progress}%`, background: 'var(--color-primary, #0ea5e9)' }}
              ></div>
            </div>
            <span className="text-caption-sm text-muted tabular-nums w-10 text-right">
              {progress}%
            </span>
          </div>
          <div className="flex items-center justify-between gap-2">
            <span className="text-caption-sm text-muted flex-1 truncate">
              {status}
              {badge ? (
                <span className="ml-2 px-1.5 py-0.5 rounded bg-surface-3 text-white text-caption-sm">
                  {badge}
                </span>
              ) : null}
            </span>
