...
| 文件 | 职责 |
|---|---|
| `contracts.js` | 契约真源（apiRegistry/EVENTS/STORAGE_KEYS/NODE_TYPES），`check:api`/`check:events`/`check:keys` 依赖 |
| `config.js` | 环境变量/常量唯一入口（端口/超时/魔法数字） |
| `contentStore.ts` | 存储横切唯一入口（按 STORAGE_KEYS 路由底层，dev 校验裸 key） |
