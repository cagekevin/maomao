    ...
    img.onload = () => {
      ...
      if (el) {
        const scale = Math.min(
          (el.clientWidth - 32) / img.naturalWidth,
          (el.clientHeight - 32) / img.naturalHeight,
          1,
        );
    ...
    if (dragBox && dragBox.w > 4 && dragBox.h > 4) {
      ...
      if (c && ctx && src) {
        const box = {
          ...
          w: Math.round(dragBox.w),
          h: Math.round(dragBox.h),
        };
        drawMosaicOnBox(
          ctx,
          mode === 'bar' || mode === 'grid' ? src : c,
          box,
          mode,
          strength,
          'rect',
    ...
    <div className="fixed inset-0 z-[9999] bg-black/80 flex flex-col" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onClose}
            className="flex items-center gap-1 px-2.5 h-7 rounded-md text-[12px] text-body bg-surface-hover hover:bg-surface-hover-strong border border-edge cursor-pointer border-none"
          >
            <X size={13} /> 取消
          </button>
          <button
      ...
      {/* 工具条 */}
      <div
        className="flex items-center gap-2 px-4 py-2 bg-surface-strong border-b border-edge-faint flex-wrap"
        onClick={(e) => e.stopPropagation()}
      >
        {MOSAIC_MODES.map((m) => (
          <button
        ...
        <button
          ...
          className="flex items-center gap-1 px-2.5 h-7 rounded-md text-[12px] text-body bg-surface-hover hover:bg-surface-hover-strong border border-edge disabled:opacity-40 cursor-pointer"
        >
          <ScanFace size={13} /> {recognizing ? '识别中…' : '自动识别人脸'}
        </button>
        <button
          onClick={() => canUndo && restoreSnapshot(histIdx - 1)}
          disabled={!canUndo}
          className="flex items-center gap-1 px-2 h-7 rounded-md text-[12px] text-body bg-surface-hover hover:bg-surface-hover-strong border border-edge disabled:opacity-40 cursor-pointer"
          title="撤销 (Ctrl+Z)"
        >
          <Undo2 size={14} />
        </button>
        <button
          onClick={() => canRedo && restoreSnapshot(histIdx + 1)}
          disabled={!canRedo}
          className="flex items-center gap-1 px-2 h-7 rounded-md text-[12px] text-body bg-surface-hover hover:bg-surface-hover-strong border border-edge disabled:opacity-40 cursor-pointer"
        ...
        <button
          onClick={() => canUndo && restoreSnapshot(0)}
          disabled={!canUndo}
          className="flex items-center gap-1 px-2 h-7 rounded-md text-[12px] text-body bg-surface-hover hover:bg-surface-hover-strong border border-edge disabled:opacity-40 cursor-pointer"
          title="重置"
        >
