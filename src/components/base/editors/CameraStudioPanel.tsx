...
function StudioViewport({
  ...
  useEffect(() => {
    ...
    const ringMaterial = new THREE.MeshBasicMaterial({ color: 0x676767, transparent: true, opacity: 0.7 })
    ...
    const camBodyMat = new THREE.MeshStandardMaterial({ color: 0x3d6fd6, roughness: 0.45, metalness: 0.3 })
    const camLensMat = new THREE.MeshStandardMaterial({ color: 0x24356e, roughness: 0.4, metalness: 0.5 })
    const camRingMat = new THREE.MeshStandardMaterial({ color: 0xe8c46a, roughness: 0.25, metalness: 0.85 }) // 金色装饰环
    const camGlassMat = new THREE.MeshPhysicalMaterial({
      color: 0x0e3a66, roughness: 0.05, metalness: 0.6, clearcoat: 1.0,
      emissive: 0x1a5fa8, emissiveIntensity: 0.45,
    })
    ...
    // 镜头筒 (放置在 +Z 轴)
    const camLensBarrel = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.22, 32), camLensMat)
    camLensBarrel.rotation.x = Math.PI / 2
    camLensBarrel.position.z = 0.19
    
    // 镜头前端金属环
    const camAccentRing = new THREE.Mesh(new THREE.CylinderGeometry(0.132, 0.132, 0.015, 32), camRingMat)
    ...
    // 金属快门按钮
    const camShutter = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 0.02, 16), camRingMat)
    ...
    const lightFrameMat = new THREE.MeshStandardMaterial({ color: 0xd06a28, roughness: 0.55, metalness: 0.25 })
    const lightMountMat = new THREE.MeshStandardMaterial({ color: 0x9c4a1a, roughness: 0.5, metalness: 0.4 })
    ...
    const lightPanel = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({
        color: 0xffffff, emissive: 0xffd98a, emissiveIntensity: 1.8,
...
function CameraStudioPanel({ isOpen, imageUrl, onClose, onGenerate }: CameraStudioPanelProps) {
  ...
  const [activeControl, setActiveControl] = useState<'camera' | 'lighting'>(() => _lastActiveControl ?? 'camera')
  const [cameraState, setCameraState] = useState<CameraStudioCameraState>(() => _lastCamera ? { ..._lastCamera } : { ...DEFAULT_CAMERA_STATE })
  const [lightState, setLightState] = useState<CameraStudioLightState>(() => _lastLight ? { ..._lastLight } : { ...DEFAULT_LIGHT_STATE })
  ...
  return createPortal(
    ...
    >
      <div
        ...
        style={{ width: 'min(96vw, 1180px)', height: 'min(90vh, 760px)', maxWidth: '98vw', maxHeight: '98vh' }}
      >
        {/* ===== 顶部栏 ===== */}
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-edge-faint bg-surface-1 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            {/* 模式 Tab */}
            <div className="flex gap-0 bg-surface rounded-lg p-0.5 border border-edge" role="tablist" aria-label="摄影棚模式">
              <button
                type="button"
                role="tab"
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <aside className="w-60 flex-shrink-0 border-l border-edge-faint bg-surface-1 flex flex-col">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateCamera({ distance: e.target.value as CameraDistance })}
                      >
            ...
            <div className="border-t border-edge-faint p-3 space-y-2 bg-surface">
              {/* 提示词预览 */}
              <div className="bg-surface-strong border border-edge rounded-lg px-3 py-2.5 text-caption-sm leading-relaxed text-body min-h-[60px] max-h-20 overflow-y-auto break-words whitespace-pre-wrap">{prompt}</div>
              {/* 按钮 */}
