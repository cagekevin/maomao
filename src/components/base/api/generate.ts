...
async function attachImages(
  ...
  const contentArr: ChatContentBlock[] = Array.isArray(last.content)
    ...
    : [
        {
          type: 'text',
          text: typeof last.content === 'string' ? last.content : String(last.content || ''),
        },
      ];
...
export async function chatStream(opts: {
  ...
  try {
    ...
    if (he && he.name === 'HttpError') {
      const fake = {
        status: he.status ?? 0,
        text: async () => (he.data != null ? JSON.stringify(he.data) : ''),
      };
      throw new Error(
        opts.parseAgentError ? await opts.parseAgentError(fake, '调用失败') : '调用失败',
      );
    }
