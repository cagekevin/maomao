...
export class PasteCommand extends Command {
  ...
  execute(): void {
    ...
    const minStart = Math.min(...this.clipboardItems.map((item) => item.element.startTime));
    ...
    for (const [trackId, items] of itemsByTrackId) {
      ...
      const sourceTrackIndex = updatedTracks.findIndex((track) => track.id === trackId);
...
function resolveTargetTrackIndex({
  ...
  const highestCompatibleIndex = tracks.findIndex((track) => track.type === trackType);
