...
export const extractTimelineAudio = async ({
  ...
  if (audioMixSources.length === 0) {
    ...
    const silentSamples = new Float32Array(Math.ceil(silentDuration * SAMPLE_RATE) * NUM_CHANNELS);
    return createWavBlob({ samples: silentSamples });
  }

  const totalSamples = Math.ceil(totalDuration * SAMPLE_RATE);
  const mixBuffers = [new Float32Array(totalSamples), new Float32Array(totalSamples)];
...
async function decodeAndMixAudioSource({
  ...
  for await (const { buffer, timestamp } of sink.buffers(source.trimStart, trimEnd)) {
    ...
    const outputStartSample = Math.floor((source.startTime + relativeTimelineTime) * SAMPLE_RATE);
    ...
    for (let ch = 0; ch < NUM_CHANNELS; ch++) {
      ...
      for (let i = 0; i < resampledLength; i++) {
        ...
        const sample1 = sourceIdx + 1 < channelData.length ? channelData[sourceIdx + 1] : sample0;
...
function writeString({ view, offset, str }: { view: DataView; offset: number; str: string }): void {
