...
export function bindAction<A extends TAction>(action: A, handler: TActionFunc<A>) {
...
export function unbindAction<A extends TAction>(action: A, handler: TActionFunc<A>) {
  const handlers = boundActions[action];
  if (!handlers) return;
...
type InvokeActionFunc = {
  (action: TActionWithOptionalArgs, args?: undefined, trigger?: TInvocationTrigger): void;
