...
async function migrateProject({
  ...
  const legacyBookmarks = Array.isArray(project.bookmarks) ? project.bookmarks : null;
  ...
  const migratedScenes = await Promise.all(
    scenes.map(async (scene) => {
      ...
      const shouldLoadTracks = !Array.isArray(existingTracks) || existingTracks.length === 0;
...
async function loadTracksFromLegacyDB({
  ...
  const adapter = new IndexedDBAdapter<LegacyTimelineData>(sceneDbName, 'timeline', 1);

  let data = await adapter.get('timeline');

  if (!data && isMain) {
    const projectAdapter = new IndexedDBAdapter<LegacyTimelineData>(projectDbName, 'timeline', 1);
...
async function transformTracks({
  ...
  return transformedTracks.filter((track): track is TimelineTrack => track !== null);
}
...
function transformTextTrack({ track }: { track: Record<string, unknown> }): TimelineTrack {
  const elements = Array.isArray(track.elements) ? track.elements : [];
...
function transformAudioTrack({ track }: { track: Record<string, unknown> }): TimelineTrack {
  const elements = Array.isArray(track.elements) ? track.elements : [];
...
function getCurrentSceneId({ value, scenes }: { value: unknown; scenes: unknown[] }): string {
...
function getNumberValue({ value, fallback }: { value: unknown; fallback: number }): number {
  return typeof value === 'number' ? value : fallback;
}

function getStringValue({ value, fallback }: { value: unknown; fallback: string }): string;
