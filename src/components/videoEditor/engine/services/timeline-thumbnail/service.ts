...
function makeKey({ mediaId, snappedTime }: { mediaId: string; snappedTime: number }): string {
...
class TimelineThumbnailCache {
  ...
  async loadThumbnailsBatch({
    ...
    try {
      ...
      videoEditorLogger.warn(`Batch thumbnail load failed for ${mediaId}:`, error);
    }
  }
  ...
  private async generateThumbnail({
    ...
    try {
      ...
      videoEditorLogger.warn(`Failed to get thumbnail for ${mediaId} at ${time}:`, error);
      return null;
    }
  }
  ...
  private addToTimeIndex({ mediaId, time }: { mediaId: string; time: number }): void {
  ...
  private async initializeSink({
    ...
    try {
      ...
      videoEditorLogger.error(`Failed to init thumbnail sink for ${mediaId}:`, error);
      return null;
    }
  }
}
