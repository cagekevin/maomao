...
export function TimelineRuler({
  ...
  const editor = useEditor('timeline', 'project');
  ...
  const bufferPx = isZoomTransition ? Math.max(200, (scrollLeft + viewportWidth) * 0.15) : 200;

  useLayoutEffect(() => {
    prevZoomRef.current = zoomLevel;
  }, [zoomLevel]);

  const visibleStartTime = Math.max(0, (scrollLeft - bufferPx) / pixelsPerSecond);
  const visibleEndTime = (scrollLeft + viewportWidth + bufferPx) / pixelsPerSecond;

  const startTickIndex = Math.max(0, Math.floor(visibleStartTime / tickIntervalSeconds));
  const endTickIndex = Math.min(tickCount - 1, Math.ceil(visibleEndTime / tickIntervalSeconds));
