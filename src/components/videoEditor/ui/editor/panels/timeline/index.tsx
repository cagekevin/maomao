...
export function Timeline() {
  ...
  const { handleRulerMouseDown: handlePlayheadRulerMouseDown } = useTimelinePlayhead({
    zoomLevel,
    rulerRef,
    rulerScrollRef: tracksScrollRef,
    tracksScrollRef,
  ...
  const contentWidth = timelineDuration * TIMELINE_CONSTANTS.PIXELS_PER_SECOND * zoomLevel;
  ...
  const showSnapIndicator =
    snappingEnabled && currentSnapPoint !== null && (dragState.isDragging || isResizing);

  const { handleTracksMouseDown, handleTracksClick, handleRulerMouseDown, handleRulerClick } =
    useTimelineSeek({
      playheadRef,
      trackLabelsRef,
      rulerScrollRef: tracksScrollRef,
      ...
      <div className="relative flex flex-1 flex-col overflow-hidden" ref={timelineRef}>
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="bg-background flex w-28 shrink-0 flex-col border-r">
            <div className="bg-background flex h-6 items-center justify-between px-3">
            ...
            {tracks.length > 0 && (
              ...
              >
                <ScrollArea className="size-full overflow-y-hidden" ref={trackLabelsScrollRef}>
                  <div className="flex flex-col gap-1">
                    {tracks.map((track, index) => {
                      ...
                      const isBeingDragged =
                        reorderState.isDragging && reorderState.dragTrackId === track.id;
                      ...
                      return (
                        ...
                        >
                          <GripVertical
                            className="ve-drag-handle group-hover:text-muted-foreground size-3.5 shrink-0 cursor-grab active:cursor-grabbing"
                            onMouseDown={(event) => {
          ...
          <div className="relative flex flex-1 flex-col overflow-hidden" ref={tracksContainerRef}>
            <SelectionBox
            ...
            <TimelinePlayhead
              ...
              isSnappingToPlayhead={showSnapIndicator && currentSnapPoint?.type === 'playhead'}
            />
            <ScrollArea
              className="size-full"
              ref={tracksScrollRef}
                        ...
                        <ContextMenu key={track.id}>
                          ...
                          <ContextMenuContent className="z-[200] w-40">
                            ...
                            >
                              <Eye />
                              <span>
                                {canTrackBeHidden(track) && track.hidden ? '显示轨道' : '隐藏轨道'}
                              </span>
                            </ContextMenuItem>
                            <ContextMenuItem
                              onClick={(e) => {
...
function TrackToggleIcon({
  ...
  return (
    <>
      {isOff ? (
        <icons.off className="text-destructive size-4 cursor-pointer" onClick={onClick} />
      ) : (
        <icons.on className="text-muted-foreground size-4 cursor-pointer" onClick={onClick} />
      )}
    </>
  );
}
