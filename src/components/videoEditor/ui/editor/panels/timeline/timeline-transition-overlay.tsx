...
function TransitionJunctionOverlay({
  ...
  const maxDuration = Math.min(fromElement.duration, toElement.duration, MAX_TRANSITION_DURATION);
  ...
  const handleDragStart = useCallback(
    ({ event, side }: { event: React.PointerEvent; side: 'left' | 'right' }) => {
      if (!transition) return;
  ...
  const handleDragMove = useCallback(
    ({ event }: { event: React.PointerEvent }) => {
      ...
      const deltaDuration = (side === 'left' ? -deltaX : deltaX) / pixelsPerSecond;
    ...
    return (
      ...
      >
        <Popover open={isOpen} onOpenChange={setIsOpen}>
          ...
          <PopoverContent className="w-56 p-0" side="top" align="center" sideOffset={8}>
            <TransitionPicker currentType={null} onSelect={handleSelectTransition} />
          </PopoverContent>
        </Popover>
      </div>
    );
