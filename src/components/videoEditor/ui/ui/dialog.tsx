...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn('fixed inset-0 z-modal-raise backdrop-blur-sm bg-black/10', className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'bg-popover fixed top-[50%] left-[50%] z-modal-raise grid w-[calc(100%-2rem)] max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-lg border shadow-lg duration-200',
        className,
      )}
      onCloseAutoFocus={(e) => {
        e.stopPropagation();
