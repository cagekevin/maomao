...
import { AddMediaAssetCommand } from '@videoEditor/engine/commands/media/add-media-asset';
...
import { findAvailableVideoTrackAbove, getVisualSourceTime } from '../element-utils';
...
describe('freeze frame timeline decisions', () => {
  ...
  test('undo removes only the generated asset', async () => {
    ...
    const deleteMediaAsset = spyOn(storageService, 'deleteMediaAsset').mockResolvedValue();

    try {
      editor.media.setAssets({ assets: [imageAsset('existing')] });
      const command = new AddMediaAssetCommand('project', imageAsset('freeze'), true);
      command.execute();
      ...
      expect(editor.media.getAssets().map(({ id }) => id)).toEqual(['existing', 'later']);
