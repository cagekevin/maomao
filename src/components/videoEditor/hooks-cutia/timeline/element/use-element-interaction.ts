    ...
    const handleMouseMove = ({ clientX, clientY }: MouseEvent) => {
      ...
      if (isPendingDrag && pendingDragRef.current) {
        ...
        if (deltaX > DRAG_THRESHOLD_PX || deltaY > DRAG_THRESHOLD_PX) {
          ...
          const adjustedTime = Math.max(0, mouseTime - pendingDragRef.current.clickOffsetTime);
      ...
      const movingElement = sourceTrack?.elements.find(({ id }) => id === dragState.elementId);
  ...
  useEffect(() => {
    ...
    const handleMouseUp = ({ clientX, clientY }: MouseEvent) => {
      ...
      const isDraggedElementSelected = selectedElements.some(
        (element) =>
          element.elementId === dragState.elementId && element.trackId === dragState.trackId,
      );
      const hasBatchSelection = isDraggedElementSelected && selectedElements.length > 1;
