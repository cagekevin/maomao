...
export function useTimelineZoom({
  ...
  const [zoomLevel, setZoomLevelRaw] = useState(() => {
    if (initialZoom !== undefined) {
      ...
      return Math.max(minZoom, Math.min(TIMELINE_CONSTANTS.ZOOM_MAX, initialZoom));
    }
    return minZoom;
  });
  ...
  useEffect(() => {
    if (initialZoom !== undefined && !hasInitializedRef.current) {
      hasInitializedRef.current = true;
      setZoomLevel(Math.max(minZoom, Math.min(TIMELINE_CONSTANTS.ZOOM_MAX, initialZoom)));
      return;
    }
    setZoomLevel((prev) => {
      if (prev < minZoom) {
  ...
  const wrappedSetZoomLevel = useCallback(
    (zoomLevelOrUpdater: number | ((prev: number) => number)) => {
      setZoomLevel((prev) => {
        const nextZoom =
          typeof zoomLevelOrUpdater === 'function' ? zoomLevelOrUpdater(prev) : zoomLevelOrUpdater;
        const clampedZoom = Math.max(minZoom, Math.min(TIMELINE_CONSTANTS.ZOOM_MAX, nextZoom));
        return clampedZoom;
      });
    },
    [minZoom, setZoomLevel],
  ...
  useLayoutEffect(() => {
    ...
    if (sliderPercent >= TIMELINE_CONSTANTS.ZOOM_ANCHOR_PLAYHEAD_THRESHOLD) {
      ...
      const playheadPixelsAfter = playheadTime * TIMELINE_CONSTANTS.PIXELS_PER_SECOND * zoomLevel;
