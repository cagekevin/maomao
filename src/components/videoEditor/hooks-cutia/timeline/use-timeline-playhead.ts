...
export function useTimelinePlayhead({
  ...
  const handleScrub = useCallback(
    ({ event }: { event: MouseEvent | React.MouseEvent }) => {
      ...
      const rawTime = Math.max(
        0,
        Math.min(duration, clampedMouseX / (TIMELINE_CONSTANTS.PIXELS_PER_SECOND * zoomLevel)),
      );
  ...
  useEffect(() => {
    ...
    const playheadPixels = playheadPosition * TIMELINE_CONSTANTS.PIXELS_PER_SECOND * zoomLevel;
  ...
  }, [playheadPosition, zoomLevel, rulerScrollRef, tracksScrollRef, isScrubbing, isPlaying]);
