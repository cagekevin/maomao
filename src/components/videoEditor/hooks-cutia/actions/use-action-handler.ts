...
export function useActionHandler<A extends TAction>(
  ...
  const stableHandler = useCallback((...parameters: [TArgOfAction<A>, TInvocationTrigger?]) => {
    (handlerRef.current as (...handlerParameters: [TArgOfAction<A>, TInvocationTrigger?]) => void)(
      ...parameters,
    );
  }, []) as TActionFunc<A>;
