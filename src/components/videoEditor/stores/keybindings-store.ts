...
export const useKeybindingsStore = create<KeybindingsState>()(
  persist(
    (set, get) => ({
      ...
      validateKeybinding: (key: ShortcutKey, action: TActionWithOptionalArgs) => {
    ...
    }),
    {
      ...
      migrate: (persisted, version) => runMigrations({ state: persisted, fromVersion: version }),
    },
  ),
);
...
function generateKeybindingString(ev: KeyboardEvent): ShortcutKey | null {
  ...
  if (isDOMElement(target) && isTypableDOMElement({ element: target as HTMLElement })) return null;
