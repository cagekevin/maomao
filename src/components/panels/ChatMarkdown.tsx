  ...
  for (const match of value.matchAll(INLINE_PATTERN)) {
    ...
    if (raw.startsWith('@')) {
      ...
      nodes.push(
        <span
          key={`ref-${start}`}
          className="inline-flex items-center rounded bg-surface-hover px-1 py-px font-mono text-caption-sm text-blue-300"
        >
          {raw}
        </span>,
      );
      ...
      nodes.push(
        <code
          key={`code-${start}`}
          className="rounded bg-surface-hover px-1 py-0.5 font-mono text-[0.9em] text-body"
        >
          {raw.slice(1, -1)}
        </code>,
      );
...
function InlineWithImages({
  ...
  if (last < value.length)
    nodes.push(<InlineContent key={`t${spans.length}`} value={value.slice(last)} />);
  return <>{nodes}</>;
}
...
function ChatMarkdown({ value, onOpenImage }: ChatMarkdownProps) {
  ...
  while (index < lines.length) {
    ...
    if (heading) {
      ...
      if (level === 1)
        blocks.push(
          <h1 key={`h-${index}`} className={headingClasses}>
            {content}
          </h1>,
        );
      else if (level === 2)
        blocks.push(
          <h2 key={`h-${index}`} className={headingClasses}>
            {content}
          </h2>,
        );
      else
        blocks.push(
          <h3 key={`h-${index}`} className={headingClasses}>
            {content}
          </h3>,
        );
      index += 1;
    ...
    if (listItem) {
      ...
      blocks.push(
        ordered ? (
          <ol key={`list-${index}`} className="my-1.5 list-decimal space-y-1 pl-5 text-body/90">
            {children}
          </ol>
        ) : (
          <ul key={`list-${index}`} className="my-1.5 list-disc space-y-1 pl-5 text-body/90">
            {children}
          </ul>
        ),
      );
