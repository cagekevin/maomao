...
export function isAssetUrl(u: string): boolean {
  u = String(u || '')
    .trim()
    .toLowerCase();
  if (!u) return false;
...
export function extractImageSpans(text: string): ImageSpan[] {
  ...
  //    排除被 blob:/ipfs:/ipns: 协议前缀包裹的 URL（与 isAssetUrl「临时协议不渲染」契约冲突）
  for (const m of text.matchAll(
    /(https?:\/\/[^\s]+?|data:image\/[^\s"]+?)(?:[)\]}'"，。、,!?；;]+)?(?=\s|$)/gi,
  )) {
    const before = text.slice(Math.max(0, (m.index ?? 0) - 5), m.index ?? 0);
    // 排除 blob:/ipfs:/ipns: 临时协议前缀（与 isAssetUrl「临时协议不渲染」契约冲突）
  ...
  return spans
    .filter((s) => isAssetUrl(s.url) && !seen.has(s.url) && seen.add(s.url))
