              ...
              <>
                ...
                {selectedIds.length > 0 && (
                  <>
                    ...
                    <button
                      className="node-btn-settings hover:text-red-400"
                      title="删除已选"
                      onClick={(e) => {
                        e.stopPropagation();
                        removeSelected();
            ...
            {!expanded && current && (
              <>
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    downloadUrl(current.url, current.label);
                  }}
                >
                  <Download size={14} />
                </button>
              ...
              <>
                ...
                {images.length > 1 && (
                  <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-1.5 py-1 rounded-full bg-black/60 backdrop-blur-sm opacity-0 group-hover/node:opacity-100 transition-opacity">
                    <button
                      className="w-6 h-6 rounded-full bg-black/60 hover:bg-black/80 text-white flex items-center justify-center cursor-pointer border-none"
                      title="上一张"
                      onClick={(e) => {
                        e.stopPropagation();
                        prev();
                    ...
                    <span className="px-1 text-caption text-white tabular-nums select-none">
                  ...
                  {images.map((img, index) => {
                    ...
                    return (
                      <div
                        ...
                        title={
                          img.label || (isSel ? '点击取消选择' : '点击选择 (按住 Ctrl 设为默认图)')
                        }
                      >
                        <LazyImage
                          ...
                          imgClassName="w-full h-full object-cover bg-surface-black"
                        />
                        <button
                        ...
                        {isActive && (
                          <span className="absolute bottom-1 left-1 px-1 py-px rounded bg-blue-500 text-white text-2xs font-medium">
                            默认
                          </span>
                        )}
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              if (menuIndex === index) {
                                ...
                                if (left < 8) left = 8;
                                if (left + 130 > window.innerWidth - 8)
                                  left = window.innerWidth - 8 - 130;
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <button
                    className="w-full text-left px-2 py-1.5 text-caption-sm text-red-400 hover:bg-surface-hover-strong rounded flex items-center gap-2 cursor-pointer"
                    onClick={() => {
                      removeAt(idx);
                      close();
                    }}
