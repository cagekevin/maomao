      ...
      try {
        ...
        if (mergeMode === 'grid') {
          ...
          const imgs = await Promise.all(
            cells.map((c) => (c ? loadImageOrNull(c) : Promise.resolve(null))),
          );
          let cellW = cellSize;
          let cellH = cellSize;
            ...
            >
              {preview && (
                <img
                  ...
                  alt="Preview"
                  className="w-full h-full max-h-[360px] object-cover block pointer-events-none"
                />
              )}
              ...
              {mergeMode === 'longImage' && (
                ...
                >
                  {longList.map((n, t) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          minWidth: longDirection === 'horizontal' ? 40 : 0,
                          minHeight: longDirection === 'vertical' ? 40 : 0,
                        }}
                      >
                        <img
                          src={render(n)}
                        ...
                        />
                        <span
                          className={`absolute m-1 px-1 py-px rounded text-meta font-mono pointer-events-none ${isDragTo ? 'bg-blue-500 text-white' : 'bg-black/60 text-white/80'}`}
                        >
                          {t + 1}
                        </span>
                      </div>
                  ...
                  {longList.length === 0 && (
                    <div className="w-full h-full flex items-center justify-center text-caption text-muted-2">
                      连线图片加入
                    </div>
                  )}
                </div>
                    ...
                    <button
                      ...
                      className={`text-caption px-2 py-0.5 rounded border transition-colors cursor-pointer ${
                        rows === p.rows && cols === p.cols
                          ...
                          : 'bg-surface-hover border-edge text-secondary hover:text-white hover:border-edge-strong'
                      }`}
                      onClick={() => {
                        setRows(p.rows);
                        setCols(p.cols);
                        setGridText(`${p.rows}x${p.cols}`);
                      }}
                    >
                      {p.label}
                    </button>
                ...
                {showCustom && (
                  <div className="flex items-center gap-1.5 text-caption text-secondary">
                    ...
                    <input
                      ...
                      value={rows}
                      onChange={(e) => {
                        const v = clamp(parseInt(e.target.value || '1', 10) || 1, 1, 20);
                        setRows(v);
                        setGridText(`${v}x${cols}`);
                      }}
                      className="w-12 bg-surface-hover text-primary rounded px-1.5 py-0.5 border border-edge outline-none"
                    />
                    <span>×</span>
                    <span>列</span>
                    <input
                    ...
                    <input
                      ...
                      className="flex-1 bg-surface-hover text-primary rounded px-1.5 py-0.5 border border-edge outline-none"
                    />
                  </div>
                )}
                <div className="flex items-center gap-2">
                  <input
                    className="flex-1 bg-surface-hover text-body text-xs rounded px-2 py-1 border border-edge outline-none"
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (v === 'auto') setAutoSize(true);
                      else {
                        setAutoSize(false);
                        setCellSize(Number(v));
                      }
                    }}
                    className="bg-surface-hover text-body text-xs rounded px-2 py-1 border border-edge outline-none flex-1"
                    title="单格尺寸"
                  >
            ...
            {mergeMode === 'longImage' && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-caption text-secondary">
                  <span>方向</span>
                  <button
                    className={`px-2 py-0.5 rounded border transition-colors cursor-pointer ${longDirection === 'vertical' ? 'bg-blue-500/15 border-blue-500/60 text-blue-300' : 'bg-surface-hover border-edge text-secondary'}`}
                    onClick={() => setLongDirection('vertical')}
                  >
                    垂直
                  </button>
                  <button
                    className={`px-2 py-0.5 rounded border transition-colors cursor-pointer ${longDirection === 'horizontal' ? 'bg-blue-500/15 border-blue-500/60 text-blue-300' : 'bg-surface-hover border-edge text-secondary'}`}
                ...
                <div className="flex items-center gap-2 text-caption text-secondary">
                  ...
                  <input
                    ...
                    className="w-20 bg-surface-hover text-primary rounded px-1.5 py-0.5 border border-edge outline-none disabled:opacity-50"
                  />
                  <span>间距</span>
                  <input
                    type="number"
                    ...
                    value={longGap}
                    onChange={(e) =>
                      setLongGap(clamp(parseInt(e.target.value || '0', 10) || 0, 0, 200))
                    }
                    className="w-14 bg-surface-hover text-primary rounded px-1.5 py-0.5 border border-edge outline-none"
                  />
                </div>
              </div>
            )}
