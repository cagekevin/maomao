...
function PanoramaNode({ id, data, selected }: PanoramaNodeProps) {
  ...
  const renderRatioSelect = () => (
    <>
      <span className="text-caption-sm text-secondary px-2 whitespace-nowrap shrink-0">
        截图比例
      </span>
      <select
        value={aspectRatio}
        onChange={(e) => setAspectRatio(e.target.value)}
        className="bg-transparent text-primary text-caption-sm pl-1 pr-4 py-0.5 outline-none cursor-pointer text-center font-bold"
      >
        {RATIO_OPTIONS.map((r) => (
      ...
      {aspectRatio === 'custom' && (
        <div className="flex items-center gap-1 ml-2 mr-2 border-l border-white/20 pl-2 shrink-0">
          <input
            type="number"
            value={customDim.w}
            onChange={(e) => setCustomDim((d) => ({ ...d, w: Number(e.target.value) }))}
            className="w-8 bg-transparent text-primary text-caption-sm outline-none text-center border-b border-transparent focus:border-white/50"
          />
          <span className="text-muted">:</span>
          <input
            type="number"
            value={customDim.h}
            onChange={(e) => setCustomDim((d) => ({ ...d, h: Number(e.target.value) }))}
            className="w-8 bg-transparent text-primary text-caption-sm outline-none text-center border-b border-transparent focus:border-white/50"
          />
        </div>
      )}
  ...
  const renderShotButtons = ({ size = 20 }: { size?: number }) => (
    <div
      className="absolute top-1/2 left-2 -translate-y-1/2 flex flex-col items-center gap-1 z-30 bg-black/60 p-1.5 rounded-xl backdrop-blur-md border border-white/10 shadow-popover nodrag"
      onClick={(e) => e.stopPropagation()}
    >
      <button
        onClick={() => doCapture([0])}
        title="当前视角截图"
        className={`p-2.5 text-secondary hover:text-white hover:bg-white/10 rounded-lg transition-all active:scale-95 cursor-pointer ${shotKind === 'current' ? 'text-white bg-white/10' : ''}`}
      >
        <CircleDot size={size} className={shotKind === 'current' ? 'animate-spin' : ''} />
      </button>
      <button
        onClick={() => doCapture([90, 180, 270, 0])}
        title="四大视角截图 (90,180,270,0度)"
        className={`p-2.5 text-secondary hover:text-white hover:bg-white/10 rounded-lg transition-all active:scale-95 cursor-pointer ${shotKind === 'four' ? 'text-white bg-white/10' : ''}`}
      >
        <Grid3X3 size={size} className={shotKind === 'four' ? 'animate-spin' : ''} />
      </button>
      <button
        onClick={() => doCapture([0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330])}
        title="12大视角截图 (每30度)"
        className={`p-2.5 text-secondary hover:text-white hover:bg-white/10 rounded-lg transition-all active:scale-95 cursor-pointer ${shotKind === 'twelve' ? 'text-white bg-white/10' : ''}`}
      >
  ...
  const renderRatioOverlay = () =>
    !capturing && (
      <div
        className="absolute inset-0 pointer-events-none z-dropdown"
        style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}
      >
        <div
          className="border-[2px] border-dashed border-white/30 shadow-[0_0_0_9999px_rgba(0,0,0,0.4)]"
          style={{ aspectRatio: ratioStr, height: '100%', maxHeight: '100%', maxWidth: '100%' }}
        />
      </div>
    );
  ...
  const renderFullscreen = () =>
    ...
    createPortal(
      <div
        className="fixed inset-0 z-modal bg-surface-sunken flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="absolute top-4 right-4 z-10 flex gap-2">
          <button
            onClick={() => doCapture([0])}
            className="bg-white hover:bg-gray-200 text-black px-5 py-2.5 rounded-lg flex items-center gap-2 shadow cursor-pointer border-none"
          >
            <Camera size={18} /> 截图
          </button>
          <button
            onClick={() => setFullscreen(false)}
            className="bg-black/50 hover:bg-white/10 text-white p-2.5 rounded-lg backdrop-blur border border-white/10 transition-colors cursor-pointer"
          >
            <X size={22} />
          </button>
        </div>
        ...
        {/* 截图比例选择器（全屏） */}
        <div
          className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center bg-black/60 backdrop-blur-md border border-white/10 rounded-lg p-1 shadow-popover nodrag"
          onClick={(e) => e.stopPropagation()}
        >
          {renderRatioSelect()}
        </div>
          ...
          <div className="flex items-center gap-2 nodrag">
            ...
            {configOpen && (
              <select
                value={panoType}
                onChange={(e) => {
                  const v = e.target.value as 'sphere' | 'cylinder';
                  setPanoType(v);
                  setNodes((ns) =>
                    ns.map((n) => (n.id === id ? { ...n, data: { ...n.data, panoType: v } } : n)),
                  );
                }}
          ...
          <div className="flex-1 relative overflow-hidden rounded-xl bg-surface-black">
            ...
            {imgError ? (
              <div className="w-full h-full flex flex-col items-center justify-center text-red-400/80 bg-surface-muted">
                ...
                <span className="text-caption mt-1 text-red-400/60">
                  图片可能已失效或跨域不可访问
                </span>
              </div>
            ) : (
