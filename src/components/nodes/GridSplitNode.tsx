...
function GridSplitNode({ id, data, selected }: GridSplitNodeProps) {
  ...
  const closedCount =
    splitMode === 'lasso'
      ? lassoShapes.filter((s) => s.closed && s.points.length >= 3).length
      : cells.length;
  // 行列计数（复刻 Lo.jsx L/R）
  const rowCount = splitMode === 'grid' ? rows : splitMode === 'manual' ? hLines.length + 1 : 1;
  const colCount =
    splitMode === 'grid' ? cols : splitMode === 'manual' ? vLines.length + 1 : closedCount;
      ...
      <>
        ...
        >
          {lassoShapes.map((s) => {
            ...
            const d =
              s.points.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x * 100} ${p.y * 100}`).join(' ') +
              (s.closed ? ' Z' : '');
            ...
            return (
              <path
                ...
                d={d}
                fill={
                  s.closed ? (isActive ? 'rgba(59,130,246,0.35)' : 'rgba(59,130,246,0.18)') : 'none'
                }
        ...
        <div ref={contentRef} className="p-2 space-y-2 relative z-10 rounded-xl w-full">
          ...
          {imageUrl ? (
            <div className="relative w-full">
              ...
              {splitMode === 'manual' && (
                <div className="mt-1 text-caption text-muted leading-tight">
                  双击空白加水平线，Shift+双击加垂直线；拖动线移动；Shift+点击线删除。
                </div>
              )}
              {splitMode === 'lasso' && (
          ...
          <div className="space-y-2 nodrag">
                ...
                {showCustom && (
                  <div className="flex items-center gap-1.5 text-caption text-secondary">
                    ...
                    <input
                      ...
                      value={rows}
                      onChange={(e) =>
                        setRows(clamp(parseInt(e.target.value || '1', 10) || 1, 1, 20))
                      }
                    ...
                    <input
                      ...
                      value={cols}
                      onChange={(e) =>
                        setCols(clamp(parseInt(e.target.value || '1', 10) || 1, 1, 20))
                      }
                      className="w-14 bg-surface-hover text-primary rounded px-1.5 py-0.5 border border-edge outline-none"
                    />
                  </div>
            ...
            {splitMode === 'manual' && (
              <div className="flex items-center justify-between text-caption text-secondary">
                <span>
                  {rowCount} 行 × {colCount} 列 = {closedCount} 块
                </span>
                <button
