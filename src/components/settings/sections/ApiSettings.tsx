/**
 * 设置分区（由 `SettingsFrame` 静态 import 装配）。
 *
 * @public 跨边界消费者：src/components/settings/SettingsFrame.tsx
 */
import React from 'react';
import {
  Check,
  Star,
  Server,
  RefreshCw,
  CircleDot,
  AlertCircle,
  Image,
  MessagesSquare,
  Video,
  Plus,
  Trash2,
} from 'lucide-react';
import { showToast } from '@/components/base/core/event/toastStore';
import { PROVIDER_PROTOCOL_LABELS } from '@/components/settings/providerUrlAdapters';
import {
  useProviders,
  load,
  select,
  setPrimary,
  test,
  fetchModels,
  applyFetchedModels,
  closeFetchedModels,
  save,
  enabledProviders,
  candidateProviders,
  toggleProviderEnabled,
  addModel,
  removeModel,
  updateProviderField,
} from '@/components/settings/providerStore';
import FetchModelsModal from './FetchModelsModal';
import type { RawModel } from '@/components/base/utils/providerModels';
import type { FetchedModelGroup } from './FetchModelsModal';

/**
 * 设置分区 · 服务商配置（新时代「切哪个用哪个」，docs/96、docs/101）。
 *
 * 【语义（用户拍板）】能实现的厂商/模型都已配进 config/providers JSON，用户想用哪个就用哪个——
 * 本页 = 「集中展示已用厂商 + 切换当前想用 + 管理模型/厂商的用户配置」（2026-09-03 增删能力落地）。
 *  - 已用厂商列表 = enabled 的厂商；未启用的作「增加厂商」候选（点击启用加入）。
 *  - 厂商增删 = 用户配置层：增=从候选启用；移出=置 enabled=false（内置 revert 出厂，不删文件）。
 *    系统 13 厂商定义不动；后端 config JSON 仍唯一数据源，save() 写回后端。
 *  - 模型增删 = 用户配置层：每能力清单可增（手动输入 id/名）删（单项移除）；也可「拉取模型→勾选」。
 */
export default function ApiSettings() {
  const {
    providers,
    selectedId,
    loading,
    dirty,
    saving,
    testingId,
    fetchingId,
    fetchedModels,
    testResult,
  } = useProviders();

  React.useEffect(() => {
    let cancelled = false;
    load().catch((_e) => {
      if (!cancelled) showToast('加载服务商失败', { type: 'error' });
    });
    return () => {
      cancelled = true;
    };
  }, []);

  // 已用（enabled）厂商作展示，未启用作「增加厂商」候选；selected 只在已用里选（全局过滤）
  const usedProviders = React.useMemo(() => enabledProviders(providers), [providers]);
  const candidate = React.useMemo(() => candidateProviders(providers), [providers]);
  const selected = React.useMemo(
    () => usedProviders.find((p) => p.id === selectedId) || usedProviders[0] || null,
    [usedProviders, selectedId],
  );
  const isPrimary = !!selected?.primary;

  const handleSave = async () => {
    const r = await save();
    showToast(r.ok ? '已保存当前服务商' : '保存失败：' + r.error, {
      type: r.ok ? 'success' : 'error',
    });
  };
  const handleSetPrimary = async () => {
    if (!selected) return;
    setPrimary(selected.id);
    showToast('已设为当前使用（记得保存）', { type: 'success' });
  };
  const handleFetch = async () => {
    if (!selected) return;
    const r = await fetchModels(selected.id);
    if (r.error) showToast('拉取模型失败：' + r.error, { type: 'error' });
    else if (r.warning) showToast(r.warning, { type: 'warning' });
    else if (r.pending)
      showToast(`已拉取 ${r.total ?? 0} 个模型，请勾选要写入的`, { type: 'success' });
    else showToast(`已拉取 ${r.total ?? 0} 个模型`, { type: 'success' });
  };
  const [showAddModal, setShowAddModal] = React.useState(false);
  const handleAddCandidate = (id: string) => {
    toggleProviderEnabled(id, true);
    select(id);
    setShowAddModal(false);
    showToast('已加入「' + (providers.find((p) => p.id === id)?.name || id) + '」— 记得保存', {
      type: 'success',
    });
  };
  const handleRemove = () => {
    if (!selected) return;
    toggleProviderEnabled(selected.id, false);
    showToast('已移出「' + (selected.name || selected.id) + '」— 记得保存', { type: 'success' });
  };

  return (
    <div className="st-section">
      {/* 顶部操作栏 */}
      <div className="st-page-head">
        <div>
          <h2 className="st-section-title">
            <Server size={17} /> 服务商配置
          </h2>
          <p className="st-section-sub">切换当前想用的服务商；节点仍可各自独立选择任意厂商模型</p>
        </div>
        <div className="st-inline">
          {dirty && <span className="st-chip">有未保存的修改</span>}
          <button
            className="st-btn st-btn--primary"
            type="button"
            onClick={handleSave}
            disabled={saving || !dirty}
          >
            <Check size={14} /> {saving ? '保存中…' : '保存更改'}
          </button>
        </div>
      </div>

      <div className="st-split">
        {/* 左：厂商列表（已用） */}
        <aside className="st-pane">
          <button
            className="st-btn st-btn--block"
            type="button"
            onClick={() => setShowAddModal(true)}
            disabled={candidate.length === 0}
          >
            <Plus size={14} /> 增加厂商
          </button>
          <div className="st-card">
            <div className="st-between">
              <span className="st-label">已用服务商</span>
              <span className="st-chip">{usedProviders.length}</span>
            </div>
            <div className="st-list">
              {loading ? (
                <div className="st-empty">加载中…</div>
              ) : usedProviders.length === 0 ? (
                <div className="st-empty">
                  暂无已用服务商
                  <br />
                  点「增加厂商」启用一个
                </div>
              ) : (
                usedProviders.map((p) => (
                  <ProviderListItem
                    key={p.id}
                    p={p}
                    active={p.id === selected?.id}
                    onSelect={() => select(p.id)}
                  />
                ))
              )}
            </div>
          </div>
          <p className="st-hint">
            已用服务商与其模型由后端 config JSON
            管理；增加/移出厂商、增删模型后点右上「保存更改」写回后端。
          </p>
        </aside>

        {/* 右：详情 */}
        <main className="st-pane">
          {!selected ? (
            <div className="st-empty">请选择一个服务商查看</div>
          ) : (
            <div className="st-stack">
              {/* header */}
              <div className="st-card">
                <div className="st-card-head">
                  <h3 className="st-card-title">{selected.name || selected.id}</h3>
                  {selected.primary && <Star size={14} />}
                  {selected.readonly && <span className="st-chip">内置</span>}
                  {/* 「当前使用」是**状态**不是动作 ⇒ 归头部（与名字 / 内置 同排），不混在动作栏里造成高低不一 */}
                  {isPrimary && (
                    <span className="st-chip st-chip--ok">
                      <Star size={14} /> 当前使用
                    </span>
                  )}
                  <span className="st-chip st-push">
                    {PROVIDER_PROTOCOL_LABELS[selected.protocol || ''] ||
                      selected.protocol ||
                      '未知'}
                  </span>
                </div>
                <input
                  className="st-input"
                  value={
                    (selected.base_url as string) ||
                    (selected._relay
                      ? ((selected._relay as { defaultBaseUrl?: string })?.defaultBaseUrl ?? '')
                      : '') ||
                    ''
                  }
                  onChange={(e) => updateProviderField(selected.id, 'base_url', e.target.value)}
                  placeholder="请求地址（如 http://127.0.0.1:18080）"
                />

                {/* 动作栏 */}
                <div className="st-toolbar">
                  {!isPrimary && (
                    <button className="st-btn st-btn--sm" type="button" onClick={handleSetPrimary}>
                      <Star size={14} /> 设为当前使用
                    </button>
                  )}
                  <button
                    className="st-btn st-btn--sm"
                    type="button"
                    onClick={() => test(selected.id)}
                    disabled={testingId === selected.id}
                  >
                    <RefreshCw size={14} /> {testingId === selected.id ? '测试中…' : '测试连接'}
                  </button>
                  <button
                    className="st-btn st-btn--sm"
                    type="button"
                    onClick={handleFetch}
                    disabled={fetchingId === selected.id}
                  >
                    <CircleDot size={14} /> {fetchingId === selected.id ? '拉取中…' : '拉取模型'}
                  </button>
                  {/* 破坏性操作：红只在这里出现（移出厂商） */}
                  <button
                    className="st-btn st-btn--sm st-btn--danger"
                    type="button"
                    onClick={handleRemove}
                  >
                    <Trash2 size={14} /> 移出服务商
                  </button>
                  {testResult && selectedId === selected.id && (
                    <div
                      className={
                        testResult.ok ? 'st-notice st-notice--ok' : 'st-notice st-notice--danger'
                      }
                    >
                      {testResult.ok ? <Check size={14} /> : <AlertCircle size={14} />}
                      {testResult.ok
                        ? `连接成功（${testResult.status ?? ''}${testResult.stage ? ' · ' + testResult.stage : ''}）`
                        : `连接失败：${testResult.error || testResult.status || '未知'}`}
                    </div>
                  )}
                </div>
              </div>

              {/* 模型清单（可编辑：每项可删 + 每能力可手输新增；也走「拉取模型→勾选」） */}
              <CapabilityModels
                title="图片模型"
                icon={<Image size={14} />}
                models={selected.image_models || []}
                providerId={selected.id}
                cap="image_models"
              />
              <CapabilityModels
                title="聊天模型"
                icon={<MessagesSquare size={14} />}
                models={selected.chat_models || []}
                providerId={selected.id}
                cap="chat_models"
              />
              <CapabilityModels
                title="视频模型"
                icon={<Video size={14} />}
                models={selected.video_models || []}
                providerId={selected.id}
                cap="video_models"
              />

              <FetchModelsModal
                open={!!fetchedModels}
                fetched={
                  fetchedModels
                    ? {
                        image_models: fetchedModels.image_models,
                        chat_models: fetchedModels.chat_models,
                        video_models: fetchedModels.video_models,
                      }
                    : null
                }
                existing={
                  selected
                    ? {
                        image_models: selected.image_models,
                        chat_models: selected.chat_models,
                        video_models: selected.video_models,
                      }
                    : null
                }
                fetching={fetchingId === selected?.id}
                onClose={() => closeFetchedModels()}
                onConfirm={(selectedModels: FetchedModelGroup) => {
                  if (fetchedModels) {
                    applyFetchedModels(fetchedModels.id, selectedModels);
                    const total =
                      (selectedModels.image_models?.length || 0) +
                      (selectedModels.chat_models?.length || 0) +
                      (selectedModels.video_models?.length || 0);
                    showToast(`已写入 ${total} 个模型（记得点「保存更改」）`, { type: 'success' });
                  }
                }}
              />

              {/* 增加厂商：从候选（未启用）选一个启用 */}
              {showAddModal && (
                <div className="st-overlay" onClick={() => setShowAddModal(false)}>
                  <div className="st-modal" onClick={(e) => e.stopPropagation()}>
                    <div className="st-modal-head">
                      <h3 className="st-modal-title">增加厂商</h3>
                      <button
                        className="st-icon-btn"
                        type="button"
                        onClick={() => setShowAddModal(false)}
                      >
                        ×
                      </button>
                    </div>
                    <div className="st-modal-body">
                      {candidate.length === 0 ? (
                        <div className="st-empty">没有可增加的厂商（都已启用）</div>
                      ) : (
                        <div className="st-list">
                          {candidate.map((p) => (
                            <button
                              key={p.id}
                              className="st-list-item"
                              type="button"
                              onClick={() => handleAddCandidate(p.id)}
                            >
                              <Plus size={14} />
                              <span className="st-list-item-title st-grow">{p.name || p.id}</span>
                              <span className="st-hint">
                                图{(p.image_models || []).length} 文{(p.chat_models || []).length}{' '}
                                视{(p.video_models || []).length}
                              </span>
                            </button>
                          ))}
                        </div>
                      )}
                    </div>
                  </div>
                </div>
              )}
            </div>
          )}
        </main>
      </div>
    </div>
  );
}

function ProviderListItem({
  p,
  active,
  onSelect,
}: {
  p: {
    id: string;
    name?: string;
    protocol?: string;
    primary?: boolean;
    image_models?: RawModel[];
    chat_models?: RawModel[];
    video_models?: RawModel[];
  };
  active: boolean;
  onSelect: () => void;
}) {
  const cap = (arr?: RawModel[]) => arr?.length || 0;
  return (
    <div
      className={`st-list-item st-list-item--stack${active ? ' is-active' : ''}`}
      onClick={onSelect}
    >
      {/* 第一行：名字（+ 主） */}
      <div className="st-inline st-cluster--nowrap">
        <span className="st-list-item-title st-grow">{p.name || p.id}</span>
        {p.primary && <Star size={12} />}
      </div>
      {/* 第二行：能力计数 + 协议（窄侧栏下不挤成一行） */}
      <div className="st-inline">
        <span className="st-chip">
          图{cap(p.image_models)} 文{cap(p.chat_models)} 视{cap(p.video_models)}
        </span>
        <span className="st-chip">
          {PROVIDER_PROTOCOL_LABELS[p.protocol || ''] || p.protocol || '——'}
        </span>
      </div>
    </div>
  );
}

function CapabilityModels({
  title,
  icon,
  models,
  providerId,
  cap,
}: {
  title: string;
  icon: React.ReactNode;
  models: RawModel[];
  providerId: string;
  cap: 'image_models' | 'chat_models' | 'video_models';
}) {
  const [adding, setAdding] = React.useState(false);
  const [draft, setDraft] = React.useState('');
  const submitAdd = () => {
    const v = draft.trim();
    if (!v) return;
    addModel(providerId, cap, { id: v, label: v }); // 只输 id 时 label 同 id；后续可拉取补全
    setDraft('');
    setAdding(false);
  };
  return (
    <section className="st-card">
      <div className="st-card-head">
        <span className="st-row-icon">{icon}</span>
        <h3 className="st-card-title">{title}</h3>
        <span className="st-chip">{models.length}</span>
        <button
          className="st-btn st-btn--sm st-push"
          type="button"
          onClick={() => setAdding((v) => !v)}
        >
          <Plus size={13} /> 新增
        </button>
      </div>

      {adding && (
        <div className="st-inline">
          <input
            className="st-input st-grow"
            autoFocus
            value={draft}
            onChange={(e) => setDraft(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === 'Enter') submitAdd();
            }}
            placeholder="输入模型 id 或名称，回车新增"
          />
          <button className="st-btn" type="button" onClick={submitAdd}>
            添加
          </button>
        </div>
      )}

      {models.length === 0 ? (
        <div className="st-hint">
          该服务商暂无此能力模型，可点「新增」手输或「拉取模型」批量勾选
        </div>
      ) : (
        <div className="st-list">
          {models.map((m, i) => (
            <div key={`${m.id || ''}-${i}`} className="st-list-item st-list-item--static">
              <span className="st-dot" />
              <span className="st-list-item-title st-grow">{m.label || m.id}</span>
              <button
                className="st-icon-btn is-danger"
                type="button"
                onClick={() => removeModel(providerId, cap, m.id || m.label || '')}
                title="移除该模型"
              >
                <Trash2 size={13} />
              </button>
            </div>
          ))}
        </div>
      )}
    </section>
  );
}
