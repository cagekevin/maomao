...
function PreviewScene({ objects, cameraData, cameraAspect, lighting, backgroundCanvas = null, animationTime = 0, performanceMode = false }) {
  return (
    <>
      ...
      {!performanceMode && <ContactShadows position={[0, 0.01, 0]} opacity={0.35} scale={18} blur={2.2} far={9} />}
...
export function MainViewport(props) {
  ...
  return (
    <Canvas shadows={props.performanceMode ? false : 'basic'} dpr={[1, 1.75]} camera={cameraSettings} onPointerMissed={() => props.onSelect(null)} gl={{ alpha: true, antialias: true, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 0.88 }}>
      <EditorScene {...props} />
    </Canvas>
  )
}
...
export function CameraPreview({ objects, cameraData, cameraAspect, lighting, backgroundCanvas = null, animationTime = 0, onCanvasReady, exportMode = false, performanceMode = false }) {
  return (
    ...
    >
      <PreviewScene objects={objects} cameraData={cameraData} cameraAspect={cameraAspect} lighting={lighting} performanceMode={performanceMode} backgroundCanvas={backgroundCanvas} animationTime={animationTime} />
    </Canvas>
  )
}
