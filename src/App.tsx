/**
 * 画布宿主（App）—— ReactFlow 画布壳 + 全局编排。
 *
 * 【文件名变更留痕（2026-09-16 · TD-18-3）】本文件原名 **`App.jsx`**（TS 迁移时改名为 `App.tsx`）。
 * `src/` 内约 **39 处注释**写着 `App.jsx`，分两类、**处置不同**（这是刻意选择，勿"统一替换"）：
 *   ① **历史叙述**（「从 App.jsx 抽出」「原 App.jsx 内…」「旧实现 App.jsx:487」）——**保留原名**。
 *      那是对「当时该文件确实叫 App.jsx」的忠实记录；改成 `App.tsx` 等于**篡改历史**，
 *      而本仓 §5.2 明令保护这类"为什么这样设计 / 从哪抽出来"的注释（只可追加，不可改）。
 *   ② **当前指引**（「供 App.jsx 复用」「App.jsx 是 nodeTypes 的唯一派生源」）——同理不回改：
