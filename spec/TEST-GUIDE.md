...
>
> **上游认知**：测试不是写码之外的额外步骤——**构思测试 = 把契约想透**。"先定数据流、再定契约、后想测试"的完整写码流程见 `CLAUDE.md` §3.0（数据流理清 → 契约先行 → 动手）。本文件只讲"测试怎么做"，不讲"契约怎么定"，两者互补不重叠。
...
> **两类用例明确禁止（判据真源 = `docs/adr/ADR-0049`，2026-09-20 用户裁定，此处只留指针不复述）**：
> ① **文档测试**（断文件存在／文本内容／文档间引用对账）；② **UI 面板存在性测试**（断"有这个图标／这个元素渲染出来了"）。
> 文档写错就改文档，UI 对不对靠手测 —— 这两类不写测试、不进闸。

### 测试测的是「契约」与「数据流」，不是「实现防御」

判定一个用例**值不值得写**，用这两条一问就懂：

1. **它锁的是契约吗？** 断言的是「输入 → 输出的约定」——用户操作触发一条数据流，每一步的产出是否符合约定。例如：合并生成视频时「勾选镜头3(5s)+镜头4(3s) → 合并时长必须是 8s」「参考图 = 所选镜头引用的资产，未勾选镜头的资产不能混入」「顺序 = 勾选顺序，镜头3 必须在镜头4 前」「缺省字段传给 AI 时标『（未指定）』，防止 AI 脑补」。**这些是用户能感知到的**——时长算错、参考图带错、顺序反了、画面缺信息，用户一眼就能发现。

2. **它是用户感知不到的垃圾边界吗？** 为「覆盖率」硬凑的异常输入防御，如 `duration='abc'` 兜底成 5、传 `null` 不崩、`undefined` 项不崩溃——这些在**真实业务流程里永远不会发生**（duration 来自第一步表格永远是正整数、shots 永远是有效数组），测它们等于自嗨，改坏了用户也无感知，**不值得写**。

> **一句话**：测试要能回答「用户走这条数据流，会不会拿到错误结果」。锁得住用户可感知的契约 → 写；只证明"我防御了不可能发生的输入" → 删。**宁可只写 5 条真契约，也不凑 50 条假边界。**
...
- ✅ **已补高频组件深度测试（2026-08-18，阶段二续）**：`TaskCenter`（任务中心面板）、`AgentPanel`（画布 AI 助手聊天面板）此前为**零测试**的高频改动组件（近 200 次提交多次改动），本轮补齐深度测试（详见各 `.test.jsx` 头部注释）。
  ...
  - `tests/unit/AgentPanel.test.jsx`：面板显隐、消息发送（回车/快捷 chip/空输入禁用/发送后清空）、图像模式直连生图（走 `sendImageMode` 不经过 LLM）、sending 态（思考中 + 停止）、Skill 应用与移除（`setCurrentSnapshot` 同步落盘）、对话管理（新建/切换/清空含 confirm 分支）、待引用图确认并入附件、执行分级切换。
