...
- 🟡 **仍可补（第二阶段全覆盖，2026-08-17 续，详见计划文档）**：当前 `src/components` 共 185 文件、已覆盖 48、仍有 **137 个零测试文件**待清零，按 7 批推进：
  ...
  4. ✅ **批 4 已完成（2026-08-18）**：`ImageBoxNode` `AgentMessage` `AgentPanel`（`Comet` `ConnectionLine` `CustomEdge` `CustomHandle` `DiscountVideoNode` `GhostTargetNode` `JianyingIcon` `NodeTitle` `ScriptBoxNode` 仍待补）
