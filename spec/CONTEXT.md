...
> **🔻 本文件的定位（2026-09-18 用户裁定，见 `docs/adr/ADR-0025`）：规则适用性地图。**
...
**判据本体在 ADR —— 本节只留索引（判据不在此复述，见 `docs/adr/ADR-0025`）：**
...
| **轮次文件 §九**   | 决策前置的**取舍**（动手前过了哪三关 / 为什么这么选） | `daily/架构日志/<NN>-*.md` 的 `§九 决策留痕`（判据本体 `docs/adr/ADR-0050`） | 不记判据（判据仍进 ADR）；不重写 §八 证据 |
...
> **本表只是「不要再写第二份」的清单**（动作指令）。每条**现状与边界**在 `spec/DATAFLOW.md` 对应链路 + 代码文件头，判据在 `docs/adr/` —— 本表**不重复**它们（见 `docs/adr/ADR-0025`）。
