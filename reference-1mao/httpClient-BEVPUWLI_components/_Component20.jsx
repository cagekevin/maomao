...
import { t, e, d, c, so, a, h, po, _Component12 } from './shared.js';
import * as Z from 'react';
import * as Q from 'react';
export default function _Component20({
  category: e,
  presetPrompts: t,
  onApply: n,
  onToast: r
  ...
  return <Component233 className={`relative nodrag flex items-center`} ref={u}>
      ...
      return e.stopPropagation();
          ...
          <Component228 className={`flex-1 overflow-y-auto custom-scrollbar px-2`}>
            ...
            return <Component226 className={`w-full flex items-center gap-1.5 mb-1 text-left px-2 py-1.5 text-[11px] rounded-md transition-colors truncate text-gray-400 hover:bg-[#2a2a2a] hover:text-gray-200`} onClick={() => {
              return h(e.content);
                      ...
                      <_Component12 size={12} className={`shrink-0 text-white`} />
                    </Component226>;
          })}
                {p.map((e, t) => {
          ...
          a(false);
              ...
              <_Component12 size={11} />
              {`提示词库`}
            </Component230>
          </Component231>
        </Component232>}
      <_cmp__Component14 open={o} onClose={() => {
      return s(false);
    }} onUse={e => {
      return n(e.content);
