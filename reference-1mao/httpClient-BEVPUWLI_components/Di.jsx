...
import { id, We, Tr, wi, ei, ii, ri, h, y, Ci, a, xi, d, Zr, X, c, _, Ie, _Component5, _Component3, _Component2, _Component8, _Component9, Ze, _Component0, _Component1, Te, _Component6, _Component11 } from './shared.js';
...
var Di = Z.memo(({
  ...
  return <Component131 className={`relative group/node w-full h-full min-w-[120px] min-h-[80px]`}>
      <_cmp_Ti id={e} data={t} defaultTitle={h === `video` ? `视频` : h === `audio` ? `音频` : h === `text` ? `文本文件` : `图片`} icon={h === `video` ? <Ie size={11} /> : h === `audio` ? <_Component5 size={11} /> : h === `text` ? <_Component3 size={11} /> : <_Component2 size={11} />} />
      <_cmp_Ei visible={!!n} minWidth={120} minHeight={80} />
      ...
      <Component115 className={`absolute -top-12 left-1/2 -translate-x-1/2 z-20 opacity-0 group-hover/node:opacity-100 transition-opacity pointer-events-none group-hover/node:pointer-events-auto nodrag pb-4`}>
        <Component114 className={`flex items-center gap-1 px-3 py-2 bg-[#1c1c1c]/90 backdrop-blur-md border border-[#333] rounded-full shadow-lg`}>
          ...
          try {
            a(e, {
              imageUrl: await _cmp_jr(u, 2048, 0.85)
            });
