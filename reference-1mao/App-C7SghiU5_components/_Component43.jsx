...
import { he, ge, e, a, pe, r, _Component7, _Component4 } from './shared.js';
import * as G from 'react';
export default function _Component43({
  onSaved: e
}) {
  let [t, n] = G.useState(() => {
    return he();
  ...
  return <Component104 className={`bg-gradient-to-br from-[#1a1a1a] to-[#0d0c0c] rounded-xl overflow-hidden shadow-sm border border-[#222]`}>
      <Component92 className={`flex items-center gap-4 p-5 border-b border-[#222] bg-[#1a1a1a]`}>
        <Component88 className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-500/15 ring-1 ring-emerald-400/25`}>
          <_Component7 size={20} className={`text-emerald-300`} />
        </Component88>
        <Component91>
        ...
        return <Component98 onClick={() => {
          return d(e.url);
              ...
              <Component97 className={`flex items-center justify-between gap-3`}>
                ...
                {i ? <_Component4 size={14} className={`animate-spin text-blue-300 flex-shrink-0`} /> : n ? <Component96 className={`text-xs text-blue-300 flex-shrink-0`}>{`当前`}</Component96> : null}
              </Component97>
            </Component98>;
      })}
        {ge.length === 0 && <Component99 className={`text-xs text-gray-500 bg-[#0d0c0c] border border-[#333] rounded-lg p-4`}>{`未配置接入点列表（VITE_API_ENDPOINTS）。`}</Component99>}
      </Component100>
