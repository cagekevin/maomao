// TODO(全局, 无需 import): open, onClose, defaultAppId, w, h, j, startX, startY, startW, startH, t, i, n, s, l, appId, appName, d, p, encodeURIComponent, f, durationDays, m, maxRuns, v, note, b, y, status, width, height, maxWidth, maxHeight, o, u, c, te, ne, re, ee, background
import { r, e, k, A, a, ut, M, I, O, We, S, E, rt, xt, g, L, C, D, T, _Component8, R, _Component9, _Component4, _Component0, F, N, _, P } from './shared.js';
...
export default function _Component11({
  ...
  let [c, l] = G.useState([]);
  let [u, d] = G.useState([]);
  let [f, p] = G.useState(r || ``);
  let [m, g] = G.useState(30);
  let [v, y] = G.useState(``);
  let [b, S] = G.useState(``);
  let [C, w] = G.useState(false);
  let [T, E] = G.useState(null);
  ...
  let L = async () => {
    ...
    s(``);
    try {
      let e = await We(`/workflow-apps/${encodeURIComponent(f)}/licenses`, {
  ...
  if (e) {
    ...
    return <Component160 className={`fixed inset-0 z-[210] bg-black/70 flex items-center justify-center p-4`}>
        ...
        maxHeight: `100vh`
            ...
            <Component128 className={`rounded-2xl border border-[#2a2a2a] bg-[#161616] p-4`}>
              ...
              <Component127 className={`grid grid-cols-1 md:grid-cols-[1.4fr_0.7fr_0.8fr_1fr_auto] gap-3 items-end`}>
                ...
                <Component126 onClick={L} disabled={C || !f} className={`h-[38px] px-4 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-500 disabled:opacity-50 flex items-center gap-2 justify-center`}>
                  {C ? <_Component4 size={14} className={`animate-spin`} /> : <_Component9 size={14} />}
                  {`创建`}
                </Component126>
              </Component127>
            </Component128>
              ...
              {i ? <Component130 className={`h-40 flex items-center justify-center text-gray-400`}>
                ...
                return <Component154 className={`grid grid-cols-1 md:grid-cols-[minmax(120px,1.4fr)_minmax(180px,2fr)_90px_140px_90px_160px] gap-2 md:gap-3 px-4 py-3 border-b border-[#1f1f1f] last:border-b-0 hover:bg-white/[0.02] items-center text-xs`} key={e.id}>
                      ...
                      return ne(e);
                            ...
                            {T === e.id ? <_Component4 size={12} className={`animate-spin`} /> : t ? <N size={12} /> : <_ size={12} />}
                            {t ? `恢复` : `冻结`}
                          </Component151>
          ...
          <Component158 onMouseDown={ee} className={`absolute bottom-0 right-0 w-5 h-5 cursor-nwse-resize`} style={{
