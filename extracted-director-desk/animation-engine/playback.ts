// ============================================================
// 3D 导演台 · 动画引擎 · 播放采样（核心）
// 抽自 Nomi scene3dPlayback.ts（完整）
// 播放头 + 轨迹绑定 → 对象/相机在 t 时刻的位姿；动作关键帧 → 姿势；手持抖动 → 确定性噪声。
// ============================================================
import * as THREE from 'three'
import { CAMERA_DEFAULT_TARGET, UNGROUPED_TRAJECTORY_GROUP_ID } from './constants'
import { buildTrajectoryCurve, clampRatio, remapTrajectoryTimeRatio, wrapRatio } from './trajectory'
import { cameraLookAtRotation, eulerToArray, objectVisualHalfHeight, vectorToArray } from './math'
import { samplePoseKeyframe } from './poseTrack'
import { cameraAimBindingId } from './bindingIds'
import type {
  Scene3DCamera,
  Scene3DObject,
  Scene3DState,
...
// ── 手持抖动：确定性多频正弦噪声，纯播放头 t 的函数、无随机源 ──
const SHAKE_MAX_AIM_RAD = 0.028
const SHAKE_MAX_OFFSET = 0.035
...
export function cameraWithPlaybackPosition(
  ...
  // 注视点优先级：① aim 轨迹 → ② follow 某物体 → ③ 静态 target。
