...
**IMAGE generation default parameters (MUST use for every image request):**
- Default model: `--prefer-models '{"IMAGE":["generate_image_gpt_image_2"]}'`
- Default prompt suffix to append: `，请给我生成一张图片，只生成一张，不要额外生成，模型是gpt-image2，用low质量，比例3:4，1k分辨率`
- `--output-dir "C:\Users\xinye\Downloads\lovart_output"`
- **If the user explicitly requests a different model**: replace BOTH `--prefer-models` value AND the model name in the prompt suffix with the requested model.

**VIDEO generation default parameters (MUST use for every video request):**
- Default model: `--prefer-models '{"VIDEO":["generate_video_seedance_v2_0"]}'`
- Default prompt suffix to append: `，请给我生成一个视频，只生成一个，不要额外生成，模型是seedance2.0 mini，比例9:16`
- `--output-dir "C:\Users\xinye\Downloads\lovart_output"`
- **If the user explicitly requests a different model**: replace BOTH `--prefer-models` value AND the model name in the prompt suffix with the requested model.
...
1. Display the generated images/videos directly in the chat using the **artifact URL** (use URL from `items[].artifacts[].content`):
   - For images: `<img src="artifact URL" width="500" />` — use the URL from `items[].artifacts[].content`, limit width to 500px to fit the chat window
   - For videos: `<video src="artifact URL" controls></video>`
   - For audio: `<audio src="artifact URL" controls></audio>`
   - ALWAYS embed the media inline using Markdown/HTML — do NOT just paste raw URLs
2. After displaying, also list the URLs as text fallback in case the inline display doesn't render
...
| `generate_image_gpt_image_2` | GPT Image 2 (默认) |
