...
describe('AI 助手表格模型（assistantTable 纯函数）', () => {
  ...
  it('parsePasted(HTML table)：保留 Tab 不被压平，多格多行还原正确', () => {
    const htm = '<table><tr><th>A</th><th>B</th></tr><tr><td>a1</td><td>b1</td></tr><tr><td>a2</td><td>b2</td></tr></table>'
  ...
  it('normalizeAssistantTable：宽松/脏数据归一成精确形状，缺省回空表', () => {
    ...
    const n2 = normalizeAssistantTable({ columns: [{ label: 'A' }, { label: '' }, 'junk'], rows: [{ values: { x: '1' } }] })
