...
import { cloneJointPose, presetJoints } from '../../src/components/director3d/rig.ts';
import {
  lerp,
  lerpAngle,
  segmentAmount,
...
// 迁移到通道结构（复用 M1 归一化入口）
const personChannels = normalizeObjectTracks(
  { 'actor-lead': legacyPersonKeys },
  { 'actor-lead': 'person' },
)['actor-lead'];
const boxChannels = normalizeObjectTracks(
...
describe('M2-C1 单通道求值：未含字段不解接管', () => {
  it('普通物体：仅 position 关键帧 → rotation/scale 保持基线', () => {
    const partial = upsertChannelKeys(
      {},
      snapshotToChannelKeys('object', { position: [3, 0, 0] }, 10, 'linear'),
    );
  ...
  it('摄像机：仅 position 关键帧 → rotation/focalLength 保持基线', () => {
    const partial = upsertChannelKeys(
      {},
      snapshotToChannelKeys('camera', { position: [2, 3, 4] }, 10, 'linear'),
    );
...
describe('M2-C2 合成器按注册顺序合并', () => {
  it('人物：仅 transform 通道 → 动作/骨骼字段保持基线，且不产生派生量', () => {
    let tOnly = {};
    tOnly = upsertChannelKeys(
      tOnly,
      snapshotToChannelKeys('person', { position: [0, 1, 0] }, 0, 'smooth'),
    );
  ...
  it('人物：仅 action 通道 → 变换保持基线', () => {
    let aOnly = {};
    aOnly = upsertChannelKeys(
      aOnly,
      snapshotToChannelKeys(
        'person',
        { pose: 'run', poseTime: 0.5, continuousMotion: true },
        0,
        'smooth',
      ),
    );
...
describe('M2-C3 迁移验证锚：旧整快照求值 vs 新通道求值 逐帧一致', () => {
  ...
  it('objectsAtFrame 三实体混合轨逐帧一致', () => {
    ...
    for (let frame = 0; frame <= 40; frame += 1) {
      ...
      const legacy = [
        legacyObjectAtFrame(person, legacyPersonKeys, frame, FPS),
        legacyObjectAtFrame(box, legacyBoxKeys, frame, FPS),
      ];
...
describe('M2-C6 查询基元：exact / 首前 / 末后 / 段内 定位', () => {
  ...
  it('对象：exact/首前/末后/段内 定位与参考一致', () => {
    for (const frame of [-3, 0, 1, 12, 24, 40]) {
      compareSubset(
        objectAtFrame(box, boxChannels, frame, FPS),
        legacyObjectAtFrame(box, legacyBoxKeys, frame, FPS),
...
describe('47 三层注册表：objectState 沿基线不被关键帧覆盖', () => {
  it('先勾后打帧再播放：continuousMotion 恒沿基线 true，不被旧 action 关键帧 false 覆盖（46 病灶）', () => {
    ...
    channels = upsertChannelKeys(
      channels,
      snapshotToChannelKeys('person', { pose: 'walk', poseTime: 0.4 }, 0, 'smooth'),
    );
    channels.action = [
      {
        frame: 0,
        interpolation: 'smooth',
        fields: { pose: 'walk', poseTime: 0.4, continuousMotion: false },
      },
    ];
...
describe('47 三层注册表：派生的 ENTITY_CHANNELS 与 OBJECT_STATE_FIELDS', () => {
  it('continuousMotion/footLock 被收进 OBJECT_STATE_FIELDS.person，且不在任何 ENTITY_CHANNELS 通道', () => {
    expect(OBJECT_STATE_FIELDS.person).toEqual(
      expect.arrayContaining(['continuousMotion', 'footLock']),
    );
