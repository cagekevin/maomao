...
async function runWithTimers<T>(p: Promise<T>): Promise<{ ok: boolean; value?: T; error?: unknown }> {
...
describe('relayProxy §R6 — relayAttachUntilDone（统一 attach 契约）', () => {
  beforeEach(() => { vi.useFakeTimers(); h.mockHttpRequest.mockReset() })
  afterEach(() => { vi.useRealTimers() })

  it('completed → {ok:true, url}（url=后端已落盘 /files/）', async () => {
    h.mockHttpRequest.mockResolvedValue(envResp({ status: 'completed', url: '/files/tasks/x.png' }))
    ...
    expect(h.mockHttpRequest).toHaveBeenCalledWith(expect.stringContaining('/api/generate/task-1'), expect.any(Object))
  })
  ...
  it('cancelOnAbort=true → signal abort 时通知后端 cancel 并抛 AbortError（in-flight）', async () => {
    ...
    const p = relayAttachUntilDone({ frontTaskId: 'task-1', signal: ctl.signal, cancelOnAbort: true })
    ...
    // 通知了后端 cancel
    expect(h.mockHttpRequest).toHaveBeenCalledWith(expect.stringContaining('/api/generate/task-1/cancel'), expect.any(Object))
  })

  it('运行中已到终态后 abort 晚到 → 不再 cancel（settled 守卫）', async () => {
    const ctl = new AbortController()
    h.mockHttpRequest.mockResolvedValue(envResp({ status: 'completed', url: '/files/tasks/done.png' }))
    const r = await runWithTimers(relayAttachUntilDone({ frontTaskId: 'task-1', signal: ctl.signal, cancelOnAbort: true }))
...
describe('relayProxy §R6 — relayGenerate = submit + attach', () => {
  ...
  it('提交拿到 taskId → attach 到 completed → {ok:true, url}', async () => {
    ...
    const p = relayGenerate({
      intent: { frontTaskId: 'task-1', type: 'image', providerId: 'lovart', capability: 'image', model: 'm', prompt: 'x' },
    })
    const r = await runWithTimers(p)
    expect(r.value).toEqual({ ok: true, url: '/files/tasks/x.png' })
    ...
    expect(JSON.parse(submitCall[1].body)).toMatchObject({
      frontTaskId: 'task-1', providerId: 'lovart', capability: 'image', model: 'm',
    })
  })

  it('提交失败（code:-1）→ {ok:false, error}（不做 attach）', async () => {
    h.mockHttpRequest.mockResolvedValue({ code: -1, data: { error: '提交失败' } })
    const p = relayGenerate({ intent: { frontTaskId: 't', type: 'image', providerId: 'p', capability: 'image', model: 'm' } })
    const r = await runWithTimers(p)
