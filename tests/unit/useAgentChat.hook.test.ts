...
vi.mock('../../src/components/agent/conversation/conversationStore.ts', () => {
  ...
  return {
    ...
    deleteConversation: vi.fn(() => {
      ...
      return {
        activeId: 'c1',
        snapshot: { id: 'c1', messages: [], skills: [], draft: '', attachments: [] },
      };
    }),
  };
});
...
type AgentChatApi = Omit<ReturnType<typeof useAgentChat>, 'messages'> & {
  messages: TestChatMessage[];
};
...
describe('useAgentChat · 真实模式 SSE 编排', () => {
  ...
  it('复合忙判定：任务进行中（发送锁 + 状态机 running）再次 send 走 steer，不触发第二次请求', async () => {
    ...
    // 发起第一轮
    act(() => {
      result.current.send('第一轮任务').catch(() => {});
    });
    ...
    act(() => {
      result.current.send('插一句补充').catch(() => {});
    });
    await waitFor(() => {
      expect(
        result.current.messages.some((m) => m.steer === true && m.content === '插一句补充'),
      ).toBe(true);
    });
  ...
  it('三阶段门禁：show_plan_for_confirm 后暂停循环，不再继续第二轮（等用户确认，防自言自语）', async () => {
    ...
    fetchMock
      .mockResolvedValueOnce(
        toolStream('call_1', 'show_plan_for_confirm', '{"plan_text":"一套策划","generations":[]}'),
      )
    ...
    expect(vi.mocked(convStore.patchCurrentWorkflow)).toHaveBeenCalledWith(
  ...
  it('【积分闸语义】残留 creditGate.pending 不打断非 execute_plan 工具循环（积分闸只拦"点生成那下"）', async () => {
    ...
    vi.mocked(convStore.getCreditGate).mockReturnValue({
      pending: true,
      gens: [{}],
      map: { g1: 'n1' },
    });
  ...
  it('【积分闸语义】本轮 execute_plan 返回 awaited:credit → 工具循环暂停等点生成（唯一停点）', async () => {
    callTool.mockReturnValue({
      ...
      data: {
        awaited: 'credit',
        steps: [{ id: 'g1', status: 'ready', nodeId: 'n1' }],
        note: '节点已建好，生成待积分确认',
      },
    });
    fetchMock
    ...
    const { result } = renderHook<AgentChatApi, unknown>(() => useAgentChat());
    await act(async () => {
      await result.current.send('生成一张图');
    });
  ...
  it('stop 中止：进行中调 stop → error=已停止，sending 回 false', async () => {
    ...
    const { result } = renderHook<AgentChatApi, unknown>(() => useAgentChat());
    act(() => {
      result.current.send('生一张图').catch(() => {});
    });
    await waitFor(() => expect(result.current.sending).toBe(true));
...
describe('useAgentChat · steer 排队（任务进行中补充指令）', () => {
  it('任务进行中再 send → 第二条进 steer 队列（标记 steer），不并发双发', async () => {
    ...
    act(() => {
      result.current.send('第一个任务').catch(() => {});
    });
    // 第一个进行中发第二个：应被拦下进 steer 队列，不再调 fetch
    act(() => {
      result.current.send('第二个任务').catch(() => {});
    });
  ...
  it('正常顺序两次 send：各自走完整循环，callTool 各执行一次', async () => {
    ...
    await act(async () => {}); // 会话水化/初始恢复为异步：flush 微任务等 restore 完成，避免其复位消息
    await act(async () => {
      await result.current.send('创建节点A');
    });
    await act(async () => {
      await result.current.send('创建节点B');
    });
...
describe('useAgentChat · roundTrip 出站（过期路径断言更新）', () => {
  ...
  it('默认路径：fetch 指向 /api/generate（统一生成入口），且 body.capability=chat', async () => {
    ...
    const { result } = renderHook<AgentChatApi, unknown>(() =>
      useAgentChat({ agentKey: 'canvas-assistant' }),
    );
    await act(async () => {
  ...
  it('【fresh-task 彻底对齐大雄】历史 user 消息（含图/文字）全部不进 LLM 上下文，只发本轮 user', () => {
    ...
    const history = [
      ...
      { role: 'assistant', content: 'ok1' },
      {
        role: 'user',
        content: '轮2',
        attachments: [{ type: 'image', url: 'http://x/b.png' }],
        refCatalog: '参考图1：B',
      },
      { role: 'assistant', content: 'ok2' },
      {
        role: 'user',
        content: '轮3',
        attachments: [{ type: 'image', url: 'http://x/c.png' }],
        refCatalog: '参考图1：C',
      },
    ...
    expect((users[0].content as ContentBlock[])[0]).toMatchObject({
      type: 'image_url',
...
describe('useAgentChat · buildRequestMessages 深度（请求体组装）', () => {
  const base = [
    { role: 'user', content: '帮我建个节点' },
    {
      ...
      content: '已建',
      tool_calls: [
        { id: 'c1', type: 'function', function: { name: 'create_node', arguments: '{}' } },
      ],
    },
  ...
  it('enhance=true 且无 system：前置注入画布准则 + 执行模型分流段（恒 auto），历史消息按顺序接在后面', () => {
    ...
    // 首条是系统准则（断言引用同一常量，不断言具体措辞——改字不红，漏注入才红）
    ...
    const msgs = [
      {
        role: 'user',
        content: '看这张图',
        attachments: [{ type: 'image', url: 'http://x/r.png' }],
        isCurrent: true,
      },
    ];
...
describe('useAgentChat · 执行模型提示词注入（恒 auto）', () => {
  const base = [
    { role: 'user', content: '生成一张猫图' },
    {
      ...
      content: 'ok',
      tool_calls: [
        { id: 'c1', type: 'function', function: { name: 'execute_plan', arguments: '{}' } },
      ],
    },
    { role: 'tool', content: '{"ok":true}', tool_call_id: 'c1' },
  ] as ChatMessage[];
...
describe('useAgentChat · parseSSEChunk 深度（SSE 增量解析）', () => {
  ...
  it('tool_calls 跨多段按 index 分包拼接（name 与 arguments 分段到达）', () => {
    ...
    parseSSEChunk(
      'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"name":"node"}}]}}]}',
      acc,
    );
...
describe('useAgentChat · parseGenerationsFromReply（回复正文解析）', () => {
  ...
  it('无代码块时回退提取最大 {...} JSON', () => {
    const content =
      '规划如下 { "generations": [ { "id": "g2", "title": "详情1", "prompt": "狗" } ] } 请确认';
...
describe('useAgentChat · 全链路 400 错误透传（发图被拒）', () => {
  it('LLM 返回 400 + error.message → 前端 error 显示真实原因（不只状态码）', async () => {
    ...
    fetchMock.mockResolvedValue(
      new Response(JSON.stringify({ error: { message: 'model does not support image_url' } }), {
        status: 400,
...
describe('useAgentChat · 阶段1B 卸载不 abort / 切 key abort', () => {
  it('组件卸载且 send 进行中 → 不触发 abort，流不被中断', async () => {
    ...
    const { result, unmount } = renderHook<AgentChatApi, unknown>(() =>
      useAgentChat({ agentKey: 'k1' }),
    );
    act(() => {
      result.current.send('挂起任务').catch(() => {});
    });
...
describe('useAgentChat · 阶段1D 薄壳化（sending/activeId 订阅 store）', () => {
  it('sending 单源：外部 setSending → hook 订阅重渲染', () => {
    ...
    expect(result.current.sending).toBe(false);
    act(() => {
      sharedConvStore.setSendingState(true);
    });
    expect(result.current.sending).toBe(true);
    act(() => {
      sharedConvStore.setSendingState(false);
    });
    expect(result.current.sending).toBe(false);
  });
  ...
  it('activeConversationId 单源：跟随 store.activeId', () => {
    ...
    expect(result.current.activeConversationId).toBe('c1');
    act(() => {
      sharedConvStore.setActiveId('c9');
    });
