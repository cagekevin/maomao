...
vi.mock('../../src/components/scriptbox/GearSettings.tsx', () => ({
  default: ({ updateData }) => (
    <div data-testid="gear-settings">
      <button
        type="button"
        data-testid="gear-save"
        onClick={() => updateData({ aspectRatio: '9:16' })}
      >
        保存设置
      </button>
    </div>
...
function setup(data: Record<string, unknown> = {}) {
  ...
  return render(
    <ScriptBoxNode
      id={nodeId}
      data={{ ...nodeData0 } as unknown as ScriptBoxNodeData}
      selected={false}
    />,
  );
}
...
describe('ScriptBoxNode — 上游接入数据同步', () => {
  it('上游文本/图片连入时，同步写回 node.data.upstreamStory / upstreamImages / upstreamTexts', () => {
    ...
    setup();
    expect(h.updateData).toHaveBeenCalledWith(
  ...
  it('无上游内容时不写上游字段', () => {
    setup();
    expect(h.updateData).not.toHaveBeenCalledWith(
      expect.objectContaining({ upstreamImages: expect.anything() }),
    );
    expect(h.updateData).not.toHaveBeenCalledWith(
      expect.objectContaining({ upstreamTexts: expect.anything() }),
    );
  });
});
...
describe('ScriptBoxNode — 端口注册（code-008 回归）', () => {
  it('in 端口走 NodeShell 的 overlayHandles（首帧即挂载，非 createPortal 延迟挂载）', () => {
    ...
    expect(
      screen.getByTestId('node-overlay').querySelector('[data-testid="handle-in"]'),
    ).toBeTruthy();
    // 必须挂在整个节点层（overlay），不能落进 children（定位基准不含标题栏 → 端口偏高）
    expect(
      screen.getByTestId('node-children').querySelector('[data-testid="handle-in"]'),
    ).toBeNull();
  });
  ...
  it('镜头 id 列表变化时重测端口，仅改镜头字段不重测', () => {
    ...
    rerender(
      <ScriptBoxNode
        id={nodeId}
        data={{ shots: [{ id: 1 }, { id: 2 }] } as unknown as ScriptBoxNodeData}
        selected={false}
      />,
    );
    ...
    rerender(
      <ScriptBoxNode
        id={nodeId}
        data={{ shots: [{ id: 1, prompt: 'x' }, { id: 2 }] } as unknown as ScriptBoxNodeData}
        selected={false}
      />,
    );
    expect(h.updateNodeInternals).toHaveBeenCalledTimes(2);
