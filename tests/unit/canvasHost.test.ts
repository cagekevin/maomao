...
/**
 * canvasHost.updateNodeData —— 字典归一化接线（TD-05-13 · I1）。
 *
 * 为什么单独测这条路径：写 node.data 有**两条**入口 —— 界面写回（`useNodeData.patchData`，
 * 见 useNodeData.test.ts）与 Agent 写回（`canvasHost.updateNodeData`，本文件）。
 * 债的症状原文即点名「含 Agent `update_node_any_field`」，故两条路径都要有行为锁，
 * 否则某一侧漏接线时单测仍全绿（假绿）。
 */
import { describe, it, expect } from 'vitest';
import { createCanvasHost } from '../../src/components/agent/canvas/canvasHost.ts';
import type { CanvasHostCtx } from '../../src/components/agent/canvas/canvasHost.ts';
import type { Node, Edge } from '@xyflow/react';

/** 最小 ctx：内存 nodes，setNodes 支持函数式更新 */
function makeCtx(initial: Node[]): CanvasHostCtx & { current: () => Node[] } {
  let nodes = initial;
  return {
    getNodes: () => nodes,
    setNodes: (updater) => {
...
describe('canvasHost.updateNodeData — 字典 GC（Agent 写回路径）', () => {
