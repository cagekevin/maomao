...
beforeEach(() => {
  ...
  mockGetNodes.mockReset(); mockGetNodes.mockReturnValue([node])
...
describe('PromptNode 保存后节点框跟随图片比例（aspectRatio 回 Auto 不污染生图）', () => {
  it('裁剪保存（dims 4:3）→ fitByRatio 让节点框为 4:3，aspectRatio 回 Auto', async () => {
    render(<PromptNode id="pn1" data={{ imageUrl: 'http://x/result.png', aspectRatio: '1:1', label: '生图' }} selected={false} />)
