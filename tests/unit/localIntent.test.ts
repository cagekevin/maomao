...
describe('意图本地判定 classifyLocalIntent（docs/76 L1）', () => {
  const cases: [string, string][] = [
    ...
    ['翻译这段话', 'content'],
    ['帮我起个名字', 'content'],
    // ── 求方法句（2026-09-08 补漏：动词在前、名词在后，原顺序敏感规则漏判）──
    ['我该怎么写提示词', 'content'],
    ['如果我想把他做成一个有空间感、产品前后排列、高低错落的横图，我应该怎么写提示词', 'content'],
    ['这个画面怎么描述比较好', 'content'],
    ['需要什么提示词才能出这种效果', 'content'],
    // ── 求方法 > 生成（2026-09-08 优先级修订：本地判定须与 CANVAS_RULES 对齐）──
    // 句中同时含「批量/出图」与「怎么写」，若生成优先就会被误判成真要出图。
    ['批量出图怎么写提示词', 'content'],
    ['这些图批量生成该怎么写 prompt', 'content'],
    // 反向护栏：求方法句不含语言文字动词时不抢 generate（宁漏勿误）
    ['怎么生成一张图', 'generate'],
    ['帮我批量生成 5 张', 'generate'],
    // 回归：裸「该怎么/该怎么做」不得被判 content（否则注入"不用工具"→ AI 装死不动手）
    ['这个该怎么弄', 'null'],
    ['这个该怎么做成节点', 'null'],
