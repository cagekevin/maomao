...
describe('M1.1 实体·通道注册表 ENTITY_CHANNELS', () => {
  it('三实体通道划分符合 PRD 草案（M1-C1）；continuousMotion 撤出 action 归 objectState（47）', () => {
    expect(ENTITY_CHANNELS.camera).toEqual({
      transform: ['position', 'rotation'],
      lens: ['focalLength'],
    });
    expect(ENTITY_CHANNELS.object).toEqual({ transform: ['position', 'rotation', 'scale'] });
    expect(ENTITY_CHANNELS.person).toEqual({
      transform: ['position', 'rotation', 'scale'],
      action: ['pose', 'poseTime'],
  ...
  it('派生量 motionStartTime 不落任何通道字段（M1-C5）', () => {
    const allFields = Object.values(ENTITY_CHANNELS).flatMap((definition) =>
      Object.values(definition).flat(),
    );
...
describe('M1.2 整快照 → 通道 key（snapshotToChannelKeys）', () => {
  it('人物整快照按 transform/action/skeleton 拆包，fields 无跨通道混入（M1-C1）', () => {
    ...
    expect(keys.transform[0].fields).toEqual({
      position: [-1.25, 0, 0.3],
      rotation: [0, 0.25, 0],
      scale: [1, 1, 1],
    });
...
describe('读侧桥 channelsToSnapshotKeys（M2 求值器落地前的过渡）', () => {
  it('人物通道 round-trip 还原整快照所有字段与帧序', () => {
    const channels = normalizeObjectTracks(
      { 'actor-lead': legacyPersonKeys },
      { 'actor-lead': 'person' },
    );
  ...
  it('snapshotKeysForTrack 对已通道化/旧数组输入均返回整快照数组', () => {
    ...
    expect(snapshotKeysForTrack(channels['actor-lead'], 'person')).toEqual(
      channelsToSnapshotKeys('person', channels['actor-lead']),
    );
    expect(snapshotKeysForTrack(legacyPersonKeys, 'person')).toEqual(
      legacyPersonKeys.map((key) => ({ ...key })),
    );
  });
});
...
describe('M1-C6 旧整快照无损迁移 → 播放逐帧一致', () => {
  it('人物：迁移+flatten 后 objectAtFrame 逐帧输出与旧实现完全一致', () => {
    const legacyResults = sampleFrames.map((frame) =>
      objectAtFrame(person, legacyPersonKeys, frame, 24),
    );
    const channels = normalizeObjectTracks(
      { 'actor-lead': legacyPersonKeys },
      { 'actor-lead': 'person' },
  ...
  it('普通物体：旧实现写入的 pose 字段被收敛后，播放输出不变', () => {
    ...
    const pickTransform = (result: any) => ({
  ...
  it('objectsAtFrame 直接消费通道化轨道，输出与整快照求值一致', () => {
    const channels = {
      'actor-lead': normalizeObjectTracks(
        { 'actor-lead': legacyPersonKeys },
        { 'actor-lead': 'person' },
      )['actor-lead'],
      'block-stage': normalizeObjectTracks(
        { 'block-stage': legacyBoxKeys },
        { 'block-stage': 'object' },
      )['block-stage'],
    };
    const viaChannels = objectsAtFrame([person, box], channels, 10, 24);
    const viaLegacy = objectsAtFrame(
      [person, box],
      { 'actor-lead': legacyPersonKeys, 'block-stage': legacyBoxKeys },
      10,
      24,
    );
...
describe('归一化与通道化工具（幂等/统计/clamp/写入口）', () => {
  it('normalizeObjectTracks 对已通道化输入幂等（跑两次结果一致）', () => {
    const once = normalizeObjectTracks(
      { 'actor-lead': legacyPersonKeys },
      { 'actor-lead': 'person' },
    );
  ...
  it('通道化 key 归一化 frame/interpolation（frame 字符串/越界自动规整）', () => {
    ...
    const noisy = {
      'actor-lead': {
        transform: [{ frame: '3', interpolation: 'hold', fields: { position: [1, 2, 3] } }],
      },
    } as unknown as ChannelTracks;
  ...
  it('upsertChannelKeys：写入口按通道合并，同帧覆盖', () => {
    ...
    channels = upsertChannelKeys(
      channels,
      snapshotToChannelKeys('person', legacyPersonKeys[0], 0, 'smooth'),
    );
    channels = upsertChannelKeys(
      channels,
      snapshotToChannelKeys('person', legacyPersonKeys[1], 24, 'linear'),
    );
  ...
  it('moveChannelFrames：把某帧在所有通道整体平移为另一帧', () => {
    ...
    expect((moved as ChannelTracks).transform.map((key: ChannelKey) => key.frame).sort()).toEqual([
      0, 12,
    ]);
    expect((moved as ChannelTracks).action.map((key: ChannelKey) => key.frame).sort()).toEqual([
      0, 12,
    ]);
