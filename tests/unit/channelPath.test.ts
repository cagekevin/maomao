...
const straightPath = normalizeCameraPath({ points: [{ x: 0, y: 0, z: 0 }, { x: 5, y: 0, z: 0 }, { x: 10, y: 0, z: 0 }], startFrame: 0, endFrame: 240, keyframeCount: 5 })
...
describe('M3-C3 仅路径、无关键帧 → position=路径位置，不回落基线', () => {
  ...
  it('objectsAtFrame 传 paths 映射逐对象生效（无路径对象保持原样）', () => {
    const box = { id: 'block-stage', type: 'box', position: [1.4, 0.45, -0.8], rotation: [0, -0.18, 0], scale: [2.8, 0.9, 2.1] }
    const [lead, stage] = objectsAtFrame([person, box], {}, 120, FPS, { 'actor-lead': straightPath })
...
describe('M3-C6 仅路径、无手动 pose 帧 → 动作/骨骼仍按状态段规则求值，不锁死 source', () => {
  ...
  const actionSkeletonTrack = {
    action: [
      { frame: 0, interpolation: 'smooth', fields: { pose: 'walk', poseTime: 0.2, continuousMotion: true } },
      { frame: 24, interpolation: 'smooth', fields: { pose: 'walk', poseTime: 0.6, continuousMotion: true } },
      { frame: 48, interpolation: 'smooth', fields: { pose: 'run', poseTime: 0.1, continuousMotion: true } },
    ],
    skeleton: [
