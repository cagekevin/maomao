...
import type {
  ProjectMemory,
  ProjectMemoryKind,
} from '../../src/components/agent/runtime/projectMemoryStore.ts';
...
describe('rankProjectMemories —— 相关度 + 类别权重 + 近因（T1）', () => {
  it('与 query 词法相关度高的记忆排前面', () => {
    ...
    const ranked = rankProjectMemories(hits, '喜欢冷色调的方案', { now });
...
describe('buildProjectMemoryBlock —— 注入块（T3）', () => {
  ...
  it('超过 MEMORY_BLOCK_CHAR_LIMIT 时截断', () => {
    const long = buildProjectMemoryBlock(
      Array.from({ length: 10 }, (_, i) => mem('字'.repeat(300), 'fact', i)),
    );
