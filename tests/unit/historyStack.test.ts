...
import { HistoryStack } from '../../src/components/canvas/structure/historyStack.ts';

// §2.3 画布历史栈（撤销/重做）：规划 §3.1 的 useCanvasHistory.test.js 遗漏项。
// 核心逻辑已从 React hook 下沉为纯类 HistoryStack，可直接单测。

// 收敛泛型参数到实际 push 的形状，去掉 6 处 `(x as any).i`
// i 在分支截断用例里会被喂字符串（'new'/'ignored'），故允许 number | string
type Snap = { i: number | string };
function pushN(stack: HistoryStack<Snap>, n: number) {
...
describe('HistoryStack MAX=15 上限', () => {
  it('push 超过 MAX 时移除最早一条，index 封顶', () => {
    const s = new HistoryStack<Snap>({ max: 15 });
    pushN(s, 20);
