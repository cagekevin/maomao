...
function stubFileReader() {
  class StubFileReader {
    ...
    readAsDataURL(blob?: Blob) {
      this.result =
        (blob && (blob as unknown as { _dataUrl?: string })._dataUrl) ||
        'data:image/png;base64,stubdata';
      this.onload?.();
    }
  }
...
describe('assetUrl · normalizeAssetUrlForSend（发送端归一化）', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    stubFileReader();
  });
  ...
  it('/files/ 本地图 → 保持相对路径（E 方案：不压缩不转码，交 localTool 出站回读）', async () => {
    const out = await normalizeAssetUrlForSend('/files/a.png');
  ...
  it('data: 本地 base64 → 压缩后 base64（同样不 fetch）', async () => {
    const out = await normalizeAssetUrlForSend('data:image/png;base64,xxx');
  ...
  it('blob: 本地图 → 压缩后 base64（走 compressImage，不 fetch 转 blob）', async () => {
    const out = await normalizeAssetUrlForSend('blob:http://x/abc');
    expect(out).toBe('data:image/png;base64,compressed:blob:http://x/abc');
    expect(httpRequest).not.toHaveBeenCalled();
  });

  it('公网 http(s) 图 → 原样透传，不压缩、不 fetch', async () => {
    await expect(normalizeAssetUrlForSend('http://x/a.png')).resolves.toBe('http://x/a.png');
    await expect(normalizeAssetUrlForSend('https://cdn/y.jpg')).resolves.toBe('https://cdn/y.jpg');
    expect(compressImage).not.toHaveBeenCalled();
    expect(httpRequest).not.toHaveBeenCalled();
  });

  it('裸 base64（无 data: 前缀）→ 原样透传（网关可解析）', async () => {
    await expect(normalizeAssetUrlForSend('iVBORw0KGgo=')).resolves.toBe('iVBORw0KGgo=');
  });
  ...
  it('blob: 本地图压缩失败 → 回退 blob 转 data base64（失败可见，不阻断发送）', async () => {
    ...
    vi.mocked(httpRequest).mockResolvedValueOnce({
      blob: async () => ({ _dataUrl: 'data:image/png;base64,stubdata' }),
    });
    const out = await normalizeAssetUrlForSend('blob:http://x/abc');
  ...
  it('preferBase64=true：本地图同样压缩转 base64（压缩结果即 base64）', async () => {
    const out = await normalizeAssetUrlForSend('/files/c.png', { preferBase64: true });
    expect(out).toBe('data:image/png;base64,compressed:http://127.0.0.1:18080/files/c.png');
    expect(compressImage).toHaveBeenCalledWith('http://127.0.0.1:18080/files/c.png', {
      maxSize: 1920,
  ...
  it('preferBase64=true：公网图保持原尺寸转 base64（不压缩）', async () => {
    vi.mocked(httpRequest).mockResolvedValueOnce({
      blob: async () => ({ _dataUrl: 'data:image/jpeg;base64,publicdata' }),
    });
    const out = await normalizeAssetUrlForSend('http://x/a.png', { preferBase64: true });
    expect(out).toBe('data:image/jpeg;base64,publicdata');
    expect(compressImage).not.toHaveBeenCalled();
    expect(httpRequest).toHaveBeenCalledWith('http://x/a.png', expect.anything());
  });

  it('normalizeAssetUrlsForSend：数组并行归一 + 过滤空值（本地图压缩、公网原样）', async () => {
    const out = await normalizeAssetUrlsForSend([
      'http://x/a.png',
      'blob:http://x/b',
      '',
      null,
  ...
  // ── 发送侧防线：缩略图端点 URL 绝不允许发出去（系统性根因治理，见 assetUrl.js thumbnailToOriginal）──
  it('发送缩略图端点 URL → 自动还原为原图相对 /files/（E 方案：不把 render 小图发给网关，也不压缩）', async () => {
    ...
    const out = await normalizeAssetUrlForSend(thumb);
  ...
  it('preferBase64=true 的缩略图端点 → 还原原图后压缩转 base64（同样不发缩略图）', async () => {
    const thumb = buildThumbnailUrl('/files/tasks/x.png', { maxDim: 320 });
    const out = await normalizeAssetUrlForSend(thumb, { preferBase64: true });
  ...
  it('普通原图 URL（绝对本地 http 但非缩略图）→ 判定本地 → 保持相对 /files/（不是公网原样，也不压缩）', async () => {
    ...
    const out = await normalizeAssetUrlForSend('http://127.0.0.1:18080/files/tasks/x.png');
    expect(out).toBe('/files/tasks/x.png');
    expect(compressImage).not.toHaveBeenCalled();
  });
});

// ── resolveAssetUrl：前端图片唯一出口（display=render 走按需小图 / send 走原图）──
describe('assetUrl · resolveAssetUrl（统一出口，render 按需小图 / send 原图）', () => {
  it('toRelativeFileUrl：/files/ 相对原样 / 绝对本地还原相对 / 非本地 null', () => {
    expect(toRelativeFileUrl('/files/a.png')).toBe('/files/a.png');
    expect(toRelativeFileUrl('http://127.0.0.1:18080/files/tasks/x.png')).toBe(
      '/files/tasks/x.png',
    );
  ...
  it('render：非本地（外部 http / data: / blob:）回退原图绝对地址，绝不请求出图端点', () => {
    expect(resolveAssetUrl('http://cdn/x.jpg', { scope: 'render' })).toBe('http://cdn/x.jpg');
    expect(resolveAssetUrl('data:image/png;base64,xxx', { scope: 'render' })).toBe(
      'data:image/png;base64,xxx',
    );
    expect(resolveAssetUrl('blob:http://x/abc', { scope: 'render' })).toBe('blob:http://x/abc');
  });

  it('send：一律原图绝对地址，不缩图（/files/ 补全绝对；绝对原样）', () => {
    expect(resolveAssetUrl('/files/a.png', { scope: 'send' })).toBe(
      'http://127.0.0.1:18080/files/a.png',
    );
    expect(resolveAssetUrl('http://127.0.0.1:18080/files/a.png', { scope: 'send' })).toBe(
      'http://127.0.0.1:18080/files/a.png',
    );
...
describe('assetUrl · toImageContentBlocks', () => {
...
describe('assetUrl · fileToDataUrl（本地文件→dataURL 收口）', () => {
  ...
  it('File → data: base64（FileReader 收口，供发送附件，result 透传）', async () => {
    ...
    await expect(
      fileToDataUrl({ _dataUrl: 'data:image/png;base64,fileimg' } as unknown as Blob),
    ).resolves.toBe('data:image/png;base64,fileimg');
  });

  it('无自定义 result → 使用 FileReader 默认读取结果', async () => {
    await expect(fileToDataUrl({} as unknown as Blob)).resolves.toBe(
      'data:image/png;base64,stubdata',
    );
  });
});

// ── classifyImageType / summarizeAssetUrls：带图可观测（发送图片形态分类）──
describe('assetUrl · classifyImageType / summarizeAssetUrls（发送图片可观测）', () => {
  ...
  it('summarizeAssetUrls：空/无 → count=0', () => {
    expect(summarizeAssetUrls([])).toEqual({ count: 0, urls: 0, base64s: 0 });
    expect(summarizeAssetUrls(null)).toEqual({ count: 0, urls: 0, base64s: 0 });
  });

  it('normalizeAssetUrlsForSend 带图发送 → 记一条图片形态 info 日志（可观测）', async () => {
    vi.mocked(logger.info).mockClear();
    await normalizeAssetUrlsForSend(['http://a.png', 'data:image/png;base64,xxx']);
    expect(logger.info).toHaveBeenCalledWith('assetUrl', '发送图片', {
      count: 2,
      urls: 1,
      base64s: 1,
      total: 2,
  ...
  it('normalizeAssetUrlsForSend 无图 → 不记发送日志', async () => {
    vi.mocked(logger.info).mockClear();
