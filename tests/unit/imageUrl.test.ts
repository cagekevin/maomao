...
import {
  ...
  normalizeImageUrlsForSend,
  toImageContentBlocks,
  fileToDataUrl,
  classifyImageType,
  summarizeImages,
} from '../../src/components/base/utils/imageUrl.ts';
...
describe('imageUrl · normalizeImageUrlForSend（发送端归一化）', () => {
  ...
  it('normalizeImageUrlsForSend：数组并行归一 + 过滤空值（本地图压缩、公网原样）', async () => {
    const out = await normalizeImageUrlsForSend([
      'http://x/a.png',
      'blob:http://x/b',
      '',
      null,
...
describe('imageUrl · resolveImageUrl（统一出口，render 按需小图 / send 原图）', () => {
  ...
  it('render：maxDim / 白名单 format 透传；webp 被钳制不产出', () => {
    const u = new URL(
      resolveImageUrl('/files/a.png', { scope: 'render', maxDim: 320, format: 'jpeg' }),
    );
    ...
    const w = new URL(
      resolveImageUrl('/files/a.png', { scope: 'render', maxDim: 320, format: 'webp' }),
    );
  ...
  it('render：非本地（外部 http / data: / blob:）回退原图绝对地址，绝不请求出图端点', () => {
    ...
    expect(resolveImageUrl('data:image/png;base64,xxx', { scope: 'render' })).toBe(
  ...
  it('send：一律原图绝对地址，不缩图（/files/ 补全绝对；绝对原样）', () => {
    expect(resolveImageUrl('/files/a.png', { scope: 'send' })).toBe(
      'http://127.0.0.1:18080/files/a.png',
    );
    expect(resolveImageUrl('http://127.0.0.1:18080/files/a.png', { scope: 'send' })).toBe(
      'http://127.0.0.1:18080/files/a.png',
    );
...
describe('imageUrl · classifyImageType / summarizeImages（发送图片可观测）', () => {
  ...
  it('normalizeImageUrlsForSend 带图发送 → 记一条图片形态 info 日志（可观测）', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('imageUrl', '发送图片', {
      count: 2,
      urls: 1,
      base64s: 1,
      total: 2,
