...
describe('providerStore §4 供应商数据层（新时代配置型）', () => {
  ...
  describe('test（连通性探测）', () => {
    ...
    it('apimart 通用探测失败时用 probe-async 补全诊断（透传原始错误）', async () => {
      ...
      h.mockTestConnection.mockResolvedValue({
        ok: false,
        status: 0,
        error: '连接失败: Connect Timeout',
      });
      h.mockProbeAsync.mockResolvedValue({
        ok: true,
        status: 400,
        stage: 'async_endpoint_ok',
    ...
    it('apimart probe-async 本身抛错时保留 test-connection 原始信息（不覆盖）', async () => {
      ...
      await mod.test('a');
      expect(mod.useProviders().testResult).toEqual({
        ok: false,
        status: 0,
        error: '连接失败: Connect Timeout',
      });
  ...
  describe('save（透传 + 回退默认 KV）', () => {
    it('save 透传整组 provider，剥离 key 通道字段，并把主供应商回写 active_api_endpoint', async () => {
      ...
      h.mockSaveProviders.mockResolvedValue({
        data: { providers: [{ id: 'a', name: 'A', primary: true }] },
      });
      ...
      // 主供应商回写 KV
      expect(h.mockKvSet).toHaveBeenCalledWith(
        'active_api_endpoint',
