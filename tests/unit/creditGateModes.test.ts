...
vi.mock('../../src/components/agent/canvas/canvasPlanExecutor.ts', async (importOriginal) => {
  const actual =
    (await importOriginal()) as unknown as typeof import('../../src/components/agent/canvas/canvasPlanExecutor.ts');
  return {
    ...actual,
    executePlan: vi.fn(async (args) => ({
      ...
      entries: [
        {
          status: args?.autoRun ? 'completed' : 'ready',
          nodeId: 'n1',
          stepId: 'g1',
          resultUrl: args?.autoRun ? 'http://r/x.png' : '',
        },
      ],
    })),
  };
...
beforeEach(() => {
  ...
  vi.mocked(convStore.getCreditGate).mockImplementation(
    () => convMock.__state.creditGate as CreditGate | null,
  );
  vi.mocked(convStore.setCreditGate).mockImplementation((g) => {
    convMock.__state.creditGate = (g || null) as CreditGate | null;
  });
