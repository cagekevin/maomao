...
const mod = await import('@/components/base/api/proxyGenerate.ts')
const { chatProxy, imageProxy } = mod
const { GEN_TIMEOUT } = await import('../../src/components/base/config.ts')
...
describe('chatProxy 红线：信封永不抛错', () => {
  it('成功：取 json.data.choices[0].message.content', async () => {
    fetchMock.mockResolvedValueOnce(jsonResp({ data: { choices: [{ message: { content: 'hi' } }] } }))
    const r = await chatProxy({ provider, body: {} })
    expect(r).toEqual({ ok: true, content: 'hi' })
  })

  it('成功：json 顶层无 data 也兼容（取 json.choices）', async () => {
    fetchMock.mockResolvedValueOnce(jsonResp({ choices: [{ message: { content: 'x' } }] }))
    const r = await chatProxy({ provider, body: {} })
  ...
  it('非2xx：返回嵌套 message，不抛、不误加「网络错误」前缀', async () => {
    fetchMock.mockResolvedValueOnce(jsonResp({ error: { message: '余额不足' } }, false, 402))
    const r = await chatProxy({ provider, body: {} })
  ...
  it('非2xx：无嵌套 message 时回退 HTTP status', async () => {
    fetchMock.mockResolvedValueOnce(jsonResp({}, false, 500))
    const r = await chatProxy({ provider, body: {} })
    expect(r.ok).toBe(false)
  ...
  it('content 为空字符串：返回「上游未返回文本内容」', async () => {
    fetchMock.mockResolvedValueOnce(jsonResp({ data: { choices: [{ message: { content: '   ' } }] } }))
    const r = await chatProxy({ provider, body: {} })
    expect(r.ok).toBe(false)
  ...
  it('非网络异常（普通 Error）不再被自加「网络错误」前缀 → 下游归 business、不可自动重试', async () => {
    ...
    fetchMock.mockRejectedValueOnce(new Error('JSON 解析失败'))
    const r = await chatProxy({ provider, body: {} })
    expect(r.ok).toBe(false)
    expect(r.error.startsWith('网络错误')).toBe(false)
