...
describe('sanitizeMemoryContent —— 脱敏（T1）', () => {
  it('脱敏密钥、凭据、本地路径，压缩空白并截断到上限', () => {
    const dirty =
      ' 密钥 sk-abcdef1234567890 且 token=SECRET12345 ，本地 /Users/kevin/data/a.png ，多    空格 ';
...
describe('saveProjectMemory / loadProjectMemories —— 保存与读取（T2）', () => {
  ...
  it('kind 非法时回退 fact，content 超长截断', async () => {
    const saved = await saveProjectMemory('gw', {
      kind: 'badKind' as unknown as Parameters<typeof saveProjectMemory>[1]['kind'],
