  ...
  it('getLastUserReferenceImages：向前找最近带图 user 消息，返回其参考图 url', () => {
    setup([
      ...
      { role: 'assistant', content: 'ok1' },
      {
        role: 'user',
        content: '轮2',
        attachments: [{ url: 'http://x/b.png' }, { url: 'http://x/c.png' }],
      },
    ]);
  ...
  it('getLastGeneratedImages：向前找最近带生成结果图的 assistant 消息（lastResults）', () => {
    setup([
      ...
      { role: 'user', content: '轮' },
      {
        role: 'assistant',
        content: '新',
        lastResults: [{ url: 'http://x/new1.png' }, { url: 'http://x/new2.png' }],
      },
    ]);
    ...
    setup([
      {
        role: 'assistant',
        content: '生成',
        lastResults: [{ url: 'http://x/gen1.png', name: '主图' }],
      },
    ]);
    // 设置当前附件（画布选中/上传的参考图）
    setCurrentSnapshot({
      messages: getCurrentSnapshot().messages,
      attachments: [{ url: 'http://x/att.png', label: '参考' }],
    });
