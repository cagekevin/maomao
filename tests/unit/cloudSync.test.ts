...
function gasCalls(action) {
  return fetchMock.mock.calls.filter((c) => {
    ...
    try { return JSON.parse(c[1].body).action === action } catch { return false }
  })
}
...
describe('cloudSync — 防覆盖保护：云端包解析（GAS 两种回包形态）', () => {
  it('形态A（完整包）：取外层 rev/updatedAt，data 剔除 __meta', () => {
    const snap = normalizeCloudPayload({
      type: 'cloud_config', version: 5, rev: 7, updatedAt: 111,
  ...
  it('形态B（GAS 只回 data 内容）：rev/updatedAt 从 data.__meta 兜底取回', () => {
    const snap = normalizeCloudPayload({ app_settings: { a: 1 }, __meta: { rev: 3, updatedAt: 222 } })
...
describe('cloudSync — 防覆盖保护：上传冲突判定（纯函数）', () => {
  ...
  it('云端为空 → none（首次上传）', () => {
    expect(decideUpload({ ...base, cloudExists: false, ledger, localHash: 'hash-v3' }).kind).toBe('none')
  })

  it('云端读不到 → cloud-unknown（不硬阻断，交用户定夺）', () => {
    expect(decideUpload({ ...base, cloudReadable: false, ledger, localHash: 'hash-v3' }).kind).toBe('cloud-unknown')
  })

  it('无台账时保守视为「本地改过」，宁可多问不可静默覆盖', () => {
    expect(decideUpload({ ...base, cloudReadable: false, ledger: null, localHash: 'x' }).localDirty).toBe(true)
  })
})
...
describe('cloudSync — 防覆盖保护：弹窗文案', () => {
  it('上传三种冲突各有明确标题与 danger 标记；cloud-newer 会引导去下载', () => {
    const both = describeUploadConflict({ kind: 'both-changed', cloudRev: 5, cloudUpdatedAt: 111, ledgerRev: 2, localDirty: true })
    expect(both.title).toContain('云端和本地都有更新')
    expect(both.danger).toBe(true)
    const newer = describeUploadConflict({ kind: 'cloud-newer', cloudRev: 5, cloudUpdatedAt: 111, ledgerRev: 2, localDirty: false })
    expect(newer.title).toContain('云端内容比你本地新')
    expect(newer.message).toContain('从云端拉取') // 引导用户往正确方向走
    const unknown = describeUploadConflict({ kind: 'cloud-unknown', cloudRev: 0, cloudUpdatedAt: 0, ledgerRev: 2, localDirty: true })
...
describe('cloudSync — 防覆盖保护：端到端（用户取消 / 用户确认）', () => {
  it('上传遇冲突且用户取消 → cancelled:true，且不发起 push（云端保持原样）', async () => {
    ...
    fetchMock.mockResolvedValue(jsonResp({
      data: {
        type: 'cloud_config', version: 5, rev: 9, updatedAt: Date.now(),
  ...
  it('上传遇冲突但用户确认 → 正常推送，且 rev = 云端 rev + 1（单调递增）', async () => {
    ...
    fetchMock.mockImplementation((_url, opt) => {
      let action = ''
      try { action = JSON.parse(opt.body).action } catch { /* 非 GAS 报文 */ }
      if (action === 'pull_data') {
        return Promise.resolve(jsonResp({
          data: { // 信封：pull 返回 res.data
            type: 'cloud_config', version: 5, rev: 4, updatedAt: 1,
            data: { app_settings: { theme: 'light' } },
          },
        }))
      }
  ...
  it('下载有覆盖项且用户取消 → cancelled:true，本地一个字节都没改', async () => {
    ...
    fetchMock.mockResolvedValue(jsonResp({
      data: {
        type: 'cloud_config', version: 5, rev: 3, updatedAt: Date.now(),
        data: { app_settings: { theme: 'light' } },
      },
    }))
