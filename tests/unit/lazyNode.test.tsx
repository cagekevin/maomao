...
const { lazyNode, HEAVY_NODE_LOADERS, prefetchHeavyNode } =
  await import('../../src/components/canvas/shell/lazyNode.tsx');
...
describe('按需加载 · 静态 import 回归拦截', () => {
  const staticImportsOf = (src: any) =>
    src.split('\n').filter((l: any) => /^\s*import\s.+from\s/.test(l));

  it('NodePalette 不得静态 import 重依赖节点（否则 vendor-3d/media 进首屏）', () => {
    const imports = staticImportsOf(readSrc('src/components/canvas/shell/NodePalette.ts'));
    const offenders = imports.filter((l: any) =>
  ...
  it('重依赖 loader 必须是字面量动态 import（供 Vite 静态分析出 chunk）', () => {
    const src = readSrc('src/components/canvas/shell/lazyNode.tsx');
    // 先剥离注释：文档里会写「禁止的反例」（import(`../nodes/${type}.jsx`)），
    // 不剥离会被下面的检测误判成违规。
    const stripComments = (s: any) =>
      s.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^[ \t]*\/\/.*$/gm, '');
    const loaderBlock = stripComments(
      src.slice(
        src.indexOf('HEAVY_NODE_LOADERS'),
        src.indexOf('export function prefetchHeavyNode'),
      ),
    );
    // 每个 loader 形如 `xxx: () => import('../nodes/X.jsx')`
    const loaders = [...loaderBlock.matchAll(/(\w+):\s*\(\)\s*=>\s*import\(/g)].map((m) => m[1]);
    expect(loaders.sort()).toEqual(['director3dNode', 'panoramaNode', 'videoProcessNode'].sort());
