...
describe('M4-C1 writeObjectTrack 统一写入口', () => {
  it('upsert 增/改一帧：同帧覆盖（整体替换该帧 key）', () => {
    ...
    expect(tracks['actor-lead'].transform[0]).toEqual({
      frame: 10,
      interpolation: 'linear',
      fields: { position: [1, 0, 0] },
    });
    // 同帧再写完整 key → 覆盖该帧（含插值）
    const replaceKeys = snapshotToChannelKeys(
      'person',
      { frame: 10, position: [9, 0, 0], rotation: [0, 1, 0], scale: [1, 1, 1] },
      10,
      'smooth',
    );
    ...
    expect(after['actor-lead'].transform[0].fields).toEqual({
      position: [9, 0, 0],
      rotation: [0, 1, 0],
      scale: [1, 1, 1],
    });
  ...
  it('move 整帧平移', () => {
    const keys = snapshotToChannelKeys(
      'person',
      { frame: 10, position: [1, 0, 0], rotation: [0, 0.5, 0] },
      10,
      'linear',
    );
    const tracks = writeObjectTrack({}, 'actor-lead', { op: 'upsert', keys });
  ...
  it('interpolation 改该帧插值（各通道一致）', () => {
    const keys = snapshotToChannelKeys(
      'person',
      { frame: 10, position: [1, 0, 0], pose: 'walk', poseTime: 0.5, continuousMotion: true },
      10,
      'linear',
    );
    const tracks = writeObjectTrack({}, 'actor-lead', { op: 'upsert', keys });
    const updated = writeObjectTrack(tracks, 'actor-lead', {
      op: 'interpolation',
      frame: 10,
      value: 'hold',
    });
  ...
  it('setChannelInterpolation 只改目标帧、不动其它帧', () => {
    ...
    const next = setChannelInterpolation(channels, 10, 'hold') as unknown as {
      transform: Array<{ interpolation: string }>;
    };
...
describe('M4-C5 batch 原子性（路径 bake 先删旧帧再整批插新）', () => {
  it('一次 batch 结果 = 顺序手写结果，手动帧不丢', () => {
    const seed = snapshotToChannelKeys(
      'person',
      { frame: 24, position: [100, 0, 100], pose: 'run', poseTime: 0.6, continuousMotion: true },
      24,
      'smooth',
    );
    let tracks = writeObjectTrack({}, 'actor-lead', { op: 'upsert', keys: seed });
    tracks = writeObjectTrack(tracks, 'actor-lead', {
      op: 'upsert',
      keys: snapshotToChannelKeys('person', { frame: 0, position: [0, 0, 0] }, 0, 'linear'),
    });
    tracks = writeObjectTrack(tracks, 'actor-lead', {
      op: 'upsert',
      keys: snapshotToChannelKeys('person', { frame: 60, position: [2.5, 0, 0] }, 60, 'linear'),
    });
...
describe('M4-C4 迁移后 M2 逐帧一致（旧参考实现对照）', () => {
  it('纯手动 legacy 轨迁移后逐帧全等（无路径场景）', () => {
    ...
    const migrated = normalizeObjectTracks(
      { 'actor-lead': legacyKeys },
      { 'actor-lead': 'person' },
      {},
    )['actor-lead'];
