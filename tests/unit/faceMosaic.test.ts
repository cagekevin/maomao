...
import type { MosaicMode } from '../../src/components/base/utils/faceMosaic.ts';
...
import {
  ...
  drawMosaicOnBox,
  MOSAIC_PALETTE,
} from '../../src/components/base/utils/faceMosaic.ts';
...
describe('loadFaceDetector', () => {
  ...
  it('加载失败抛出且单例清空，下次调用可重试', async () => {
    ...
    const { loadFaceDetector: load2 } =
      await import('../../src/components/base/utils/faceMosaic.ts');
    ...
    vi.mocked(FaceDetector.createFromOptions).mockResolvedValueOnce(
      fakeFaceDetector as unknown as import('@mediapipe/tasks-vision').FaceDetector,
    );
...
describe('drawMosaicOnBox', () => {
  ...
  it('非法 mode 不抛错（回退默认分支）', () => {
    const ctx = makeCtx();
    expect(() =>
      drawMosaicOnBox(ctx, SRC, BOX, 'unknown-mode' as unknown as MosaicMode, 0.5),
    ).not.toThrow();
  });
});
