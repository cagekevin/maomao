 ...
 * 覆盖：
 *   - normalizeAttachmentsForSend：每条 { ...a, url } 经 normalizeAssetUrlForSend（含 preferBase64 透传）；返回 Promise。
 *   - buildRefCatalog：只对图片附件编号、>0 时生成目录文本、无图片 → 空串、nodeId 记录、label/name 兜底。
 * 策略：mock normalizeAssetUrlForSend 验证透传与 Promise 形态（纯函数，不 import store）。
 */
...
describe('normalizeAttachmentsForSend — 附件统一归一出口', () => {
  it('每条附件经 normalizeAssetUrlForSend，保留其它字段', async () => {
  ...
  it('preferBase64 透传给归一函数', async () => {
    ...
    expect(normalizeAssetUrlForSend).toHaveBeenCalledWith('http://a', { preferBase64: true });
  });

  it('缺省 preferBase64=false', async () => {
    await normalizeAttachmentsForSend([{ url: 'http://a' }]);
    expect(normalizeAssetUrlForSend).toHaveBeenCalledWith('http://a', { preferBase64: false });
  });
