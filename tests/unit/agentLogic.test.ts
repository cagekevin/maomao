...
describe('AI 助手 buildRequestMessages（发 LLM 消息组装）§2.15', () => {
  // 只验"注入的结构契约"（enhance 时准则作为首条 system，不被覆盖），不断言具体文案——
  ...
  it('【fresh-task】历史消息（含 system）不进 LLM：只保留本轮 user + 前置注入的准则/systemPrompt', () => {
    ...
    expect(sys).toHaveLength(3); // 画布准则 + 外部准则 + 执行模型分流段(auto)（历史 system「已有」被丢弃）
  ...
  it('正常 assistant(tool_calls) + tool 配对透传，tool_call_id 匹配', () => {
    const msgs: ChatMessage[] = [
      { role: 'user', content: '生成' },
      {
        ...
        content: '',
        tool_calls: [
          { id: 'c_real', type: 'function', function: { name: 'create_node', arguments: '{}' } },
        ],
      },
    ...
    const msgs: ChatMessage[] = [
      {
        ...
        content: '',
        tool_calls: [
          { id: 'c1', type: 'function', function: { name: 'read_canvas', arguments: '{}' } },
        ],
      },
      { role: 'tool', content: '{"ok":true}', tool_call_id: 'c1' },
    ...
    const msgs: ChatMessage[] = [
      {
        role: 'user',
        content: '按这张图改',
        attachments: [{ type: 'image', url: '/files/a.png', nodeId: 'n1', x: 100, y: 200 }],
      },
    ];
    const out = buildRequestMessages(msgs, '', false);
    const user = out.find((m) => m.role === 'user');
