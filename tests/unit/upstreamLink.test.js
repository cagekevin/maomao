...
vi.mock('../../src/components/base/logger.js', () => ({ logger: loggerState }))
...
describe('useUpstreamAutoTrigger — 下游触发失败可见（§3.3）', () => {
  ...
  it('下游触发失败 → logger.warn 留痕，且不往上抛 rejection', async () => {
    ...
    await act(async () => { await busState.handler({ sourceNodeId: 'n1' }) })
  ...
  it('下游触发成功 → 仅 info 不 warn（失败才留痕）', async () => {
    ...
    await act(async () => { await busState.handler({ sourceNodeId: 'n1' }) })
    expect(runNodeGenerationMock).toHaveBeenCalledWith('n2')
    expect(loggerState.info).toHaveBeenCalledWith('拓扑', '[G1] 上游完成 → 触发直接下游', expect.anything())
