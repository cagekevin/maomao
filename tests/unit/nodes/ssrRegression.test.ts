...
const cases = [
  ...
  {
    name: '视频抽帧 VideoExtractNode',
    Comp: VideoExtractNode,
  ...
  {
    name: '视频处理 VideoProcessNode',
    Comp: VideoProcessNode,
    type: 'videoProcessNode',
    expect: ['group/node', 'react-flow__handle', '视频处理'],
  },
  {
    name: '人脸打码 FaceMosaicNode',
    Comp: FaceMosaicNode,
    type: 'faceMosaicNode',
    expect: [
      'group/node',
  ...
  },
  {
    ...
    Comp: GroupNode,
    type: 'group',
    expect: ['group/node', 'bg-surface-raised', 'rounded-xl', 'border-edge', '编组'],
  },
  {
    name: '剧本盒子 ScriptBoxNode',
    Comp: ScriptBoxNode,
    type: 'scriptBoxNode',
    expect: ['group/node', 'bg-surface-raised', 'rounded-xl', 'border-edge', '剧本盒子'],
  },
];
    ...
    test(`${c.name} 结构完整`, () => {
      const html = renderToString(
        React.createElement(
          ...
          null,
          React.createElement(
            c.Comp as React.ComponentType<{ id: string; data: unknown; selected: boolean }>,
            {
              id: c.type + '-1',
