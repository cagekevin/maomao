...
import { buildCanvasMenuItems, buildNodeMenuItems, buildSelectionMenuItems, menuForState, type MenuActionCtx } from '../../src/components/base/canvas/canvasContextMenu.tsx'
...
const keyed = (items: ContextMenuItem[]): Array<{ key?: string; items?: Array<{ key?: string }> }> =>
  items.filter((i) => i.type !== 'divider') as Array<{ key?: string; items?: Array<{ key?: string }> }>
...
describe('buildNodeMenuItems（单选节点右键）', () => {
  it('图片类节点（promptNode）有「复制图片」', () => {
    const items = buildNodeMenuItems(makeCtx({ nodeById: () => ({ type: 'promptNode' }) as never }), 'node-id')
    const ls = labels(items)
  ...
  it('imageNode 也有「复制图片」', () => {
    const ls = labels(buildNodeMenuItems(makeCtx({ nodeById: () => ({ type: 'imageNode' }) as never }), 'node-id'))
    expect(ls).toContain('复制图片')
  })

  it('group 节点有「取消编组」，普通节点没有', () => {
    const groupLs = labels(buildNodeMenuItems(makeCtx({ nodeById: () => ({ type: 'group' }) as never }), 'node-id'))
    expect(groupLs).toContain('取消编组')
    const plainLs = labels(buildNodeMenuItems(makeCtx({ nodeById: () => ({ type: 'textNode' }) as never }), 'node-id'))
