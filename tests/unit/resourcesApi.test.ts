...
describe('rescanResources', () => {
  ...
  it('非 2xx 抛 HttpError', async () => {
    mockFetchOnce({ error: 'x' }, { ok: false, status: 500 });
    await expect(ra.rescanResources()).rejects.toMatchObject({
      name: 'HttpError',
      status: 500,
      message: 'x',
    });
...
describe('deleteResource', () => {
  ...
  it('非 2xx 抛 HttpError', async () => {
    mockFetchOnce({ error: 'x' }, { ok: false, status: 500 });
    await expect(ra.deleteResource('id1')).rejects.toMatchObject({
      name: 'HttpError',
      status: 500,
      message: 'x',
    });
...
describe('openLocalFolder / openFileDir', () => {
  ...
  it('openFileDir 正常请求 open-dir', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toContain(
      'open-dir?filepath=' + encodeURIComponent('tasks/a.png'),
    );
  });

  it('openLocalFolder 非 2xx 抛 HttpError', async () => {
    mockFetchOnce({ error: 'x' }, { ok: false, status: 500 });
    await expect(fl.openLocalFolder('tasks')).rejects.toMatchObject({
      name: 'HttpError',
      status: 500,
      message: 'x',
    });
  ...
  it('openFileDir 非 2xx 抛 HttpError', async () => {
    mockFetchOnce({ error: 'x' }, { ok: false, status: 500 });
    await expect(fl.openFileDir('tasks/a.png')).rejects.toMatchObject({
      name: 'HttpError',
      status: 500,
      message: 'x',
    });
...
describe('relativePathFromUrl', () => {
  it('去掉 /files/ 前缀并解码', () => {
    expect(fl.relativePathFromUrl('http://127.0.0.1:18080/files/tasks/a%20b.png')).toBe(
      'tasks/a b.png',
    );
  });
