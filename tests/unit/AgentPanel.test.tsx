...
const h = vi.hoisted(() => {
  ...
  const contentSubscribe = vi.fn((key, cb) => {
    ...
    return subscribeUnsubs[subscribeUnsubs.length - 1];
  });
...
describe('AgentPanel — 消息发送', () => {
  ...
  beforeEach(() => {
    h.setProviders([
      { id: 'modelscope', name: '魔搭', primary: true, chat_models: [{ id: 'qwen-max' }] },
    ]);
  });
  it('输入文本 → 回车发送（携带输入内容）', () => {
    render(<AgentPanel {...OPEN_PROPS} />);
...
describe('AgentPanel — 设置改模型/供应商即生效（方案 B，无需刷新）', () => {
  it('挂载 → 订阅 agent_chat_model 变更（contentSubscribe 注册）', () => {
    ...
    expect(h.contentSubscribe).toHaveBeenCalledWith(
      expect.stringMatching(/^agent_chat_model$/),
      expect.any(Function),
    );
  });
  ...
  it('触发设置变更 → setModel 同步为新 modelId（model 进 useAgentChat）', () => {
    render(<AgentPanel {...OPEN_PROPS} />);
    act(() => {
      h.fireAgentModelChange({ providerId: 'p1', modelId: 'gpt-5', streamMode: 'stream' });
    });
  ...
  it('触发设置变更 → useAgentChat 收到新 provider（agentProvider 重算）', () => {
    ...
    expect(h.useAgentChat).toHaveBeenCalledWith(
      expect.objectContaining({ provider: expect.objectContaining({ id: 'p2' }) }),
    );
  });
