...
const { appendMsg, setHistory, updateLastStreaming, endStreaming, stripStreaming } =
  await import('../../src/components/agent/runtime/agentMessages.ts');
...
describe('updateLastStreaming — 流式增量（仅通知）', () => {
  ...
  it('只改最后一条；前面的消息不动', () => {
    store.set([
      { id: 'a', role: 'user', content: 'keep' },
      { id: 's', role: 'assistant', content: '', streaming: true },
    ]);
...
describe('endStreaming — 结束流式', () => {
  it('保留占位 id 替换为完整 assistant', () => {
    store.set([
      ...
      { id: 'sid', role: 'assistant', content: '', streaming: true },
    ]);
