  ...
  it('migration 键指向的目标必须存在', () => {
    for (const [key, entry] of Object.entries(STORAGE_KEYS)) {
      if (entry.migration) {
        ...
        const found = Object.keys(STORAGE_KEYS).find((k) =>
          k.startsWith(targetKey.replace(/\{.*\}/, '')),
        );
        expect(
          found,
          `${key} 的 migration 目标 "${targetKey}" 在 STORAGE_KEYS 中不存在`,
        ).toBeTruthy();
      }
    }
  });
...
describe('STORAGE_KEYS 内容验证', () => {
  ...
  it('旧迁移键标记了 migration 字段', () => {
    ...
    expect(STORAGE_KEYS.agent_active_conversation_id.migration).toBe(
...
describe('getLocalKeys', () => {
  ...
  it('返回的键数量与 STORAGE_KEYS 中 local+migration+pattern 过滤一致', () => {
    const expected = Object.entries(STORAGE_KEYS).filter(
      ([, v]) => v.backend === 'local' && !v.migration && !v.pattern,
    ).length;
