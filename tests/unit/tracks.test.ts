...
const manualSnapshot = (frame, position, pose = 'run') => ({
  frame, interpolation: 'smooth', position,
  rotation: [0, 0.5, 0], scale: [1.2, 1, 1],
  pose, poseTime: 0.6, continuousMotion: true, rigRoot: [0, 0, 0], joints: runJoints,
})
...
const camSnapshot = (frame, position, focalLength = 42) => ({
  frame, interpolation: 'linear', position,
  rotation: [0.3, 0, 0], focalLength,
})
...
describe('bakeObjectPath 对象路径烘焙', () => {
  const seedTracks = () => {
    let tracks = {}
    tracks = upsertObjectSnapshot(tracks, 'actor-lead', 'person', manualSnapshot(24, [100, 0, 100]))
    tracks = upsertObjectSnapshot(tracks, 'actor-lead', 'person', { frame: 0, interpolation: 'linear', position: [0, 0, 0] })
    tracks = upsertObjectSnapshot(tracks, 'actor-lead', 'person', { frame: 60, interpolation: 'linear', position: [2.5, 0, 0] })
    return tracks
  }

  it('先删旧路径帧再整批插新（原子 batch），手动帧保留、路径帧只落 transform.position', () => {
    const tracks = seedTracks()
    const next = bakeObjectPath(tracks, 'actor-lead', 'person', [
  ...
  it('空 removeFrames 时仅插新帧（首次烘焙无旧路径帧）', () => {
    const tracks = {}
    const next = bakeObjectPath(tracks, 'actor-lead', 'person', [{ frame: 0, position: [0, 0, 0] }], [])
...
describe('bakeCameraPath 相机路径烘焙', () => {
  it('先删旧路径帧、再按曲线逐帧写 transform.position/rotation + lens.focalLength', () => {
    ...
    const next = bakeCameraPath(tracks, straightPath, camera, [
      { frame: 0, position: [0, 0, 0] },
      { frame: 120, position: [5, 0, 0] },
    ], [0, 60])
...
describe('duplicateObjectTrack / clearObjectTrack', () => {
  const sampleTracks = () => {
    let tracks = {}
    tracks = upsertObjectSnapshot(tracks, 'actor-lead', 'person', manualSnapshot(0, [0, 0, 0], 'walk'))
