...
describe('volumePolicy · truncateTo', () => {
  ...
  it('非字符串原样返回', () => {
    expect(truncateTo(undefined as unknown as string, 10)).toBe(undefined);
    expect(truncateTo(null as unknown as string, 10)).toBe(null);
  });
  it('超长补省略标记', () => {
...
describe('volumePolicy · sanitizeLastResults', () => {
  ...
  it('按 url 去重并保留最近顺序', () => {
    ...
    expect(sanitizeLastResults(list).map((i) => (i as { id: number }).id)).toEqual([2, 3]); // 保原时序：b 在前，较新的 a(3) 顶上
  });
  it('超上限只留最近 LAST_RESULTS_MAX 条', () => {
    const list = Array.from({ length: LAST_RESULTS_MAX + 5 }, (_, i) => ({
      url: `http://h/u${i}.png`,
    }));
  ...
  it('过滤无效项', () => {
    // 故意喂 null/字符串 等非 LastResultItem 脏数据测容错；unknown as 标注此处为故意
    const out = sanitizeLastResults([
      null,
      { url: 'http://h/x.png' },
      'bad',
    ] as unknown as LastResultItem[]);
...
describe('volumePolicy · capConversationMemory', () => {
  ...
  it('facts 超限保最近 FACTS_MAX', () => {
    const mem = {
      facts: Array.from({ length: FACTS_MAX + 3 }, (_, i) => ({ k: `k${i}`, v: 'v' })),
    };
  ...
  it('artifacts 超限保最近 ARTIFACTS_MAX', () => {
    const mem = {
      artifacts: Array.from({ length: ARTIFACTS_MAX + 5 }, (_, i) => ({ id: String(i) })),
    };
    const out = capConversationMemory(mem);
  ...
  it('不 mutate 入参', () => {
    const mem = {
      facts: Array.from({ length: FACTS_MAX + 1 }, (_, i) => ({ k: `k${i}`, v: 'v' })),
    };
...
describe('volumePolicy · 整包预算', () => {
  ...
  it('入参内存态不被 mutate（投影副本原则）', () => {
    const huge = 'x'.repeat(100000);
    const convs = [
      { id: 'c1', messages: [{ id: 'm1', role: 'user', content: huge, streaming: true }] },
    ];
        ...
        messages: [
          {
            ...
            role: 'assistant',
            content: 'r',
            lastResults: [{ url: 'u1' }, { url: 'u1' }, { url: 'u2' }, { url: 'u2' }],
          },
        ],
      },
    ];
