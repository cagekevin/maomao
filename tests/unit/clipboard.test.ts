...
describe('clipboard — buildNodesFromClipboard（粘贴节点组重建）', () => {
  ...
  it('空 nodes → null', () => {
    expect(
      buildNodesFromClipboard('{"type":"mutiwindow-nodes","nodes":[]}', { x: 0, y: 0 }),
    ).toBeNull();
  });
...
describe('clipboard — resolveDownloadFilename（下载文件名推导）', () => {
  ...
  it('无 label → 用 URL 末尾文件名', () => {
    expect(resolveDownloadFilename('', 'http://127.0.0.1:18080/files/tasks/abc.png')).toBe(
      'abc.png',
    );
  });
  ...
  it('视频语义：ext=mp4 / fallback=video.mp4', () => {
    expect(
      resolveDownloadFilename('片段', 'http://x/a.mp4', { ext: 'mp4', fallback: 'video.mp4' }),
    ).toBe('片段.mp4');
    expect(
      resolveDownloadFilename('', 'blob:http://x/abc', { ext: 'mp4', fallback: 'video.mp4' }),
    ).toBe('video.mp4');
  });
});
