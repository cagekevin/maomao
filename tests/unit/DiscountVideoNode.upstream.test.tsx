...
describe('DiscountVideoNode 上游文本/图片合并（修复点）', () => {
  it('上游文本节点连入时，文字合并进生视频 prompt', async () => {
    connectedInputs = {
      images: [],
      texts: [{ id: 't1', text: '产品开箱视频', sourceNodeId: 's1' }],
    };
    setup({ prompt: '快节奏剪辑' });
    fireEvent.click(screen.getByText('生成'));

    await waitFor(() => expect(mockGenerateVideo).toHaveBeenCalled());
    const call = mockGenerateVideo.mock.calls[0][0] as unknown as {
      prompt: string;
      images?: unknown;
    };
  ...
  it('上游有文本但本地 prompt 为空时，校验通过', async () => {
    ...
    fireEvent.click(screen.getByText('生成'));
    await waitFor(() => expect(mockGenerateVideo).toHaveBeenCalled());
    expect((mockGenerateVideo.mock.calls[0][0] as unknown as { prompt: string }).prompt).toContain(
      '夏日促销',
    );
  });
