...
describe('A1 adjustDepthValue — 深度值对比度/反色', () => {
  ...
  it('contrast=1.6 放大偏离', () => {
    expect(adjustDepthValue(200, { contrast: 1.6, invert: false })).toBe(Math.round(128 + (200 - 128) * 1.6))
...
describe('A2 灰度化 grayFromRaw / grayFromTensor', () => {
  ...
  it('tensor 归一化 float 放大到 0..255', () => {
    ...
    const out = grayFromTensor(new Float32Array([0, 0.5, 1]), [1, 1, 3], { contrast: 1, invert: false })
    expect(Array.from(out)).toEqual([0, Math.round(127.5), 255])
  })
  it('tensor 全等不除零、不产生 NaN', () => {
    const out = grayFromTensor(new Float32Array([0.5, 0.5, 0.5]), [1, 1, 3], { contrast: 1, invert: false })
...
describe('A5 资源前缀 buildRuntimeModels（同源根相对）', () => {
  it('返回同源根相对路径（页面 origin 即资源 origin）', () => {
    ...
    expect(p.wasmPaths.wasm).toBe('/depth-video/vendor/onnxruntime/ort-wasm-simd-threaded.asyncify.wasm')
...
describe('A6 下游节点规格 buildDepthChildSpec + 命名', () => {
  it('下游为 imageNode 视频态，imageUrl=outputUrl，expanded=true，label=name', () => {
    const spec = buildDepthChildSpec('http://127.0.0.1:18080/files/canvas/video-process/x_depth.mp4', 'scene_depth.mp4')
    expect(spec.type).toBe('imageNode')
    expect(spec.data.mediaType).toBe('video')
    expect(spec.data.imageUrl).toBe('http://127.0.0.1:18080/files/canvas/video-process/x_depth.mp4')
