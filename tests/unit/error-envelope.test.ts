...
describe('extractErrorDetail — 错误信封唯一解析入口', () => {
  it('error 为字符串 → 字符串兜底 {code:UNKNOWN}', () => {
    expect(extractErrorDetail({ error: 'bad request' })).toEqual({
      code: 'UNKNOWN',
      message: 'bad request',
    });
  });
  ...
  it('error 为结构化 {code,message} → 透传结构化', () => {
    expect(extractErrorDetail({ error: { code: 'business', message: '余额不足' } })).toEqual({
      code: 'business',
      message: '余额不足',
    });
  });
  ...
  it('error 结构化缺 code → 回落 UNKNOWN；缺 message 回落 detail', () => {
    expect(extractErrorDetail({ error: { detail: '磁盘满' } })).toEqual({
      code: 'UNKNOWN',
      message: '磁盘满',
    });
  });
  ...
  it('平铺 detail → 取 detail', () => {
    expect(extractErrorDetail({ detail: 'db down' })).toEqual({
      code: 'UNKNOWN',
      message: 'db down',
    });
  });
  ...
  it('优先级：error.message > data.detail > data.message', () => {
    expect(extractErrorDetail({ error: { message: 'A' }, detail: 'B', message: 'C' })).toEqual({
      code: 'UNKNOWN',
      message: 'A',
    });
  });
