...
vi.mock('../../src/components/base/editors/FaceMosaicEditor.tsx', () => ({
  default: ({ imageUrl: _imageUrl, onSave, onClose }) =>
    React.createElement(
      ...
      React.createElement(
        'button',
        { onClick: () => onSave('data:image/png;base64,QUFBQQ==') },
        '保存手动打码',
      ),
...
function setup(
  ...
  connected: { images?: unknown[]; texts?: unknown[] } = { images: [], texts: [] },
) {
...
describe('FaceMosaicNode — AI打码', () => {
  it('有图 → 打码成功输出 imageNode + 结果信息', async () => {
    ...
    await waitFor(() => {
      expect(h.applyMosaicMock).toHaveBeenCalledWith('ABS:http://x/in1.png', {
        mode: 'mosaic',
        strength: 0.5,
        color: '#000000',
      });
