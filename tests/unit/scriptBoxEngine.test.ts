  ...
  describe('assembleShotUser', () => {
    ...
    it('合并 shot 字段 + 参考资源 + 全局风格', () => {
      ...
      const shot = {
        ...
        motion: '推镜',
        description: '主角登场',
        dialogue: [{ kind: '旁白', role: '旁白', text: '开始' }],
        sound: '风声',
      };
    ...
    it('P1-3：开启上一镜尾帧且锁定参考 URL → 注入视觉起点约束', () => {
      const shot = {
        ...
        prevShotImageRefUrls: ['/files/tail.jpg'],
      };
      const out = assembleShotUser(shot, [], '');
    ...
    it('P1-3：未开启尾帧 → 不注入视觉起点', () => {
      const out = assembleShotUser(
        { index: 2, description: 'x', prevShotImageRefUrls: ['/files/tail.jpg'] },
        [],
        '',
      );
    ...
    it('P2-3：首镜/末镜/中段分别命中对应位置语义', () => {
      ...
      expect(assembleShotUser({ index: 3, description: 'x' }, [], '', { totalShots: 3 })).toContain(
        '结尾镜',
      );
      expect(assembleShotUser({ index: 2, description: 'x' }, [], '', { totalShots: 3 })).toContain(
        '中段镜',
      );
    });
    ...
    it('对齐官方：命中四象限剧情职责（前 20% / 中段 / 后 80%+）', () => {
      expect(
        assembleShotUser({ index: 1, description: 'x' }, [], '', {
          totalShots: 10,
          shotIndexInStory: 0,
        }),
      ).toContain('交代信息/建立悬念/锚定冲突');
      expect(
        assembleShotUser({ index: 5, description: 'x' }, [], '', {
          totalShots: 10,
          shotIndexInStory: 4,
        }),
      ).toContain('冲突升级/情绪累积/压力堆叠');
      expect(
        assembleShotUser({ index: 10, description: 'x' }, [], '', {
          totalShots: 10,
          shotIndexInStory: 9,
        }),
      ).toContain('收束留钩/余韵钩子');
...
describe('scriptBoxEngine · 引擎编排', () => {
  function makeEngine(initial) {
    ...
    const setEdges = vi.fn((updater) => {
      store._edges = updater(store._edges || []);
    });
  ...
  it('onGenerateScript 把上游接入图片传给 chatCompletions（编剧 AI 看产品外观）', async () => {
    ...
    const { engine, store } = makeEngine({
      ...
      upstreamImages: [
        { id: 'i1', url: 'http://127.0.0.1:18080/files/p.png' },
        { id: 'i2', url: '/files/q.png' },
      ],
    });
    await engine.onGenerateScript();
    expect(chatCompletions).toHaveBeenCalled();
    ...
    expect(imgNode.data.images).toMatchObject([
      {
        ...
        url: 'http://127.0.0.1:18080/files/城堡.png',
        label: '城堡',
      },
    ]);
  ...
  it('onConnectShot 透传宽高比（image→aspectRatio；video→size+时长+durationFromScript）', async () => {
    const { engine, addNodes } = makeEngine({
      aspectRatio: '9:16',
      shots: [
        { id: 's1', index: 1, description: 'x', prompt: 'p', videoPrompt: 'v', duration: '7s' },
      ],
    });
    // image 下游
    engine.onConnectShot('s1', 'image');
  ...
  it('onConnectShot 宽高比 custom 用 customAspectRatio；4:4 归一为 1:1', async () => {
    const { engine, addNodes } = makeEngine({
      ...
      shots: [
        { id: 's1', index: 1, description: 'x', prompt: 'p', videoPrompt: 'v', duration: '5s' },
      ],
    });
    engine.onConnectShot('s1', 'image');
    ...
    const { engine: e2, addNodes: addNodes2 } = makeEngine({
      aspectRatio: '4:4',
      shots: [
        { id: 's2', index: 1, description: 'x', prompt: 'p', videoPrompt: 'v', duration: '5s' },
      ],
    });
...
describe('scriptBoxEngine · P11 批量写回合并', () => {
  ...
  it('合并不丢数据：错误分镜（JSON 不完整）也正确回退且动画关闭', async () => {
    ...
    vi.mocked(chatCompletions).mockResolvedValueOnce({
      ok: true,
      content: JSON.stringify({ prompt: 'ok-p', videoPrompt: 'ok-v' }),
    });
    vi.mocked(chatCompletions).mockResolvedValueOnce({ ok: true, content: '不是json' });
