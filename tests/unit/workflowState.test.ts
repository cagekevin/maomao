...
const { wfStart, wfSteer, wfFinish, wfAwaitConfirm, wfNextSteer, WORKFLOW_STATUS } =
  await import('../../src/components/agent/runtime/workflowState.ts');
...
describe('wfSteer — 补充指令入队', () => {
  it('向当前 workflow.steerQueue 追加；attachments 缺省为 []', () => {
    vi.mocked(convStore.getCurrentWorkflow).mockReturnValue({
      id: 'wf',
      status: 'running',
      nodeIds: [],
      steerQueue: [{ text: 'a', attachments: [] }],
      startedAt: 0,
      updatedAt: 0,
    });
...
describe('wfFinish — 终态推导', () => {
  it('ok=true → completed', () => {
    expect(wfFinish(true).status).toBe('completed');
  });
  it('ok=false, aborted=false → failed', () => {
    expect(wfFinish(false).status).toBe('failed');
  });
  it('ok=false, aborted=true → stopped', () => {
    expect(wfFinish(false, true).status).toBe('stopped');
  });
});

describe('wfAwaitConfirm — 等待确认', () => {
  it('status → awaiting_confirm', () => {
    expect(wfAwaitConfirm().status).toBe('awaiting_confirm');
  });
});
