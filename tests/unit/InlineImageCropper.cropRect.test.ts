...
describe('cropRectFromSelection', () => {
  it('默认百分比整图（100%）→ 原始整图区域', () => {
    const r = cropRectFromSelection({
      sel: { x: 0, y: 0, width: 100, height: 100, unit: '%' },
      ...R,
    });
  ...
  it('百分比 50% → 原图一半区域', () => {
    const r = cropRectFromSelection({
      sel: { x: 0, y: 0, width: 50, height: 50, unit: '%' },
      ...R,
    });
  ...
  it('像素选区（onComplete 返回）→ 按 natural/render 映射到原图', () => {
    const r = cropRectFromSelection({
      sel: { x: 50, y: 40, width: 100, height: 60, unit: 'px' },
      ...R,
    });
  ...
  it('像素选区偏移正确（非零起点）', () => {
    const r = cropRectFromSelection({
      sel: { x: 150, y: 100, width: 75, height: 50, unit: 'px' },
      ...R,
    });
  ...
  it('越界选区钳制到图内（sw/sh 不超出原图残留）', () => {
    const r = cropRectFromSelection({
      sel: { x: 250, y: 180, width: 200, height: 200, unit: 'px' },
      ...R,
    });
  ...
  it('无效选区（无宽度/高度）→ null', () => {
    ...
    expect(
      cropRectFromSelection({ sel: { x: 0, y: 0, width: 0, height: 0, unit: '%' }, ...R }),
    ).toBeNull();
  });
});
