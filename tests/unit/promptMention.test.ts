...
import {
  ...
  MENTION_FLIP_MIN_H,
} from '../../src/components/base/prompt/promptMention.ts';
...
describe('computeMentionPlacement', () => {
  ...
  it('上方充足 → 向上（底对齐）', () => {
    const p = computeMentionPlacement(
      { top: 400, bottom: 425, left: 100 },
  ...
  it('翻转阈值 = MENTION_FLIP_MIN_H，且两侧都不够时取空间大的一侧', () => {
    ...
    const p = computeMentionPlacement(
      { top: 280, bottom: 305, left: 100 },
      { viewportW: 400, viewportH: 400 },
    );
    expect(p.placement).toBe('up');
    // 上 100 < 160 且 < 下 807 → down
    const p2 = computeMentionPlacement(
      { top: 108, bottom: 133, left: 100 },
  ...
  it('左右溢出 clamp 到边距', () => {
    expect(
      computeMentionPlacement({ top: 400, bottom: 425, left: 5 }, { viewportW: vw, viewportH: vh })
        .left,
    ).toBe(8);
    expect(
      computeMentionPlacement(
        { top: 400, bottom: 425, left: vw - 10 },
        { viewportW: vw, viewportH: vh },
      ).left,
    ).toBe(vw - 280 - 8);
  });
  ...
  it('最小高度兜底：宁可溢出也不塌成一条', () => {
    const p = computeMentionPlacement(
      { top: 500, bottom: 525, left: 100 },
      { viewportW: vw, viewportH: 600 },
    );
