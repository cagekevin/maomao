...
describe('ensurePolling · 注册表去重（一个 taskId 只有一个 poller）', () => {
  it('重复 ensurePolling 同 taskId → 返回同一句柄，register 不被驱动两次', async () => {
    ...
    const opts: EnsurePollingOptions = { register: registered, pollIntervalMs: 100, timeoutMs: 10000 }
  ...
  it('register 返回 true(终态) → poller 自停并释放注册', async () => {
    ...
    const h = ensurePolling('term-task', { register: registered, pollIntervalMs: 100, timeoutMs: 10000 })
...
describe('ensurePolling · 总超时强停', () => {
  it('register 永不终态 → 超时后 poller 自停释放（防无限挂起）', async () => {
    ...
    const h = ensurePolling('timeout-task', { register: registered, pollIntervalMs: 20, timeoutMs: 100 })
...
describe('ensurePolling · 单轮驱动节奏', () => {
  it('未到终态时按 pollIntervalMs 周期驱动 register', async () => {
    let calls = 0
    const registered = vi.fn(async () => { calls++; return calls >= 3 }) // 第 3 轮才终态
...
describe('ensurePolling · occupyOnly 纯占位（in-flight 防恢复重复接管）', () => {
  it('occupyOnly 注册后 isPolling=true（恢复扫描据此跳过），且不起定时器/不驱动 register', async () => {
    const registered = vi.fn(async () => false)
    const h = ensurePolling('inflight-task', { register: registered, occupyOnly: true, pollIntervalMs: 10, timeoutMs: 10000 })
  ...
  it('occupyOnly 占位与驱动 poller 互斥：占位期间 ensurePolling(驱动) 复用占位，不重复起', async () => {
    ...
    ensurePolling('mix-task', { register: occupyReg, occupyOnly: true, pollIntervalMs: 10, timeoutMs: 10000 })
    // 恢复扫描想对同一 taskId 驱动 → 见已有占位(isPolling)，应跳过（不重复注册驱动）
    const driven = vi.fn(async () => true)
    const h2 = ensurePolling('mix-task', { register: driven, pollIntervalMs: 10, timeoutMs: 10000 })
    await vi.advanceTimersByTimeAsync(50)
