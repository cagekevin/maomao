...
import { registerTool, getTools, resetTools, type ToolDef } from '../../src/components/base/canvas/toolRegistry.ts'
...
describe('toolRegistry（docs/25 阶段2：工具轴注册表）', () => {
  ...
  it('注册条目保留 toolDef 字段并可挂 mutating（供 buildCanvasAgentTools 压撤销栈）', () => {
    registerTool({ name: 'mut_tool', description: 'd', parameters: {}, execute: () => {}, mutating: true })
    const t = getTools()[0]
