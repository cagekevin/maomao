  ...
  providers: [
    {
      ...
      isPrimary: true,
      protocol: 'apimart',
      chat_models: [{ id: 'lovart-chat' }],
      image_models: [{ id: 'gpt-image-2' }],
      video_models: [],
    },
  ],
  primary: {
...
describe('剧本盒引擎深度业务 §2.7', () => {
  ...
  it('onGenerateShotPrompts：开启上一镜尾帧时，@图片1 缺失则后处理强制补写', async () => {
    chatCompletionsMock.mockResolvedValue({
      ok: true,
      content: JSON.stringify({ prompt: '画面', videoPrompt: '视频' }),
    });
    data = {
      shots: [
        {
  ...
  it('onGenerateShotPrompts：videoPrompt 已含 @视频1 时不重复补写标签', async () => {
    chatCompletionsMock.mockResolvedValue({
      ok: true,
      content: JSON.stringify({ prompt: '画面', videoPrompt: '@视频1 从尾帧延续' }),
    });
    data = {
      shots: [
        {
  ...
  it('onGenerateShotPrompts：单个 shotId 只生成该分镜', async () => {
    chatCompletionsMock.mockResolvedValue({
      ok: true,
      content: JSON.stringify({ prompt: 'P', videoPrompt: 'V' }),
    });
    data = {
      shots: [
  ...
  it('onGenerateShotPrompts：chat 失败 → 该分镜复位 promptLoading', async () => {
    ...
    data = {
      shots: [{ id: 's1', description: '镜头1', promptLoading: true, prompt: '', videoPrompt: '' }],
    };
    const eng = createScriptBoxEngine(ctx());
  ...
  it('onConnectShot：建 promptNode 下游 + 自动连线，且带入 @资产参考图', () => {
    ...
    expect(node.data.images).toMatchObject([
      { id: 'script-asset-a1', url: 'http://127.0.0.1:18080/files/r.png', label: '小红帽' },
    ]);
    // 自动连线 sb-1 → 新节点
  ...
  it('onGenerateShotImage：成功 → 写回 imgGen{type,label,prompt}', async () => {
    ...
    data = {
      shots: [{ id: 's1', description: '镜头1', prompt: '', videoPrompt: '' }],
      globalStyle: '皮克斯',
    };
    const eng = createScriptBoxEngine(ctx());
    await eng.onGenerateShotImage('s1', 'keyframe');
    const last = patches[patches.length - 1];
    expect(last.shots[0].imgGen).toMatchObject({
      type: 'keyframe',
      label: '关键帧',
      prompt: '关键帧画面提示词',
    });
  ...
  it('onGenerateShotImage：无 provider → 报「请先配置文本大模型」', async () => {
    const eng = createScriptBoxEngine({
      ...ctx(),
      getProviderState: () => ({ providers: [], primary: null }),
    });
