...
vi.mock('../../src/components/base/filesApi.js', () => ({ toAbsoluteFileUrl: (u) => `ABS:${u}` }))
...
describe('AgentMessage — 图片 URL 判定与文本切图', () => {
  ...
  it('markdown 图片 ![](url) → 渲染成图（img src=ABS:url）', () => {
    render(<AgentMessage message={{ role: 'assistant', content: '看这张图：![alt](http://x/a.png)' }} />)
  ...
  it('blob:/ipfs:/ipns: URL → 不当图片，保留为文本', () => {
    render(<AgentMessage message={{ role: 'assistant', content: 'blob:http://x/abc 和 ipfs://QmXyz' }} />)
    expect(document.querySelector('img')).toBeNull()
    expect(screen.getByText(/blob:http:\/\/x\/abc/)).toBeTruthy()
  })

  it('网页类后缀 URL（.html/.json 等）→ 保留为文本，不渲染图', () => {
    render(<AgentMessage message={{ role: 'assistant', content: '参考 https://api.x/list.json' }} />)
    expect(document.querySelector('img')).toBeNull()
...
describe('AgentMessage — user 消息', () => {
  it('渲染 content 与已使用 Skill 标签', () => {
    render(
      <AgentMessage
        message={{ role: 'user', content: '帮我生成图', skills: [{ name: '生图' }, { name: '放大' }] }}
      />
    )
  ...
  it('渲染 attachments 缩略图', () => {
    render(
      <AgentMessage message={{ role: 'user', content: '参考这张', attachments: [{ url: 'http://x/ref.png' }] }} />
    )
    expect(document.querySelector('img')).toBeTruthy()
  })
})
...
describe('AgentMessage — assistant 消息（思考/工具/步骤卡片）', () => {
  ...
  it('streaming=true 时 reasoning 显示「思考中...」且默认展开', () => {
    render(<AgentMessage message={{ role: 'assistant', content: '', reasoning: '过程', streaming: true }} />)
    ...
    render(
      <AgentMessage
        message={{
          role: 'assistant',
          content: 'done',
          tool_calls: [{ function: { name: 'generate_node', arguments: '{"nodeType":"imageNode","prompt":"猫"}' } }],
        }}
      />
    )
  ...
  it('awaiting_confirm → 点击「确认，按此执行」触发 onConfirmPlan', () => {
    ...
    render(<AgentMessage message={{ role: 'assistant', content: 'plan', awaiting_confirm: true }} onConfirmPlan={onConfirmPlan} />)
    fireEvent.click(screen.getByText('确认，按此执行'))
    expect(onConfirmPlan).toHaveBeenCalledTimes(1)
  })

  it('streaming 消息不渲染确认按钮', () => {
    render(<AgentMessage message={{ role: 'assistant', content: 'plan', awaiting_confirm: true, streaming: true }} onConfirmPlan={noop} />)
...
describe('AgentMessage — tool 消息（成败与重试）', () => {
  ...
  it('失败且带 nodeId → 显示「重试」，点击触发 onRetryStep(nodeId)', () => {
    ...
    render(
      <AgentMessage
        message={{ role: 'tool', content: JSON.stringify({ ok: false, nodeId: 'n2', error: '超时' }) }}
