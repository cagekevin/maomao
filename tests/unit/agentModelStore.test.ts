...
import type { AgentStreamMode } from '../../src/components/base/store/agentModelStore.ts'
...
describe('agentModelStore §4 读取/保存 AI 聊天模型偏好', () => {
  ...
  it('loadAgentChatModel 无数据返回 null', async () => {
    const { loadAgentChatModel } = await import('../../src/components/base/store/agentModelStore.ts')
    expect(loadAgentChatModel()).toBeNull()
  })

  it('saveAgentChatModel 写入后可被 load 读回（含 stream 默认）', async () => {
    const { loadAgentChatModel, saveAgentChatModel } = await import('../../src/components/base/store/agentModelStore.ts')
...
describe('agentModelStore §4.5 历史回传轮数 load/save', () => {
  ...
  it('未设置时默认返回 6', async () => {
    const { loadAgentHistoryTurns } = await import('../../src/components/base/store/agentModelStore.ts')
    expect(loadAgentHistoryTurns()).toBe(6)
  })

  it('save 后 load 读回（支持 0）', async () => {
    const m = await import('../../src/components/base/store/agentModelStore.ts')
