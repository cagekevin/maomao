...
export interface StableRequestOptions {
   ...
   * 导致**每个**调用点自动重发、不分 method（非幂等 POST 也会被静默重发）——
   * 违反红线「只有网络错误才能重试」。现缺省不重试状态码；网络层错误仍自动重试（不受本项影响）。
   *
   * 【合法用途】调用方**确知**某码在本场景是瞬态，且有幂等依据（TD-08-25 实证：CDN「已发布但尚未可读」
   * 窗口返回 **403**，非权限问题、几秒后自愈，且下载是只读 GET）。
   * 传 `[...显式码]` 时**并不自动继承**中央集 —— 要哪些就给哪些（如 `[403, ...RETRYABLE_HTTP_STATUSES]`）。
   */
