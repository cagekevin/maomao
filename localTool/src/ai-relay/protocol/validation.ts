/**
 * protocol/validation — 声明式调用协议的校验与归一化。
 * 协议一律按不可信输入处理：路径必须同源相对、变量必须在白名单内、
 * 鉴权与危险 Header 不可被协议改写；version 1 的扁平字段升级为 version 2 结构。
 * 对应 AI-Canvas-tauri 的 modelProtocolValidation.ts。
 */
...
function validateConditionalTemplateDirectives(
  ...
  if (hasCondition || hasConditionalValue) {
    ...
    const whenPresent = value[WHEN_PRESENT_KEY];
    if (typeof whenPresent !== 'string' || !FULL_TEMPLATE_RE.test(whenPresent)) {
...
function pollRequestUsesTaskId(request: unknown, taskIdPath: string): boolean {
...
export function upgradeLegacyProtocolValue(value: unknown): unknown {
...
function validateResultConfig(value: unknown, label: string, requirePath: boolean, errors: string[]): void {
  ...
  if (value.base64Transform !== undefined) {
    if (!isRecord(value.base64Transform) || value.base64Transform.type !== 'pcm-s16le-to-wav') {
      ...
      const sampleRate = Number(value.base64Transform.sampleRate);
      const channels = Number(value.base64Transform.channels ?? 1);
...
export { ModelProtocolResponsePreviewEntry };
