/**
 * protocol/shared — 声明式模型协议的共享常量与基础校验。
 * 对应 AI-Canvas-tauri 的 modelProtocolShared.ts（纯逻辑，无 DOM 依赖）。
 */

import { PROTOCOL_VARIABLE_NAMES } from './variables.js';
...
/**
 * 仅允许在请求体数组里展开参考素材 URL 数组，避免任意协议值变成隐式表达式语言。
 */
...
export function visitTemplateStrings(value: unknown, visit: (template: string) => void): void {
...
export function resolveAuthentication(auth: AuthConfig | undefined): AuthConfig {
