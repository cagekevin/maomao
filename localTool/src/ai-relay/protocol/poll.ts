  ...
  const result = await pollTask({
    fetchState: async () => {
      if (pendingExtraDelayMs > 0) {
        // 【`maxDurationMs` 的消费点清单（2026-09-22 **二次复核**修正）】本字段**只被本链消费**：
        //   · **本处**：sync 链多轮循环（`pollResolvedModelProtocol` ← `executeModelProtocol`，后者不传 timeoutMs）
        //     ⇒ 它是这条链「不无限挂」的执行者。**结论：不删**（删了这条链就失去唯一超时执行者）。
        //   · `pollModelProtocolOnce`（单轮打点）**不消费它** —— 其函数体（本文件 `:114-178`）**无 `pollTask` 调用**，
        //     官方注释 `:92-101` 即写明「只做一轮、**不代管 interval/maxDuration**」⇒ 句柄链超时**只由 `budgetMsFor` 管**。
        //   ⚠️ 曾被「勘误」成"两条链都消费"——那是**行号归属误判**（`maxDuration:` 在本函数内、不在单轮函数内）。
