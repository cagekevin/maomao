...
import { XAI_BASE_URL, XAI_MODEL_MANIFEST } from './manifests/xai';
import {
  GOOGLE_GEMINI_BASE_URL,
  GOOGLE_MODEL_MANIFEST,
} from './manifests/google';
import {
  SORA2U_BASE_URL,
  SORA2U_MODEL_MANIFEST,
  SORA2U_REQUEST_QUERY,
} from './manifests/sora2u';
...
function mergeRemoteCatalogMetadata(
  ...
  return remoteModels.map((remote) => {
    ...
    return {
      ...
      // 远程或本地清单任一具备视频能力时，以本地清单为基础、远程字段覆盖合并；
      // 两者皆无时结果为 undefined。
      videoCapability:
        remote.videoCapability || fallback.videoCapability
...
async function fetchCatalogResponse(
  ...
  return relayFetch(url, { method: 'GET', headers, signal });
}
