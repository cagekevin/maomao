 ...
 */
import type { GeneralModelCategory, ImageReferenceRequestMode } from '../4-types/connection';
import type {
...
import { validateModelExecutionProtocol } from './schema';
import {
  IMAGE_ARRAY_FIELDS,
  IMAGE_SINGLE_FIELDS,
...
function parseOpenApiRequests(source: string): ParsedRequest[] {
  ...
  // 收集所有匹配的操作，优先 POST（生成类接口多为 POST 提交），再退回任意 method。
  // 原先在第一个匹配处直接 return，会跳过同一文档里真正的生成接口（如先命中 GET /status）。
  const matches: Array<{ path: string; method: 'post' | 'get'; request: ParsedRequest }> = [];
  ...
  // POST 优先（生成类接口多为提交动作），其次任意首个匹配。
  const chosen = matches.find((match) => match.method === 'post') ?? matches[0];
