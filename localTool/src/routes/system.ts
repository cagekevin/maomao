...
async function handleProxyJson(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  // ── [E 方案 · docs/72] 出站回读：把请求体里的本机 /files/ 图片（相对 + 绝对自指）统一内联成 base64。
  //    前端会话/内存态只存 /files/；本地网关 9004 与外部 provider 都读不到用户本机 127.0.0.1:18080，
  //    由 localTool（唯一出站口）统一读 uploads/ → 压缩≤1920 → base64 再转发。不再依赖网关
  //    resolve_attachments 的回环支持（网关只是众多上游之一）。失败保留原 URL，上游显性失败。 ──
  if (fetchBody) {
    try {
      const parsed = JSON.parse(fetchBody);
      const resolved = await resolveLocalImages(parsed);
      const next = JSON.stringify(resolved);
      if (next !== fetchBody) fetchBody = next;
    } catch (e) {
      console.error(`[proxy:inline-img] 出站回读失败，保留原 body: ${(e as Error)?.message || e}`);
    }
  }
