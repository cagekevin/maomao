...
function makeRes() {
  const r = {
    ...
    end(data) { r.writableEnded = true; if (data !== undefined) r.body = (r.body || '') + (Buffer.isBuffer(data) ? data.toString('utf-8') : String(data)); return r; },
  };
  return r;
}
...
test('[computePaddedRect] 外扩 10% 且钳制图内', () => {
  ...
  assert.equal(r.x, 90); assert.equal(r.y, 90);
  // 外扩 100*0.1=10，x2=100+100-1+10=209，w=209-90+1=120
  assert.equal(r.w, 120); assert.equal(r.h, 120);
  ...
  assert.ok(edge.w > 50); assert.ok(edge.h > 50);
});
...
test('[crop] 裁出 paddedRect 且生成 cropContext', async () => {
  ...
  const { img, cropContext } = await ops.cropLocalPatch(diskOf(url), { x: 50, y: 50, w: 60, h: 60 }, { sourceUrl: url });
...
test('[merge] 单图拼接回原位置，padded 外不受影响', async () => {
  ...
  const { img: local, cropContext } = await ops.cropLocalPatch(diskOf(url), { x: 50, y: 50, w: 60, h: 60 });
  const green = solid(local.getWidth(), local.getHeight(), { r: 0, g: 255, b: 0 });
  const patchUrl = await writeOut(green, 'merge_patch.png');

  const merged = await ops.mergeLocalPatches(diskOf(url), [{ patchPath: diskOf(patchUrl), cropContext }], { colorMatch: false });
...
test('[merge] 源图文件变更 → 409', async () => {
  ...
  // 篡改源图 → 源指纹变
  fs.writeFileSync(diskOf(url), await solid(200, 200, { r: 0, g: 0, b: 255 }).getBufferAsync(Jimp.MIME_PNG));
  ...
  await assert.rejects(
    ops.mergeLocalPatches(diskOf(url), [{ patchPath: diskOf(pUrl), cropContext }], { colorMatch: false }),
    (e) => e.status === 409
  );
});
...
test('[merge] 2× 等比放大 patch 能缩回融合', async () => {
  ...
  const merged = await ops.mergeLocalPatches(diskOf(url), [{ patchPath: diskOf(pUrl), cropContext }], { colorMatch: false });
  const pr = cropContext.paddedRect;
...
test('[crop handler] 返回 file.url + cropContext 且落盘新文件', async () => {
  ...
  const res = makeRes();
  await routes.handleLocalPatchCrop(makeJsonReq({ source_url: srcUrl, selection: { x: 40, y: 40, w: 50, h: 50 }, padding_ratio: 0.1 }), res);
...
test('[merge handler] 缺上下文 → 400；>16 张 → 400', async () => {
  ...
  const res1 = makeRes();
  await routes.handleLocalPatchMerge(makeJsonReq({ original_url: srcUrl, patches: [{ patch_url: srcUrl, crop_context: null }] }), res1);
  assert.equal(res1.status, 400);
  // >16 张（复用同一合法 patch）
  const cropRes = makeRes();
  await routes.handleLocalPatchCrop(makeJsonReq({ source_url: srcUrl, selection: { x: 10, y: 10, w: 40, h: 40 } }), cropRes);
...
test('[merge handler] 正常融合 → file 带 localPatchFullImage/ContextReset', async () => {
  ...
  const cropRes = makeRes();
  await routes.handleLocalPatchCrop(makeJsonReq({ source_url: srcUrl, selection: { x: 40, y: 40, w: 60, h: 60 } }), cropRes);
  const ctx = parseBody(cropRes).data.file.cropContext;
  const pUrl = await writeOut(solid(ctx.paddedRect.w, ctx.paddedRect.h, { r: 0, g: 255, b: 0 }), 'hm_ok_patch.png');
  const res = makeRes();
  await routes.handleLocalPatchMerge(makeJsonReq({ original_url: srcUrl, patches: [{ patch_url: pUrl, crop_context: ctx }], color_match: false }), res);
  assert.equal(res.status, 200);
