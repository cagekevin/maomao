...
test('B6 自然语言路：prompt 含可读模型名（gpt-image-2 → GPT Image 2 Low）', () => {
  const p = buildLovartPrompt('gpt-image-2-low', 'a red dog');
  assert.match(p, /\[model: GPT Image 2 Low\]/);
  assert.ok(p.endsWith('a red dog'), '用户 prompt 原样保留在末尾');
});

test('B6 视频：prompt 含 seedance 可读名', () => {
  const p = buildLovartPrompt('seedance-2', 'make a wave');
  assert.match(p, /\[model: Seedance 2\]/);
  assert.ok(p.endsWith('make a wave'));
});

test('C3 尺寸归一：size 参数写进 prompt', () => {
  const p = buildLovartPrompt('gpt-image-2-low', 'a photo', '1024x1024');
  assert.match(p, /\[size: 1024x1024\]/);
  const noSize = buildLovartPrompt('gpt-image-2-low', 'a photo');
  assert.ok(!noSize.includes('[size:'), '未给 size 时不写 [size]');
});
...
test('参考图声明（对齐 main.build_gen_prefix）：IMAGE 带参考图写 Reference attached + Generate ONE', () => {
  ...
  assert.match(withRef, /Reference image attached\. Generate exactly ONE image\./);
  assert.ok(withRef.endsWith('a red dog'), '用户 prompt 原样保留在末尾');
});
...
test('参考图声明（对齐 main）：IMAGE 无参考图不写 Reference，但写 Generate exactly ONE image', () => {
  ...
  assert.match(noRef, /Generate exactly ONE image\./);
});
...
test('参考图声明（对齐 main）：VIDEO 写 Generate exactly ONE video（参考图经 attachments 携带）', () => {
  ...
  assert.match(p, /Generate exactly ONE video\./);
