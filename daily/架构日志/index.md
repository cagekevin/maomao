...
| 09 | 类型诚实性（横切：假收窄/无守卫断言） | 见 `docs/类型诚实性审计-假收窄清单.md` + `contracts.ts`（护栏：`type-check` + `check:node-data --strict`） | **已验证(显式断言面无债) / 隐式 any 面待翻新**（TD-09-1：src `noImplicitAny` 实测 1435 处，strict.probe 不含 src；**选项 A 已落地第一步：新增闸 `npm run check:strict-src` + `core/` 白名单清零，存量 1435→1424；执行手册 `docs/STRICT-TYPING-HANDOFF.md` + 报告工具 `scripts/strict-report.mjs` 已交付**） | [09-类型诚实性-2026-09-11.md](09-类型诚实性-2026-09-11.md) · [09-类型诚实性-隐式any复核-2026-09-12.md](09-类型诚实性-隐式any复核-2026-09-12.md) |
